 ...
 * 5. Shell chrome functionality (header, navigation, REEVU AI)
...
test.describe('§5 Shell Chrome & Navigation', () => {
  ...
  test('header should have branding and navigation controls', async ({ page }) => {
    ...
    // Should have BijMantra branding
    const branding = page.getByText('BijMantra').first()
  ...
  test('should have REEVU AI assistant toggle', async ({ page }) => {
    await navigateAuthenticated(page, '/dashboard')
    
    const assistantBtn = page.getByRole('button', { name: /reevu|ai assistant/i }).first()
    const hasAssistant = await assistantBtn.isVisible({ timeout: 5000 }).catch(() => false)
...
test.describe('§9 BrAPI API Compliance', () => {
  ...
  test.beforeAll(async ({ request }) => {
    // Reset rate limits
    await request.post(`${API_BASE}/api/auth/reset-rate-limit`, {
        headers: { 'X-RateLimit-Reset-Token': RATE_LIMIT_RESET_TOKEN }
    }).catch(() => {})
