...
import { apiClient, PedigreeIndividual, PedigreeStats, PedigreeRecord } from '@/lib/api-client';
...
export function PedigreeAnalysis() {
  ...
  const loadPedigreeMutation = useMutation({
    mutationFn: async () => {
      ...
      return apiClient.pedigreeAnalysisService.loadPedigree(pedigree);
    },
    onSuccess: (data) => {
  ...
  const coancestryMutation = useMutation({
    mutationFn: () => apiClient.pedigreeAnalysisService.calculateCoancestry(coancestryId1, coancestryId2),
    onError: (error: Error) => {
  ...
  const ancestorsMutation = useMutation({
    mutationFn: (id: string) => apiClient.pedigreeAnalysisService.getAncestors(id, 4),
    onError: (error: Error) => {
  ...
  const descendantsMutation = useMutation({
    mutationFn: (id: string) => apiClient.pedigreeAnalysisService.getDescendants(id, 3),
    onError: (error: Error) => {
  ...
  const getInbreedingColor = (f: number): string => {
    if (f >= 0.25) return 'text-red-600 dark:text-red-400';
    if (f >= 0.125) return 'text-orange-600 dark:text-orange-400';
    if (f >= 0.0625) return 'text-yellow-600 dark:text-yellow-400';
