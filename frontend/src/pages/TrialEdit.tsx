...
interface TrialEditFormProps {
  initialData: TrialData
  programs: Program[]
  trialDbId: string
}

function TrialEditForm({ initialData, programs, trialDbId }: TrialEditFormProps) {
  const navigate = useNavigate()
  const [formData, setFormData] = useState({
    trialName: initialData.trialName || '',
    trialDescription: initialData.trialDescription || '',
    programDbId: initialData.programDbId || '',
  ...
  const mutation = useMutation({
    mutationFn: (data: TrialData) => apiClient.trialService.updateTrial(trialDbId, data),
    onSuccess: () => {
    ...
    <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-lg shadow p-6 space-y-6">
      ...
      {mutation.isError && (
        <div className="p-4 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-md">
          <p className="text-sm text-red-600 dark:text-red-400">Error updating trial. Please try again.</p>
        </div>
      )}
...
export function TrialEdit() {
  ...
  if (isTrialLoading || isProgramsLoading) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
      ...
      {trialData?.result && trialDbId && (
        <TrialEditForm
          initialData={trialData.result}
          programs={programs}
          trialDbId={trialDbId}
        />
      )}
