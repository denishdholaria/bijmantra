...
function TransactionHistory({ seedLotId, units }: { seedLotId: string; units: string }) {
  ...
  const { data: transactions = [], isLoading } = useQuery({
    ...
    queryFn: async (): Promise<Transaction[]> => {
      try {
        const response = await apiClient.seedLotService.getSeedlotTransactionsById(seedLotId, { pageSize: 100 });
  ...
  const typeColors: Record<string, string> = {
    addition: 'text-green-600 dark:text-green-400',
    removal: 'text-red-600 dark:text-red-400',
...
export function SeedLotDetail() {
  ...
  const { data: germplasmData } = useQuery({
    ...
    queryFn: () => apiClient.germplasmService.getGermplasmById(data!.result.germplasmDbId),
    enabled: !!data?.result?.germplasmDbId,
  })

  const deleteMutation = useMutation({
    mutationFn: () => apiClient.seedLotService.deleteSeedLot(id!),
    onSuccess: () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
      ...
      {/* Stock Status Card */}
      <Card className={stockStatus === 'good' ? 'border-green-200 bg-green-50/50 dark:bg-green-900/20 dark:border-green-800' : stockStatus === 'low' ? 'border-orange-200 bg-orange-50/50 dark:bg-orange-900/20 dark:border-orange-800' : 'border-red-200 bg-red-50/50 dark:bg-red-900/20 dark:border-red-800'}>
