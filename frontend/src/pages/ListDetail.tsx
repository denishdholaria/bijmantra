...
export function ListDetail() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['list', id],
    queryFn: () => apiClient.listService.getList(id!),
    enabled: !!id,
  })

  const deleteMutation = useMutation({
    mutationFn: () => apiClient.listService.deleteList(id!),
    onSuccess: () => {
  ...
  const list = data?.result as List | undefined;
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" asChild aria-label="Back to lists"><Link to="/lists">←</Link></Button>
          <div>
