    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Select value={selectedGermplasm} onValueChange={setSelectedGermplasm}>
            ...
            <SelectContent>
              {germplasmLoading ? (
                ...
                germplasmList.map((g: GermplasmListItem) => (
          ...
          <Button variant="outline" onClick={() => setIsFullscreen(!isFullscreen)} disabled={!hasData} aria-label="Toggle Fullscreen">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="pt-4 text-center">
            <p className="text-3xl font-bold text-blue-600 dark:text-blue-400">{nodes.filter(n => n.type === 'root').length}</p>
            <p className="text-sm text-muted-foreground">Founders</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 text-center">
            <p className="text-3xl font-bold text-purple-600 dark:text-purple-400">{nodes.length > 0 ? Math.max(...nodes.map(n => n.generation)) : 0}</p>
            <p className="text-sm text-muted-foreground">Generations</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 text-center">
            <p className="text-3xl font-bold text-green-600 dark:text-green-400">{edges.length}</p>
          ...
          <Card className={`${selectedNode ? 'lg:col-span-2' : ''} ${isFullscreen ? 'fixed inset-4 z-50' : ''}`}>
            <CardHeader className="pb-2">
              <CardTitle className="flex items-center gap-2">
                ...
                Pedigree Tree {selectedGermplasm && `- ${germplasmList?.find((g: GermplasmListItem) => g.germplasmDbId === selectedGermplasm)?.germplasmName || selectedGermplasm}`}
              </CardTitle>
