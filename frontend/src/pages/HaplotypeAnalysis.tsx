...
import { AlertCircle, Dna, BarChart3, Target, Network } from 'lucide-react'
...
// Demo germplasm haplotypes for breeding tab (static display data)
const demoGermplasmHaplotypes = [
...
export function HaplotypeAnalysis() {
  ...
  const getHapBadgeColor = (hap: string) => {
    const colors: { [key: string]: string } = {
      'H1': 'bg-blue-100 text-blue-700', 'H2': 'bg-green-100 text-green-700',
      'H3': 'bg-orange-100 text-orange-700', 'H4': 'bg-purple-100 text-purple-700',
      'H5': 'bg-pink-100 text-pink-700', 'H6': 'bg-cyan-100 text-cyan-700',
    }
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {blocksError && (
        <Alert variant="destructive">
          ...
          <AlertDescription>Failed to load haplotype data. Please try again.</AlertDescription>
        </Alert>
      )}
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {demoGermplasmHaplotypes.map((germ) => (
                      <tr key={germ.name} className="border-b hover:bg-muted/50">
                        ...
                        {blocks.slice(0, 6).map(b => (
                          <td key={b.id} className="p-2 text-center">
                            <div className={`w-6 h-6 rounded mx-auto flex items-center justify-center text-xs text-white ${getHapColor(germ.haplotypes[b.id as keyof typeof germ.haplotypes] || 'H1')}`}>
                              {(germ.haplotypes[b.id as keyof typeof germ.haplotypes] || 'H1').replace('H', '')}
                            </div>
                          </td>
                        ))}
