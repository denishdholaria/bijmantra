...
import { apiClient, HaplotypeBlock, Haplotype, HaplotypeDiversitySummary, HaplotypeStatistics } from '@/lib/api-client'
...
export function HaplotypeAnalysis() {
  ...
  const { data: blocksData, isLoading: blocksLoading, error: blocksError, refetch: refetchBlocks } = useQuery<{ data: HaplotypeBlock[] }>({
    queryKey: ['haplotype-blocks', selectedChromosome],
    queryFn: () => apiClient.molecularBreedingService.getHaplotypeBlocks(selectedChromosome === 'all' ? undefined : selectedChromosome),
  })

  // Fetch haplotypes for selected block
  const { data: haplotypesData, isLoading: haplotypesLoading } = useQuery<{ data: Haplotype[] }>({
    queryKey: ['haplotype-block-haplotypes', selectedBlock],
    queryFn: () => apiClient.molecularBreedingService.getBlockHaplotypes(selectedBlock),
  ...
  const { data: diversityData } = useQuery({
    ...
    queryFn: () => apiClient.molecularBreedingService.getHaplotypeDiversity(),
  })

  // Fetch statistics
  const { data: statsData } = useQuery({
    queryKey: ['haplotype-statistics'],
    queryFn: () => apiClient.molecularBreedingService.getHaplotypeStatistics(),
  })
  ...
  const blocks: Array<{
    ...
    diversity: number
    trait?: string
  }> = (blocksData?.data || []).map((b: HaplotypeBlock) => ({
    id: b.block_id || b.id,
  ...
  const haplotypes: Array<{
    ...
    effect: number
    germplasm: string[]
  }> = (haplotypesData?.data || []).map((h: Haplotype) => ({
    id: h.haplotype_id,
    block_id: h.block_id,
  ...
  const getHapColor = (hap: string) => {
    const colors: { [key: string]: string } = {
      'H1': 'bg-blue-500 dark:bg-blue-600', 'H2': 'bg-green-500 dark:bg-green-600', 'H3': 'bg-orange-500 dark:bg-orange-600',
      'H4': 'bg-purple-500 dark:bg-purple-600', 'H5': 'bg-pink-500 dark:bg-pink-600', 'H6': 'bg-cyan-500 dark:bg-cyan-600',
    }
    return colors[hap] || 'bg-gray-500 dark:bg-gray-600'
  }
