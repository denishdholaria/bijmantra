...
export function SeedLots() {
  ...
  const seedLots = (data?.result?.data || []) as SeedLot[]
  ...
  return (
    <div className="space-y-4 lg:space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <Button asChild>
          <Link to="/seedlots/new"><Package className="h-4 w-4 mr-2" /> New Seed Lot</Link>
        </Button>
      </div>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <Button asChild>
                <Link to="/seedlots/new"><Package className="h-4 w-4 mr-2" /> Create First Seed Lot</Link>
              </Button>
            </div>
          ) : (
            <>
                  ...
                  <tbody className="divide-y">
                    {seedLots.map((lot) => (
                      <tr key={lot.seedLotDbId} className="hover:bg-muted/30">
                        ...
                        <td className="px-4 py-3">
                          <Badge variant={(lot.amount || 0) > 100 ? 'default' : 'destructive'}>
                            {(lot.amount || 0) > 100 ? 'In Stock' : 'Low'}
                          </Badge>
                        </td>
