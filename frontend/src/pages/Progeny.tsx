...
export function Progeny() {
  ...
  const { data: parentsData, isLoading: parentsLoading, refetch: refetchParents } = useQuery({
    ...
    queryFn: () => apiClient.progenyService.getParents({
  ...
  const { data: statsData, isLoading: statsLoading } = useQuery({
    ...
    queryFn: () => apiClient.progenyService.getStatistics(),
    retry: 1,
  })
  ...
  const getParentTypeColor = (type: string) => {
    const colors: Record<string, string> = {
      FEMALE: 'bg-pink-100 text-pink-800 dark:bg-pink-900/20 dark:text-pink-400',
      MALE: 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400',
      SELF: 'bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-400',
      POPULATION: 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400',
    }
  ...
  const exportProgeny = async () => {
    try {
      const blob = await apiClient.progenyService.export(filteredData.map(p => p.id), 'csv')
      ...
      {statistics && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-blue-100 dark:bg-blue-900/20 rounded-lg"><Users className="h-5 w-5 text-blue-600 dark:text-blue-400" /></div>
                <div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-green-100 dark:bg-green-900/20 rounded-lg"><Sprout className="h-5 w-5 text-green-600 dark:text-green-400" /></div>
                <div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-purple-100 dark:bg-purple-900/20 rounded-lg"><TrendingUp className="h-5 w-5 text-purple-600 dark:text-purple-400" /></div>
                <div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-yellow-100 dark:bg-yellow-900/20 rounded-lg"><Award className="h-5 w-5 text-yellow-600 dark:text-yellow-400" /></div>
                <div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1">
              <Input
                ...
                aria-label="Search progeny"
              />
            </div>
            <Select value={selectedParent} onValueChange={setSelectedParent}>
              <SelectTrigger className="w-[200px]" aria-label="Filter by parent">
            ...
            <Select value={parentTypeFilter} onValueChange={setParentTypeFilter}>
              <SelectTrigger className="w-[150px]" aria-label="Filter by parent type">
