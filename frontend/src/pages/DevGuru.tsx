...
async function sendChatMessage(message: string, projectId?: string, history?: ChatMessage[]): Promise<{ message: string, provider: string, sources?: any[], reasoning?: string }> {
  const token = apiClient.getToken()
...
async function fetchProposals(): Promise<Proposal[]> {
  const token = apiClient.getToken()
  const response = await fetch('/api/v2/proposals?status=draft', {
    headers: token ? { 'Authorization': `Bearer ${token}` } : {}
  })
...
async function reviewProposal(id: number, approved: boolean, notes?: string): Promise<Proposal> {
  const token = apiClient.getToken()
  const response = await fetch(`/api/v2/proposals/${id}/review`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      ...(token ? { 'Authorization': `Bearer ${token}` } : {})
...
function ProposalsTab({ proposals, isLoading, onReview }: ProposalsTabProps) {
  if (isLoading) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
