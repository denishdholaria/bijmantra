...
export function PhenomicSelection() {
  ...
  const { data: datasets = [], isLoading: loadingDatasets } = useQuery({
    ...
    queryFn: () => apiClient.phenomicSelectionService.getDatasets({
  ...
  const { data: models = [], isLoading: loadingModels } = useQuery({
    ...
    queryFn: () => apiClient.phenomicSelectionService.getModels()
  })

  // Fetch statistics
  const { data: stats, isLoading: loadingStats } = useQuery({
    queryKey: ['phenomic-statistics'],
    queryFn: () => apiClient.phenomicSelectionService.getStatistics()
  })

  const getAccuracyColor = (acc: number) => {
    if (acc >= 0.85) return 'text-green-600 dark:text-green-400'
    if (acc >= 0.75) return 'text-yellow-600 dark:text-yellow-400'
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Select value={selectedCrop} onValueChange={setSelectedCrop}>
            <SelectTrigger className="w-32" aria-label="Select crop">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="pt-4">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-green-500/10 dark:bg-green-900/20 rounded-lg">
        ...
        <Card>
          <CardContent className="pt-4">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-blue-500/10 dark:bg-blue-900/20 rounded-lg">
