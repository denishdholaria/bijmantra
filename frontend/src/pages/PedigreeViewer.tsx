...
import { pedigreeViewerAPI, PedigreeNode } from '@/lib/api-client'
...
export function PedigreeViewer() {
  ...
  const { data: germplasmListData } = useQuery({
    ...
    queryFn: async () => {
      const response = await pedigreeViewerAPI.getGermplasmList({ limit: 50 })
  ...
  const { data: pedigreeData, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await pedigreeViewerAPI.getPedigree(selectedGermplasm, generations)
      return response.data
    },
  ...
  if (error) {
    return (
      <div className="text-center py-12 text-red-500">
        Failed to load pedigree. Please try again.
      </div>
    )
  }
