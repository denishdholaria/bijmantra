...
export function FieldScanner() {
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: () => apiClient.fieldScannerService.getStats(),
  });

  // Fetch scan history
  const { data: scansData, isLoading: loadingScans } = useQuery({
    queryKey: ['field-scanner', 'scans'],
    queryFn: () => apiClient.fieldScannerService.getFieldScans({ limit: 50 }),
  });
  ...
  const createScan = useMutation({
    mutationFn: (data: {
      ...
      thumbnail?: string;
      notes?: string;
    }) => apiClient.fieldScannerService.createScan(data),
    onSuccess: () => {
  ...
  const deleteScan = useMutation({
    mutationFn: (scanId: string) => apiClient.fieldScannerService.deleteScan(scanId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['field-scanner'] });
    },
  });
  ...
  const handleExport = async (format: 'json' | 'csv') => {
    try {
      const data = await apiClient.fieldScannerService.exportScans(format);
                ...
                {scanResults.filter(s => s.location).length > 0 ? (
                  <div className="h-full flex flex-col">
                    ...
                    <div className="flex-1 grid grid-cols-4 gap-2">
                      {scanResults.filter(s => s.location).slice(0, 8).map((scan) => (
