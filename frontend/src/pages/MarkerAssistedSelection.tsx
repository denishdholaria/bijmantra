...
import { AlertCircle, Target, Dna } from 'lucide-react'
...
// Hardcoded data removed — Zero Mock Data Policy (Session 95)
// Data now fetched from /api/v2/mas/markers and /api/v2/mas/stats

export function MarkerAssistedSelection() {
  const [activeTab, setActiveTab] = useState('foreground')
  const [selectedMarkers, setSelectedMarkers] = useState<string[]>([])

  // Fetch markers from MAS API
  const { data: markersData, isLoading: loadingMarkers } = useQuery({
    queryKey: ['mas-markers'],
    queryFn: () => apiClient.get<MASMarkerResponse>('/api/v2/mas/markers'),
  })

  // Map API markers to TargetMarker shape
  const targetMarkers: TargetMarker[] = useMemo(() => (markersData?.markers || []).map((m) => ({
    id: m.marker_id || m.id || '',
    name: m.name,
    chromosome: m.chromosome,
    position: m.position,
    trait: m.linked_trait || '',
    alleles: [m.target_allele, '?'] as [string, string],
    favorableAllele: m.target_allele,
    effect: m.distance_to_qtl === 0 ? 'Major' : 'Moderate',
    validated: true,
  })), [markersData]);

  // No candidates until genotype scoring is run — show empty state
            ...
            <CardContent>
              {loadingMarkers ? (
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-semibold mb-2">No markers registered</h3>
                  <p className="text-muted-foreground mb-4">
                    Register target markers via POST /api/v2/mas/markers to begin foreground selection.
                  </p>
                  <Button variant="outline">📥 Import Genotypes</Button>
                </div>
              ) : (
            ...
            <CardContent>
              {candidates.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="font-medium">No genotype data scored</p>
                  <p className="text-sm text-muted-foreground mt-1">
                    Score genotypes via POST /api/v2/mas/score, then run foreground selection.
                  </p>
                </div>
              ) : (
              <>
