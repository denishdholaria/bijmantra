...
import { apiClient, type StressType, type ToleranceGene, type StressIndices } from '@/lib/api-client';
...
const categoryColors: Record<string, string> = {
  ...
  radiation: 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300',
};
...
export function AbioticStress() {
  ...
  const { data: stressesData, isLoading: stressesLoading } = useQuery({
    ...
    queryFn: () => apiClient.abioticStressService.getStressTypes({
  ...
  const { data: genesData, isLoading: genesLoading } = useQuery({
    ...
    queryFn: () => apiClient.abioticStressService.getGenes({
      search: searchQuery || undefined,
    }),
  });
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: () => apiClient.abioticStressService.getStatistics(),
  });

  // Calculate indices mutation
  const calculateMutation = useMutation({
    mutationFn: () => apiClient.abioticStressService.calculateIndices(
      ...
      <div className="flex gap-4">
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="pl-10"
            aria-label="Search stresses or genes"
          />
        </div>
