...
import { diseaseResistanceAPI, type Disease, type ResistanceGene } from '@/lib/api-client';
...
export function DiseaseResistance() {
  ...
  const { data: diseasesData, isLoading: diseasesLoading } = useQuery({
    ...
    queryFn: () => diseaseResistanceAPI.getDiseases({
  ...
  const { data: genesData, isLoading: genesLoading } = useQuery({
    ...
    queryFn: () => diseaseResistanceAPI.getGenes({
      search: searchQuery || undefined,
    }),
  });
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: () => diseaseResistanceAPI.getStatistics(),
  });

  // Fetch pyramiding strategies
  const { data: pyramidingData } = useQuery({
    queryKey: ['pyramiding-strategies'],
    queryFn: () => diseaseResistanceAPI.getPyramidingStrategies(),
  });
