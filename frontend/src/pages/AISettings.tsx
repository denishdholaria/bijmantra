...
export function AISettings() {
  ...
  const testCloud = async () => {
    ...
    try {
      ...
      if (e.name === 'AbortError') {
        toast.error("Connection timed out. Check your internet or API key.");
      } else {
  ...
  // Mode state
  const [setupMode, setSetupMode] = useState<"standard" | "pro">("standard");
  ...
  // Wizard step navigation (Pro Mode Only)
  ...
  // Switch to Standard Mode (Clear Custom Config)
  const activateStandardMode = () => {
    const newConfig = {
      mode: "cloud" as AIMode,
      cloud: {
        provider: "google", // Default backend provider
        apiKey: "", // Empty key = Managed Mode
        model: "gemini-2.5-flash",
        tested: true,
        lastTestResult: "success" as const
      }
    ...
    toast.success("Switched to Managed Cloud (Standard)");
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Configure how Veena connects to AI models
          </p>
        </div>
