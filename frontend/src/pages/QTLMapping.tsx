    ...
    <div className="space-y-6 animate-fade-in">
            ...
            <DialogContent className="max-h-[80vh] overflow-y-auto">
              ...
              <div className="grid gap-3 py-2">
                ...
                <Select value={gwasConfig.variant_set_id} onValueChange={e => setGwasConfig({ ...gwasConfig, variant_set_id: e })}>
                  ...
                  <SelectContent>
                    {(variantSetsData?.result?.data || []).map((vs: any) => (
      ...
      <Tabs value={analysisType} onValueChange={setAnalysisType}>
        ...
        <TabsContent value="candidates" className="space-y-4 mt-4">
          <Card><CardHeader><CardTitle>GO Enrichment</CardTitle></CardHeader><CardContent>{goEnrichment.slice(0, 10).map((g, i) => <div key={i} className="text-sm py-1">{g.term}: {g.p_value}</div>)}</CardContent></Card>
        </TabsContent>
      </Tabs>
    </div>
  )
