...
export function QTLMapping() {
  ...
  const [gwasConfig, setGwasConfig] = useState({
      run_name: "GWAS_Run_" + new Date().toISOString().split('T')[0],
      trait_name: "Yield",
      method: "mlm", // glm, mlm
      sample_size: 100
  });
  ...
  const handleRunGWAS = async () => {
      setRunningGWAS(true);
      try {
          // Generate simulated genotype/phenotype input for GWAS computation
          const n = gwasConfig.sample_size;
          const m = 500; // markers
          
          const markers = Array(m).fill(0).map((_, i) => ({
              name: `M${i}`,
              chromosome: `${Math.floor(i/50) + 1}`,
              position: i * 1000
          }));
          
          const genotypes = Array(n).fill(0).map(() => 
              Array(m).fill(0).map(() => Math.floor(Math.random() * 3))
          );
          const phenotypes = Array(n).fill(0).map(() => Math.random() * 100);

          const res = await apiClient.genomicSelectionService.runGWAS({
          ...
          toast({
              title: "GWAS Analysis Complete",
              description: `Found ${res.significant_markers} significant markers. Run ID: ${res.run_id}`,
          });
          setOpenGWAS(false);
      } catch (err: any) {
          toast({
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Dialog open={openGWAS} onOpenChange={setOpenGWAS}>
              ...
              <DialogContent>
                  <DialogHeader>
                      <DialogTitle>Run GWAS Analysis</DialogTitle>
                      <DialogDescription>
                          Genome-Wide Association Study using Bio-Analytics Engine.
                      </DialogDescription>
                  </DialogHeader>
                  <div className="grid gap-4 py-4">
                      <div className="grid grid-cols-4 items-center gap-4">
                          <Label className="text-right">Run Name</Label>
                          <Input value={gwasConfig.run_name} onChange={e => setGwasConfig({...gwasConfig, run_name: e.target.value})} className="col-span-3" />
                      </div>
                      <div className="grid grid-cols-4 items-center gap-4">
                          <Label className="text-right">Trait</Label>
                          <Input value={gwasConfig.trait_name} onChange={e => setGwasConfig({...gwasConfig, trait_name: e.target.value})} className="col-span-3" />
                      </div>
                      <div className="grid grid-cols-4 items-center gap-4">
                          ...
                          <Select value={gwasConfig.method} onValueChange={e => setGwasConfig({...gwasConfig, method: e})}>
                              ...
                              <SelectContent>
                                  <SelectItem value="glm">GLM (Fast)</SelectItem>
                                  <SelectItem value="mlm">MLM (Accurate)</SelectItem>
                              </SelectContent>
                          </Select>
                      </div>
                      <div className="grid grid-cols-4 items-center gap-4">
                          <Label className="text-right">Samples</Label>
                          <Input type="number" value={gwasConfig.sample_size} onChange={e => setGwasConfig({...gwasConfig, sample_size: parseInt(e.target.value)})} className="col-span-3" />
                      </div>
                  </div>
                  <div className="flex justify-end">
                      <Button onClick={handleRunGWAS} disabled={runningGWAS}>
                          {runningGWAS ? "Running..." : "Start Analysis"}
                      </Button>
                  </div>
              </DialogContent>
          </Dialog>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {goEnrichment.length > 0 ? goEnrichment.map((go: any, i: number) => (
                  ...
                  <p className="text-muted-foreground text-center py-8">Run a QTL analysis to see GO enrichment results.</p>
                )}
              </div>
            </CardContent>
          </Card>
