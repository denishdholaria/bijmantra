...
export function SelectionIndexCalculator() {
  ...
  const { data: methods } = useQuery({
    ...
    queryFn: async () => {
      try {
        const res = await fetch(`${API_URL}/api/v2/selection/methods`);
  ...
  const { data: defaultWeights } = useQuery({
    ...
    queryFn: async () => {
      try {
        const res = await fetch(`${API_URL}/api/v2/selection/default-weights`);
        if (!res.ok) return null;
        return res.json();
      } catch {
        return null;
  ...
  const smithHazelMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`${API_URL}/api/v2/selection/smith-hazel`, {
        method: 'POST',
  ...
  const desiredGainsMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`${API_URL}/api/v2/selection/desired-gains`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          traits: traits.map(t => t.name),
  ...
  const baseIndexMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`${API_URL}/api/v2/selection/base-index`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          traits: traits.map(t => t.name),
  ...
  const independentCullingMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`${API_URL}/api/v2/selection/independent-culling`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          traits: traits.map(t => t.name),
  ...
  const predictResponseMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`${API_URL}/api/v2/selection/predict-response`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          traits: traits.map(t => t.name),
    ...
    <div className="p-6 space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <div className="overflow-x-auto pb-2">
          <TabsList className="grid grid-cols-5 w-full min-w-[600px]">
              ...
              {traits.map((trait, index) => (
                <div key={index} className="p-3 border rounded-lg space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    />
                    <Button size="sm" variant="ghost" onClick={() => removeTrait(index)} aria-label="Remove Trait">
                      <Trash2 className="h-4 w-4 text-red-500" />
                    </Button>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <Label htmlFor={`heritability-${index}`} className="text-xs">Heritability (h²)</Label>
                      <Input
                        id={`heritability-${index}`}
                        type="number"
                        step="0.01"
                        min="0"
                        max="1"
                    ...
                    <div>
                      <Label htmlFor={`variance-${index}`} className="text-xs">Phenotypic Variance</Label>
                      <Input
                        id={`variance-${index}`}
                        type="number"
                        step="0.1"
                    ...
                    <div>
                      <Label htmlFor={`weight-${index}`} className="text-xs">Economic Weight</Label>
                      <Input
                        id={`weight-${index}`}
                        type="number"
                        step="0.1"
                    ...
                    {activeTab === 'desired-gains' && (
                      <div>
                        <Label htmlFor={`gain-${index}`} className="text-xs">Desired Gain</Label>
                        <Input
                          id={`gain-${index}`}
                          type="number"
                          step="0.1"
                    ...
                    {activeTab === 'independent-culling' && (
                      <div>
                        <Label htmlFor={`threshold-${index}`} className="text-xs">Threshold</Label>
                        <Input
                          id={`threshold-${index}`}
                          type="number"
                          step="0.1"
