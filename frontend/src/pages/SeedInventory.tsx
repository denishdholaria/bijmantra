...
import { SeedLot, SeedInventorySummary } from '@/lib/api-client'
...
export function SeedInventory() {
  ...
  const { data: lotsData, isLoading } = useQuery({
    ...
    queryFn: () => apiClient.inventoryService.getLots(filter !== 'all' ? { status: filter } : undefined),
  })

  // Fetch summary stats from API
  const { data: summaryData } = useQuery({
    queryKey: ['seed-inventory-summary'],
    queryFn: () => apiClient.inventoryService.getSummary(),
  })
