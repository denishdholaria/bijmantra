...
export function Germplasm() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => apiClient.germplasmService.getGermplasm(page, pageSize, search),
  })

  const deleteMutation = useMutation({
    mutationFn: (germplasmDbId: string) => apiClient.germplasmService.deleteGermplasm(germplasmDbId),
    onSuccess: () => {
      ...
      <Card>
        <CardContent className="p-4">
          <form onSubmit={handleSearch} className="flex gap-2 w-full">
            <Label htmlFor="germplasm-search" className="sr-only">Search</Label>
            <Input
              id="germplasm-search"
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <Button
                variant="outline"
                onClick={() => queryClient.invalidateQueries({ queryKey: ['germplasm'] })}
              >
                    ...
                    {germplasm.map((g: {
                      ...
                      <tr key={g.germplasmDbId} className="hover:bg-muted/30">
                        ...
                        <td className="px-6 py-4 text-right space-x-2">
                          ...
                          <Link to={`/germplasm/${g.germplasmDbId}/edit`} className="text-blue-700 dark:text-blue-400 hover:underline text-sm">Edit</Link>
                          <button onClick={() => setDeleteId(g.germplasmDbId)} className="text-red-700 dark:text-red-400 hover:underline text-sm">Delete</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
