...
import { CheckCircle2, XCircle, HelpCircle, RefreshCw, Dna, Users, FlaskConical } from 'lucide-react'
...
export function ParentageAnalysis() {
  ...
  const getConclusionBadge = (conclusion: string) => {
    switch (conclusion) {
      case 'CONFIRMED':
        return <Badge className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"><CheckCircle2 className="h-3 w-3 mr-1" />Confirmed</Badge>
      case 'EXCLUDED':
        return <Badge className="bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"><XCircle className="h-3 w-3 mr-1" />Excluded</Badge>
      case 'INCONCLUSIVE':
        return <Badge className="bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400"><HelpCircle className="h-3 w-3 mr-1" />Inconclusive</Badge>
      default:
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'match':
        return <Badge className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">Match</Badge>
      case 'female_exclusion':
        return <Badge className="bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400">♀ Exclusion</Badge>
      case 'male_exclusion':
        return <Badge className="bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400">♂ Exclusion</Badge>
      default:
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <Card className="bg-green-50 border-green-200 dark:bg-green-900/20 dark:border-green-800">
          <CardContent className="pt-4 text-center">
            <p className="text-3xl font-bold text-green-700 dark:text-green-400">{stats.confirmed}</p>
            <p className="text-sm text-green-600 dark:text-green-400">Confirmed</p>
          </CardContent>
        </Card>
        <Card className="bg-red-50 border-red-200 dark:bg-red-900/20 dark:border-red-800">
          <CardContent className="pt-4 text-center">
            <p className="text-3xl font-bold text-red-700 dark:text-red-400">{stats.excluded}</p>
            <p className="text-sm text-red-600 dark:text-red-400">Excluded</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 text-center">
            <p className="text-3xl font-bold text-purple-600 dark:text-purple-400">{stats.confirmation_rate}%</p>
                ...
                <div className="mt-6 space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded text-center">
                      <p className="text-2xl font-bold text-blue-700 dark:text-blue-400">{verificationResult.summary.total_markers}</p>
                      <p className="text-xs text-blue-600 dark:text-blue-400">Markers Tested</p>
                    </div>
                    <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded text-center">
                      <p className="text-2xl font-bold text-green-700 dark:text-green-400">{verificationResult.summary.matches}</p>
                      <p className="text-xs text-green-600 dark:text-green-400">Matches</p>
                    </div>
                    <div className="p-3 bg-red-50 dark:bg-red-900/20 rounded text-center">
                      <p className="text-2xl font-bold text-red-700 dark:text-red-400">{verificationResult.summary.female_exclusions}</p>
                      <p className="text-xs text-red-600 dark:text-red-400">♀ Exclusions</p>
                    </div>
                    <div className="p-3 bg-orange-50 dark:bg-orange-900/20 rounded text-center">
                      <p className="text-2xl font-bold text-orange-700 dark:text-orange-400">{verificationResult.summary.male_exclusions}</p>
                      <p className="text-xs text-orange-600 dark:text-orange-400">♂ Exclusions</p>
                    </div>
                  </div>
                ...
                <div className="mt-6 space-y-4">
                  {findParentsResult.likely_parents.length > 0 && (
                    <div className="p-4 bg-green-50 border border-green-200 rounded-lg dark:bg-green-900/20 dark:border-green-800">
                      <h4 className="font-semibold text-green-800 dark:text-green-400 mb-2">Likely Parents (No Exclusions)</h4>
                      <div className="flex flex-wrap gap-2">
                        {findParentsResult.likely_parents.map(p => (
                          <Badge key={p.candidate_id} className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">
                  ...
                  <div className="space-y-2">
                    {findParentsResult.all_candidates.map((cand, idx) => (
                      <div key={cand.candidate_id} className={`p-3 border rounded-lg ${cand.likely_parent ? 'border-green-500 bg-green-50 dark:border-green-700 dark:bg-green-900/20' : ''}`}>
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="flex items-center gap-4 text-sm">
                            <span className="text-green-600 dark:text-green-400">{cand.matches} matches</span>
                            <span className="text-red-600 dark:text-red-400">{cand.exclusions} exclusions</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
