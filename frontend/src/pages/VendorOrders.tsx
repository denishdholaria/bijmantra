...
export function VendorOrders() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => apiClient.platesService.getVendorOrders({
  ...
  const createMutation = useMutation({
    mutationFn: (data: typeof newOrder) => apiClient.platesService.createVendorOrder(data),
    onSuccess: () => {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) => apiClient.platesService.updateVendorOrderStatus(id, status),
    onSuccess: () => {
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          <DialogTrigger asChild><Button><Package className="h-4 w-4 mr-2" /> New Order</Button></DialogTrigger>
          <DialogContent>
