...
import { AlertCircle, Cloud, Droplets, Wind, Sun, Thermometer, Calendar, Activity, RefreshCw, Leaf } from 'lucide-react'
...
interface Forecast {
  ...
  optimal_windows?: ActivityWindow[];
  gdd_forecast?: GDDData[];
}
...
export function Weather() {
  ...
  const locations: Location[] = locationsData?.result?.data || []

  // Get location name for selected location
  const selectedLocationData = locations.find((l) => l.locationDbId === selectedLocation)
  const locationName = selectedLocationData?.locationName || 'Research Station'

  // Fetch weather forecast from real API
  const { data: forecast, isLoading, error, refetch } = useQuery<Forecast>({
    queryKey: ['weather-forecast', selectedLocation, forecastDays],
    queryFn: async () => {
      const data = await apiClient.weatherService.getWeatherForecast(selectedLocation, locationName, forecastDays)
        ...
        <>
          ...
          <Tabs defaultValue="forecast" className="space-y-4">
            <TabsList>
              ...
              <TabsTrigger value="ag-metrics" className="flex items-center gap-2">
            ...
            {/* Ag Metrics Tab */}
            <TabsContent value="ag-metrics">
              <Card>
                <CardHeader>
                  <CardTitle>Agricultural Metrics</CardTitle>
                  <CardDescription>Soil and atmospheric conditions for crop management</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b">
                          <th className="text-left py-2 px-3">Date</th>
                          <th className="text-right py-2 px-3">Soil Moisture</th>
                          <th className="text-right py-2 px-3">Soil Temp</th>
                          <th className="text-right py-2 px-3">ET₀</th>
                          <th className="text-right py-2 px-3">Solar Rad.</th>
                          <th className="text-right py-2 px-3">VPD</th>
                        </tr>
                      </thead>
                      <tbody>
                        {forecast.daily_forecast?.slice(0, forecastDays).map((day, i) => (
                          <tr key={i} className="border-b hover:bg-muted/50">
                            <td className="py-2 px-3">{formatDate(day.date)}</td>
                            <td className="text-right py-2 px-3 font-mono">
                              {day.soil_moisture ? `${day.soil_moisture.toFixed(1)}%` : '-'}
                            </td>
                            <td className="text-right py-2 px-3 font-mono">
                              {day.soil_temperature ? `${day.soil_temperature.toFixed(1)}°C` : '-'}
                            </td>
                            <td className="text-right py-2 px-3 font-mono">
                              {day.et0 ? `${day.et0.toFixed(2)} mm` : '-'}
                            </td>
                            <td className="text-right py-2 px-3 font-mono">
                              {day.solar_radiation ? `${(day.solar_radiation / 3.6).toFixed(1)} wh/m²` : '-'} 
                            </td>
                            <td className="text-right py-2 px-3 font-mono">
                              {day.vapor_pressure_deficit ? `${day.vapor_pressure_deficit.toFixed(2)} kPa` : '-'}
                            </td>
                          </tr>
                        ))}
                      </tbody>
