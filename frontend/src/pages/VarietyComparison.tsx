...
import { apiClient, VarietyData } from '@/lib/api-client'
...
export function VarietyComparison() {
  ...
  const { data: varietiesData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.varietyComparisonService.getVarieties()
  ...
  if (error) {
    return (
      <div className="p-12 text-center">
        ...
        <h3 className="text-lg font-semibold text-red-600 mb-2">Error Loading Varieties</h3>
        <p className="text-muted-foreground mb-4">{error instanceof Error ? error.message : 'Unknown error'}</p>
        <Button variant="outline" onClick={() => queryClient.invalidateQueries({ queryKey: ['variety-comparison-varieties'] })}>
