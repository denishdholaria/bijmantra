...
const TRAIT_DEMANDS: TraitDemand[] = [
...
export function MarketAnalysis() {
  ...
  const filteredData = useMemo(() => MARKET_DATA.filter(d =>
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">${Math.round(MARKET_DATA.reduce((s, d) => s + d.price, 0) / MARKET_DATA.length)}/ton</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{MARKET_DATA.filter(d => d.demand === 'high').length}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{new Set(MARKET_DATA.map(d => d.region)).size}</div>
      ...
      <div className="flex gap-4">
        ...
        <Select value={selectedRegion} onValueChange={setSelectedRegion}>
          ...
          <SelectContent>
            ...
            {[...new Set(MARKET_DATA.map(d => d.region))].map(r => (<SelectItem key={r} value={r}>{r}</SelectItem>))}
          </SelectContent>
        </Select>
      </div>
