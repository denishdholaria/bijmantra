...
export function PlotHistory() {
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: () => apiClient.plotHistoryService.getPlotHistoryStats(selectedField !== 'all' ? selectedField : undefined),
  })

  // Fetch event types
  const { data: eventTypesData } = useQuery({
    queryKey: ['plot-history-event-types'],
    queryFn: () => apiClient.plotHistoryService.getPlotHistoryEventTypes(),
  })

  // Fetch fields
  const { data: fieldsData } = useQuery({
    queryKey: ['plot-history-fields'],
    queryFn: () => apiClient.plotHistoryService.getPlotHistoryFields(),
  })

  // Fetch plots
  const { data: plotsData, isLoading: plotsLoading } = useQuery({
    queryKey: ['plot-history-plots', selectedField, searchQuery],
    queryFn: () => apiClient.plotHistoryService.getPlotHistoryPlots({
  ...
  const { data: selectedPlot, isLoading: plotLoading } = useQuery({
    ...
    queryFn: () => selectedPlotId ? apiClient.plotHistoryService.getPlotHistoryPlot(selectedPlotId) : null,
    enabled: !!selectedPlotId,
  })

  // Create event mutation
  const createEventMutation = useMutation({
    mutationFn: ({ plotId, data }: { plotId: string; data: Parameters<typeof apiClient.plotHistoryService.createPlotHistoryEvent>[1] }) =>
      apiClient.plotHistoryService.createPlotHistoryEvent(plotId, data),
    onSuccess: () => {
  ...
  const getEventColor = (type: string) => {
    const colors: Record<string, string> = {
      planting: 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400',
      observation: 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400',
      treatment: 'bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400',
      harvest: 'bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-400',
      maintenance: 'bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-400',
      sampling: 'bg-cyan-100 text-cyan-800 dark:bg-cyan-900/20 dark:text-cyan-400',
    }
    ...
    return (
      <div className="flex items-center justify-center h-[50vh]">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Please select a workspace to view plot history.</p>
        </div>
      </div>
    )
  }
              ...
              <div className="divide-y max-h-[400px] overflow-auto">
                {plots.map((plot: Plot) => (
                  <div
                    ...
                    role="button"
                    tabIndex={0}
                    aria-label={`Select plot ${plot.name}`}
                  >
