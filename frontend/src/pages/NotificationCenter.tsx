...
export function NotificationCenter() {
  ...
  const { data: statsData } = useQuery({
    ...
    queryFn: () => apiClient.notificationService.getSummary(),
  })

  const stats = statsData?.data

  // UI structure for preferences form - these are default values for the form controls
  // Not demo data - just initial form state until backend preferences API is implemented
  ...
  const [quietHours, setQuietHours] = useState({ enabled: false, start_time: '22:00', end_time: '07:00' })
  const preferencesLoading = false

  // Mark read mutation
  const markReadMutation = useMutation({
    mutationFn: (notificationId: string) => apiClient.notificationService.markAsRead(notificationId),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (notificationId: string) => apiClient.notificationService.deleteNotification(notificationId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['notifications'] })
      queryClient.invalidateQueries({ queryKey: ['notification-stats'] })
