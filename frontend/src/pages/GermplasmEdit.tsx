...
export function GermplasmEdit() {
  ...
  useEffect(() => {
    if (data?.result) {
      const g = data.result as GermplasmResult
      reset({
  ...
  const mutation = useMutation({
    mutationFn: (formData: GermplasmFormData) => {
      const payload = {
        ...
        biologicalStatusOfAccessionCode: formData.biologicalStatusOfAccessionCode || undefined,
      ...
      return apiClient.germplasmService.updateGermplasm(germplasmDbId!, payload)
    },
    onSuccess: () => {
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" asChild aria-label="Back to germplasm detail">
              <Link to={`/germplasm/${germplasmDbId}`}>←</Link>
            </Button>
            <div>
              ...
              <TabsContent value="basic" className="space-y-4 mt-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    {errors.germplasmName && <p className="text-sm text-red-600 dark:text-red-400">{errors.germplasmName.message}</p>}
                  </div>
                  <div className="space-y-2">
