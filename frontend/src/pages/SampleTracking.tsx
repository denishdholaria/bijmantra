...
import { Package, Search, QrCode, MapPin, Clock, CheckCircle, AlertCircle, Truck, Loader2, RefreshCw } from 'lucide-react'
import { apiClient, Sample } from '@/lib/api-client'
...
export function SampleTracking() {
  ...
  const { data: samplesData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.sampleTrackingService.getSamples(params)
  ...
  const { data: statsData } = useQuery({
    ...
    queryFn: () => apiClient.sampleTrackingService.getStats(),
  })
  ...
  const getStatusColor = (status: string) => {
    const colors: Record<string, string> = { 
      ...
      stored: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
  ...
  const getTypeColor = (type: string) => {
    const colors: Record<string, string> = { 
      leaf: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
      seed: 'bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300',
      dna: 'bg-pink-100 text-pink-800 dark:bg-pink-900/30 dark:text-pink-300'
    }
  ...
  if (error) {
    return (
      <div className="p-12 text-center">
        ...
        <Button variant="outline" onClick={() => queryClient.invalidateQueries({ queryKey: ['sample-tracking'] })}>
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 grid-cols-2 md:grid-cols-5">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{stats.collected}</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              <p className="text-2xl font-bold text-green-600 dark:text-green-400">{stats.stored}</p>
