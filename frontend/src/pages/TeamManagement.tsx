...
import { teamManagementAPI } from '@/lib/api-client'
...
export function TeamManagement() {
  ...
  const { data: membersData, isLoading: membersLoading } = useQuery({
    ...
    queryFn: () => teamManagementAPI.getMembers({
  ...
  const { data: teamsData } = useQuery({
    queryKey: ['teams'],
    queryFn: () => teamManagementAPI.getTeams(),
  })

  // Fetch invites
  const { data: invitesData } = useQuery({
    queryKey: ['team-invites'],
    queryFn: () => teamManagementAPI.getInvites(),
  })

  // Fetch stats
  const { data: statsData } = useQuery({
    queryKey: ['team-stats'],
    queryFn: () => teamManagementAPI.getStats(),
  })

  // Update member mutation
  const updateMemberMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => teamManagementAPI.updateMember(id, data),
    onSuccess: () => {
  ...
  const deleteMemberMutation = useMutation({
    mutationFn: (id: string) => teamManagementAPI.deleteMember(id),
    onSuccess: () => {
  ...
  const resendInviteMutation = useMutation({
    mutationFn: (id: string) => teamManagementAPI.resendInvite(id),
    onSuccess: () => {
  ...
  const deleteInviteMutation = useMutation({
    mutationFn: (id: string) => teamManagementAPI.deleteInvite(id),
    onSuccess: () => {
                  ...
                  {members.map((member: any) => (
                    <div key={member.id} className="flex items-center justify-between p-4">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => deleteMemberMutation.mutate(member.id)}><Trash2 className="h-4 w-4" /></Button>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  {invites.map((invite: any) => (
                    <div key={invite.id} className="flex items-center justify-between p-4 border rounded-lg">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => deleteInviteMutation.mutate(invite.id)}><Trash2 className="h-4 w-4" /></Button>
                      </div>
                    </div>
                  ))}
                </div>
