...
import { GeneticGainData, HeritabilityData, SelectionResponseData, QuickInsight } from '@/lib/api/analytics/types'
...
export function AnalyticsDashboard() {
  ...
  const { data: analyticsData, isLoading, refetch } = useQuery({
    ...
    queryFn: () => apiClient.analyticsService.getAnalytics({
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="icon" aria-label="Refresh analytics" onClick={() => refetch()}>
                      ...
                      {correlations.matrix[ri].map((val: number, ci: number) => (
                        <div
                          ...
                          className={`p-2 text-center rounded ${
                            ri === ci ? 'bg-gray-200 dark:bg-gray-700' :
                            val > 0.3 ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' :
                            val > 0 ? 'bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400' :
                            val < -0.2 ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400' :
