...
export function Statistics() {
  ...
  const { data: trialsData } = useQuery({
    ...
    queryFn: () => apiClient.statisticsService.getTrials(),
  })

  // Fetch traits
  const { data: traitsData } = useQuery({
    queryKey: ['statistics-traits'],
    queryFn: () => apiClient.statisticsService.getTraits(),
  })

  // Fetch overview
  const { data: overview, isLoading: overviewLoading } = useQuery({
    queryKey: ['statistics-overview', selectedTrial],
    queryFn: () => apiClient.statisticsService.getOverview(selectedTrial),
  })

  // Fetch summary stats
  const { data: summaryData, isLoading: summaryLoading } = useQuery({
    queryKey: ['statistics-summary', selectedTrial],
    queryFn: () => apiClient.statisticsService.getSummary({ trial_id: selectedTrial }),
  })

  // Fetch correlations
  const { data: correlationsData, isLoading: correlationsLoading } = useQuery({
    queryKey: ['statistics-correlations', selectedTrial],
    queryFn: () => apiClient.statisticsService.getCorrelations({ trial_id: selectedTrial }),
  })
  ...
  const { data: distributionData, isLoading: distributionLoading } = useQuery({
    ...
    queryFn: () => apiClient.statisticsService.getDistribution(selectedDistTrait, { trial_id: selectedTrial }),
  })
