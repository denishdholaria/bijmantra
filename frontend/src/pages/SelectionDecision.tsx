...
export function SelectionDecision() {
  ...
  const reevuTaskContext = useMemo(() => ({
    entity_type: 'selection_candidate',
    selected_entity_ids: candidates.map(candidate => candidate.id),
    active_filters: {
      ...(selectedProgram ? { program_id: selectedProgram } : {}),
      ...(selectedTrial ? { trial_id: selectedTrial } : {}),
      ...(decisionFilter ? { decision_status: decisionFilter } : {}),
      ...(searchQuery ? { search: searchQuery } : {}),
      pending_changes_count: Object.keys(localDecisions).length,
    },
    visible_columns: ['name', 'generation', 'gebv', 'yield_estimate', 'pedigree', 'traits', 'current_decision', 'notes'],
    attached_context: candidates.map(candidate => ({
      ...
      metadata: {
        ...
        gebv: candidate.gebv,
        yield_estimate: candidate.yield_estimate,
        pedigree: candidate.pedigree,
        traits: candidate.traits,
        current_decision: localDecisions[candidate.id]?.decision || candidate.decision || 'pending',
        pending_notes: localDecisions[candidate.id]?.notes || candidate.decision_notes || null,
        advanced_count: stats.advanced,
        rejected_count: stats.rejected,
        on_hold_count: stats.on_hold,
      },
    })),
  }), [candidates, decisionFilter, localDecisions, searchQuery, selectedProgram, selectedTrial, stats.advanced, stats.on_hold, stats.rejected])
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 text-sm mb-2">
                            <span className="flex items-center gap-1"><TrendingUp className="h-4 w-4 text-green-600" />GEBV: {formatMetric(candidate.gebv)}</span>
                            {candidate.yield_estimate !== null && candidate.yield_estimate !== undefined && <span>Yield: {candidate.yield_estimate} t/ha</span>}
                        ...
                        <div className="flex gap-2 ml-4">
                          <Button
                            variant={currentDecision === 'advance' ? 'default' : 'outline'}
                            size="sm"
                            onClick={() => setDecision(candidate.id, 'advance')}
                            onContextMenu={(e) => { e.preventDefault(); handleDecisionWithNotes(candidate.id, 'advance') }}
                            className={currentDecision !== 'advance' ? 'text-green-600 hover:text-green-700 hover:bg-green-50' : ''}
                          ...
                          <Button
                            ...
                            className={currentDecision !== 'hold' ? 'text-yellow-600 hover:text-yellow-700 hover:bg-yellow-50' : ''}
                            title="Left-click to hold, right-click to add notes"
                            aria-label="Hold Candidate"
                          >
                          ...
                          <Button
                            ...
                            className={currentDecision !== 'reject' ? 'text-red-600 hover:text-red-700 hover:bg-red-50' : ''}
