...
import { selectionDecisionsAPI, SelectionCandidate, SelectionStatistics } from '@/lib/api-client'
...
export function SelectionDecision() {
  ...
  const { data: candidatesData, isLoading: isLoadingCandidates, refetch } = useQuery({
    ...
    queryFn: () => selectionDecisionsAPI.getCandidates({
      program_id: selectedProgram || undefined,
      trial_id: selectedTrial || undefined,
      decision_status: decisionFilter || undefined,
  ...
  const { data: statsData, isLoading: isLoadingStats } = useQuery({
    ...
    queryFn: () => selectionDecisionsAPI.getStatistics({
      program_id: selectedProgram || undefined,
      trial_id: selectedTrial || undefined,
    }),
  })
  ...
  const { data: programsData } = useQuery({
    ...
    queryFn: () => selectionDecisionsAPI.getPrograms(),
  })

  const { data: trialsData } = useQuery({
    queryKey: ['selection-trials', selectedProgram],
    queryFn: () => selectionDecisionsAPI.getTrials(selectedProgram || undefined),
  })

  const decisionMutation = useMutation({
    mutationFn: ({ candidateId, decision, notes }: { candidateId: string; decision: string; notes?: string }) =>
      selectionDecisionsAPI.recordDecision(candidateId, decision, notes),
  ...
  const bulkSaveMutation = useMutation({
    mutationFn: () => {
      ...
      return selectionDecisionsAPI.recordBulkDecisions(decisions)
    },
    onSuccess: (data) => {
