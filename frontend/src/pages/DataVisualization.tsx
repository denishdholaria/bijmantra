...
import { ChartConfig, ChartTypeInfo, DataSourceInfo } from '@/lib/api/analytics/visualization'
...
export function DataVisualization() {
  ...
  const { data: chartsData, isLoading: chartsLoading } = useQuery({
    ...
    queryFn: () => apiClient.dataVisualizationService.getCharts({
  ...
  const { data: statsData } = useQuery({
    ...
    queryFn: () => apiClient.dataVisualizationService.getStatistics(),
  })
  ...
  const { data: sourcesData } = useQuery({
    ...
    queryFn: () => apiClient.dataVisualizationService.getDataSources(),
  })

  const dataSources: DataSourceInfo[] = sourcesData?.data || []

  // Fetch chart types
  const { data: typesData } = useQuery({
    queryKey: ['visualizations-types'],
    queryFn: () => apiClient.dataVisualizationService.getChartTypes(),
  })

  const chartTypes: ChartTypeInfo[] = typesData?.data || []

  // Delete mutation
  const deleteMutation = useMutation({
    mutationFn: (chartId: string) => apiClient.dataVisualizationService.deleteChart(chartId),
    onSuccess: () => {
  ...
  const handleExport = async (chartId: string) => {
    try {
      const result = await apiClient.dataVisualizationService.exportChart(chartId, 'png')
