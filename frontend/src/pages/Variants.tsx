...
export function Variants() {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: () => apiClient.genotypingResultsService.getVariants({
  ...
  const { data: refsData } = useQuery({
    ...
    queryFn: () => apiClient.genomicMapService.getReferences({ pageSize: 100 }),
    enabled: !!activeWorkspace
  })
  ...
  const createMutation = useMutation({
    mutationFn: (data: Partial<Variant> & { variantSetDbId: string }) => apiClient.genotypingResultsService.createVariant(data),
    onSuccess: () => {
  ...
  const variants = useMemo(() => (data?.result?.data || []) as Variant[], [data])
  const references = useMemo(() => (refsData?.result?.data || []) as Reference[], [refsData])
  const variantSets = useMemo(() => (variantSetsData?.result?.data || []) as VariantSet[], [variantSetsData])
  ...
  const stats = useMemo(() => ({
    total: totalCount,
    snps: variants.filter((v) => v.variantType === 'SNP').length,
    indels: variants.filter((v) => v.variantType === 'INDEL').length,
