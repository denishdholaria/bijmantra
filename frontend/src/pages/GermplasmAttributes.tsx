...
import { Loader2, Plus, Search, Tags, Database, RefreshCw, Trash2 } from 'lucide-react'
import { apiClient } from '@/lib/api-client'
import { GermplasmAttribute } from '@/lib/api/brapi/attributes'
...
export function GermplasmAttributes() {
  ...
  const { data: attributesData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.germplasmAttributesService.getAttributes(params)
  ...
  const { data: categoriesData } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.germplasmAttributesService.getCategories()
      return response.result.data
    },
  })
  ...
  const createMutation = useMutation({
    mutationFn: (data: typeof newAttribute) => apiClient.germplasmAttributesService.createAttribute(data),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiClient.germplasmAttributesService.deleteAttribute(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['germplasm-attributes'] })
  ...
  const getDataTypeBadge = (dataType?: string) => {
    switch (dataType?.toLowerCase()) {
      case 'categorical': return <Badge variant="outline" className="bg-purple-50 dark:bg-purple-900/30">Categorical</Badge>
      case 'numerical': return <Badge variant="outline" className="bg-blue-50 dark:bg-blue-900/30">Numerical</Badge>
      case 'scale': return <Badge variant="outline" className="bg-green-50 dark:bg-green-900/30">Scale</Badge>
      case 'text': return <Badge variant="outline" className="bg-gray-50 dark:bg-gray-800">Text</Badge>
  ...
  if (error) {
    return (
      <div className="p-12 text-center">
        ...
        <h3 className="text-lg font-semibold text-red-600 mb-2">Error Loading Attributes</h3>
        <p className="text-muted-foreground mb-4">{error instanceof Error ? error.message : 'Unknown error'}</p>
        <Button variant="outline" onClick={() => queryClient.invalidateQueries({ queryKey: ['germplasm-attributes'] })}>
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (confirm('Are you sure you want to delete this attribute?')) {
                                deleteMutation.mutate(attr.attributeDbId)
                              }
                            }}
                          >
