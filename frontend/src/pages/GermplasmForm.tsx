...
export function GermplasmForm() {
  ...
  const mutation = useMutation({
    mutationFn: (data: GermplasmFormData) => {
      ...
      return apiClient.germplasmService.createGermplasm(payload)
    },
    onSuccess: (response) => {
      queryClient.invalidateQueries({ queryKey: ['germplasm'] })
      toast.success('Germplasm created successfully!')
      // Response is single object according to type definition
      const id = response.result?.germplasmDbId
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" asChild aria-label="Back to germplasm list">
              <Link to="/germplasm">←</Link>
            </Button>
            <div>
              <CardTitle>Add New Germplasm</CardTitle>
              <CardDescription>Register a new genetic material entry (BrAPI-compatible)</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
