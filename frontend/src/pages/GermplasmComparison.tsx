...
import { apiClient, ComparisonGermplasm, ComparisonTrait } from '@/lib/api-client';
...
export function GermplasmComparison() {
  ...
  const { data: listData, isLoading: isLoadingList } = useQuery({
    ...
    queryFn: () => apiClient.germplasmComparisonService.listGermplasm({ search: searchQuery || undefined, limit: 50 }),
  });
  ...
  const { data: comparisonData, isLoading: isLoadingComparison, refetch: refetchComparison } = useQuery({
    ...
    queryFn: () => apiClient.germplasmComparisonService.compare(selectedIds),
  ...
  const { data: traitsData } = useQuery({
    ...
    queryFn: () => apiClient.germplasmComparisonService.getTraits(),
  });
  ...
  const comparisonTraits: ComparisonTrait[] = comparisonData?.traits || traitsData?.data || [];
  ...
  const getResistanceColor = (value: string): string => {
    switch (value) {
      case 'R': case 'T': return 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400';
      case 'MR': case 'MT': return 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400';
      case 'MS': return 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400';
      case 'S': return 'bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400';
      case 'HS': return 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400';
