        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {selectedGermplasm.map(g => (
            <Card key={g.id} className="relative">
              <Button variant="ghost" size="icon" className="absolute top-2 right-2 h-6 w-6" onClick={() => removeGermplasm(g.id)}><X className="h-4 w-4" /></Button>
              <CardHeader className="pb-2">
                <div className="flex items-start gap-2">
                  <Button variant="ghost" size="icon" className="h-6 w-6 -ml-1" onClick={() => toggleFavorite(g.id)}>
