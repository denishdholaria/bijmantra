...
export function DataValidation() {
  ...
  const { data: statsData } = useQuery({
    ...
    queryFn: () => apiClient.dataValidationService.getStats(),
  })

  // Fetch validation issues
  const { data: issuesData, isLoading: issuesLoading } = useQuery({
    queryKey: ['validation-issues'],
    queryFn: () => apiClient.dataValidationService.getIssues({ status: 'open', limit: 50 }),
  })

  // Fetch validation rules
  const { data: rulesData } = useQuery({
    queryKey: ['validation-rules'],
    queryFn: () => apiClient.dataValidationService.getRules(),
  })

  // Run validation mutation
  const runValidationMutation = useMutation({
    mutationFn: () => apiClient.dataValidationService.runValidation(),
    onSuccess: (data) => {
  ...
  const updateIssueMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) => 
      apiClient.dataValidationService.updateIssueStatus(id, { status: status as any }),
    onSuccess: () => {
  ...
  const toggleRuleMutation = useMutation({
    mutationFn: ({ id, enabled }: { id: string; enabled: boolean }) =>
      apiClient.dataValidationService.updateRule(id, { enabled }),
    onSuccess: () => {
  ...
  const deleteIssueMutation = useMutation({
    mutationFn: (id: string) => apiClient.dataValidationService.deleteIssue(id),
    onSuccess: () => {
  ...
  const handleExport = async () => {
    try {
      const response = await apiClient.dataValidationService.exportReport('json')
      const data = response.data; // exportReport likely returns ApiSuccessResponse
                    ...
                    <div key={issue.id} className={`p-4 border rounded-lg ${issue.type === 'error' ? 'border-red-200 bg-red-50' : issue.type === 'warning' ? 'border-yellow-200 bg-yellow-50' : 'border-blue-200 bg-blue-50'}`}>
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex gap-1">
                          <Button variant="ghost" size="icon" aria-label="View issue"><Eye className="h-4 w-4" /></Button>
                          <Button variant="ghost" size="icon" aria-label="Resolve issue" onClick={() => updateIssueMutation.mutate({ id: issue.id, status: 'resolved' })}><CheckCircle2 className="h-4 w-4" /></Button>
                          <Button variant="ghost" size="icon" aria-label="Delete issue" onClick={() => deleteIssueMutation.mutate(issue.id)}><Trash2 className="h-4 w-4" /></Button>
                        </div>
                      </div>
                    </div>
                  ))}
