...
import { apiClient, RankedEntry, RankingStatistics } from '@/lib/api-client'
...
export function PerformanceRanking() {
  ...
  const { data: rankingsData, isLoading: isLoadingRankings, refetch } = useQuery({
    ...
    queryFn: () => apiClient.performanceRankingService.getRankings({
  ...
  const { data: statsData, isLoading: isLoadingStats } = useQuery({
    ...
    queryFn: () => apiClient.performanceRankingService.getStatistics({
      program_id: selectedProgram || undefined,
      trial_id: selectedTrial || undefined,
    }),
  })
  ...
  const { data: programsData } = useQuery({
    ...
    queryFn: () => apiClient.performanceRankingService.getPrograms(),
  })

  const { data: trialsData } = useQuery({
    queryKey: ['performance-trials', selectedProgram],
    queryFn: () => apiClient.performanceRankingService.getTrials(selectedProgram || undefined),
  })
  ...
  const getRankChange = (current: number, previous: number) => {
    if (current < previous) return { icon: <ArrowUp className="h-4 w-4 text-green-500 dark:text-green-400" />, text: `+${previous - current}`, color: 'text-green-500 dark:text-green-400' }
    if (current > previous) return { icon: <ArrowDown className="h-4 w-4 text-red-500 dark:text-red-400" />, text: `-${current - previous}`, color: 'text-red-500 dark:text-red-400' }
    return { icon: <Minus className="h-4 w-4 text-gray-400 dark:text-gray-500" />, text: '0', color: 'text-gray-400 dark:text-gray-500' }
  }

  const getMedalColor = (rank: number) => {
    if (rank === 1) return 'bg-yellow-100 text-yellow-700 border-yellow-300 dark:bg-yellow-900/20 dark:text-yellow-400 dark:border-yellow-800'
    if (rank === 2) return 'bg-gray-100 text-gray-700 border-gray-300 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-700'
    if (rank === 3) return 'bg-orange-100 text-orange-700 border-orange-300 dark:bg-orange-900/20 dark:text-orange-400 dark:border-orange-800'
    return 'bg-muted'
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoadingStats ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">{[1,2,3,4,5].map(i => <Skeleton key={i} className="h-24" />)}</div>
      ) : statistics && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          <Card><CardContent className="p-4"><div className="flex items-center gap-3"><div className="p-2 bg-blue-100 dark:bg-blue-900/20 rounded-lg"><BarChart3 className="h-5 w-5 text-blue-600 dark:text-blue-400" /></div><div><div className="text-2xl font-bold">{statistics.total_entries}</div><div className="text-sm text-muted-foreground">Total Entries</div></div></div></CardContent></Card>
          <Card><CardContent className="p-4"><div className="flex items-center gap-3"><div className="p-2 bg-green-100 dark:bg-green-900/20 rounded-lg"><TrendingUp className="h-5 w-5 text-green-600 dark:text-green-400" /></div><div><div className="text-2xl font-bold">{statistics.avg_yield?.toFixed(1) || '-'}</div><div className="text-sm text-muted-foreground">Avg Yield</div></div></div></CardContent></Card>
          <Card><CardContent className="p-4"><div className="flex items-center gap-3"><div className="p-2 bg-purple-100 dark:bg-purple-900/20 rounded-lg"><Star className="h-5 w-5 text-purple-600 dark:text-purple-400" /></div><div><div className="text-2xl font-bold">{statistics.avg_gebv?.toFixed(2) || '-'}</div><div className="text-sm text-muted-foreground">Avg GEBV</div></div></div></CardContent></Card>
          <Card><CardContent className="p-4"><div className="flex items-center gap-3"><div className="p-2 bg-yellow-100 dark:bg-yellow-900/20 rounded-lg"><Trophy className="h-5 w-5 text-yellow-600 dark:text-yellow-400" /></div><div><div className="text-lg font-bold truncate">{statistics.top_performer || '-'}</div><div className="text-sm text-muted-foreground">Top Performer</div></div></div></CardContent></Card>
          <Card><CardContent className="p-4"><div className="flex items-center gap-3"><div className="p-2 bg-emerald-100 dark:bg-emerald-900/20 rounded-lg"><ArrowUp className="h-5 w-5 text-emerald-600 dark:text-emerald-400" /></div><div><div className="text-lg font-bold truncate">{statistics.most_improved ? `${statistics.most_improved.name} (+${statistics.most_improved.improvement})` : '-'}</div><div className="text-sm text-muted-foreground">Most Improved</div></div></div></CardContent></Card>
        </div>
      )}

      {isLoadingRankings ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">{[1,2,3].map(i => <Skeleton key={i} className="h-48" />)}</div>
      ) : topThree.length > 0 && (
        ...
        <CardContent className="p-4">
          <div className="flex flex-wrap items-center gap-4">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <Input placeholder="Search entries..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" aria-label="Search entries" />
            </div>
                ...
                {rankings.length === 0 ? (
                  ...
                  return (
                    <div key={entry.id} className="flex items-center gap-4 p-4 border rounded-lg hover:bg-muted/50 transition-colors">
                      ...
                      <Button variant="ghost" size="icon" aria-label="Star entry"><Star className="h-4 w-4" /></Button>
                    </div>
                  )
                })}
              </div>
