...
export function TraitDetail() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => apiClient.observationService.getObservationVariable(id!),
    enabled: !!id,
  })

  const deleteMutation = useMutation({
    mutationFn: () => apiClient.observationService.deleteObservationVariable(id!),
    onSuccess: () => {
  ...
  const variable: ObservationVariable | undefined = data?.result
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" aria-label="Back to traits" asChild><Link to="/traits">←</Link></Button>
          <div>
        ...
        <TabsContent value="trait" className="mt-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {variable.trait?.synonyms && variable.trait.synonyms.length > 0 && (
                <div>
