...
export function AdvancedReports() {
  ...
  const { data: statsData } = useQuery({
    ...
    queryFn: () => apiClient.reportsService.getStats(),
  })

  const { data: templatesData, isLoading: templatesLoading } = useQuery({
    queryKey: ['report-templates', selectedCategory, searchQuery],
    queryFn: () => apiClient.reportsService.getTemplates(),
  })

  const { data: schedulesData, isLoading: schedulesLoading } = useQuery({
    queryKey: ['report-schedules'],
    queryFn: () => apiClient.reportsService.getSchedules(),
  })

  const { data: generatedData, isLoading: generatedLoading } = useQuery({
    queryKey: ['generated-reports'],
    queryFn: () => apiClient.reportsService.getGeneratedReports(),
  })

  // Mutations
  const generateMutation = useMutation({
    mutationFn: (templateId: string) => apiClient.reportsService.generateReport(templateId, 'pdf'),
    onSuccess: () => {
  ...
  const toggleScheduleMutation = useMutation({
    mutationFn: ({ scheduleId, enabled }: { scheduleId: string; enabled: boolean }) =>
      apiClient.reportsService.toggleSchedule(scheduleId, enabled),
    onSuccess: () => {
  ...
  const runScheduleMutation = useMutation({
    mutationFn: (scheduleId: string) => apiClient.reportsService.runSchedule(scheduleId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['generated-reports'] })
      queryClient.invalidateQueries({ queryKey: ['report-schedules'] })
    },
