...
export function GermplasmAttributeValues() {
  ...
  const { data: germplasmData, isLoading: germplasmLoading, isError: germplasmError } = useQuery({
    queryKey: ['germplasm', 'all'],
    queryFn: () => apiClient.germplasmService.getGermplasm(0, 100),
  })

  // Fetch observation variables as "attributes"
  const { data: variablesData, isLoading: variablesLoading, isError: variablesError } = useQuery({
    queryKey: ['variables', 'all'],
    queryFn: () => apiClient.observationService.getObservationVariables(0, 100),
  })
  ...
  const germplasm = useMemo(() => {
    ...
    return data.map((g: { germplasmDbId: string; germplasmName: string }) => ({ id: g.germplasmDbId, name: g.germplasmName }))
  }, [germplasmData])

  const attributes = useMemo(() => {
    const data = variablesData?.result?.data || []
    return data.map((v: { observationVariableDbId: string; observationVariableName: string }) => ({ id: v.observationVariableDbId, name: v.observationVariableName }))
  }, [variablesData])

  if (germplasmError || variablesError) {
    return (
