...
export function SeedLotForm() {
  ...
  const germplasm = (germplasmData?.result?.data || []) as GermplasmOption[]
  const locations = (locationsData?.result?.data || []) as LocationOption[]
  const programs = (programsData?.result?.data || []) as ProgramOption[]
          ...
          <form onSubmit={handleSubmit((data) => mutation.mutate(data))} className="space-y-6">
            ...
            <div className="flex gap-3 pt-4 border-t">
              <Button type="submit" disabled={mutation.isPending} className="flex-1">
                {mutation.isPending ? <><Package className="h-4 w-4 mr-2 animate-spin" /> Saving...</> : isEdit ? <><Package className="h-4 w-4 mr-2" /> Update Seed Lot</> : <><Package className="h-4 w-4 mr-2" /> Create Seed Lot</>}
              </Button>
