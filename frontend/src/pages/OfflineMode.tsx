...
export function OfflineMode() {
  ...
  const { data: pendingChanges = [], isLoading: loadingPending } = useQuery({
    ...
    queryFn: () => apiClient.offlineSyncService.getPendingChanges(),
  })
  ...
  const deletePendingMutation = useMutation({
    mutationFn: (itemId: string) => apiClient.offlineSyncService.deletePendingChange(itemId),
    onSuccess: () => {
  ...
  const updateCacheMutation = useMutation({
    mutationFn: (category: string) => apiClient.offlineSyncService.updateCache(category),
    onSuccess: () => {
  ...
  const clearCacheMutation = useMutation({
    mutationFn: (category?: string) => apiClient.offlineSyncService.clearCache(category),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['offline-sync'] })
  ...
  const updateSettingsMutation = useMutation({
    mutationFn: (data: Record<string, boolean>) => apiClient.offlineSyncService.updateSettings(data),
    onSuccess: () => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-2">
            {isOnline ? <Wifi className="h-8 w-8 text-green-600 dark:text-green-400" /> : <WifiOff className="h-8 w-8 text-red-600 dark:text-red-400" />}
            Offline Mode
          </h1>
        ...
        <div className="flex items-center gap-4">
          ...
          <Button onClick={handleSync} disabled={!isOnline || isSyncing} aria-label="Sync data now">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-blue-100 dark:bg-blue-900/20 rounded-lg"><HardDrive className="h-5 w-5 text-blue-600 dark:text-blue-400" /></div>
              <div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-yellow-100 dark:bg-yellow-900/20 rounded-lg"><Upload className="h-5 w-5 text-yellow-600 dark:text-yellow-400" /></div>
              <div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-green-100 dark:bg-green-900/20 rounded-lg"><CheckCircle2 className="h-5 w-5 text-green-600 dark:text-green-400" /></div>
              <div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-red-100 dark:bg-red-900/20 rounded-lg"><AlertCircle className="h-5 w-5 text-red-600 dark:text-red-400" /></div>
              <div>
                ...
                <div className="space-y-3">
                  {pendingChanges.map((item) => (
                    <div key={item.id} className="flex items-center justify-between p-4 border rounded-lg">
                      <div className="flex items-center gap-3">
                        {item.status === 'pending' ? <Clock className="h-5 w-5 text-yellow-600 dark:text-yellow-400" /> : <AlertCircle className="h-5 w-5 text-red-600 dark:text-red-400" />}
                        <div>
                          <div className="font-medium">{item.name}</div>
                          <div className="text-sm text-muted-foreground">{item.type} • {item.size}</div>
                          {item.error_message && (
                            <div className="text-xs text-red-600 dark:text-red-400 mt-1">{item.error_message}</div>
                          )}
                        </div>
                      </div>
