...
import { languagesAPI } from '@/lib/api-client'
import {
  Languages,
  Globe,
  Check,
...
export function LanguageSettings() {
  ...
  const { data: languagesData, isLoading: languagesLoading } = useQuery({
    queryKey: ['languages'],
    queryFn: () => languagesAPI.listLanguages(),
  })
  ...
  const { data: keysData } = useQuery({
    ...
    queryFn: () => languagesAPI.listTranslationKeys({ limit: 10 }),
  })
  ...
  const updateLanguageMutation = useMutation({
    mutationFn: ({ code, data }: { code: string; data: { is_enabled?: boolean; is_default?: boolean } }) =>
      languagesAPI.updateLanguage(code, data),
    onSuccess: () => {
  ...
  const exportMutation = useMutation({
    mutationFn: (languageCode: string) => languagesAPI.exportTranslations(languageCode),
    onSuccess: (data) => {
  ...
  const autoTranslateMutation = useMutation({
    mutationFn: (languageCode: string) => languagesAPI.autoTranslate(languageCode),
    onSuccess: (data) => {
      toast.success(data.message)
