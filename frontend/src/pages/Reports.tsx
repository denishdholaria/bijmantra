...
export function Reports() {
  ...
  const { data: studiesData, isLoading: loadingStudies } = useQuery({
    queryKey: ['studies'],
    queryFn: () => apiClient.studyService.getStudies(0, 100),
  })

  const { data: observationsData, isLoading: loadingObs } = useQuery({
    queryKey: ['observations'],
    queryFn: () => apiClient.observationService.getObservations(undefined, 0, 100),
  })
  ...
  }), [programs, germplasm, trials, studies, observations])
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="overview" className="space-y-6 mt-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-4xl font-bold text-green-600 dark:text-green-400">{isLoading ? '-' : stats.totalGermplasm}</div>
                <div className="text-sm text-muted-foreground">Germplasm</div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-4xl font-bold text-blue-600 dark:text-blue-400">{isLoading ? '-' : stats.totalTrials}</div>
                <div className="text-sm text-muted-foreground">Trials</div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-4xl font-bold text-purple-600 dark:text-purple-400">{isLoading ? '-' : stats.totalStudies}</div>
                <div className="text-sm text-muted-foreground">Studies</div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-4xl font-bold text-orange-600 dark:text-orange-400">{isLoading ? '-' : stats.totalObservations}</div>
