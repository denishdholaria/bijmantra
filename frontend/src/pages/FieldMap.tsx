...
import { fieldMapAPI, type Field, type Plot } from '@/lib/api-client'
...
export function FieldMap() {
  ...
  const { data: summary } = useQuery({
    ...
    queryFn: () => fieldMapAPI.getSummary(),
  })

  // Fetch stations
  const { data: stations } = useQuery({
    queryKey: ['fieldMapStations'],
    queryFn: () => fieldMapAPI.getStations(),
  })

  // Fetch plots for selected field
  const { data: plots, isLoading: plotsLoading } = useQuery({
    queryKey: ['fieldPlots', selectedField],
    queryFn: () => selectedField ? fieldMapAPI.getPlots(selectedField) : Promise.resolve([]),
    enabled: !!selectedField,
  })

  // Create field mutation
  const createFieldMutation = useMutation({
    mutationFn: (data: typeof newField) => fieldMapAPI.createField({
      name: data.name,
