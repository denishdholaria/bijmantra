...
interface ObservationVariable {
  observationVariableDbId: string
  observationVariableName: string
  trait?: { traitName: string }
  scale?: {
    dataType: string
    validValues?: { min: number; max: number }
  }
}
...
export function DataCollect() {
  ...
  const { data: unitsData } = useQuery({
    ...
    queryFn: () => apiClient.observationService.getObservationUnits(selectedStudy, 0, 100),
    enabled: !!selectedStudy,
  })

  const studies = studiesData?.result?.data || []
  const variables: ObservationVariable[] = variablesData?.result?.data || []
  const units: ObservationUnit[] = unitsData?.result?.data || []

  const submitMutation = useMutation({
    mutationFn: (data: ObservationEntry[]) => apiClient.observationService.createObservations(data),
    onSuccess: () => {
