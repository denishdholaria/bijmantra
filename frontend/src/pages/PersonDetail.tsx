    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-muted/50 rounded-lg">
        <div className="text-center">
          <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{data.programs.length}</p>
          <p className="text-xs text-muted-foreground">Programs</p>
        </div>
        <div className="text-center">
          <p className="text-2xl font-bold text-green-600 dark:text-green-400">{data.studies.length}</p>
          <p className="text-xs text-muted-foreground">Studies</p>
        </div>
        <div className="text-center">
          <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">{data.observations.toLocaleString()}</p>
          <p className="text-xs text-muted-foreground">Observations</p>
        </div>
        <div className="text-center">
          <p className="text-2xl font-bold text-orange-600 dark:text-orange-400">{data.germplasm}</p>
...
export function PersonDetail() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['person', id],
    queryFn: () => apiClient.peopleService.getPerson(id!),
    enabled: !!id,
  })

  const deleteMutation = useMutation({
    mutationFn: () => apiClient.peopleService.deletePerson(id!),
    onSuccess: () => {
