...
export function VariantSets() {
  ...
  const { data: refSetsData } = useQuery({
    ...
    queryFn: () => apiClient.referencesService.getReferenceSets(),
  })
  ...
  const referenceSets = (refSetsData?.result?.data || []) as ReferenceSet[]
              ...
              <TabsContent value="create" className="space-y-4 py-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={newVS.referenceSetDbId} onValueChange={(v) => setNewVS({ ...newVS, referenceSetDbId: v })}>
                    ...
                    <SelectContent>{referenceSets.map((rs) => <SelectItem key={rs.referenceSetDbId} value={rs.referenceSetDbId}>{rs.referenceSetName}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
              </TabsContent>
