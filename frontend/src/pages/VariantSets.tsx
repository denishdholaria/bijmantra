...
export function VariantSets() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: typeof newVS) => apiClient.createVariantSet(data),
    onSuccess: () => {
      toast.success('Variant set created')
      setIsCreateOpen(false)
      setNewVS({ variantSetName: '', studyName: '', referenceSetDbId: '' })
      queryClient.invalidateQueries({ queryKey: ['variantSets'] })
    },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card><CardContent className="pt-6"><div className="text-2xl font-bold">{((variantSets.reduce((a, vs) => a + vs.variantCount, 0)) / 1000).toFixed(0)}K</div><p className="text-sm text-muted-foreground">Total Variants</p></CardContent></Card>
        <Card><CardContent className="pt-6"><div className="text-2xl font-bold">{summary.callSets || variantSets.reduce((a, vs) => a + vs.callSetCount, 0)}</div><p className="text-sm text-muted-foreground">Total Samples</p></CardContent></Card>
        <Card><CardContent className="pt-6"><div className="text-2xl font-bold">{summary.referenceSets || referenceSets.length}</div><p className="text-sm text-muted-foreground">Reference Genomes</p></CardContent></Card>
      </div>
