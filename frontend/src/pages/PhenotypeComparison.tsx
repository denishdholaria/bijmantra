...
export function PhenotypeComparison() {
  ...
  const getTraitColor = (value: number, trait: string, allValues: number[]) => {
    ...
    if (isTop) return 'bg-green-100 text-green-700 font-bold dark:bg-green-900/20 dark:text-green-400'
    if (isBottom) return 'bg-red-100 text-red-700 dark:bg-red-900/20 dark:text-red-400'
    return ''
  }
                          ...
                          {comparisonData.map((g) => {
                            ...
                            return (
                              <td key={g.id} className={`p-2 text-center ${colorClass}`}>
                                ...
                                {diffInfo && !isCheck && (
                                  <span className={`text-xs ${diffInfo.isPositive ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
