...
import { phenotypeComparisonAPI } from '@/lib/api-client'
...
export function PhenotypeComparison() {
  ...
  const { data: germplasmData, isLoading: loadingGermplasm } = useQuery({
    ...
    queryFn: async () => {
      const response = await phenotypeComparisonAPI.getGermplasm({ limit: 20 })
  ...
  const { data: observationsData, isLoading: loadingObservations } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await phenotypeComparisonAPI.getObservations(selected)
