...
import { breedingValuesAPI } from '@/lib/api-client'
...
export function BreedingValues() {
  ...
  const { data: analysesData, isLoading: loadingAnalyses } = useQuery({
    ...
    queryFn: () => breedingValuesAPI.listAnalyses(),
  })

  // Fetch methods from API
  const { data: methodsData } = useQuery({
    queryKey: ['breeding-value-methods'],
    queryFn: () => breedingValuesAPI.getMethods(),
  })
  ...
  const { data: analysisData, isLoading: loadingAnalysis } = useQuery({
    ...
    queryFn: async () => {
      const result = await breedingValuesAPI.getAnalysis(selectedTrait)
      if (result.data) {
        return {
          ...result,
          data: {
  ...
  const calculateMutation = useMutation({
    mutationFn: async () => {
      ...
      if (method === 'BLUP') {
        return breedingValuesAPI.estimateBLUP({
          phenotypes,
          trait: 'Grain Yield',
          heritability: 0.65,
        })
      } else {
        return breedingValuesAPI.estimateGBLUP({
          phenotypes,
  ...
  // Default demo data when no analysis is selected
  const defaultData: TraitAnalysis = {
    id: 'demo',
    trait: 'Grain Yield',
    method: method,
    heritability: 0.65,
    genetic_variance: 0.42,
    mean: 4.5,
    breeding_values: [
      { id: 'G001', name: 'Elite-2024-001', ebv: 0.85, accuracy: 0.92, rank: 1, parent_mean: 0.62, mendelian_sampling: 0.23 },
      { id: 'G002', name: 'Elite-2024-002', ebv: 0.72, accuracy: 0.88, rank: 2, parent_mean: 0.55, mendelian_sampling: 0.17 },
      { id: 'G003', name: 'Elite-2024-003', ebv: 0.68, accuracy: 0.85, rank: 3, parent_mean: 0.48, mendelian_sampling: 0.20 },
      { id: 'G004', name: 'Elite-2024-004', ebv: 0.55, accuracy: 0.90, rank: 4, parent_mean: 0.42, mendelian_sampling: 0.13 },
      { id: 'G005', name: 'Elite-2024-005', ebv: 0.48, accuracy: 0.82, rank: 5, parent_mean: 0.38, mendelian_sampling: 0.10 },
      { id: 'G006', name: 'Elite-2024-006', ebv: 0.35, accuracy: 0.78, rank: 6, parent_mean: 0.30, mendelian_sampling: 0.05 },
      { id: 'G007', name: 'Elite-2024-007', ebv: 0.22, accuracy: 0.75, rank: 7, parent_mean: 0.25, mendelian_sampling: -0.03 },
      { id: 'G008', name: 'Elite-2024-008', ebv: -0.15, accuracy: 0.80, rank: 8, parent_mean: -0.10, mendelian_sampling: -0.05 },
    ],
  }

  const displayData: TraitAnalysis = currentAnalysis || defaultData
      ...
      {isLoading ? (
        ...
        <Tabs defaultValue="estimates">
          ...
          <TabsContent value="estimates" className="space-y-6 mt-4">
                    ...
                    <Select value={selectedTrait} onValueChange={setSelectedTrait}>
                      ...
                      <SelectContent>
                        {analyses.length > 0 ? (
                          ...
                          <SelectItem value="__demo__" disabled>Demo: Grain Yield</SelectItem>
                        )}
                      </SelectContent>
                    </Select>
                  </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle>Estimated Breeding Values - {displayData.trait}</CardTitle>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {displayData.breeding_values.map((bv, i) => (
                        <tr key={bv.id} className={`border-b hover:bg-muted/50 ${i < selectedCount ? 'bg-green-50' : ''}`}>
                          <td className="p-3">
