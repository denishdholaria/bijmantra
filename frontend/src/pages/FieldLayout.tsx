...
import { fieldLayoutAPI, FieldPlot, FieldStudy } from '@/lib/api-client'
...
export function FieldLayout() {
  ...
  const { data: studiesData, isLoading: studiesLoading } = useQuery({
    ...
    queryFn: () => fieldLayoutAPI.getStudies(),
  })
  ...
  const { data: layoutData, isLoading: layoutLoading } = useQuery({
    ...
    queryFn: () => fieldLayoutAPI.getLayout(selectedStudy),
  ...
  const handleExport = async () => {
    ...
    try {
      const result = await fieldLayoutAPI.exportLayout(selectedStudy, 'csv')
