...
// Mock study data
const mockStudies = [
  { studyDbId: 'study001', studyName: 'Rice Yield Trial 2024', rows: 10, cols: 8 },
  { studyDbId: 'study002', studyName: 'Wheat Disease Screening', rows: 8, cols: 12 },
  { studyDbId: 'study003', studyName: 'Maize Drought Trial', rows: 6, cols: 10 },
]

// Generate mock plots
const generateMockPlots = (rows: number, cols: number): Plot[] => {
  const plots: Plot[] = []
  const germplasm = ['IR64', 'Nipponbare', 'Kasalath', 'Azucena', 'Check-1', 'Check-2']
  for (let r = 1; r <= rows; r++) {
    for (let c = 1; c <= cols; c++) {
      plots.push({
        plotNumber: `P${r.toString().padStart(2, '0')}${c.toString().padStart(2, '0')}`,
        row: r,
        col: c,
        germplasmName: germplasm[Math.floor(Math.random() * germplasm.length)],
        blockNumber: Math.ceil(r / 2),
        replicate: ((r - 1) % 2) + 1,
        entryType: Math.random() > 0.9 ? 'CHECK' : 'TEST',
      })
    }
  }
  return plots
...
export function FieldLayout() {
  ...
  const study = mockStudies.find(s => s.studyDbId === selectedStudy)
  
  const { data: plotsData, isLoading } = useQuery({
    queryKey: ['fieldLayout', selectedStudy],
    queryFn: async () => {
      await new Promise(r => setTimeout(r, 500))
      if (!study) return []
      return generateMockPlots(study.rows, study.cols)
    },
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => toast.success('Layout exported (demo)')}>
            📥 Export
          </Button>
          <Button variant="outline" onClick={() => toast.success('Print layout (demo)')}>
            🖨️ Print
          </Button>
        </div>
      </div>
          ...
          <div className="lg:col-span-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>{study?.rows} rows × {study?.cols} columns = {plots.length} plots</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    {Array.from({ length: study?.rows || 0 }, (_, rowIdx) => (
                      <div key={rowIdx} className="flex">
                        ...
                        {Array.from({ length: study?.cols || 0 }, (_, colIdx) => {
                          const plot = plots.find(p => p.row === rowIdx + 1 && p.col === colIdx + 1)
