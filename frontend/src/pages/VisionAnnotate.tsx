...
import { 
  ...
  ZoomIn, ZoomOut, Move, AlertCircle, RefreshCw
} from 'lucide-react';
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (datasetRes.ok) {
        ...
        setError('Failed to load dataset. It may not exist.');
      }
      if (imagesRes.ok) {
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-16 border-r bg-muted/30 p-2 flex flex-col gap-2">
          ...
          <Button
            ...
            aria-label="Classification tool"
          >
          ...
          <Button variant="ghost" size="icon" onClick={() => setZoom(z => Math.min(z + 0.25, 3))} aria-label="Zoom in">
            <ZoomIn className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="icon" onClick={() => setZoom(z => Math.max(z - 0.25, 0.5))} aria-label="Zoom out">
