...
export function StudyForm() {
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => apiClient.createStudy(data),
    onSuccess: (response) => {
      const studyDbId = response.result?.data?.[0]?.studyDbId
      if (studyDbId) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 space-y-6">
        ...
        <div>
          <label htmlFor="trialDbId" className="block text-sm font-medium text-gray-700 mb-2">
            Trial
          </label>
          <select
            id="trialDbId"
        ...
        <div>
          <label htmlFor="locationDbId" className="block text-sm font-medium text-gray-700 mb-2">
            Location
          </label>
          <select
            id="locationDbId"
        ...
        {mutation.isError && (
          <div className="p-4 bg-red-50 border border-red-200 rounded-md">
            <p className="text-sm text-red-600">Error creating study. Please try again.</p>
          </div>
        )}
