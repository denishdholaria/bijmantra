...
interface Forecast {
  ...
  alerts?: string[];
  impacts?: any[]; // Keep flexible or define strict
...
export function WeatherForecast() {
  ...
  const { data: forecast, isLoading, error, refetch } = useQuery<Forecast>({
    ...
    queryFn: async () => {
      const data = await apiClient.weatherService.getWeatherForecast(selectedLocation, locationName, 14)
  ...
  const gddForecast = forecast?.gdd_forecast || [];
  const fieldConditions = gddForecast.length > 0 ? [
    { 
      field: `${locationName} - Rice`, 
      soilMoisture: currentWeather?.soil_moisture ? Math.round(currentWeather.soil_moisture) : 65, 
      gdd: Math.round(gddForecast[gddForecast.length - 1]?.gdd_cumulative || 0),
      lastRain: forecast?.daily_forecast?.find((d) => d.precipitation > 5) ? 'Recent' : '3+ days ago',
    ...
    { 
      ...
      gdd: Math.round((gddForecast[gddForecast.length - 1]?.gdd_cumulative || 0) * 0.8),
      lastRain: '4 days ago',
      status: 'optimal'
    },
  ] : []
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {forecast.optimal_windows.map((window, index) => (
