...
export function PersonForm() {
  ...
  const mutation = useMutation({
    mutationFn: (data: PersonFormData) => {
      ...
      return isEdit 
        ? apiClient.peopleService.updatePerson(id!, payload)
        : apiClient.peopleService.createPerson(payload)
    },
    onSuccess: (response) => {
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" asChild aria-label="Back to people"><Link to="/people">←</Link></Button>
            <div>
