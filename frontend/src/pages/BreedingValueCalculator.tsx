...
interface BreedingValueCalculatorProps {
  initialTab?: string;
}

export function BreedingValueCalculator({ initialTab = 'blup' }: BreedingValueCalculatorProps) {
  ...
  const individuals: Individual[] = useMemo(() => individualsData?.data || [], [individualsData]);
  ...
  // Memoize sorted individuals to avoid expensive re-sorting on every render
  const sortedIndividuals = useMemo(() => {
    return [...individuals].sort((a, b) => b.ebv - a.ebv);
  }, [individuals]);

  // Calculate selection candidates
  const selectionThreshold = useMemo(() =>
    Math.ceil(individuals.length * (selectionIntensity / 100)),
    [individuals.length, selectionIntensity]
  );

  const selectedIndividuals = useMemo(() =>
    sortedIndividuals.slice(0, selectionThreshold),
    [sortedIndividuals, selectionThreshold]
  );

  const avgSelectedEBV = useMemo(() =>
