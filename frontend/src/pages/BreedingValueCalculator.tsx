...
import { breedingValueCalculatorAPI } from '@/lib/api-client';
...
export function BreedingValueCalculator() {
  ...
  const { data: individualsData, isLoading } = useQuery({
    ...
    queryFn: () => breedingValueCalculatorAPI.getIndividuals(trait),
  });
  ...
  const blupMutation = useMutation({
    mutationFn: () => breedingValueCalculatorAPI.runBLUP({ trait }),
    onSuccess: (data) => {
      setBLUPResult(data);
      toast.success('BLUP analysis completed');
    },
    onError: () => toast.error('BLUP analysis failed'),
  });

  // GBLUP calculation
  const gblupMutation = useMutation({
    mutationFn: () => breedingValueCalculatorAPI.runGBLUP({ trait }),
    onSuccess: (data) => {
      setBLUPResult(data);
  ...
  const predictCrossMutation = useMutation({
    mutationFn: () => breedingValueCalculatorAPI.predictCross({ parent1, parent2, trait }),
    onSuccess: (data) => {
