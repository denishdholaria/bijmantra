...
const DEMO_INDIVIDUALS: Individual[] = [
  { id: 'G001', name: 'IR64', ebv: 2.45, accuracy: 0.85, rank: 1 },
  { id: 'G002', name: 'Swarna', ebv: 1.82, accuracy: 0.82, rank: 2 },
  { id: 'G003', name: 'MTU1010', ebv: 1.56, accuracy: 0.78, rank: 3 },
  { id: 'G004', name: 'BPT5204', ebv: 1.23, accuracy: 0.80, rank: 4 },
  { id: 'G005', name: 'Samba Mahsuri', ebv: 0.98, accuracy: 0.75, rank: 5 },
  { id: 'G006', name: 'Pusa Basmati', ebv: 0.67, accuracy: 0.72, rank: 6 },
  { id: 'G007', name: 'Jaya', ebv: 0.34, accuracy: 0.70, rank: 7 },
  { id: 'G008', name: 'Ratna', ebv: -0.12, accuracy: 0.68, rank: 8 },
  { id: 'G009', name: 'Kavya', ebv: -0.45, accuracy: 0.65, rank: 9 },
  { id: 'G010', name: 'Rasi', ebv: -0.78, accuracy: 0.62, rank: 10 },
];
...
export function BreedingValueCalculator() {
  ...
  const { data: individuals = DEMO_INDIVIDUALS } = useQuery<Individual[]>({
    queryKey: ['breeding-values', trait],
    queryFn: async () => {
      try {
        const res = await fetch(`${API_BASE}/api/v2/breeding-value/individuals?trait=${trait}`);
        if (!res.ok) return DEMO_INDIVIDUALS;
        const data = await res.json();
        return Array.isArray(data) ? data : (data.individuals || DEMO_INDIVIDUALS);
      } catch {
        return DEMO_INDIVIDUALS;
      }
    },
  });
  ...
  const blupMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`${API_BASE}/api/v2/breeding-value/blup`, {
        method: 'POST',
      ...
      if (!res.ok) {
        // Return demo result
        return {
          individuals: DEMO_INDIVIDUALS,
          heritability: 0.45,
          genetic_variance: 1.25,
          residual_variance: 1.53,
        };
      }
      return res.json();
    },
  ...
  const gblupMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`${API_BASE}/api/v2/breeding-value/gblup`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ trait }),
      });
      if (!res.ok) {
        // Return demo result with higher accuracy
        return {
          individuals: DEMO_INDIVIDUALS.map(i => ({ ...i, accuracy: Math.min(i.accuracy + 0.1, 0.95) })),
          heritability: 0.52,
          genetic_variance: 1.35,
  ...
  const predictCrossMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`${API_BASE}/api/v2/breeding-value/predict-cross`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ parent1, parent2, trait }),
      });
      if (!res.ok) {
        // Calculate demo prediction
        const p1 = individuals.find(i => i.id === parent1);
        const p2 = individuals.find(i => i.id === parent2);
        if (p1 && p2) {
          const mean = (p1.ebv + p2.ebv) / 2;
          return {
            parent1: p1.name,
            parent2: p2.name,
            predicted_mean: mean,
            predicted_variance: 0.5,
            probability_superior: mean > 1 ? 0.7 : 0.3,
          };
        }
        throw new Error('Select both parents');
      }
      return res.json();
    },
    onSuccess: (data) => {
