...
export function DataQuality() {
  ...
  const { data: issuesData, isLoading: issuesLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        return await apiClient.dataQualityService.getQualityIssues(params) as { status: string; data: QualityIssue[]; total: number }
      } catch {
  ...
  const { data: metricsData } = useQuery({
    ...
    queryFn: async () => {
      try {
        return await apiClient.dataQualityService.getQualityMetrics() as QualityMetric[]
      } catch {
        return []
      }
    },
  ...
  const { data: scoreData } = useQuery({
    ...
    queryFn: async () => {
      try {
        return await apiClient.dataQualityService.getQualityScore()
      } catch {
...
interface QualityStats {
  ...
  // Fetch statistics
  const { data: statsData } = useQuery<QualityStats>({
    queryKey: ['qualityStatistics'],
    queryFn: async () => {
      try {
        const response = await apiClient.dataQualityService.getDataQualityStatistics();
        const data = response.data;
        return {
          totalIssues: data.total_issues || 0,
          openIssues: data.open_issues || 0,
          resolvedIssues: data.resolved_issues || 0,
          ignoredIssues: data.ignored_issues || 0,
          resolutionRate: data.total_issues ? (data.resolved_issues / data.total_issues) * 100 : 0,
          bySeverity: data.bySeverity
        };
      } catch {
        // Fallback data matching QualityStats interface
        return { 
          totalIssues: 7, 
          openIssues: 5, 
          resolvedIssues: 1, 
  ...
  const runValidationMutation = useMutation({
    mutationFn: () => apiClient.dataQualityService.runDataValidation(),
    onSuccess: (data) => {
  ...
  const resolveIssueMutation = useMutation({
    mutationFn: ({ issueId, notes }: { issueId: string; notes: string }) => 
      apiClient.dataQualityService.resolveQualityIssue(issueId, 'Current User', notes),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['qualityIssues'] })
      queryClient.invalidateQueries({ queryKey: ['qualityScore'] })
  ...
  const ignoreIssueMutation = useMutation({
    mutationFn: (issueId: string) => apiClient.dataQualityService.ignoreQualityIssue(issueId, 'Not applicable'),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['qualityIssues'] })
  ...
  const stats = statsData || { totalIssues: 0, openIssues: 0, resolvedIssues: 0, resolutionRate: 0, ignoredIssues: 0, bySeverity: {} }
  ...
  const openResolveDialog = (issue: QualityIssue) => {
        ...
        <CardContent className="pt-6">
          ...
          <div className="mt-4 grid grid-cols-4 gap-4 text-center">
            ...
            <div>
                ...
                <div className="text-2xl font-bold">{Math.round(stats.resolutionRate || 0)}%</div>
                </div><p className="text-xs text-white/80">Resolution Rate</p>
            </div>
          </div>
        </CardContent>
      </Card>
                  ...
                  <TableBody>
                    {issues.map((issue) => (
                      <TableRow key={issue.id}>
                        <TableCell>
                          <div>
                            <p className="font-medium">{issue.entity || issue.entity_type}</p>
                            <p className="text-xs text-muted-foreground">{issue.entityName || issue.record_id}</p>
                          </div>
                        </TableCell>
                        <TableCell>
                          <Badge className={getIssueTypeBadge(issue.issueType || issue.type || 'info')}>{issue.issueType || issue.type}</Badge>
                        </TableCell>
                        <TableCell className="font-mono text-sm">{issue.field}</TableCell>
                        <TableCell className="max-w-xs truncate">{issue.description || issue.message}</TableCell>
                        <TableCell>
                          <Badge className={getSeverityBadge(issue.severity || (issue.type === 'error' ? 'critical' : issue.type === 'warning' ? 'medium' : 'low'))}>{issue.severity || issue.type}</Badge>
                        </TableCell>
                        <TableCell>
