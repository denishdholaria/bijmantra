...
interface LDDecayResponse {
  data: {
    decay_curve: LDDecayPoint[];
  };
}

interface LDMatrixResponse {
  data: {
    matrix: number[][];
  };
}
  ...
  const runAnalysis = async () => {
    ...
    try {
      if (useServer) {
        ...
        const pairs: MarkerPair[] = data.pairs.map((p) => ({
            marker1: p.marker1,
            marker2: p.marker2,
            distance: p.distance,
            r2: p.r2,
        ...
        // Fetch matrix
        try {
            const matrixRes = await apiClient.get('/api/v2/ld/matrix/region1?variant_set_id=demo-set') as unknown as LDMatrixResponse;
          ...
          for (let j = i + 1; j < Math.min(i + 10, nMarkers); j++) {
            ...
            if (isReady && wasm) {
                try {
                    ...
                    // Use a more robust type assertion or check
