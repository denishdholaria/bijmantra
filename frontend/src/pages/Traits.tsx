...
export function Traits() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => apiClient.observationService.getObservationVariables(page, pageSize),
  })

  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiClient.observationService.deleteObservationVariable(id),
    onSuccess: () => {
  ...
  const variables: ObservationVariable[] = data?.result?.data || []
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <Button
                variant="outline"
                onClick={() => queryClient.invalidateQueries({ queryKey: ['variables'] })}
              >
                      ...
                      <tr key={v.observationVariableDbId} className="hover:bg-muted/30">
                        ...
                        <td className="px-6 py-4 text-right space-x-2">
                          ...
                          <Button
                            variant="link"
                            size="sm"
                            onClick={() => setDeleteId(v.observationVariableDbId)}
