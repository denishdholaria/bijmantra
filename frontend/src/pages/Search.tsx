...
export function Search() {
  ...
  const { data: germplasmData, isLoading: loadingGermplasm } = useQuery({
    ...
    queryFn: () => apiClient.germplasmService.getGermplasm(0, 10, searchTerm),
  ...
  const { data: programsData, isLoading: loadingPrograms } = useQuery({
    ...
    queryFn: () => apiClient.programService.getPrograms(0, 10),
  ...
  const { data: trialsData, isLoading: loadingTrials } = useQuery({
    ...
    queryFn: () => apiClient.trialService.getTrials(0, 10),
  ...
  const { data: studiesData, isLoading: loadingStudies } = useQuery({
    ...
    queryFn: () => apiClient.studyService.getStudies(0, 10),
  ...
  const { data: locationsData, isLoading: loadingLocations } = useQuery({
    ...
    queryFn: () => apiClient.locationService.getLocations(0, 10),
  ...
  const { data: traitsData, isLoading: loadingTraits } = useQuery({
    ...
    queryFn: () => apiClient.observationService.getObservationVariables(0, 10),
  ...
  const filteredGermplasm = useMemo(() => filterByQuery(germplasm, ['germplasmName', 'accessionNumber', 'species']), [germplasm, searchTerm]);
  const filteredPrograms = useMemo(() => filterByQuery(programs, ['programName', 'abbreviation']), [programs, searchTerm]);
  const filteredTrials = useMemo(() => filterByQuery(trials, ['trialName', 'trialDescription']), [trials, searchTerm]);
  const filteredStudies = useMemo(() => filterByQuery(studies, ['studyName', 'studyDescription']), [studies, searchTerm]);
  const filteredLocations = useMemo(() => filterByQuery(locations, ['locationName', 'countryName']), [locations, searchTerm]);
  const filteredTraits = useMemo(() => filterByQuery(traits, ['observationVariableName', 'trait.traitName']), [traits, searchTerm]);
