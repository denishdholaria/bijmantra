...
interface StabilityComparison {
  variety_id: string
  variety_name: string
  eberhart_rank: number
  shukla_rank: number
  wricke_rank: number
  linbinns_rank: number
  ammi_rank: number
  mean_rank: number
}

interface Recommendations {
  wide_adaptation?: StabilityRecommendation
  favorable_environments?: StabilityRecommendation
  unfavorable_environments?: StabilityRecommendation
}
...
export function StabilityAnalysis() {
  ...
  const { data: varietiesData, isLoading, refetch } = useQuery({
    ...
    queryFn: () => apiClient.stabilityAnalysisService.getVarieties(),
  })

  // Fetch stability methods
  const { data: methodsData } = useQuery({
    queryKey: ['stability-methods'],
    queryFn: () => apiClient.stabilityAnalysisService.getMethods(),
  })

  // Fetch recommendations
  const { data: recommendationsData } = useQuery({
    queryKey: ['stability-recommendations'],
    queryFn: () => apiClient.stabilityAnalysisService.getRecommendations(),
  })

  // Fetch method comparison
  const { data: comparisonData } = useQuery({
    queryKey: ['stability-comparison'],
    queryFn: () => apiClient.stabilityAnalysisService.getComparison(),
  })

  const varieties: StabilityVariety[] = varietiesData?.varieties || []
  const methods: StabilityMethod[] = methodsData?.methods || []
  const recommendations: Recommendations = recommendationsData?.recommendations || {}
  const comparison: StabilityComparison[] = comparisonData?.comparison || []
  ...
  const analyzeMutation = useMutation({
    mutationFn: (data: { variety_ids: string[]; methods: string[] }) => apiClient.stabilityAnalysisService.analyze(data),
    onSuccess: (result) => {
      toast.success(`Analysis complete. Processed ${result.variety_count} varieties.`)
      refetch() // Refresh the list with new results
    },
  ...
  const handleCalculate = () => {
    ...
    analyzeMutation.mutate({
      ...
      methods: [selectedMethod, 'nonparametric'] // Include current and non-parametric
    })
  }
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={handleCalculate} disabled={analyzeMutation.isPending || varieties.length === 0}>
             {analyzeMutation.isPending ? 'Calculating...' : '📊 Calculate'}
          </Button>
        </div>
      </div>
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {varieties.map((v) => {
                      // Metrics fetched from backend (derived from stability_rank if missing)
            ...
            <CardContent className="space-y-4">
              ...
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                ...
                <div className="flex flex-wrap gap-2">
                  {recommendations.wide_adaptation?.varieties?.map((v) => (
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <div className="flex flex-wrap gap-2">
                  {recommendations.favorable_environments?.varieties?.map((v) => (
              ...
              <div className="p-4 bg-orange-50 border border-orange-200 rounded-lg">
                ...
                <div className="flex flex-wrap gap-2">
                  {recommendations.unfavorable_environments?.varieties?.map((v) => (
