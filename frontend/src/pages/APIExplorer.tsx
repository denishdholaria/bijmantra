          ...
          <CardContent className="p-0">
            <div className="divide-y max-h-[500px] overflow-auto">
              {endpoints.map((ep, idx) => (
                <button
                  key={idx}
                  className={`w-full text-left p-3 cursor-pointer hover:bg-accent focus:outline-none focus:bg-accent ${selectedEndpoint === ep ? 'bg-accent' : ''}`}
                  onClick={() => setSelectedEndpoint(ep)}
                  aria-label={`Select endpoint ${ep.method} ${ep.path}`}
                >
