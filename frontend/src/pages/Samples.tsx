...
export function Samples() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => apiClient.sampleService.getSamples(page, pageSize),
  })

  const createMutation = useMutation({
    mutationFn: () => apiClient.sampleService.createSample({
  ...
  const germplasm = (germplasmData?.result?.data || []) as Germplasm[]
  const samples = (data?.result?.data || []) as Sample[]
