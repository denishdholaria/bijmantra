...
import { Search, Leaf, MapPin, Dna, Star, Download, Plus, Eye, SlidersHorizontal, X, AlertCircle, RefreshCw } from 'lucide-react'
...
export function GermplasmSearch() {
  ...
  const { data: filterData } = useQuery({
    ...
    queryFn: () => apiClient.germplasmSearchService.getFilters(),
    retry: 1,
  })
  ...
  const selectedResults = useMemo(
    () => results.filter(item => selectedItems.includes(item.id)),
    [results, selectedItems],
  )

  const reevuTaskContext = useMemo(() => ({
    entity_type: 'germplasm',
    selected_entity_ids: selectedItems,
    active_filters: {
    ...
    visible_columns: ['name', 'accession', 'species', 'origin', 'collection', 'traits'],
    attached_context: selectedResults.map(item => ({
      kind: 'germplasm',
      entity_id: item.id,
      label: item.name,
      metadata: {
  ...
  }), [filters.collection, filters.origin, filters.species, searchQuery, selectedItems, selectedResults])
      ...
      {searchError && (
        <Alert variant="destructive">
          ...
          <AlertDescription className="flex items-center gap-2">
            Failed to load germplasm data. {searchError instanceof Error ? searchError.message : 'Please try again.'}
        ...
        <CardContent className="p-4">
          <div className="flex items-center gap-4">
            <div className="relative flex-1">
              ...
              <Label htmlFor="search-germplasm" className="sr-only">Search</Label>
              <Input
                id="search-germplasm"
                placeholder="Search by name, accession, or trait..."
                value={searchQuery}
