...
export function GenomicSelection() {
  ...
  const trainableVariantSets = variantSets.filter((vs: any) => Boolean(vs.variantSetDbId));
    ...
    <div className="space-y-6 animate-fade-in">
                            ...
                            <SelectContent>
                              {trainableVariantSets.length > 0 ? trainableVariantSets.map((vs: any) => (
                                ...
                                <SelectItem value="__empty__" disabled>
                                  No trainable variant sets available
                                </SelectItem>
                              )}
                            </SelectContent>
                        </Select>
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <Card>
            <CardContent className="pt-4 text-center">
              <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{summary.average_accuracy}</p>
              <p className="text-xs text-muted-foreground">Avg Accuracy</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4 text-center">
              <p className="text-2xl font-bold text-green-600 dark:text-green-400">{summary.total_predictions}</p>
              <p className="text-xs text-muted-foreground">Predictions</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4 text-center">
              <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">{summary.selected_candidates}</p>
              <p className="text-xs text-muted-foreground">Selected</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4 text-center">
              <p className="text-2xl font-bold text-orange-600 dark:text-orange-400">{summary.traits_covered?.length || 0}</p>
                        ...
                        <tr key={model.id} className="border-b hover:bg-muted/50 cursor-pointer" onClick={() => setSelectedModel(model.id)}>
                          ...
                          <td className="p-3 text-right">
                            <span className={`font-bold ${
                              model.accuracy >= 0.7 ? 'text-green-600 dark:text-green-400' :
                              model.accuracy >= 0.5 ? 'text-yellow-600 dark:text-yellow-400' :
                          ...
                          <td className="p-3 text-center">
                            <Badge className={
                              model.status === 'trained' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {predictions.map((pred) => (
                        <tr key={pred.germplasm_id} className={`border-b hover:bg-muted/50 ${pred.selected ? 'bg-green-50 dark:bg-green-900/20' : ''}`}>
                          <td className="p-3 text-center font-bold">{pred.rank}</td>
                          <td className="p-3 font-medium">{pred.germplasm_name}</td>
                          <td className="p-3 text-right">
                            <span className={`font-bold ${
                              pred.gebv >= 2 ? 'text-green-600 dark:text-green-400' :
                              pred.gebv >= 1 ? 'text-yellow-600 dark:text-yellow-400' :
                          ...
                          <td className="p-3 text-center">
                            {pred.selected ? <Badge className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">Yes</Badge> : <Badge variant="outline">No</Badge>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="p-4 bg-muted rounded-lg text-center">
                  <p className="text-3xl font-bold text-green-600 dark:text-green-400">37.8%</p>
                  <p className="text-sm text-muted-foreground">Response %</p>
                  <p className="text-xs text-muted-foreground mt-1">Relative to mean</p>
                </div>
                <div className="p-4 bg-muted rounded-lg text-center">
                  <p className="text-3xl font-bold text-blue-600 dark:text-blue-400">1.755</p>
