...
export function GenomicSelection() {
  ...
  const { data: modelsData, isLoading: loadingModels } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch('/api/v2/genomic-selection/models');
  ...
  const { data: predictionsData, isLoading: loadingPredictions } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/v2/genomic-selection/models/${selectedModel}/predictions`);
      if (!response.ok) throw new Error('Failed to fetch predictions');
      return response.json();
    },
    enabled: !isDemoMode && !!selectedModel,
  });
  ...
  const { data: summaryData } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch('/api/v2/genomic-selection/summary');
  ...
  const { data: methodsData } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch('/api/v2/genomic-selection/methods');
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <Card>
            <CardContent className="pt-4 text-center">
              <p className="text-2xl font-bold text-primary">{summaryData.trained_models}</p>
              <p className="text-xs text-muted-foreground">Trained Models</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4 text-center">
              <p className="text-2xl font-bold text-blue-600">{summaryData.average_accuracy}</p>
              <p className="text-xs text-muted-foreground">Avg Accuracy</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4 text-center">
              <p className="text-2xl font-bold text-green-600">{summaryData.total_predictions}</p>
              <p className="text-xs text-muted-foreground">Predictions</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4 text-center">
              <p className="text-2xl font-bold text-purple-600">{summaryData.selected_candidates}</p>
              <p className="text-xs text-muted-foreground">Selected</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4 text-center">
              <p className="text-2xl font-bold text-orange-600">{summaryData.traits_covered?.length || 0}</p>
