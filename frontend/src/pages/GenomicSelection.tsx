...
import { genomicSelectionAPI, GSModel, GEBVPrediction, GSMethod, GSSummary } from '@/lib/api-client';
...
export function GenomicSelection() {
  ...
  const { data: modelsData, isLoading: loadingModels } = useQuery({
    ...
    queryFn: () => genomicSelectionAPI.getModels(),
  });
  ...
  const { data: predictionsData, isLoading: loadingPredictions } = useQuery({
    ...
    queryFn: () => genomicSelectionAPI.getPredictions(selectedModel),
  ...
  const { data: summaryData } = useQuery({
    ...
    queryFn: () => genomicSelectionAPI.getSummary(),
  });

  // Fetch methods
  const { data: methodsData } = useQuery({
    queryKey: ['genomic-selection', 'methods'],
    queryFn: () => genomicSelectionAPI.getMethods(),
  });
