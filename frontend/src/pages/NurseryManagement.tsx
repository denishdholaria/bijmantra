...
import { seedlingBatchAPI } from '@/lib/api-client'
...
export function NurseryManagement() {
  ...
  const { data: batchesData, isLoading } = useQuery({
    ...
    queryFn: () => seedlingBatchAPI.getBatches(),
  })

  // Fetch stats from API
  const { data: statsData } = useQuery({
    queryKey: ['nursery-stats'],
    queryFn: () => seedlingBatchAPI.getStats(),
  })

  // Update status mutation
  const updateStatusMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) =>
      seedlingBatchAPI.updateBatchStatus(id, status),
    onSuccess: () => {
