...
export function SeasonPlanning() {
  ...
  const getSeasonIcon = (seasonType: string) => {
    switch (seasonType?.toLowerCase()) {
      case 'kharif': return <CloudRain className="h-5 w-5 text-blue-500 dark:text-blue-400" />
      case 'rabi': return <Snowflake className="h-5 w-5 text-cyan-500 dark:text-cyan-400" />
      case 'summer': return <Sun className="h-5 w-5 text-amber-500 dark:text-amber-400" />
  ...
  const getStatusColor = (status: string) => {
    switch (status?.toLowerCase()) {
      ...
      case 'planning': return 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300'
