...
import { Network, MapPin, Calendar, Users, Leaf, BarChart3, Globe, Filter, Download, Plus, Eye, Settings, TrendingUp, AlertCircle } from 'lucide-react'
import { trialNetworkAPI } from '@/lib/api-client'
...
export function TrialNetwork() {
  ...
  const { data: sitesData, isLoading: sitesLoading, error: sitesError } = useQuery({
    ...
    queryFn: () => trialNetworkAPI.getSites({ season: selectedSeason }),
    retry: 1,
  })
  ...
  const { data: statsData } = useQuery({
    ...
    queryFn: () => trialNetworkAPI.getStatistics(selectedSeason),
    retry: 1,
  })
  ...
  const { data: germplasmData } = useQuery({
    ...
    queryFn: () => trialNetworkAPI.getSharedGermplasm(5),
    retry: 1,
  })
  ...
  const { data: performanceData } = useQuery({
    ...
    queryFn: () => trialNetworkAPI.getPerformance(),
    retry: 1,
  })
    ...
    <div className="space-y-6">
      ...
      {sitesError && (
        <Alert variant="destructive">
          ...
          <AlertDescription>Failed to load trial network data. Showing demo data.</AlertDescription>
        </Alert>
      )}
