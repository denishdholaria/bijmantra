...
export function Studies() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => apiClient.studyService.getStudies(page, pageSize),
  })

  const deleteMutation = useMutation({
    mutationFn: (studyDbId: string) => apiClient.studyService.deleteStudy(studyDbId),
    onSuccess: () => {
  ...
  const studies: Study[] = data?.result?.data || []
          ...
          <div className="p-6">
            <Alert variant="destructive">
              ...
              <AlertDescription className="flex items-center justify-between">
                <span>Failed to load studies. {error instanceof Error ? error.message : 'Please try again.'}</span>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => queryClient.invalidateQueries({ queryKey: ['studies'] })}
                  className="ml-4"
                >
                    ...
                    <tr key={study.studyDbId} className="hover:bg-gray-50 dark:hover:bg-slate-700">
                      ...
                      <td className="px-6 py-4 text-right text-sm font-medium space-x-2">
                        ...
                        <Button
                          ...
                          className="text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300 h-auto p-0 hover:bg-transparent font-normal"
                        >
                          Delete
                        </Button>
                      </td>
