...
import { Call, CallSet, VariantSet } from '@/lib/api/brapi/genotyping/types'
...
export function Calls() {
  ...
  const calls: Call[] = callsData?.result?.data || []
  const callSets: CallSet[] = callSetsData?.result?.data || []
  const variantSets: VariantSet[] = variantSetsData?.result?.data || []
  ...
  const stats = useMemo(() => ({
    total: totalCount,
    heterozygous: calls.filter(c => {
      const gt = c.genotype?.values || []
      return gt.length === 2 && gt[0] !== gt[1]
    }).length,
    homozygousRef: calls.filter(c => {
      const gt = c.genotype?.values || []
      return gt.length === 2 && gt[0] === gt[1]
    }).length,
    missing: calls.filter(c => {
      const gt = c.genotype?.values || []
      return gt.includes('.') || gt.length === 0
    }).length,
  }), [calls, totalCount])

  const getGenotypeDisplay = (call: Call) => {
    const gt = call.genotype?.values || []
    if (gt.length === 0) return './.'
    return gt.join('/')
  }

  const getGenotypeColor = (call: Call) => {
    const gt = call.genotype?.values || []
        ...
        <Card>
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>
                {filteredCalls.slice(0, 100).map((call, i: number) => (
