...
import { trialPlanningAPI, type PlannedTrial, type TrialType, type TrialDesign } from '@/lib/api-client'
...
export function TrialPlanning() {
  ...
  const { data: trialsData, isLoading } = useQuery({
    ...
    queryFn: () => trialPlanningAPI.getTrials({
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: () => trialPlanningAPI.getStatistics(),
  })

  // Fetch trial types
  const { data: trialTypes } = useQuery({
    queryKey: ['trialTypes'],
    queryFn: () => trialPlanningAPI.getTypes(),
  })

  // Fetch designs
  const { data: designs } = useQuery({
    queryKey: ['trialDesigns'],
    queryFn: () => trialPlanningAPI.getDesigns(),
  })

  // Create trial mutation
  const createTrialMutation = useMutation({
    mutationFn: (data: typeof newTrial) => trialPlanningAPI.createTrial({
      name: data.name,
      type: data.type,
      season: data.season,
  ...
  const approveTrialMutation = useMutation({
    mutationFn: (trialId: string) => trialPlanningAPI.approveTrial(trialId, 'Current User'),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['plannedTrials'] })
  ...
  const startTrialMutation = useMutation({
    mutationFn: (trialId: string) => trialPlanningAPI.startTrial(trialId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['plannedTrials'] })
  ...
  const completeTrialMutation = useMutation({
    mutationFn: (trialId: string) => trialPlanningAPI.completeTrial(trialId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['plannedTrials'] })
