...
// Demo fallback data
const DEMO_TRIALS: PlannedTrial[] = [
  { id: 'TRL-001', name: 'Advanced Yield Trial 2025', type: 'AYT', season: 'Rabi 2025', year: 2025, locations: ['Location A', 'Location B', 'Location C'], entries: 25, reps: 3, design: 'RCBD', status: 'planning', progress: 30, startDate: '2025-01-15', totalPlots: 225 },
  { id: 'TRL-002', name: 'Multi-Location Trial 2025', type: 'MLT', season: 'Rabi 2025', year: 2025, locations: ['Location A', 'Location B', 'Location C', 'Location D', 'Location E'], entries: 15, reps: 3, design: 'Alpha-Lattice', status: 'approved', progress: 60, startDate: '2025-01-20', totalPlots: 225 },
  { id: 'TRL-003', name: 'Preliminary Yield Trial', type: 'PYT', season: 'Kharif 2024', year: 2024, locations: ['Location A'], entries: 100, reps: 2, design: 'Augmented', status: 'active', progress: 85, startDate: '2024-07-01', totalPlots: 200 },
  { id: 'TRL-004', name: 'Disease Screening Trial', type: 'DST', season: 'Kharif 2024', year: 2024, locations: ['Location B'], entries: 50, reps: 2, design: 'RCBD', status: 'completed', progress: 100, startDate: '2024-06-15', totalPlots: 100 },
]
...
export function TrialPlanning() {
  ...
  const { data: trialsData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        return await apiClient.getPlannedTrials(params)
      } catch {
        return DEMO_TRIALS
      }
    },
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      try {
        return await apiClient.getTrialPlanningStatistics()
      } catch {
        return {
          totalTrials: DEMO_TRIALS.length,
          planning: 2, active: 1, completed: 1,
          totalPlots: DEMO_TRIALS.reduce((s, t) => s + t.totalPlots, 0),
        }
      }
    },
  })
  ...
  const { data: trialTypes } = useQuery({
    ...
    queryFn: async () => {
      try {
        return await apiClient.getTrialPlanningTypes()
      } catch {
        return [
  ...
  const { data: designs } = useQuery({
    ...
    queryFn: async () => {
      try {
        return await apiClient.getTrialPlanningDesigns()
      } catch {
        return [
  ...
  const createTrialMutation = useMutation({
    mutationFn: (data: typeof newTrial) => apiClient.createPlannedTrial({
      name: data.name,
      type: data.type,
      season: data.season,
  ...
  const approveTrialMutation = useMutation({
    mutationFn: (trialId: string) => apiClient.approvePlannedTrial(trialId, 'Current User'),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['plannedTrials'] })
  ...
  const startTrialMutation = useMutation({
    mutationFn: (trialId: string) => apiClient.startPlannedTrial(trialId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['plannedTrials'] })
  ...
  const completeTrialMutation = useMutation({
    mutationFn: (trialId: string) => apiClient.completePlannedTrial(trialId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['plannedTrials'] })
  ...
  const trials: PlannedTrial[] = trialsData || DEMO_TRIALS
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select value={newTrial.type} onValueChange={v => setNewTrial({ ...newTrial, type: v })}>
                    ...
                    <SelectContent>
                      {(trialTypes || []).map((t: any) => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div>
                ...
                <div>
                  ...
                  <Select value={newTrial.design} onValueChange={v => setNewTrial({ ...newTrial, design: v })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {(designs || []).map((d: any) => <SelectItem key={d.value} value={d.value}>{d.label}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="col-span-2">
      ...
      <div className="flex gap-4 flex-wrap">
        ...
        <Select value={typeFilter} onValueChange={setTypeFilter}>
          ...
          <SelectContent>
            ...
            {(trialTypes || []).map((t: any) => <SelectItem key={t.value} value={t.value}>{t.value}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
