...
export function Seasons() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => apiClient.seasonService.getSeasons(page, pageSize),
  })

  const createMutation = useMutation({
    mutationFn: () => apiClient.seasonService.createSeason({
  ...
  const seasons = (data?.result?.data || []) as Season[]
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <Dialog open={showCreate} onOpenChange={setShowCreate}>
          <DialogTrigger asChild>
            <Button><CalendarDays className="h-4 w-4 mr-2" /> New Season</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
            ...
            <div className="space-y-4 py-4">
              ...
              <Button onClick={() => createMutation.mutate()} disabled={!newSeasonName || createMutation.isPending} className="w-full">
                {createMutation.isPending ? 'Creating...' : <><CalendarDays className="h-4 w-4 mr-2" /> Create Season</>}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <Button onClick={() => setShowCreate(true)}><CalendarDays className="h-4 w-4 mr-2" /> Create First Season</Button>
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {seasons.map((season) => (
