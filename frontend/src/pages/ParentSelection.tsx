...
import { parentSelectionAPI } from '@/lib/api-client'
import {
  Users,
  Search,
  Filter,
...
export function ParentSelection() {
  ...
  const { data: parentsData, isLoading: parentsLoading, refetch: refetchParents } = useQuery({
    ...
    queryFn: () => parentSelectionAPI.getParents({
  ...
  const { data: recsData, isLoading: recsLoading, refetch: refetchRecs } = useQuery({
    ...
    queryFn: () => parentSelectionAPI.getRecommendations({ limit: 5 }),
    retry: 1,
  })
  ...
  const predictCross = async () => {
    ...
    setPredicting(true)
    try {
      const result = await parentSelectionAPI.predictCross(selectedParents[0], selectedParents[1])
  ...
  const exportParents = async () => {
    try {
      const blob = await parentSelectionAPI.export(parents.map(p => p.id), 'csv')
