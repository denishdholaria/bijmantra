...
interface LDStats {
  ...
  decay_curve: { distance: number; mean_r2: number }[];
  chromosome_stats: { chromosome: string; mean_r2: number }[];
  n_markers: number;
  pairs: LDPair[];
}
...
export function LinkageDisequilibrium() {
  ...
  const [ldData, setLdData] = useState<{ data: LDStats } | null>(null)
  ...
  const filteredPairs = useMemo(() => ldData?.data?.pairs.filter((ld) =>
    (selectedChromosome === 'all' || ld.chromosome === selectedChromosome) &&
    ld.r2 >= r2Threshold[0]
  ) || [], [ldData, selectedChromosome, r2Threshold]);
              ...
              {isLoading ? (
                ...
                <div className="h-64 bg-muted rounded-lg p-4">
                  <div className="h-full flex items-end justify-between gap-1">
                    {ldData?.data?.decay_curve.map((point, i) => (
          ...
          <Card>
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="space-y-3">
                  {ldData?.data?.chromosome_stats.map((chr) => (
