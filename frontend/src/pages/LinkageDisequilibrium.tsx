...
interface LDData {
  ...
  chromosome_stats: ChromosomeStat[]
}
...
export function LinkageDisequilibrium() {
  ...
  // Fetch LD demo data
  const { data: ldData, isLoading, error, refetch } = useQuery<LDData>({
    queryKey: ['ld-data'],
    queryFn: async () => {
      const response = await fetch('/api/v2/gwas/ld/demo')
      if (!response.ok) throw new Error('Failed to fetch LD data')
      return response.json()
    },
  })

  const filteredPairs = ldData?.pairs.filter(ld => 
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              {isLoading ? (
                ...
                <div className="h-64 bg-muted rounded-lg p-4">
                  <div className="h-full flex items-end justify-between gap-1">
                    {ldData?.decay_curve.map((point, i) => (
              ...
              <div className="flex justify-between text-sm text-muted-foreground">
                ...
                <span>r² = 0.2 at ~{ldData?.ld_decay_distance || 15} kb (LD decay distance)</span>
              </div>
            </CardContent>
          </Card>
              ...
              <>
                <Card>
                  <CardContent className="pt-4 text-center">
                    <p className="text-3xl font-bold text-primary">{ldData?.ld_decay_distance} kb</p>
                    <p className="text-sm text-muted-foreground">LD Decay (r²=0.2)</p>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="pt-4 text-center">
                    <p className="text-3xl font-bold text-green-600">{ldData?.mean_r2.toFixed(2)}</p>
                    <p className="text-sm text-muted-foreground">Mean r²</p>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="pt-4 text-center">
                    <p className="text-3xl font-bold text-blue-600">{ldData?.n_pairs.toLocaleString()}</p>
                    <p className="text-sm text-muted-foreground">Marker Pairs</p>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="pt-4 text-center">
                    <p className="text-3xl font-bold text-purple-600">{ldData?.n_high_ld.toLocaleString()}</p>
          ...
          <Card>
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="space-y-3">
                  {ldData?.chromosome_stats.map((chr) => (
              ...
              {isLoading ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {filteredPairs.slice(0, 50).map((ld, i) => (
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                <Card className="bg-blue-50 border-blue-200">
                  <CardContent className="pt-4 text-center">
                    <p className="text-3xl font-bold text-blue-700">{ldData?.n_markers.toLocaleString() || '12,500'}</p>
