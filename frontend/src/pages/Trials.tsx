...
export function Trials() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => apiClient.getTrials(page, pageSize),
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: (trialDbId: string) => apiClient.deleteTrial(trialDbId),
    onSuccess: () => {
    ...
    <div className="space-y-4 lg:space-y-6 animate-fade-in">
      ...
      <div className="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl lg:rounded-2xl shadow-xl p-4 lg:p-8 text-white">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <Link
            to="/trials/new"
            className="px-4 lg:px-6 py-2 lg:py-3 bg-white text-purple-600 rounded-lg font-semibold hover:bg-purple-50 transition-colors shadow-lg hover:shadow-xl text-sm lg:text-base whitespace-nowrap"
          >
            ➕ New Trial
          </Link>
        </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-12 text-red-500">Error loading trials</div>
        ) : trials.length === 0 ? (
          <div className="text-center py-12">
            <div className="text-6xl mb-4">🧪</div>
            <p className="text-gray-500 mb-4">No trials yet</p>
            <Link
              to="/trials/new"
              ...
              <table className="min-w-full divide-y divide-gray-200" style={{ minWidth: '640px' }}>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {trials.map((trial: any) => (
                    <tr key={trial.trialDbId} className="hover:bg-gray-50">
