...
export function ProgramEdit() {
  ...
  const { data, isLoading: isLoadingProgram } = useQuery({
    ...
    queryFn: () => apiClient.programService.getProgram(programDbId!),
    enabled: !!programDbId,
  })
  ...
  const updateMutation = useMutation({
    mutationFn: (data: ProgramFormData) => apiClient.programService.updateProgram(programDbId!, data),
    onSuccess: () => {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="programName">
              Program Name <span className="text-red-500 dark:text-red-400">*</span>
            </Label>
            <Input
              id="programName"
              type="text"
          ...
          {updateMutation.isError && (
            <div className="p-4 bg-red-50 border-l-4 border-red-500 rounded-r-lg dark:bg-red-900/20 dark:border-red-600">
              <p className="text-sm text-red-800 font-medium flex items-center gap-2 dark:text-red-300">
                <span>❌</span>
