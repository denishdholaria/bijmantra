...
export function References() {
  ...
  const filteredRefs = useMemo(() => search
  ...
  const totalLength = useMemo(() => filteredRefs.reduce((a, r) => a + (r.length || 0), 0), [filteredRefs])
  const uniqueSpecies = useMemo(() => new Set(referenceSets.map(rs => rs.species?.genus)).size, [referenceSets])
