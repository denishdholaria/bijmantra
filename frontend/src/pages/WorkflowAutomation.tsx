...
export function WorkflowAutomation() {
  ...
  const { data: workflowsData, isLoading: workflowsLoading } = useQuery({
    queryKey: ['workflows'],
    queryFn: () => apiClient.workflowService.listWorkflows(),
  })
  ...
  const { data: statsData } = useQuery({
    ...
    queryFn: apiClient.workflowService.getStats,
  })

  const stats: WorkflowStats | undefined = statsData?.data

  // Fetch templates
  const { data: templatesData } = useQuery({
    queryKey: ['workflow-templates'],
    queryFn: apiClient.workflowService.listTemplates,
  })

  const workflowTemplates: WorkflowTemplate[] = templatesData?.data || []

  // Fetch run history
  const { data: runsData } = useQuery({
    queryKey: ['workflow-runs'],
    queryFn: () => apiClient.workflowService.getWorkflowRuns({ limit: 10 }),
  })

  const recentRuns: WorkflowRun[] = runsData?.data || []

  // Toggle workflow mutation
  const toggleMutation = useMutation({
    mutationFn: (workflowId: string) => apiClient.workflowService.toggleWorkflow(workflowId),
    onSuccess: () => {
  ...
  const runMutation = useMutation({
    mutationFn: (workflowId: string) => apiClient.workflowService.runWorkflow(workflowId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['workflows'] })
  ...
  const deleteMutation = useMutation({
    mutationFn: (workflowId: string) => apiClient.workflowService.deleteWorkflow(workflowId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['workflows'] })
      queryClient.invalidateQueries({ queryKey: ['workflow-stats'] })
  ...
  const useTemplateMutation = useMutation({
    mutationFn: ({ templateId, name }: { templateId: string; name: string }) =>
      apiClient.workflowService.useTemplate(templateId, name),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['workflows'] })
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          />
                          <Button variant="ghost" size="icon" aria-label="Workflow settings"><Settings className="h-4 w-4" /></Button>
                          <Button 
                            variant="ghost" 
                            size="icon"
