...
export function LabelPrinting() {
  ...
  const { data: templatesData, isLoading: loadingTemplates } = useQuery({
    ...
    queryFn: () => apiClient.labelService.getLabelTemplates(),
  });

  // Fetch label data based on source type
  const { data: labelData, isLoading: loadingData } = useQuery({
    queryKey: ['label-data', sourceType],
    queryFn: () => apiClient.labelService.getLabelData(sourceType),
  });

  // Fetch print job history
  const { data: jobsData } = useQuery({
    queryKey: ['label-jobs'],
    queryFn: () => apiClient.labelService.getLabelPrintJobs(undefined, 20),
  });

  // Fetch stats
  const { data: statsData } = useQuery({
    queryKey: ['label-stats'],
    queryFn: () => apiClient.labelService.getLabelPrintingStats(),
  });

  const stats = statsData as LabelStats | undefined;

  // Create print job mutation
  const createJob = useMutation({
    mutationFn: (data: { template_id: string; items: LabelData[]; copies: number }) =>
      apiClient.labelService.createLabelPrintJob(data),
    onSuccess: (job) => {
      ...
      // Simulate completion
      setTimeout(() => {
        apiClient.labelService.updateLabelPrintJobStatus(job.id, 'completed');
        queryClient.invalidateQueries({ queryKey: ['label-jobs'] });
        queryClient.invalidateQueries({ queryKey: ['label-stats'] });
      }, 2000);
    },
  ...
  const templates: LabelTemplate[] = useMemo(() => templatesData?.templates || [], [templatesData]);
  const data: LabelData[] = useMemo(() => labelData?.data || [], [labelData]);
  const jobs: PrintJob[] = useMemo(() => jobsData?.jobs || [], [jobsData]);
  ...
  const getFieldValue = (item: LabelData, field: string): string => {
    ...
    return fieldMap[field] || item[field]?.toString() || '';
  };
