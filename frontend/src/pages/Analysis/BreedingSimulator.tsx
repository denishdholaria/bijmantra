...
export default function BreedingSimulator() {
  ...
  const initPopulation = () => {
    ...
    for (let i = 0; i < size; i++) {
      ...
      const g_val = genotype.reduce<number>((a, b) => a + b, 0)
      newPop.push({
          id: `Gen0_Ind${i}`,
          genotype,
          g_val,
  ...
  const calculateVariance = (values: number[]) => {
      ...
      return values.reduce((a: number, b: number) => a + Math.pow(b - mean, 2), 0) / values.length
  }
  ...
  const runGeneration = async () => {
    ...
    try {
        ...
        // 3. Process Offspring
        const newInds: Ind[] = result.offspring_dosages.map((gt: number[], idx: number) => ({
            id: `Gen${generation + 1}_Ind${idx}`,
            genotype: gt,
            g_val: gt.reduce((a: number, b: number) => a + b, 0),
            p_val: 0 // Placeholder
        }))
