...
export function AuditLog() {
  ...
  const { data: entriesData, isLoading: entriesLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      return apiClient.auditLogService.getEntries(params)
    },
  })
  ...
  const { data: statsData, isLoading: statsLoading } = useQuery({
    ...
    queryFn: () => apiClient.auditLogService.getStats(hoursFilter !== 'all' ? parseInt(hoursFilter) : 24),
  })

  // Search audit logs
  const { data: searchData, isLoading: searchLoading } = useQuery({
    queryKey: ['auditSearch', search],
    queryFn: () => apiClient.auditLogService.searchLogs(search),
  ...
  const exportMutation = useMutation({
    mutationFn: async (format: 'json' | 'csv') => {
      ...
      return apiClient.auditLogService.exportLogs({
