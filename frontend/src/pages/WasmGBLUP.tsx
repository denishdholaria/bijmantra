...
interface ComputeGBLUPResponse {
  output: {
    breeding_values: number[];
    reliability?: number[] | null;
    accuracy?: number[] | null;
    genetic_variance?: number | null;
...
function WasmGBLUP() {
  ...
  const [markerCount] = useState(500);
  ...
  const generateNewPopulation = useCallback(() => {
  ...
  const runGMatrix = async () => {
    if (engine === 'wasm') {
      ...
      try {
        ...
        const res = await apiCall('/api/v2/compute/grm', 'POST', {
          genotypes: matrix,
          method: 'vanraden1'
        }) as ComputeGRMResponse;

        setServerGMatrix(res.output.matrix);
        toast.success(`Server calculated G-Matrix using ${res.output.n_markers} markers`);
      } catch (err: unknown) {
  ...
  const runAnalysis = async () => {
    if (engine === 'wasm') {
      ...
      try {
        ...
        const res = await apiCall('/api/v2/compute/gblup', 'POST', {
          phenotypes,
          genotypes: matrix,
          heritability
        }) as ComputeGBLUPResponse;

        setServerResults({
          gebv: res.output.breeding_values,
          reliability: res.output.reliability ?? undefined,
          accuracy: res.output.accuracy ?? undefined,
          genetic_variance: res.output.genetic_variance ?? 0,
          error_variance: res.output.error_variance ?? undefined,
          mean: res.output.mean,
        });
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Slider
                  ...
                  onValueCommit={() => generateNewPopulation()}
                  min={10}
                  max={200}
                  step={10}
                />
