...
import { Loader2, Plus, Search, Pencil, Trash2, FolderKanban, Wheat, RefreshCw } from 'lucide-react'
import { apiClient, CrossingProject } from '@/lib/api-client'
...
export function CrossingProjects() {
  ...
  const { data: projectsData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.crossingProjectsService.getProjects(params)
  ...
  const createMutation = useMutation({
    mutationFn: (data: typeof newProject) => apiClient.crossingProjectsService.createProject(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<CrossingProject> }) => 
      apiClient.crossingProjectsService.updateProject(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['crossing-projects'] })
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiClient.crossingProjectsService.deleteProject(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['crossing-projects'] })
  ...
  if (error) {
    return (
      <div className="p-12 text-center">
        ...
        <h3 className="text-lg font-semibold text-red-600 mb-2">Error Loading Crossing Projects</h3>
        <p className="text-muted-foreground mb-4">{error instanceof Error ? error.message : 'Unknown error'}</p>
        <Button variant="outline" onClick={() => queryClient.invalidateQueries({ queryKey: ['crossing-projects'] })}>
