...
import { yieldMapAPI } from '@/lib/api-client'
...
export function YieldMap() {
  ...
  const { data: studiesData, isLoading: loadingStudies } = useQuery({
    ...
    queryFn: () => yieldMapAPI.getStudies({ limit: 50 }),
  })
  ...
  const { data: plotsData, isLoading: loadingPlots } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await yieldMapAPI.getFieldPlotData({ study_id: studyId, trait: selectedTrait })
      return response.result.data.map((unit) => {
        ...
        return {
          row: parseInt(unit.observationUnitPosition?.positionCoordinateY || '1'),
          col: parseInt(unit.observationUnitPosition?.positionCoordinateX || '1'),
          plotId: unit.observationUnitName || unit.observationUnitDbId,
          germplasm: unit.germplasmName || 'Unknown',
          yield: parseFloat(yieldObs?.value || '0'),
          rep: `R${Math.ceil(parseInt(unit.observationUnitPosition?.positionCoordinateY || '1') / 2)}`,
        }
      })
    },
  })
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Select value={selectedStudy} onValueChange={setSelectedStudy}>
            ...
            <SelectContent>
              {studies.map(s => (
                <SelectItem key={s.studyDbId} value={s.studyDbId}>
                  {s.studyName}
                </SelectItem>
              ))}
            </SelectContent>
