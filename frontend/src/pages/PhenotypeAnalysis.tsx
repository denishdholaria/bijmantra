...
G4: 5.5, 5.4, 5.6`);
  ...
  const statsMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await fetch(`${API_URL}/api/v2/phenotype/stats`, {
        method: 'POST',
  ...
  const heritabilityMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await fetch(`${API_URL}/api/v2/phenotype/heritability`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const selectionMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`${API_URL}/api/v2/phenotype/selection-response`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const correlationMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await fetch(`${API_URL}/api/v2/phenotype/correlation`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const getHeritabilityColor = (h2: number): string => {
    if (h2 >= 0.6) return 'text-green-600 dark:text-green-400';
    if (h2 >= 0.3) return 'text-yellow-600 dark:text-yellow-400';
    return 'text-red-600 dark:text-red-400';
  };

  const getCorrelationColor = (r: number): string => {
    const abs = Math.abs(r);
    if (abs >= 0.7) return r > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400';
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Yield"
                    aria-label="Trait name for statistics"
                  />
                </div>
                <div className="space-y-2">
