...
export function MarkerPositions() {
  ...
  const markers: MarkerPosition[] = useMemo(() => data?.result?.data || [], [data]);
  const maps: Map[] = useMemo(() => mapsData?.result?.data || [], [mapsData]);
  const chromosomes = useMemo(() => [...new Set(markers.map(m => m.linkageGroupName))].sort(), [markers]);

  const filteredMarkers = useMemo(() => search
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Select value={mapFilter} onValueChange={setMapFilter}>
              ...
              <SelectContent>
                ...
                {maps.map((m) => <SelectItem key={m.mapDbId} value={m.mapDbId}>{m.mapName}</SelectItem>)}
              </SelectContent>
            </Select>
