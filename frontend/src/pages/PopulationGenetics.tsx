...
// Demo data fallback
const DEMO_POPULATIONS = [
  { id: 'pop1', name: 'Elite Lines 2024', size: 120, region: 'South Asia', admixture: [0.70, 0.20, 0.10] },
  { id: 'pop2', name: 'Core Collection', size: 95, region: 'East Asia', admixture: [0.15, 0.75, 0.10] },
  { id: 'pop3', name: 'Landrace Collection', size: 78, region: 'Africa', admixture: [0.10, 0.15, 0.75] },
  { id: 'pop4', name: 'Breeding Population', size: 85, region: 'Europe', admixture: [0.40, 0.35, 0.25] },
  { id: 'pop5', name: 'Introgression Lines', size: 62, region: 'Americas', admixture: [0.30, 0.25, 0.45] },
];
...
export function PopulationGenetics() {
  ...
  const { data: populationsData, isLoading: loadingPops } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch('/api/v2/population-genetics/populations');
  ...
  const { data: structureData, isLoading: loadingStructure } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/v2/population-genetics/structure?k=${kValue[0]}`);
  ...
  const { data: pcaData, isLoading: loadingPCA } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch('/api/v2/population-genetics/pca');
  ...
  const { data: fstData, isLoading: loadingFst } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch('/api/v2/population-genetics/fst');
      if (!response.ok) throw new Error('Failed to fetch Fst');
      return response.json();
    },
    enabled: !isDemoMode,
  });
  ...
  const { data: migrationData, isLoading: loadingMigration } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch('/api/v2/population-genetics/migration');
  ...
  const { data: summaryData } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch('/api/v2/population-genetics/summary');
  ...
  const populations = populationsData?.populations || DEMO_POPULATIONS;
  const structure = structureData || { 
    populations: DEMO_POPULATIONS.map(p => ({
      population_id: p.id,
      population_name: p.name,
      sample_size: p.size,
      proportions: p.admixture.slice(0, kValue[0]).map((prop, i) => ({ cluster: i + 1, proportion: prop })),
    })), 
    delta_k_analysis: [
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="fst" className="space-y-6 mt-4">
          ...
          {(fstData?.global_statistics || isDemoMode) && (
