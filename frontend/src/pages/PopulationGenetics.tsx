...
export function PopulationGenetics() {
  ...
  const { data: populationsData, isLoading: loadingPops } = useQuery({
    ...
    queryFn: () => apiClient.populationGeneticsService.getPopulations(),
  });
  ...
  const getClusterColor = (index: number) => {
    const colors = ['bg-blue-500 dark:bg-blue-600', 'bg-green-500 dark:bg-green-600', 'bg-orange-500 dark:bg-orange-600', 'bg-purple-500 dark:bg-purple-600', 'bg-pink-500 dark:bg-pink-600'];
    return colors[index % colors.length];
  };

  const getPopColor = (popName: string) => {
    const colors: Record<string, string> = {
      'Elite Lines 2024': 'bg-blue-500 dark:bg-blue-600',
      'Core Collection': 'bg-green-500 dark:bg-green-600',
      'Landrace Collection': 'bg-orange-500 dark:bg-orange-600',
      'Breeding Population': 'bg-purple-500 dark:bg-purple-600',
      'Introgression Lines': 'bg-pink-500 dark:bg-pink-600',
    };
    return colors[popName] || 'bg-gray-500 dark:bg-gray-600';
  };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {summaryData && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          ...
          <Card>
            <CardContent className="pt-4 text-center">
              <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{summaryData.total_samples}</p>
              <p className="text-xs text-muted-foreground">Total Samples</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4 text-center">
              <p className="text-2xl font-bold text-green-600 dark:text-green-400">{summaryData.mean_expected_heterozygosity}</p>
              <p className="text-xs text-muted-foreground">Mean He</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4 text-center">
              <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">{summaryData.global_fst}</p>
              <p className="text-xs text-muted-foreground">Global Fst</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4 text-center">
              <p className="text-2xl font-bold text-orange-600 dark:text-orange-400">{summaryData.mean_allelic_richness}</p>
