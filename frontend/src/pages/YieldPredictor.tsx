...
export function YieldPredictor() {
  ...
  const { data: predictionsData, isLoading, refetch } = useQuery({
    ...
    queryFn: () => apiClient.yieldPredictorService.getPredictions({ environment: selectedEnvironment }),
  });

  // Fetch environmental factors
  const { data: factorsData } = useQuery({
    queryKey: ['environmental-factors'],
    queryFn: () => apiClient.yieldPredictorService.getEnvironmentalFactors(),
  });

  // Run prediction mutation
  const runPredictionMutation = useMutation({
    mutationFn: () => apiClient.yieldPredictorService.runPrediction({ environment: selectedEnvironment }),
    onSuccess: () => {
  ...
  const scenarioMutation = useMutation({
    mutationFn: () => apiClient.yieldPredictorService.getScenarioAnalysis({
      ndvi: ndviValue[0],
  ...
  const avgPrediction = predictions.length > 0 
    ? predictions.reduce((sum: number, p) => sum + p.predicted_yield, 0) / predictions.length
    : 0;
  const topYield = predictions.length > 0 
    ? Math.max(...predictions.map((p) => p.predicted_yield))
    : 0;

  const factors: EnvironmentalFactor[] = factorsData?.factors || [
