...
export function LocationForm() {
  ...
  const mutation = useMutation({
    mutationFn: (data: LocationCreate) => apiClient.locationService.createLocation(data),
    onSuccess: (response) => {
      // Response is single object, not array
      const locationDbId = response.result?.locationDbId || response.result?.data?.[0]?.locationDbId
      if (locationDbId) {
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-lg shadow p-6 space-y-6">
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-md focus:ring-2 focus:ring-orange-500 focus:border-transparent bg-white dark:bg-slate-700 text-gray-900 dark:text-gray-100"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label htmlFor="countryName" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Country Name
            </label>
            <input
              type="text"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            <label htmlFor="altitude" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Altitude (m)
            </label>
            <input
              type="number"
        ...
        {mutation.isError && (
          <div className="p-4 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-md">
            <p className="text-sm text-red-600 dark:text-red-400">Error creating location. Please try again.</p>
          </div>
        )}
