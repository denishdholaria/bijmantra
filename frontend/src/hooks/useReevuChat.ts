      ...
      for await (const event of readReevuStreamEvents(response.body)) {
        if (event.type === 'start') {
          ...
          setMessages(prev => patchReevuAssistantEvidence(prev, {
            ...
            retrievalAudit: event.retrieval_audit,
            planExecutionSummary: event.plan_execution_summary,
          }))
        } else if (event.type === 'stage') {
          if (event.stage === 'policy_validation' && event.status === 'completed' && event.safe_failure) {
            setMessages(prev => patchReevuAssistantSafeFailure(prev, {
              assistantMessageId: assistantMsgId,
              provider: actualProvider,
              model: actualModel,
              safeFailure: event.safe_failure!,
            }))
          }
        } else if (event.type === 'error') {
          if (event.safe_failure) {
            handledSafeFailure = true
            setMessages(prev => patchReevuAssistantSafeFailure(prev, {
              assistantMessageId: assistantMsgId,
              provider: actualProvider,
              model: actualModel,
