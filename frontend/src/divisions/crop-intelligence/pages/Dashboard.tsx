...
export function CropIntelligenceDashboard() {
  // Fetch real crop intelligence stats from backend
  const { data: statsData, isLoading } = useQuery({
    queryKey: ['future', 'dashboard-stats'],
    queryFn: async () => {
      const [gddRes, calRes, suitRes, yieldRes] = await Promise.allSettled([
        apiClient.get<BrAPIResponse<any>>('/api/v2/crop-intelligence/gdd-logs?pageSize=0'),
        apiClient.get<BrAPIResponse<any>>('/api/v2/crop-intelligence/calendars?pageSize=0'),
        apiClient.get<BrAPIResponse<any>>('/api/v2/crop-intelligence/suitability?pageSize=0'),
        apiClient.get<BrAPIResponse<any>>('/api/v2/crop-intelligence/yield-predictions?pageSize=0'),
      ])
      return {
        gddLogs: gddRes.status === 'fulfilled' ? ((gddRes.value as any)?.metadata?.pagination?.totalCount ?? 0) : 0,
        calendars: calRes.status === 'fulfilled' ? ((calRes.value as any)?.metadata?.pagination?.totalCount ?? 0) : 0,
        suitabilityAssessments: suitRes.status === 'fulfilled' ? ((suitRes.value as any)?.metadata?.pagination?.totalCount ?? 0) : 0,
        yieldPredictions: yieldRes.status === 'fulfilled' ? ((yieldRes.value as any)?.metadata?.pagination?.totalCount ?? 0) : 0,
      }
    },
  })
