...
export function PestMonitorPage() {
  ...
  const getSeverityColor = (severity: string) => {
    ...
    return colors[severity] || 'bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-400'
  }

  const getStatusColor = (status: string) => {
    const colors: Record<string, string> = {
      active: 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400',
      treated: 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
        <Card><CardContent className="pt-6"><div className="flex items-center gap-3"><AlertTriangle className="h-8 w-8 text-red-500 dark:text-red-400" /><div><p className="text-2xl font-bold">{stats.active}</p><p className="text-xs text-muted-foreground">Active</p></div></div></CardContent></Card>
        <Card><CardContent className="pt-6"><div className="flex items-center gap-3"><Shield className="h-8 w-8 text-blue-500 dark:text-blue-400" /><div><p className="text-2xl font-bold">{stats.treated}</p><p className="text-xs text-muted-foreground">Treated</p></div></div></CardContent></Card>
        <Card><CardContent className="pt-6"><div className="flex items-center gap-3"><CheckCircle className="h-8 w-8 text-green-500 dark:text-green-400" /><div><p className="text-2xl font-bold">{stats.resolved}</p><p className="text-xs text-muted-foreground">Resolved</p></div></div></CardContent></Card>
        <Card><CardContent className="pt-6"><div className="flex items-center gap-3"><MapPin className="h-8 w-8 text-orange-500 dark:text-orange-400" /><div><p className="text-2xl font-bold">{stats.totalArea} ha</p><p className="text-xs text-muted-foreground">Affected</p></div></div></CardContent></Card>
      </div>
                ...
                {filteredReports.map((report) => (
                  <div key={report.id} className="flex items-center justify-between p-4">
                    <div className="flex items-center gap-4">
                      <Bug className={`h-8 w-8 ${report.severity === 'high' ? 'text-red-500 dark:text-red-400' : report.severity === 'medium' ? 'text-orange-500 dark:text-orange-400' : 'text-yellow-500 dark:text-yellow-400'}`} />
                      <div>
