...
export function Devices() {
  ...
  const fetchDevices = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError('Failed to load devices. Server returned an error.');
      }
    } catch (err) {
  ...
  const fetchDeviceTypes = async () => {
    try {
      ...
      // Keep hardcoded fallback for types only as it's static data
      setDeviceTypes([
  ...
  }, [statusFilter, typeFilter, activeWorkspace]);
      ...
      {error && (
        <Alert variant="destructive">
          ...
          <AlertDescription className="flex items-center justify-between">
            <span>{error}</span>
            <Button variant="outline" size="sm" onClick={() => { setLoading(true); fetchDevices(); }}>
              <RefreshCw className="h-4 w-4 mr-2" />
              Retry
            </Button>
          </AlertDescription>
                ...
                <TableRow key={device.id}>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      <Button variant="ghost" size="icon" aria-label={`Settings for ${device.name}`}>
                        <Settings className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => handleDeleteDevice(device.id)} aria-label={`Delete ${device.name}`}>
