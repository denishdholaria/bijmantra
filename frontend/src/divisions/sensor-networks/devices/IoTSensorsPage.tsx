...
export function IoTSensorsPage() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'online': return <Wifi className="h-4 w-4 text-green-500 dark:text-green-400" />
      case 'offline': return <WifiOff className="h-4 w-4 text-red-500 dark:text-red-400" />
  ...
  const getTypeIcon = (type: string) => {
    if (type === 'weather') return <Sun className="h-5 w-5" />
    if (type === 'soil') return <Droplets className="h-5 w-5" />
    if (type === 'plant') return <Thermometer className="h-5 w-5" />
    if (type === 'water') return <Droplets className="h-5 w-5" />
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
        <Card><CardContent className="pt-6"><div className="flex items-center gap-3"><Cpu className="h-8 w-8 text-blue-500 dark:text-blue-400" /><div><p className="text-2xl font-bold">{stats.total}</p><p className="text-xs text-muted-foreground">Total Sensors</p></div></div></CardContent></Card>
        <Card><CardContent className="pt-6"><div className="flex items-center gap-3"><Wifi className="h-8 w-8 text-green-500 dark:text-green-400" /><div><p className="text-2xl font-bold">{stats.online}</p><p className="text-xs text-muted-foreground">Online</p></div></div></CardContent></Card>
        <Card><CardContent className="pt-6"><div className="flex items-center gap-3"><AlertTriangle className="h-8 w-8 text-yellow-500 dark:text-yellow-400" /><div><p className="text-2xl font-bold">{stats.warning}</p><p className="text-xs text-muted-foreground">Warning</p></div></div></CardContent></Card>
        <Card><CardContent className="pt-6"><div className="flex items-center gap-3"><WifiOff className="h-8 w-8 text-red-500 dark:text-red-400" /><div><p className="text-2xl font-bold">{stats.offline}</p><p className="text-xs text-muted-foreground">Offline</p></div></div></CardContent></Card>
      </div>
