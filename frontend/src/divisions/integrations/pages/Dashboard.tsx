...
export function Dashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [availableRes, userRes] = await Promise.all([
        fetch(`${API_URL}/api/v2/integrations/available`),
        fetch(`${API_URL}/api/v2/integrations/`),
      ]);
  ...
  const handleAddIntegration = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/v2/integrations/`, {
        method: 'POST',
  ...
  const handleTestConnection = async (integrationId: string) => {
    try {
      const res = await fetch(`${API_URL}/api/v2/integrations/${integrationId}/test`, {
        method: 'POST',
      });
  ...
  const handleDeleteIntegration = async (integrationId: string) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/v2/integrations/${integrationId}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-gray-100">Integration Hub</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">Connect with external systems and services</p>
        </div>
