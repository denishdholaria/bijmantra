...
const CropList: React.FC = () => {
  const { crops, fetchCrops, loading, deleteCrop } = useAgronomy();

  useEffect(() => {
    fetchCrops();
  }, []); // eslint-disable-line

  if (loading && crops.length === 0) return <div>Loading...</div>;
            ...
            {crops.map((crop) => (
              <tr key={crop.id}>
                <td className="px-6 py-4 whitespace-nowrap">
                  <Link to={`/agronomy/crops/${crop.id}`} className="text-blue-600 hover:text-blue-900 font-medium">
                    {crop.name}
                  </Link>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{crop.scientific_name}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{crop.variety}</td>
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <Link to={`/agronomy/crops/${crop.id}/edit`} className="text-indigo-600 hover:text-indigo-900 mr-4">
                    Edit
                  </Link>
                  <button
