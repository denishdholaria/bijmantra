...
import { 
  ...
  Package, Calendar, MapPin, AlertCircle 
} from 'lucide-react';
...
export function Lineage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const [lotRes, lineageRes, descendantsRes] = await Promise.all([
        apiClient.getTraceabilityLot(lotNumber),
        apiClient.traceLotLineage(lotNumber),
        apiClient.getLotDescendants(lotNumber),
      ]);
      ...
      if (lotNumber.toUpperCase().startsWith('LOT')) {
        setLineageData({
          ...
          parents: [
            { lot_id: 'LOT-2023-050', variety_name: 'IR64', crop: 'Rice', seed_class: 'foundation', production_year: 2023, quantity_kg: 100, status: 'depleted' },
            { lot_id: 'LOT-2022-025', variety_name: 'IR64', crop: 'Rice', seed_class: 'breeder', production_year: 2022, quantity_kg: 20, status: 'depleted' },
          ],
          descendants: [
            { lot_id: 'LOT-2024-101', variety_name: 'IR64', crop: 'Rice', seed_class: 'certified', production_year: 2024, quantity_kg: 200, status: 'active' },
            { lot_id: 'LOT-2024-102', variety_name: 'IR64', crop: 'Rice', seed_class: 'certified', production_year: 2024, quantity_kg: 150, status: 'active' },
            { lot_id: 'LOT-2024-103', variety_name: 'IR64', crop: 'Rice', seed_class: 'truthful', production_year: 2024, quantity_kg: 300, status: 'active' },
          ],
        });
        setError(null);
      } else {
  ...
  const getSeedClassColor = (seedClass: string) => {
    switch (seedClass) {
      case 'breeder': return 'bg-purple-100 text-purple-700 border-purple-300';
      case 'foundation': return 'bg-white text-gray-700 border-gray-300';
      case 'certified': return 'bg-blue-100 text-blue-700 border-blue-300';
      case 'truthful': return 'bg-green-100 text-green-700 border-green-300';
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'depleted': return <Badge className="bg-gray-100 text-gray-700">Depleted</Badge>;
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-4 text-sm">
            ...
            <div className="flex items-center gap-2">
              <Badge className="bg-white border border-gray-300 text-gray-700">Foundation</Badge>
              <span className="text-gray-500">→</span>
            </div>
            <div className="flex items-center gap-2">
