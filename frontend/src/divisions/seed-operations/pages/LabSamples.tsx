...
export function LabSamples() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [samplesRes, summaryRes] = await Promise.all([
        apiClient.getQCSamples(activeTab === 'all' ? undefined : activeTab),
        apiClient.getQCSummary(),
      ]);
      ...
      setSamples([
        { sample_id: 'QC-000001', lot_id: 'LOT-2024-0156', variety: 'BG-II', sample_date: '2024-12-05', sample_weight: 500, source: 'Processing Plant A', status: 'pending', tests: [] },
        { sample_id: 'QC-000002', lot_id: 'LOT-2024-0155', variety: 'JS-335', sample_date: '2024-12-05', sample_weight: 500, source: 'Processing Plant B', status: 'passed', tests: [] },
        { sample_id: 'QC-000003', lot_id: 'LOT-2024-0154', variety: 'HD-2967', sample_date: '2024-12-04', sample_weight: 500, source: 'Processing Plant A', status: 'failed', tests: [] },
      ]);
      setSummary({ total_samples: 156, pending: 12, passed: 136, failed: 8 });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRegisterSample = async () => {
    try {
      await apiClient.registerQCSample(newSample);
