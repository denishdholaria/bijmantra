...
export function TrackLot() {
  ...
  const renderEventDetails = (details: TraceEvent['details']) => {
    ...
    const orderedEntries = [
      ...prioritizedDetailKeys
        .filter(key => key in details)
        .map(key => [key, details[key]] as const),
      ...entries.filter(([key]) => !prioritizedDetailKeys.includes(key)),
    ];

    return (
      <div className="mt-2 grid gap-2 md:grid-cols-2">
        {orderedEntries.map(([key, value]) => (
          <div key={key} className="rounded-md bg-gray-50 px-3 py-2 text-sm">
            <p className="text-xs uppercase tracking-wide text-gray-500">
              {detailLabels[key] || formatEventType(key)}
            </p>
            <p className="font-medium text-gray-700">
              {key === 'quantity_kg' ? `${value} kg` : String(value)}
            </p>
          </div>
        ))}
      </div>
