...
export function Dashboard() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [qcSummary, inventoryAlerts, processingStats, dispatchStats] = await Promise.all([
        apiClient.qualityControlService.getQCSummary().catch(() => null),
        apiClient.inventoryService.getAlerts().catch(() => null),
        apiClient.processingService.getProcessingStatistics().catch(() => null),
        apiClient.dispatchService.getDispatchStatistics().catch(() => null),
      ]);

      const processingByStatus = processingStats?.data?.by_status || {};
      const dispatchByStatus = dispatchStats?.data?.by_status || {};

      setStats({
        pendingTests: qcSummary?.pending || 0,
        activeBatches: (processingByStatus.in_progress || 0) + (processingByStatus.pending || 0),
        dispatchReady: (dispatchByStatus.approved || 0) + (dispatchByStatus.packed || 0),
        lowStockAlerts: inventoryAlerts?.alert_count || 0,
      });

      // Fetch recent samples for test display
      const samplesRes = await apiClient.qualityControlService.getQCSamples().catch(() => null);
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Seed Commerce</h1>
          <p className="text-gray-500 text-sm mt-1">Lab testing, processing, inventory, dispatch & licensing</p>
        </div>
