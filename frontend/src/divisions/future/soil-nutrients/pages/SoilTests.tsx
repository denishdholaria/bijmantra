...
export function SoilTests() {
  ...
  const { data: tests, isLoading, error, refetch } = useQuery({
    ...
    queryFn: () => apiClient.get('/api/v2/future/soil-test/'),
  })

  const createMutation = useMutation({
    mutationFn: (data: typeof newTest) => apiClient.post('/api/v2/future/soil-test/', data),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiClient.delete(`/api/v2/future/soil-test/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['future', 'soil-tests'] })
