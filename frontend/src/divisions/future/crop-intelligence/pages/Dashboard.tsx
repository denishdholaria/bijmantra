  ...
  const { data: statsData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [gddRes, calRes, suitRes, yieldRes] = await Promise.allSettled([
        apiClient.get('/api/v2/crop-intelligence/gdd-logs?pageSize=0'),
        apiClient.get('/api/v2/crop-intelligence/calendars?pageSize=0'),
        apiClient.get('/api/v2/crop-intelligence/suitability?pageSize=0'),
        apiClient.get('/api/v2/crop-intelligence/yield-predictions?pageSize=0'),
      ])
      return {
        gddLogs: gddRes.status === 'fulfilled' ? (gddRes.value?.metadata?.pagination?.totalCount ?? 0) : 0,
        calendars: calRes.status === 'fulfilled' ? (calRes.value?.metadata?.pagination?.totalCount ?? 0) : 0,
        suitabilityAssessments: suitRes.status === 'fulfilled' ? (suitRes.value?.metadata?.pagination?.totalCount ?? 0) : 0,
        yieldPredictions: yieldRes.status === 'fulfilled' ? (yieldRes.value?.metadata?.pagination?.totalCount ?? 0) : 0,
      }
    },
  })
