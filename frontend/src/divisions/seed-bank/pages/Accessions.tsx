...
import { SeedBankAccessionListResponse } from '@/lib/api/seed-bank/accessions';
...
export function Accessions() {
  ...
  const { data, isLoading, error } = useQuery<SeedBankAccessionListResponse>({
    queryKey: ['seed-bank', 'accessions', page, search],
    queryFn: () => apiClient.accessionService.getAccessions(page, pageSize, search || undefined),
  });
                  ...
                  <tbody className="divide-y">
                    {accessions.map((acc) => (
                      <tr key={acc.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-sm text-gray-600">{acc.vault_id || 'Unassigned'}</td>
