...
import type { TaxonomyValidationResult } from '@/lib/api/seed-bank/taxonomy';
...
export default function TaxonomyValidator() {
  ...
  const [result, setResult] = useState<TaxonomyValidationResult | null>(null);
  ...
  const handleValidate = async () => {
    ...
    try {
      const data = await apiClient.taxonomyService.validateTaxonomy(
        genus.trim(),
        species.trim(),
        subspecies.trim() || undefined,
      );
      setResult(data);
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Could not validate taxonomy",
        variant: "destructive",
      });
    } finally {
      setValidating(false);
