...
import type { CreateMTAPayload, MTAComplianceResult, MTARecord, MTAStatistics, MTATemplate } from '@/lib/api/seed-bank';
import {
  FileText, Plus, Search, Building2, Globe, Calendar, CheckCircle2,
  Clock, XCircle, AlertTriangle, FileSignature, Shield, Users,
} from 'lucide-react';

type MTABenefitSharingType = NonNullable<CreateMTAPayload['benefit_sharing']>['type'];
...
export function MTAManagement() {
  ...
  const [selectedMTA, setSelectedMTA] = useState<MTARecord | null>(null);
  ...
  const { data: mtasData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      return apiClient.mtaService.getMTAs({
        mtaType: typeFilter !== 'all' ? typeFilter : undefined,
  ...
  const { data: stats } = useQuery({
    queryKey: ['mta-stats'],
    queryFn: () => apiClient.mtaService.getStatistics(),
  });

  // Fetch templates
  const { data: templatesData } = useQuery({
    queryKey: ['mta-templates'],
    queryFn: () => apiClient.mtaService.getTemplates(),
  });

  const mtas: MTARecord[] = mtasData?.mtas || [];
...
function CreateMTADialog({
  ...
  const [formData, setFormData] = useState({
    mta_type: 'smta' as CreateMTAPayload['mta_type'],
    provider: { institution: 'Bijmantra Seed Bank', country: 'India', contact: '', email: '' },
    recipient: { institution: '', country: '', contact: '', email: '' },
    accessions: '',
    crops: '',
    purpose: '',
    benefit_sharing: { type: 'none' as MTABenefitSharingType, details: '', royalty_rate: 0 },
  });
  ...
  const handleSubmit = async () => {
    try {
      const payload: CreateMTAPayload = {
        ...formData,
        accessions: formData.accessions.split(',').map(a => a.trim()).filter(Boolean),
        crops: formData.crops.split(',').map(c => c.trim()).filter(Boolean),
        benefit_sharing: {
          ...formData.benefit_sharing,
          royalty_rate: Number.isFinite(formData.benefit_sharing.royalty_rate)
            ? formData.benefit_sharing.royalty_rate
            : undefined,
        },
      };
      await apiClient.mtaService.createMTA(payload);
      onSuccess();
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Failed to create MTA';
      console.error(error);
      ...
      {step === 1 && (
        <div className="space-y-4 py-4">
          <div>
            ...
            <Select value={formData.mta_type} onValueChange={(v) => setFormData({ ...formData, mta_type: v as CreateMTAPayload['mta_type'] })}>
...
function MTADetailView({
  ...
  const performAction = async (action: string, body?: object) => {
    ...
    try {
      if (action === 'submit') {
        await apiClient.mtaService.submitMTA(mta.id);
      } else if (action === 'approve') {
        await apiClient.mtaService.approveMTA(mta.id, (body as { approver: string }).approver);
      } else if (action === 'reject') {
        const payload = body as { reason: string; rejector: string };
        await apiClient.mtaService.rejectMTA(mta.id, payload.reason, payload.rejector);
      } else if (action === 'sign') {
        await apiClient.mtaService.signMTA(mta.id, (body as { signatory: string }).signatory);
      } else if (action === 'terminate') {
        const payload = body as { reason: string; terminator: string };
        await apiClient.mtaService.terminateMTA(mta.id, payload.reason, payload.terminator);
      } else {
...
function ComplianceCheck({ mtaId }: { mtaId: string }) {
  const { data, isLoading } = useQuery<MTAComplianceResult>({
    queryKey: ['mta-compliance', mtaId],
    queryFn: () => apiClient.mtaService.getCompliance(mtaId),
  });
