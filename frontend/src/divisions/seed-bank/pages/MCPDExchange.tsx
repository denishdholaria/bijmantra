...
export default function MCPDExchange() {
  ...
  const [importResult, setImportResult] = useState<MCPDImportResult | null>(null);
  const [previewData, setPreviewData] = useState<any[] | null>(null);

  // Fetch reference codes
  const { data: biologicalStatus } = useQuery({
    queryKey: ['mcpd-biological-status'],
    queryFn: () => apiClient.mcpdService.getBiologicalStatusCodes(),
  });

  const { data: acquisitionSource } = useQuery({
    queryKey: ['mcpd-acquisition-source'],
    queryFn: () => apiClient.mcpdService.getAcquisitionSourceCodes(),
  });

  const { data: storageType } = useQuery({
    queryKey: ['mcpd-storage-type'],
    queryFn: () => apiClient.mcpdService.getStorageTypeCodes(),
  });
  ...
  const exportMutation = useMutation({
    mutationFn: async (format: 'csv' | 'json') => {
      const blob = format === 'csv'
        ? await apiClient.mcpdService.exportCsv()
        : new Blob([
            JSON.stringify(await apiClient.mcpdService.exportJson(), null, 2),
  ...
  const downloadTemplate = async () => {
    try {
      const blob = await apiClient.mcpdService.downloadTemplate();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
  ...
  const importMutation = useMutation({
    mutationFn: (file: File) => apiClient.mcpdService.importCsv(file),
    onSuccess: (data) => {
      setImportResult(data);
      if (data.imported > 0) {
        toast.success(`Imported ${data.imported} accessions`);
      }
      if (data.skipped > 0) {
        toast.info(`${data.skipped} duplicate rows were skipped`);
      }
        ...
        <TabsContent value="import" className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {/* Import Result */}
              {importResult && (
                <Card className={errorCount > 0 ? 'border-amber-200' : 'border-green-200'}>
                  <CardContent className="p-4">
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {importResult.total_records} rows processed, {importResult.imported} imported, {importResult.skipped} skipped, {errorCount} errors
                        </p>
                      </div>
                    </div>
        ...
        <TabsContent value="codes" className="space-y-4">
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="max-h-[300px] overflow-y-auto space-y-1">
                  {biologicalStatusCodes.map((code: MCPDCodeEntry) => (
            ...
            <Card>
              ...
              <CardContent>
                <div className="max-h-[300px] overflow-y-auto space-y-1">
                  {acquisitionSourceCodes.map((code: MCPDCodeEntry) => (
                    <div key={code.code} className="flex items-center gap-2 text-sm p-1">
                      <Badge variant="outline" className="w-10 justify-center">
                        {code.code}
                      </Badge>
            ...
            <Card>
              ...
              <CardContent>
                <div className="max-h-[300px] overflow-y-auto space-y-1">
                  {storageTypeCodes.map((code: MCPDCodeEntry) => (
                    <div key={code.code} className="flex items-center gap-2 text-sm p-1">
                      <Badge variant="outline" className="w-10 justify-center">
                        {code.code}
                      </Badge>
...
function formatImportError(error: MCPDImportError): string {
  const accession = error.accession_number ? ` (${error.accession_number})` : '';
  return `Row ${error.row}${accession}: ${error.errors.join('; ')}`;
}
