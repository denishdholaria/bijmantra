...
export function RegenerationPlanning() {
  ...
  const getReasonIcon = (reason: string): ReactNode => {
    switch (reason) {
      case 'low-viability': return '⚠️';
      case 'low-quantity': return <TrendingDown className="h-4 w-4 text-red-500 inline" />;
      case 'scheduled': return <CalendarDays className="h-4 w-4 text-blue-500 inline" />;
      default: return '📋';
    }
  };
