...
export function GrowingDegrees() {
  ...
  const { data: fields, isLoading: isLoadingFields } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await apiClient.fieldMapService.getFields();
  ...
  const { data: fieldGddData, isLoading: isLoadingGdd, refetch: refetchGdd } = useQuery({
    ...
    queryFn: async () => {
      // Get summary
      const summary = await apiClient.gddService.getFieldSummary(selectedFieldId);
      
      // Get predictions
      const predictions = await apiClient.gddService.getFieldPredictions(selectedFieldId);
      
      // Get recent history (mocked if empty)
      // In a real scenario, we'd fetch this range dynamically
      const endDate = new Date().toISOString().split('T')[0];
      const startDate = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
      const history = await apiClient.gddService.getFieldHistory(selectedFieldId, startDate, endDate);
  ...
  const createPredictionMutation = useMutation({
    mutationFn: async () => {
      ...
      await apiClient.gddService.createFieldPrediction(selectedFieldId, {
