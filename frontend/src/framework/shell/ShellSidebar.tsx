...
function SidebarSection({
  ...
  const hasItems = section.children && section.children.length > 0
  ...
  return (
    <div>
      ...
      >
        ...
        <span className="text-shell-muted text-[10px]">{section.children!.length}</span>
      </button>
...
export function ShellSidebar() {
  ...
  const activeSection = useMemo<SidebarMenuItem | null>(() => {
    ...
    return activeDivision.children.find(section => 
      location.pathname.startsWith(section.path)
    ) || null
  }, [activeDivision, location.pathname])

  const handleDivisionClick = useCallback((division: SidebarMenuItem) => {
        ...
        <aside className="bg-shell-chrome border-shell flex flex-col w-14 h-full border-r flex-shrink-0">
          ...
          <div className="flex-1 overflow-y-auto py-2 px-1.5 space-y-1">
            {sidebarItems.map(division => {
              ...
              const gradient = divisionGradients[division.divisionId || division.id] || 'from-slate-500 to-slate-600'
  ...
  return (
    <aside className="bg-shell-chrome border-shell flex flex-col w-60 h-full border-r flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto py-2 px-2">
        {activeDivision && activeDivision.children && activeDivision.children.length > 0 ? (
          activeSection ? (
            ...
            <div className="space-y-2">
              <div className="bg-[hsl(var(--app-shell-panel)/0.78)] border-shell rounded-2xl border px-3 py-2 shadow-shell">
                ...
                <div className="text-shell text-sm font-semibold truncate">{activeSection.label}</div>
              </div>
            ...
            <div className="space-y-0.5">
              {activeDivision.children.map(section => (
                <SidebarSection
                  key={section.id}
                  section={section}
                  divisionRoute={activeDivision.path}
                />
              ))}
            </div>
          )
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <div className="text-sm font-medium truncate">{division.label}</div>
