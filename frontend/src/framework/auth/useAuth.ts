...
export function useAuth() {
   ...
   * For now, returns true for authenticated users (implement RBAC later)
   */
  const hasPermission = (permission: string): boolean => {
    if (!authStore.isAuthenticated) return false;
    
    // TODO: Implement proper RBAC when backend supports it
    // For now, superusers have all permissions
    if (authStore.user?.is_superuser) return true;
    
    // Basic permission check - authenticated users have read access
    if (permission.startsWith('read:')) return true;
    
    // Write permissions require authentication
    if (permission.startsWith('write:')) return authStore.isAuthenticated;
