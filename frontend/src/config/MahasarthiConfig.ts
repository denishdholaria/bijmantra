...
export interface GatewayConfig {
  ...
  workspaces: string[]; // Related workspace or division identifiers used for grouping
}
...
export function getGatewayForRoute(pathname: string): GatewayId | null {
  ...
  if (pathname.includes("breeding") || pathname.includes("genomics"))
    return "srijan";
  if (pathname.includes("seed-bank") || pathname.includes("seed-ops") || pathname.includes("seed-operations"))
    return "sampada";
