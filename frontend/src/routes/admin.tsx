...
const AIDiagnostics = lazy(() => import('@/pages/AIDiagnostics').then(m => ({ default: m.AIDiagnostics })));
...
const DeveloperControlPlaneRoute = lazy(() => import('@/features/dev-control-plane/routes').then(m => ({ default: m.DeveloperControlPlaneRoute })));

// Helper
const wrap = (
  Component: React.LazyExoticComponent<any> | React.ComponentType,
  options?: { requireSuperuser?: boolean }
) => (
  <ProtectedRoute requireSuperuser={options?.requireSuperuser}>
...
export const adminRoutes: RouteObject[] = [
  ...
  { path: '/ai-settings', element: wrap(AISettings, { requireSuperuser: true }) },
  { path: '/ai-diagnostics', element: wrap(AIDiagnostics, { requireSuperuser: true }) },
  ...
  { path: '/admin/developer/master-board', element: wrap(DeveloperControlPlaneRoute, { requireSuperuser: true }) },
];
