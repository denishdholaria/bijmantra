 ...
 * 
 * @see .ai/decisions/ADR-007-frontend-routing-and-network-ownership.md (replaces stale archival doc)
 * @see .ai/decisions/ADR-008-governance-authority-and-drift-control.md
 */
...
import { Icon as AppIcon, resolveIconName } from '@/lib/icons';
import { 
  WORKSPACE_ICONS, 
  WORKSPACE_GRADIENTS, 
...
export function MahasarthiDwar() {
  ...
  const Icon = WORKSPACE_ICONS[activeWorkspace.id] as any;
  const decorativeIconName = resolveIconName(WORKSPACE_EMOJIS[activeWorkspace.id]);
    ...
    >
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 pb-8">
          ...
          >
            ...
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[200px] opacity-5 select-none">
                {decorativeIconName ? (
                  <AppIcon name={decorativeIconName} className="w-[200px] h-[200px]" strokeWidth={1.25} />
                ) : (
        ...
        <div className="mt-8 flex flex-wrap items-center justify-center gap-2">
          {workspaces.map((workspace, index) => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const PillIcon = WORKSPACE_ICONS[workspace.id] as any;
