...
import { APP_SHORTCUTS, groupShortcutsByCategory, SHORTCUT_CATEGORY_ORDER } from '@/lib/keyboardShortcuts';
...
const QUICK_TIPS = [
  ...
  { icon: <Keyboard className="h-4 w-4" />, tip: 'Press ? to open shortcuts and ⌘/ to toggle REEVU' },
  ...
  { icon: <Lightbulb className="h-4 w-4" />, tip: 'Ask REEVU AI for help with any task' },
];
                    ...
                    <div key={category}>
                      ...
                      <div className="space-y-2">
                        {shortcuts.map((shortcut) => (
                          <ShortcutRow key={shortcut.id} keys={shortcut.keys} description={shortcut.description} />
                        ))}
                      </div>
                    </div>
                  );
