...
import { DEFAULT_REEVU_BYOK_MODEL, DEFAULT_REEVU_BYOK_PROVIDER } from '@/lib/ai-model-catalog';
...
// AI Config storage key (shared with useReevuChat)
...
export function useAgentArsenal(): UseAgentArsenalReturn {
      ...
      if (saved) {
        ...
        if (parsed && parsed.cloud && typeof parsed.cloud === 'object') {
          setAiConfig({
            mode: 'cloud',
            cloud: {
              provider: parsed.cloud.provider || DEFAULT_REEVU_BYOK_PROVIDER,
              apiKey: parsed.cloud.apiKey || '',
              model: parsed.cloud.model || DEFAULT_REEVU_BYOK_MODEL,
  ...
  useEffect(() => {
    ...
    AGENT_REGISTRY.agents.forEach(agent => {
      // Check if REEVU orchestrator is configured
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      if (agent?.endpoint) {
        ...
        // REEVU orchestrator - use streaming backend
              ...
              if (data.type === 'start') {
                ...
                setSessions(prev => ({
                  ...
                  [activeAgentId]: {
                    ...
                    messages: prev[activeAgentId].messages.map(msg =>
                      msg.id === assistantMsgId
                        ? { ...msg, metadata: { ...metadata, proposal: data.data } }
                        : msg
                    ),
                  },
                }));
