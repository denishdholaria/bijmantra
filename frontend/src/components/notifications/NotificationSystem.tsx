...
type NotificationType = 
  ...
  | 'warning' 
  | 'error' 
  | typeof LEGACY_REEVU_NOTIFICATION_TYPE  // AI insights
    ...
    if (token) {
      try {
        const response = await fetch(`/api/v2/notifications/?user_id=${user?.id || 1}&organization_id=${user?.organization_id || 1}`, {
          ...
          body: JSON.stringify({
            type: notification.type === LEGACY_REEVU_NOTIFICATION_TYPE || notification.type === 'collaboration' || notification.type === 'data' || notification.type === 'weather' 
              ? 'info' 
              : notification.type,
