...
import { isRouteInWorkspace, getWorkspace } from '@/framework/registry/workspaces';
...
const workspaceIcons: Record<WorkspaceId, React.ReactNode> = {
  ...
  admin: <Settings className="h-3 w-3" />,
};
...
export function GlobalSearch({ open, onOpenChange }: GlobalSearchProps) {
  ...
  const results = React.useMemo(() => {
    ...
    let filtered = demoResults.filter(
      (r) =>
    ...
    if (filterByWorkspace && activeWorkspace) {
      filtered = filtered.filter(r => isRouteInWorkspace(r.path, activeWorkspace.id));
    }
    
    return filtered;
  }, [query, filterByWorkspace, activeWorkspace]);
