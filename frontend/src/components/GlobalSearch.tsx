...
export function GlobalSearch({ open, onOpenChange }: GlobalSearchProps) {
  ...
  const { data: germplasmResults, isLoading: germplasmLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const response = await apiClient.getGermplasm(0, 10, debouncedQuery);
  ...
  const { data: trialResults, isLoading: trialsLoading } = useQuery({
    ...
    queryFn: async () => {
      if (!debouncedQuery || debouncedQuery.length < 2) return [];
      try {
        const response = await apiClient.getTrials(0, 10);
        const data = response?.result?.data || [];
  ...
  const { data: studyResults, isLoading: studiesLoading } = useQuery({
    ...
    queryFn: async () => {
      if (!debouncedQuery || debouncedQuery.length < 2) return [];
      try {
        const response = await apiClient.getStudies(0, 10);
        const data = response?.result?.data || [];
        return data
          .filter((s: any) => 
  ...
  const { data: locationResults, isLoading: locationsLoading } = useQuery({
    ...
    queryFn: async () => {
      if (!debouncedQuery || debouncedQuery.length < 2) return [];
      try {
        const response = await apiClient.getLocations(0, 10);
        const data = response?.result?.data || [];
        return data
          .filter((l: any) => 
