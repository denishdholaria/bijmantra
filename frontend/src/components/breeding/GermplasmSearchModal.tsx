            ...
            <div className="space-y-2">
              {results.map((item) => (
                <div
                  key={item.id}
                  className={cn(
                    "flex items-center gap-3 p-3 rounded-lg border border-border cursor-pointer hover:bg-muted/50 transition-colors",
                  )}
                  onClick={() => handleSelect(item)}
                >
                  <div className="p-2 bg-prakruti-patta-pale dark:bg-prakruti-patta/20 rounded-lg">
                    <Leaf className="h-5 w-5 text-prakruti-patta dark:text-prakruti-patta-light" />
                  </div>
