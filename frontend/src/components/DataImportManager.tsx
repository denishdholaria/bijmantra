...
import { parseStructuredRecordsFile } from '@/lib/spreadsheet';
...
export function DataImportManager({
  ...
  const handleFileSelect = useCallback(async (selectedFile: File) => {
    ...
    if (extension === 'csv' || extension === 'tsv') {
      ...
      setError(LEGACY_XLS_ERROR);
      return;
    } else {
      setError('Unsupported file format. Please use CSV, TSV, JSON, or XLSX files.');
      return;
    }
        ...
        {step === 'upload' && (
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Supports CSV, TSV, JSON, and XLSX files (max {maxFileSize}MB)
            </p>
            <div className="flex gap-2">
              ...
              <Badge variant="outline"><FileSpreadsheet className="h-3 w-3 mr-1" />XLSX</Badge>
            </div>
            <input
              ref={fileInputRef}
              type="file"
                  ...
                  {mappings.map((mapping) => (
                    <div key={mapping.targetColumn} className="flex items-center gap-3">
                      ...
                      <select
                        ...
                        aria-label={`Map source column to ${mapping.targetColumn}`}
                        className={cn(
...
async function parseFile(file: File, _format: ImportFormat): Promise<Record<string, unknown>[]> {
  if (_format !== 'csv' && _format !== 'json' && _format !== 'xlsx') {
    throw new Error('Unsupported import format');
  }

  return parseStructuredRecordsFile(file);
}
