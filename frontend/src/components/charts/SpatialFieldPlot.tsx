...
export function SpatialFieldPlot({
  ...
  const plotIdMap = useMemo(() => {
    const map = new Map<string, PlotData>();
    plots.forEach((p) => map.set(p.plotId, p));
    return map;
  }, [plots]);
      ...
      <CardContent className="p-2">
        ...
        <div className="flex gap-4 mb-3 text-xs">
          <span className="flex items-center gap-1">
            <span
              ...
              style={{ backgroundColor: STATUS_COLORS.planted }}
            />
          ...
          <span className="flex items-center gap-1">
            <span
              className="w-3 h-3 rounded"
              style={{ backgroundColor: STATUS_COLORS.measured }}
            />
          ...
          <span className="flex items-center gap-1">
            <span
              className="w-3 h-3 rounded"
              style={{ backgroundColor: STATUS_COLORS.missing }}
            />
        ...
        {hoveredPlot && (
          <div className="mt-2 p-2 bg-muted rounded text-xs">
            {(() => {
              const plot = plotIdMap.get(hoveredPlot);
              if (!plot) return null;
              return (
                <div className="flex gap-4">
                  ...
                  <span>
                    <strong>Position:</strong> R{plot.row}C{plot.col}
                  </span>
                  {plot.genotype && (
                    <span>
                      <strong>Genotype:</strong> {plot.genotype}
                    </span>
                  )}
                  {plot.value !== undefined && (
                    <span>
                      <strong>Value:</strong> {plot.value.toFixed(2)}
                    </span>
                  )}
                  {plot.status && (
                    <span>
