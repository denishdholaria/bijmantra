...
export function LiveSensorChart({
  ...
  // Generate demo data if no fetchData provided
  const generateDemoReading = useCallback((sensor: SensorConfig): SensorReading => {
    ...
    const base = baseValues[sensor.type] || 50;
    const variation = base * 0.1;
    const value = base + (Math.random() - 0.5) * variation * 2;
  ...
  // Fetch or generate data for all selected sensors
    ...
    for (const sensorId of selectedSensors) {
      ...
      if (fetchData) {
        try {
          ...
          reading = generateDemoReading(sensor);
        }
      } else {
        reading = generateDemoReading(sensor);
      }
  ...
  }, [data, selectedSensors, sensors, fetchData, generateDemoReading, maxDataPoints]);
