...
export function ScatterPlot({
  ...
  const option: EChartsOption = useMemo(() => ({
    ...
    legend: showLegend && Object.keys(groupedData).length > 1 ? {
      top: title ? 30 : 10,
      textStyle: {
    ...
    series,
  }), [data, series, title, xAxisLabel, yAxisLabel, showLegend, groupedData, isDark]);
  ...
  return (
    <ReactECharts
      ...
      opts={{ renderer: data.length > 10000 ? 'canvas' : 'svg' }}
    />
  );
}
