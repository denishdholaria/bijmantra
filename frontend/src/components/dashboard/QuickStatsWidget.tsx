    ...
    queryFn: async () => {
      try {
        ...
        const [programs, germplasm, trials, locations] = await Promise.all([
          fetch(`${API_URL}/brapi/v2/programs?pageSize=1`).then(r => r.ok ? r.json() : null),
          fetch(`${API_URL}/brapi/v2/germplasm?pageSize=1`).then(r => r.ok ? r.json() : null),
          fetch(`${API_URL}/brapi/v2/trials?pageSize=1`).then(r => r.ok ? r.json() : null),
          fetch(`${API_URL}/brapi/v2/locations?pageSize=1`).then(r => r.ok ? r.json() : null),
        ]);
