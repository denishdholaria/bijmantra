 ...
 * 
 * Refactored Phase 6F-2: Dock Intelligence
 * - Uses useMahasarthiNavigation hook for context-aware filtering
 * - Shares the same "Brain" as the main sidebar
 */
...
import {
  ...
  Pin, PinOff, GripVertical, Minus, MoreHorizontal, Home,
  type LucideIcon,
} from 'lucide-react';
...
export function MahasarthiDock({ onBrowserOpen, onSearchOpen, onNavigate, isMobile = false }: MahasarthiDockProps) {
  ...
  const { unpinItem, reorderPinned } = useDockStore();
  
  // Use the shared "Brain" for navigation intelligence
  const { 
    activeWorkspace, 
    gatewayPinnedItems, 
    gatewayRecentItems 
  ...
  if (isMobile) {
    ...
    const mobileItems = gatewayPinnedItems.slice(0, 4);
      ...
      <div className="flex flex-col h-full py-2 px-1">
        ...
        <div className="flex-1 space-y-1">
          {gatewayPinnedItems.length === 0 ? (
            ...
            gatewayPinnedItems.map((item, index) => {
            const IconComponent = getIcon(item.icon);
            const isActive = location.pathname === item.path || 
                            location.pathname.startsWith(item.path + '/');
