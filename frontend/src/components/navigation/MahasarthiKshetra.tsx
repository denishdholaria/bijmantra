...
import { useAllSelectablePages } from '@/hooks/useCustomWorkspace';
...
export function MahasarthiWorkspace({ compact = false, showLabel = true }: MahasarthiWorkspaceProps) {
  ...
  const selectablePages = useAllSelectablePages();
  ...
  const handleCustomWorkspaceSelect = (customWorkspace: CustomWorkspace) => {
    ...
    // Navigate to the first configured page route; fallback to dashboard if route cannot be resolved.
    const firstPageId = customWorkspace.pageIds[0];
    const firstPageRoute = firstPageId
      ? selectablePages.find(page => page.id === firstPageId)?.route
      : null;
    navigate(firstPageRoute || '/dashboard');
    setIsOpen(false);
  };
  ...
  const getActiveGradient = (): string => {
    if (isCustomActive && activeCustomWorkspace) {
      const colorKey = String(activeCustomWorkspace.color);
      // @ts-ignore
      return (customWorkspaceGradients as any)[colorKey] || customWorkspaceGradients.slate;
    }
    if (activeWorkspace) {
  ...
  const getActiveTextColor = (): string => {
    if (isCustomActive && activeCustomWorkspace) {
      const colorKey = String(activeCustomWorkspace.color);
      // @ts-ignore
      return (customWorkspaceTextColors as any)[colorKey] || customWorkspaceTextColors.slate;
    }
    if (activeWorkspace) {
          ...
          {recentWorkspaces.length > 0 && !isCustomActive && (
            <>
              ...
              {recentWorkspaces.map((workspace) => {
                ...
                const Icon = workspaceIcons[workspace.id] as any;
...
// Alias export for renamed file
export { MahasarthiWorkspace as MahasarthiKshetra };
