...
import { LEGACY_REEVU_ROUTE_SEGMENT } from '@/lib/legacyReevu';
...
const routeLabels: Record<string, string> = {
  ...
  [LEGACY_REEVU_ROUTE_SEGMENT]: 'REEVU Chat',
 ...
 */
function findDivisionForRoute(pathname: string): {
  division: string
  ...
  for (const division of divisions) {
    if (pathname.startsWith(division.route)) {
      ...
      return { division: division.name, divisionRoute: division.route };
    }

    // Also resolve absolute section/item routes that do not start with division.route.
    if (division.sections) {
      for (const section of division.sections) {
        const sectionRoute = section.isAbsolute ? section.route : `${division.route}${section.route}`;
        if (pathname.startsWith(sectionRoute)) {
...
export function Breadcrumbs({ className, showHome = true, maxItems = 4 }: BreadcrumbsProps) {
  ...
  const breadcrumbs = useMemo<BreadcrumbItem[]>(() => {
    ...
    const firstSegmentLabel = segments[0] ? getLabel(segments[0]) : null;

    // Build breadcrumb trail
    // Inject division/section context when the route's first segment does not already represent
    // the division label (for example /plannedcrosses or /programs), so breadcrumb and sidebar
    // context stay aligned.
    if (divisionContext && firstSegmentLabel !== divisionContext.division) {
      items.push({
        label: divisionContext.division,
        path: divisionContext.divisionRoute,
      });

      if (
        divisionContext.section &&
        divisionContext.sectionRoute &&
        divisionContext.sectionRoute !== divisionContext.divisionRoute &&
        divisionContext.section !== firstSegmentLabel
      ) {
        items.push({
          label: divisionContext.section,
          path: divisionContext.sectionRoute,
        });
      }
    }
