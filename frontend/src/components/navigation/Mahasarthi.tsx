  ...
  return (
    <>
      <div className="flex flex-col h-full">
        ...
        {/* =========================================================================== */}
        {/* CONTEXT-AWARE NAVIGATION (New Design)                                   */}
        {/* =========================================================================== */}
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-6">
          ...
          {activeWorkspace ? (
            <div className="space-y-4">
              {getWorkspaceModules(activeWorkspace.id).map(module => (
                <div key={module.id} className="space-y-1">
                  ...
                  <div className="px-2 text-[10px] font-bold text-emerald-500/80 uppercase tracking-wider flex items-center gap-1.5 opacity-80">
                    {/* Optional: Icon for module header */}
                    {/* {(() => { const Icon = getIcon(module.icon); return <Icon className="w-3 h-3" />; })()} */}
                    {module.name}
                  </div>
                  ...
                  <div className="space-y-0.5">
                    {module.pages.map(page => {
                      ...
                      // Use module icon for pages if page doesn't have specific icon (pages in registry don't imply icons yet, could default to Dot)
                      // For now, using a simple layout
                      return (
                        <Link
                          key={page.id}
                          to={page.route}
                        ...
                        >
                          <div className="flex items-center gap-2">
                             {/* Optional: Add icons to pages in registry later. For now, subtle bullet or just text. */}
              ...
              {/* If no modules (shouldn't happen for valid workspaces), show fallback */}
              {getWorkspaceModules(activeWorkspace.id).length === 0 && (
          ...
          {/* 2. GATEWAY SWITCHER (Secondary Navigation) */}
          <div className="pt-4 border-t border-white/5">
             ...
             <div className="space-y-0.5">
              {filteredDivisions.map(division => {
                  const isActive = activeWorkspaceId === division.id; // Rough check
