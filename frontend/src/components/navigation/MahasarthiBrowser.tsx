...
interface MahasarthiBrowserProps {
  isOpen: boolean;
  onClose: () => void;
  onNavigate?: () => void;
  isMobile?: boolean;
...
export function MahasarthiBrowser({ isOpen, onClose, onNavigate, isMobile = false }: MahasarthiBrowserProps) {
  const navigate = useNavigate();
  const { navigableDivisions } = useDivisionRegistry();
  const activeWorkspace = useActiveWorkspace();
  const activeWorkspaceId = useWorkspaceStore((state) => state.activeWorkspaceId);
  const { pinnedItems, pinItem, unpinItem, isPinned } = useDockStore();
  ...
  const { primaryDivisions, crossDomainDivisions } = useMemo(() => {
    if (!activeWorkspaceId) {
      return { primaryDivisions: navigableDivisions, crossDomainDivisions: [] };
    }
    ...
    navigableDivisions.forEach(division => {
      const isInWorkspace = isRouteInWorkspace(division.route, activeWorkspaceId) ||
      ...
      if (isInWorkspace) {
        ...
        // Has content, could be cross-domain
        crossDomain.push(division);
      }
    });

    return { primaryDivisions: primary, crossDomainDivisions: crossDomain.slice(0, 3) };
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <button
                                          ...
                                          aria-label={isPinned(path) ? 'Unpin' : 'Pin'}
                                        >
                                          {isPinned(path) ? (
                                            <PinOff className="w-3 h-3 text-emerald-400" />
                                          ) : (
...
export default MahasarthiBrowser;
