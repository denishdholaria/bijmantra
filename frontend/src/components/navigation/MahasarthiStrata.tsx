...
export function MahasarthiStrata({ isOpen, onClose, onNavigate, isMobile = false }: MahasarthiStrataProps) {
  ...
  const rootItems = useMemo<StrataItem[]>(() => {
    ...
    const items: StrataItem[] = navigableDivisions.map(div => ({
      ...
      badge: div.status === 'preview' ? 'Preview' : undefined
    }));
  ...
  const handleDesktopToolOpen = (tool: 'filesystem' | 'editor') => {
    openDesktopTool(tool);
    onClose();
    onNavigate?.();
  };
    ...
    <>
      ...
      <div className={cn(
        ...
        <div className="flex flex-col h-full w-full">
          ...
          {isDesktopRoute && (
            <div className="flex-none border-b border-slate-200/50 bg-white/60 px-6 py-4 dark:border-slate-800/50 dark:bg-slate-900/40">
              <div className="mx-auto flex w-full max-w-6xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    type="button"
                    onClick={() => handleDesktopToolOpen('filesystem')}
                    className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-primary/30 hover:text-primary dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => handleDesktopToolOpen('editor')}
                    className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-primary/30 hover:text-primary dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"
                  >
