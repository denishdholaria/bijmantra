...
describe('createApiErrorFromResponse', () => {
  ...
  it('should capture backend trace id from response headers', async () => {
    const response = new Response(JSON.stringify({ message: 'Forbidden' }), {
      status: 403,
      headers: {
        'content-type': 'application/json',
        'X-Trace-Id': 'backend-trace-12345678',
      },
    });

    const error = await createApiErrorFromResponse(response);

    expect(error.context.traceId).toBe('backend-trace-12345678');
  });
});
...
describe('ApiError.isAuthError', () => {
  it('returns true for unauthorized errors', () => {
    const error = new ApiError('Unauthorized', ApiErrorType.UNAUTHORIZED, 401);
    expect(error.isAuthError()).toBe(true);
  });

  it('returns false for forbidden errors', () => {
    const error = new ApiError('Forbidden', ApiErrorType.FORBIDDEN, 403);
