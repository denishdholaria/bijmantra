...
export class CrossService {
  ...
  async getCrosses(page = 0, pageSize = 100) {
    return this.client.get<BrAPIListResponse<Cross>>(
      `/brapi/v2/crosses?page=${page}&pageSize=${pageSize}`
    );
  }

  async getCross(crossDbId: string) {
    return this.client.get<BrAPIResponse<Cross>>(`/brapi/v2/crosses/${crossDbId}`);
  }

  async createCross(data: Partial<Cross>) {
    return this.client.post<BrAPIResponse<Cross>>("/brapi/v2/crosses", data);
  }

  async updateCross(crossDbId: string, data: Partial<Cross>) {
    return this.client.put<BrAPIResponse<Cross>>(
