...
export interface MABCSelectionCandidate {
  individual_id: string;
  overall_score: number;
  foreground_score_percent: number;
  background_score_percent: number;
}

export interface MABCSelectionResponse {
  selected: MABCSelectionCandidate[];
}
...
export class MarkerAssistedService {
  ...
  async mabcSelection(data: MABCRequest) {
    return this.client.post<MABCSelectionResponse>("/api/v2/mas/mabc", data);
  }
