...
export class GenomicSelectionService {
  ...
  /**
   * Train a new RR-BLUP Model (Bio-Analytics Module)
   */
  async trainRRBLUP(payload: {
    model_name: string;
    trait_name: string;
    markers: number[][];
    phenotypes: number[];
    ...
    return this.client.post<any>(
      "/api/v2/bio-analytics/train/rr-blup",
      payload
    );
  }

  /**
   * Run GWAS Analysis (Bio-Analytics Module)
   */
  async runGWAS(payload: {
    run_name: string;
    trait_name: string;
    method: "glm" | "mlm";
    markers: { name: string; chromosome: string; position: number }[];
    genotypes: number[][]; // 0,1,2
    phenotypes: number[];
    kinship?: number[][];
  }) {
    return this.client.post<any>(
      "/api/v2/bio-analytics/gwas/run",
      payload
    );
  }
   ...
   */
  async trainFromStorage(payload: {
    model_name: string;
    trait_name: string;
    ...
    return this.client.post<any>(
      "/api/v2/bio-analytics/train/from-storage",
      payload
    );
  }
}
