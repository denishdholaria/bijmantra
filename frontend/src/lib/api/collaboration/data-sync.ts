...
export interface PendingSyncItemsResponse {
  items: SyncItem[];
  total: number;
}
...
export interface SyncSettingsUpdateResponse {
...
export class DataSyncService {
  ...
  async getPending() {
    return this.client.get<PendingSyncItemsResponse>("/api/v2/data-sync/pending");
  }

  async sync(action: string = "full_sync") {
    return this.client.post<SyncRequestResponse>(
  ...
  async getOfflineData() {
    return this.client.get<OfflineDataSummary>("/api/v2/data-sync/offline-data");
  }
  ...
  async updateSettings(settings: Partial<SyncSettings>) {
    ...
    return this.client.patch<SyncSettingsUpdateResponse>(
