/**
 * Legacy AI Data Context Engine.
 *
 * This helper performs broad application data dumps for prompt construction and
 * is non-canonical for REEVU production chat. New work should send a scoped
 * task context envelope to the backend REEVU API instead.
 */
...
/** @deprecated Replace broad context dumps with scoped REEVU task context. */
class AIDataContextEngine {
  ...
  private async fetchGermplasm(): Promise<any[]> {
    try {
      const response = await apiClient.germplasmService.getGermplasm(0, 100)
  ...
  private async fetchTrials(): Promise<any[]> {
    try {
      const response = await apiClient.trialService.getTrials(0, 50)
      return response?.result?.data || []
    } catch {
      return []
    }
  ...
  private async fetchStudies(): Promise<any[]> {
    try {
      const response = await apiClient.studyService.getStudies(0, 50)
      return response?.result?.data || []
    } catch {
      return []
    }
  ...
  private async fetchObservations(): Promise<any[]> {
    try {
      const response = await apiClient.observationService.getObservations(undefined, 0, 200)
      return response?.result?.data || []
    } catch {
      return []
    }
  ...
  private async fetchTraits(): Promise<any[]> {
    try {
      const response = await apiClient.observationService.getObservationVariables(0, 50)
      return response?.result?.data || []
    } catch {
      return []
    }
  ...
  private async fetchLocations(): Promise<any[]> {
    try {
      const response = await apiClient.locationService.getLocations(0, 50)
      return response?.result?.data || []
    } catch {
      return []
    }
  ...
  private async fetchPrograms(): Promise<any[]> {
    try {
      const response = await apiClient.programService.getPrograms(0, 50)
      return response?.result?.data || []
    } catch {
      return []
    }
