...
class RealtimeClient {
    ...
    try {
      ...
      this.eventSource.onmessage = (event) => {
        try {
          ...
          logger.error('[Realtime] Failed to parse event', e instanceof Error ? e : new Error(String(e)))
        }
      }
      ...
      logger.warn('[Realtime] SSE not available, using polling fallback', {
  ...
  private scheduleReconnect() {
    ...
    logger.info(`[Realtime] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})`)
  ...
  private emit(eventType: string, data: unknown) {
    ...
    this.listeners.get(eventType)?.forEach(callback => {
      try {
        ...
        logger.error(`[Realtime] Error in ${eventType} listener`, e instanceof Error ? e : new Error(String(e)))
      }
    })
    ...
    this.listeners.get('*')?.forEach(callback => {
      try {
        ...
        logger.error('[Realtime] Error in wildcard listener', e instanceof Error ? e : new Error(String(e)))
      }
    })
  }
