...
import { LEGACY_REEVU_ICON_KEY } from '@/lib/legacyReevu'
...
export const icons = {
  ...
  bot: Bot,
  ai: Brain,
  [LEGACY_REEVU_ICON_KEY]: Sparkles,
...
export function SmartIcon({ icon, size = 'md', className, strokeWidth = 2 }: SmartIconProps) {
  const resolvedName = resolveIconName(icon)

  if (resolvedName) {
    return <Icon name={resolvedName} size={size} className={className} strokeWidth={strokeWidth} />
  }

  return (
    <span className={cn('inline-flex items-center justify-center', fallbackSizeMap[size], className)}>
      {icon}
    </span>
  )
}
