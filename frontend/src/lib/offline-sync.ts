...
const getIndexeddbPersistence = async () => {
  if (!IndexeddbPersistence) {
    try {
      ...
      console.warn('[OfflineSync] IndexedDB persistence not available:', e)
    }
  }
...
class OfflineSyncService {
    ...
    if (PersistenceClass) {
      ...
      await new Promise<void>((resolve) => {
        this.persistence!.once('synced', () => {
          console.log('[OfflineSync] IndexedDB synced')
          resolve()
        })
      })
    } else {
      console.warn('[OfflineSync] Running without IndexedDB persistence')
    }
  ...
  private setupNetworkListeners(): void {
    ...
    window.addEventListener('offline', () => {
      ...
      console.log('[OfflineSync] Offline - changes will be queued')
    })
  }
  ...
  async syncToServer(): Promise<void> {
    ...
    try {
      ...
      console.log(`[OfflineSync] Synced ${pending.length} documents`)
    } catch (error) {
  ...
  private async syncDocumentType(
    ...
    for (const doc of docs) {
      try {
        ...
        if (token) {
          ...
          if (!response.ok) {
            ...
            if (response.status === 401) {
              console.warn('[OfflineSync] Auth token expired, skipping sync')
              return
            }
            // For other errors, log but continue with local marking
            console.warn(`[OfflineSync] Server sync failed for ${type}/${doc.id}: ${response.status}`)
          }
        }
        ...
        console.error(`[OfflineSync] Failed to sync ${type}/${doc.id}:`, error)
  ...
  async pullFromServer(): Promise<void> {
    ...
    const token = localStorage.getItem('auth_token')
    if (!token) {
      console.log('[OfflineSync] No auth token, skipping pull')
      return
    }

    console.log('[OfflineSync] Pulling from server...')
    ...
    for (const type of types) {
      try {
        ...
        if (!response.ok) {
          if (response.status === 401) {
            console.warn('[OfflineSync] Auth token expired')
            return
          }
          continue
        }
        ...
        console.error(`[OfflineSync] Failed to pull ${type}:`, error)
      }
    }

    console.log('[OfflineSync] Pull complete')
  }
  ...
  private emit(event: SyncEventType, data: unknown): void {
    this.listeners.get(event)?.forEach((callback) => {
      try {
        ...
        console.error(`[OfflineSync] Error in ${event} listener:`, e)
      }
    })
  }
