  ...
  persist(
    (set, get) => ({
      ...
      setActiveWorkspace: (workspaceId: WorkspaceId) => {
        ...
        // Deactivate any custom workspace — only one active-workspace concept at a time.
        // Import is deferred to avoid a circular store dependency at module load time.
        import('@/store/customWorkspaceStore').then(({ useCustomWorkspaceStore }) => {
          if (useCustomWorkspaceStore.getState().activeCustomWorkspaceId !== null) {
