...
def main() -> int:
    ...
    try:
        ...
        with TestClient(app) as client, patch("app.api.bijmantra.chat.get_llm_service") as mock_llm_service, patch(
            "app.api.bijmantra.chat.get_breeding_service"
        ) as mock_breeding_service, patch(
            "app.api.bijmantra.chat.AIQuotaService.check_and_increment_usage",
            new=AsyncMock(return_value=None),
        ), patch(
            "app.api.bijmantra.chat.FunctionCallingService.detect_function_call",
            new=AsyncMock(return_value=None),
        ), patch(
            "app.api.bijmantra.chat.ReevuService.get_or_create_user_context",
            new=AsyncMock(return_value=None),
        ), patch(
            "app.api.bijmantra.chat.ReevuService.update_interaction_stats",
            new=AsyncMock(return_value=None),
        ), patch(
            "app.api.bijmantra.chat.ReevuService.save_episodic_memory",
            new=AsyncMock(return_value=None),
        ):
