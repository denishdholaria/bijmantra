...
def main() -> int:
    ...
    try:
        ...
        with TestClient(app) as client, patch("app.api.v2.chat.get_llm_service") as mock_llm_service, patch(
            ...
            "app.api.v2.chat.ReevuService.get_or_create_user_context",
            new=AsyncMock(return_value=None),
        ), patch(
            "app.api.v2.chat.ReevuService.update_interaction_stats",
            new=AsyncMock(return_value=None),
        ), patch(
            "app.api.v2.chat.ReevuService.save_episodic_memory",
            new=AsyncMock(return_value=None),
        ):
