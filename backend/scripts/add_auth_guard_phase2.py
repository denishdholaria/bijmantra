...
def add_router_deps(filepath: Path):
    ...
    if (
        "from app.api.deps import get_current_user" not in content
        and "from app.api.bijmantra.dependencies import get_current_user" not in content
    ):
    ...
    if "Depends" not in content.split("router")[0]:
        content = re.sub(
            ...
            lambda m: (
                m.group(0) if "Depends" in m.group(2) else f"{m.group(1)}{m.group(2)}, Depends"
            ),
            content,
            count=1,
        )
