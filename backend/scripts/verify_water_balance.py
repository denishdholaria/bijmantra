...
async def main():
    ...
    async with async_session() as db:
        ...
        # Verify Summary
        summary = await water_balance_service.get_field_summary(
            db, field_id=field.id, org_id=org.id
        )
        print("Summary Result:", summary)

        assert summary["total_precipitation_mm"] == 10.0, (
            f"Expected 10.0, got {summary['total_precipitation_mm']}"
        )
        assert summary["total_irrigation_mm"] == 5.0, (
            f"Expected 5.0, got {summary['total_irrigation_mm']}"
        )
        assert summary["total_evapotranspiration_mm"] == 4.0, (
            f"Expected 4.0, got {summary['total_evapotranspiration_mm']}"
        )
        assert summary["record_count"] == 1, f"Expected 1, got {summary['record_count']}"

        print(
            "VERIFICATION SUCCESSFUL: Water Balance Module is Production Ready (Backend Logic Verified)"
        )
