"""Migrate all Integer primary keys to BigInteger for enterprise scale.
...
4. Widen all INTEGER id/organization_id/user_id columns to BIGINT
...
def _get_integer_columns(conn) -> list[tuple[str, str]]:
    """All INTEGER columns named id/organization_id/user_id in public schema."""
    result = conn.execute(sa.text("""
        SELECT table_name, column_name
        FROM information_schema.columns
        WHERE table_schema = 'public'
          AND data_type    = 'integer'
          AND column_name  IN ('id', 'organization_id', 'user_id')
...
def upgrade() -> None:
    ...
    # ── Step 5: Widen all integer id/organization_id/user_id columns ──────
