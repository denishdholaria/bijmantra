...
class DemoGenotypingSeeder(BaseSeeder):
    ...
    def _seed_reference_sets(self, org_id: int) -> int:
        ...
        reference_sets_data = [
            {
                "reference_set_db_id": stable_demo_id("refset", "IRGSP-1.0"),
            ...
            {
                "reference_set_db_id": stable_demo_id("refset", "IWGSC RefSeq v2.1"),
        ...
        for rs_data in reference_sets_data:
            ...
            for ref_data in refs:
                ref = Reference(
                    ...
                    reference_db_id=stable_demo_id(
                        "ref", ref_set.reference_set_name, ref_data["name"]
                    ),
    ...
    def _seed_genome_maps(self, org_id: int) -> int:
        ...
        maps_data = [
            {
                "map_db_id": stable_demo_id("map", "Rice High-Density SNP Map"),
    ...
    def _seed_variant_sets(self, org_id: int) -> int:
        ...
        variant_set = VariantSet(
            organization_id=org_id,
            variant_set_db_id=stable_demo_id("vs", "Rice 3K SNP Dataset"),
        ...
        for var_data in variants_data:
            variant = Variant(
                ...
                variant_db_id=stable_demo_id("var", var_data["name"]),
    ...
    def _seed_call_sets(self, org_id: int) -> int:
        ...
        # Get variants
        variants = (
            self.db.execute(select(Variant).where(Variant.variant_set_id == variant_set.id))
            .scalars()
            .all()
        )
        ...
        for i, cs_data in enumerate(call_sets_data):
            call_set = CallSet(
                organization_id=org_id,
                call_set_db_id=stable_demo_id("cs", cs_data["name"]),
            ...
            for j, variant in enumerate(variants):
                ...
                call = Call(
                    ...
                    call_db_id=stable_demo_id("call", cs_data["name"], variant.variant_name),
                    genotype_value=gt,
    ...
    def _seed_plates(self, org_id: int) -> int:
        ...
        for plate_data in plates_data:
            plate = Plate(
                organization_id=org_id,
                plate_db_id=stable_demo_id("plate", plate_data["barcode"]),
    ...
    def _seed_vendor_orders(self, org_id: int) -> int:
        ...
        for order_data in orders_data:
            order = VendorOrder(
                organization_id=org_id,
                order_db_id=stable_demo_id("order", order_data["order_id"]),
    ...
    def clear(self) -> int:
        ...
        total += (
            self.db.query(MarkerPosition).filter(MarkerPosition.organization_id == org_id).delete()
        )
