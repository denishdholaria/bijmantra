...
class DemoBrAPIPhenotypingSeeder(BaseSeeder):
    ...
    def seed(self) -> int:
        ...
        for data in methods_data:
            existing = (
                self.db.query(Method).filter(Method.method_db_id == data["method_db_id"]).first()
            )
            if not existing:
        ...
        for data in levels_data:
            existing = (
                self.db.query(ObservationLevel)
                .filter(
        ...
        for data in attrs_data:
            existing = (
                self.db.query(GermplasmAttributeDefinition)
                .filter(GermplasmAttributeDefinition.attribute_db_id == data["attribute_db_id"])
                .first()
            )
            if not existing:
        ...
        germplasm_list = (
            self.db.query(Germplasm).filter(Germplasm.organization_id == org_id).limit(2).all()
        )
        ...
        if germplasm_list:
            ...
            for i, germ in enumerate(germplasm_list):
                for j, val_data in enumerate(values_data):
                    ...
                    existing = (
                        self.db.query(GermplasmAttributeValue)
                        .filter(GermplasmAttributeValue.attribute_value_db_id == av_db_id)
                        .first()
                    )
                    if not existing:
    ...
    def clear(self) -> int:
        ...
        count += (
            self.db.query(GermplasmAttributeValue)
            .filter(GermplasmAttributeValue.organization_id == demo_org.id)
            .delete()
        )
        count += (
            self.db.query(GermplasmAttributeDefinition)
            .filter(GermplasmAttributeDefinition.organization_id == demo_org.id)
            .delete()
        )
        count += self.db.query(Trait).filter(Trait.organization_id == demo_org.id).delete()
        count += (
            self.db.query(ObservationLevel)
            .filter(ObservationLevel.organization_id == demo_org.id)
            .delete()
        )
        count += self.db.query(Method).filter(Method.organization_id == demo_org.id).delete()
        count += self.db.query(Scale).filter(Scale.organization_id == demo_org.id).delete()
