...
from app.core.demo_dataset import DEMO_DATASET_ORG_NAME, stable_demo_id
...
class DemoBrAPISeeder(BaseSeeder):
    ...
    def seed(self) -> int:
        ...
        existing_locations = {
            ...
            for location in self.db.query(Location).filter(Location.organization_id == org_id).all()
        }
        existing_programs = {
            program.program_name: program
            for program in self.db.query(Program).filter(Program.organization_id == org_id).all()
        }
        existing_trials = {
            trial.trial_name: trial
            for trial in self.db.query(Trial).filter(Trial.organization_id == org_id).all()
        }
        existing_studies = {
        ...
        locations_data = [
            {
                "location_db_id": stable_demo_id("demo_loc", "IRRI Research Station"),
                ...
                "latitude": 14.1677,
                "longitude": 121.2410,
                "coordinate_uncertainty": "1000m",
                "coordinate_description": "Approximate research-station centroid used for deterministic demo weather joins.",
            },
            {
                "location_db_id": stable_demo_id("demo_loc", "CRRI Research Farm"),
                ...
                "longitude": 85.9338,
                "coordinate_uncertainty": "1000m",
                "coordinate_description": "Approximate research-farm centroid used for deterministic demo weather joins.",
            },
            {
                "location_db_id": stable_demo_id("demo_loc", "IARI Experimental Farm"),
                ...
                "longitude": 77.1644,
                "coordinate_uncertainty": "1000m",
                "coordinate_description": "Approximate experimental-farm centroid used for deterministic demo weather joins.",
            },
            {
                "location_db_id": stable_demo_id("demo_loc", "PAU Research Station"),
        ...
        for loc_data in locations_data:
            ...
            existing = existing_locations.get(loc_data["location_name"])
            if existing:
                loc = existing
            else:
                loc = Location(organization_id=org_id)
                self.db.add(loc)
                total += 1

            loc.location_db_id = loc_data["location_db_id"]
            loc.location_name = loc_data["location_name"]
            loc.location_type = loc_data["location_type"]
            loc.abbreviation = loc_data["abbreviation"]
            loc.country_code = loc_data["country_code"]
            loc.country_name = loc_data["country_name"]
            loc.altitude = loc_data["altitude"]
            loc.coordinate_uncertainty = loc_data["coordinate_uncertainty"]
            loc.coordinate_description = loc_data["coordinate_description"]
            loc.coordinates = from_shape(Point(longitude, latitude), srid=4326)
        ...
        programs_data = [
            {
                "program_db_id": stable_demo_id("demo_prog", "Rice Improvement Program"),
                "program_name": "Rice Improvement Program",
                "abbreviation": "RIP",
                "objective": "Develop high-yielding, disease-resistant rice varieties",
            },
            {
                "program_db_id": stable_demo_id("demo_prog", "Wheat Breeding Program"),
                "program_name": "Wheat Breeding Program",
                "abbreviation": "WBP",
                "objective": "Develop climate-resilient wheat varieties",
            },
            {
                "program_db_id": stable_demo_id("demo_prog", "Maize Hybrid Development"),
        ...
        for prog_data in programs_data:
            existing = existing_programs.get(prog_data["program_name"])
            if existing:
                program_map[prog_data["program_name"]] = existing
                continue
        ...
        trials_data = [
            {
                "trial_db_id": stable_demo_id("demo_trial", "Rice Yield Trial 2024"),
            ...
            {
                "trial_db_id": stable_demo_id("demo_trial", "Wheat Disease Resistance Trial"),
            ...
            {
                "trial_db_id": stable_demo_id("demo_trial", "Maize Hybrid Evaluation 2024"),
        ...
        for trial_data in trials_data:
            ...
            existing = existing_trials.get(trial_data["trial_name"])
            if existing:
                trial_map[trial_data["trial_name"]] = existing
                continue
        ...
        studies_data = [
            {
                "study_db_id": stable_demo_id("demo_study", "RYT-2024-LOC1"),
            ...
            {
                "study_db_id": stable_demo_id("demo_study", "RYT-2024-LOC2"),
            ...
            {
                "study_db_id": stable_demo_id("demo_study", "WDR-2024-SCREEN"),
        ...
        for study_data in studies_data:
            ...
            if existing_studies.get(study_data["study_name"]):
                continue
