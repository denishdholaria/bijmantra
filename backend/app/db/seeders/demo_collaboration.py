...
from app.core.demo_dataset import demo_dataset_datetime
...
DEMO_NOW = demo_dataset_datetime()
    ...
    def _seed_workspaces(self, org_id: int, user_id: int) -> int:
        ...
        tasks = [
            CollaborationTask(
                ...
                due_date=DEMO_NOW + timedelta(days=2)
            ),
            CollaborationTask(
                organization_id=org_id,
                workspace_id=workspaces[1].id,
                ...
                due_date=DEMO_NOW + timedelta(days=5)
            ),
            CollaborationTask(
                organization_id=org_id,
                ...
                status=TaskStatus.TODO,
                priority=TaskPriority.HIGH,
                due_date=DEMO_NOW + timedelta(days=7)
            ),
            CollaborationTask(
                organization_id=org_id,
                ...
                priority=TaskPriority.MEDIUM,
                due_date=DEMO_NOW - timedelta(days=1),
                completed_at=DEMO_NOW - timedelta(days=1)
            ),
        ]
    ...
    def _seed_sync_data(self, org_id: int, user_id: int) -> int:
        ...
        sync_items = [
            SyncItem(
                ...
                size_bytes=24576,
                last_modified=DEMO_NOW - timedelta(hours=1)
            ),
            SyncItem(
                organization_id=org_id,
                user_id=user_id,
                ...
                status=SyncStatus.PENDING,
                size_bytes=8192,
                last_modified=DEMO_NOW - timedelta(hours=2)
            ),
            SyncItem(
                organization_id=org_id,
                user_id=user_id,
                ...
                status=SyncStatus.PENDING,
                size_bytes=47185920,
                last_modified=DEMO_NOW - timedelta(hours=3)
            ),
            SyncItem(
                organization_id=org_id,
                user_id=user_id,
                ...
                last_modified=DEMO_NOW - timedelta(hours=6)
            ),
        ]
        ...
        history = [
            SyncHistory(
                ...
                status="success",
                started_at=DEMO_NOW - timedelta(minutes=15),
                completed_at=DEMO_NOW - timedelta(minutes=10)
            ),
            SyncHistory(
                organization_id=org_id,
                user_id=user_id,
                ...
                status="success",
                started_at=DEMO_NOW - timedelta(minutes=30),
                completed_at=DEMO_NOW - timedelta(minutes=25)
            ),
            SyncHistory(
                organization_id=org_id,
                user_id=user_id,
                ...
                status="success",
                started_at=DEMO_NOW - timedelta(hours=1),
                completed_at=DEMO_NOW - timedelta(minutes=55)
            ),
        ]
        ...
        cache_entries = [
            OfflineDataCache(
                ...
                item_count=24,
                size_bytes=2516582,
                last_updated=DEMO_NOW - timedelta(hours=1)
            ),
            OfflineDataCache(
                organization_id=org_id,
                user_id=user_id,
            ...
            OfflineDataCache(
                ...
                item_count=3420,
                size_bytes=47806873,
                last_updated=DEMO_NOW - timedelta(hours=2)
            ),
            OfflineDataCache(
                organization_id=org_id,
                user_id=user_id,
                ...
                last_updated=DEMO_NOW - timedelta(minutes=30)
            ),
            OfflineDataCache(
                organization_id=org_id,
                user_id=user_id,
                ...
                last_updated=DEMO_NOW - timedelta(hours=3)
            ),
        ]
