...
from app.models.core import Ontology, Organization, Person, Season
...
class DemoCoreSeeder(BaseSeeder):
    ...
    def seed(self) -> int:
        ...
        for data in DEMO_PEOPLE:
            ...
            existing = (
                self.db.query(Person).filter(Person.email_address == data["email_address"]).first()
            )
            if existing:
                continue
            ...
            person = Person(
                ...
                person_db_id=stable_demo_id("person", data["email_address"]),
        ...
        for data in DEMO_SEASONS:
            existing = (
                self.db.query(Season).filter(Season.season_name == data["season_name"]).first()
            )
            if existing:
                continue
            ...
            season = Season(
                organization_id=org.id,
                season_db_id=stable_demo_id("season", data["season_name"]),
        ...
        for data in DEMO_ONTOLOGIES:
            existing = (
                ...
                .filter(Ontology.ontology_name == data["ontology_name"])
                .first()
            )
            if existing:
                continue
            ...
            ontology = Ontology(
                organization_id=org.id,
                ontology_db_id=stable_demo_id("onto", data["ontology_name"]),
        ...
        for data in DEMO_LISTS:
            existing = (
                self.db.query(ListModel).filter(ListModel.list_name == data["list_name"]).first()
            )
            if existing:
                continue
            ...
            list_obj = ListModel(
                organization_id=org.id,
                list_db_id=stable_demo_id("list", data["list_name"]),
    ...
    def clear(self) -> int:
        ...
        # Delete people
        count = self.db.query(Person).filter(Person.organization_id == org.id).delete()
        total += count

        # Delete lists
        count = self.db.query(ListModel).filter(ListModel.organization_id == org.id).delete()
        total += count

        # Delete ontologies
        count = self.db.query(Ontology).filter(Ontology.organization_id == org.id).delete()
        total += count

        # Delete seasons
        count = self.db.query(Season).filter(Season.organization_id == org.id).delete()
        total += count
