...
DEMO_GERMPLASM = [
    ...
    },
    {
        ...
        "common_crop_name": "Wheat",
        "country_of_origin_code": "IND",
        "accession_number": "DEMO-UP-001",
        "institute_code": "CSAUAT",
        "institute_name": "Chandra Shekhar Azad University of Agriculture and Technology",
        "biological_status_of_accession_code": "500",
    },
]
...
def get_or_create_demo_organization(db: Session):
    ...
    org = db.query(Organization).filter(Organization.name == DEMO_DATASET_ORG_NAME).first()
    if not org:
        org = Organization(
...
class DemoGermplasmSeeder(BaseSeeder):
    ...
    def seed(self) -> int:
        ...
        accession_numbers = [
            data["accession_number"] for data in DEMO_GERMPLASM if data.get("accession_number")
        ]
        existing_accession_numbers = set()

        if accession_numbers:
            existing_records = (
                self.db.query(Germplasm.accession_number)
                .filter(Germplasm.accession_number.in_(accession_numbers))
                .all()
            )
        ...
        for data in DEMO_GERMPLASM:
            ...
            germplasm_db_id = stable_demo_id(
                "demo_germplasm",
                data.get("accession_number") or data["germplasm_name"],
            )
    ...
    def clear(self) -> int:
        ...
        count = self.db.query(Germplasm).filter(Germplasm.organization_id == org.id).delete()
