...
from app.models.germplasm import CrossingProject, Germplasm, PlannedCross
...
class DemoCrossingSeeder(BaseSeeder):
    ...
    def seed(self) -> int:
        ...
        # Pre-fetch existing projects to avoid N+1 query
        project_names = [p["crossing_project_name"] for p in DEMO_CROSSING_PROJECTS]
        existing_projects = (
            self.db.query(CrossingProject)
            .filter(CrossingProject.crossing_project_name.in_(project_names))
            .all()
        )
        existing_project_map = {p.crossing_project_name: p.id for p in existing_projects}

        for data in DEMO_CROSSING_PROJECTS:
            if data["crossing_project_name"] in existing_project_map:
                project_ids[data["crossing_project_name"]] = existing_project_map[
                    data["crossing_project_name"]
                ]
                continue

            project = CrossingProject(
                organization_id=org.id,
                crossing_project_db_id=stable_demo_id(
                    "cp", data["crossing_project_name"]
                ),
        ...
        planned_cross_names = [c["planned_cross_name"] for c in DEMO_PLANNED_CROSSES]
        existing_crosses = (
            self.db.query(PlannedCross.planned_cross_name)
            .filter(PlannedCross.planned_cross_name.in_(planned_cross_names))
            .all()
        )
        existing_cross_names = {c[0] for c in existing_crosses}

        for data in DEMO_PLANNED_CROSSES:
            if data["planned_cross_name"] in existing_cross_names:
                continue

            cross = PlannedCross(
                organization_id=org.id,
                crossing_project_id=project_ids.get(data.get("project_name")),
                planned_cross_db_id=stable_demo_id("pc", data["planned_cross_name"]),
                ...
                parent2_db_id=germplasm_ids.get(data.get("parent2_name"))
    ...
    def clear(self) -> int:
        ...
        count = self.db.query(PlannedCross).filter(PlannedCross.organization_id == org.id).delete()
        total += count

        # Delete crossing projects
        count = (
            self.db.query(CrossingProject)
            .filter(CrossingProject.organization_id == org.id)
            .delete()
        )
        total += count
