...
class DemoIoTSeeder(BaseSeeder):
    ...
    def seed(self) -> int:
        ...
        demo_org = (
            self.db.query(Organization).filter(Organization.name == settings.DEMO_ORG_NAME).first()
        )
        if not demo_org:
        ...
        for data in devices_data:
            existing = (
                ...
                .filter(IoTDevice.device_db_id == data["device_db_id"])
                .first()
            )
            if not existing:
        ...
        sensors_data = [
            ...
            {
                "sensor_db_id": "SEN-004-CT",
                "device_db_id": "DEV-PS-001",
                "sensor_type": "canopy_temperature",
        ...
        for data in sensors_data:
            ...
            existing = (
                ...
                .filter(
                    IoTSensor.sensor_db_id == data["sensor_db_id"], IoTSensor.device_id == device_id
                )
                .first()
            )
            if not existing:
        ...
        rules_data = [
            {
                "rule_db_id": stable_demo_code("RULE", "High Temperature Alert"),
            ...
            {
                "rule_db_id": stable_demo_code("RULE", "Low Soil Moisture Alert"),
            ...
            {
                "rule_db_id": stable_demo_code("RULE", "Critical Temperature Alert"),
            ...
            {
                "rule_db_id": stable_demo_code("RULE", "Frost Warning"),
                "name": "Frost Warning",
        ...
        for data in rules_data:
            existing = (
                ...
                .filter(IoTAlertRule.name == data["name"], IoTAlertRule.organization_id == org_id)
                .first()
            )
            if not existing:
        ...
        events_data = [
            {
                "event_db_id": stable_demo_code(
                    "ALERT", "Low battery warning on Soil Probe Block B (15%)"
                ),
            ...
            {
                "event_db_id": stable_demo_code(
                    "ALERT", "High temperature alert: 38.5°C exceeds threshold of 35°C"
                ),
            ...
            {
                "event_db_id": stable_demo_code(
                    "ALERT", "Low soil moisture: 28% below threshold of 30%"
                ),
                "alert_type": "threshold",
                "severity": "warning",
        ...
        for data in events_data:
            ...
            existing = (
                ...
                .filter(IoTAlertEvent.message == data["message"])
                .first()
            )
            if not existing:
        ...
        for env_id in environment_ids:
            for day_offset in range(7):
                ...
                temp_mean = stable_demo_float(
                    22.0, 30.0, env_id, day_offset, "air_temperature_mean", digits=1
                )
                aggregates = [
                    {
                    ...
                    },
                    {
                        ...
                        "value": stable_demo_choice(
                            [0.0, 0.0, 0.0, 0.0, 2.5, 5.0, 12.0, 25.0],
                            env_id,
                            day_offset,
                            "precipitation_total",
                        ),
                    ...
                    },
                    {
                        ...
                        "value": stable_demo_float(
                            30.0, 55.0, env_id, day_offset, "soil_moisture_mean", digits=1
                        ),
                        "unit": "%",
                        "period": "daily",
                        "start_time": date,
