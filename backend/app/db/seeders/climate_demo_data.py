...
from app.models.climate.carbon_monitoring import CarbonMeasurement, CarbonStock
from app.models.climate.emissions import EmissionFactor, EmissionSource, VarietyFootprint
...
async def get_demo_locations(db: AsyncSession, org_id: int, count: int = 10) -> list[Location]:
    ...
    result = await db.execute(
        select(Location).where(Location.organization_id == org_id).limit(count)
    )
...
async def create_emission_sources(db: AsyncSession, org_id: int, locations: list[Location]):
    ...
    for _i, location in enumerate(locations[:7]):  # First 7 locations
        for month in range(0, 12, 4):
            for emission_type, activity_data in emission_types:
                ...
                if emission_type == "fertilizer":
                    co2e = (
                        (activity_data["n_kg"] * 5.87)
                        + (activity_data["p_kg"] * 0.2)
                        + (activity_data["k_kg"] * 0.15)
                    )
                elif emission_type == "fuel":
                    co2e = (activity_data["diesel_liters"] * 2.68) + (
                        activity_data.get("petrol_liters", 0) * 2.31
                    )
                else:  # irrigation
...
async def create_emission_factors(db: AsyncSession, org_id: int):
    ...
    for factor_data in factors:
        factor = EmissionFactor(organization_id=org_id, **factor_data)
        db.add(factor)
...
async def create_variety_releases(db: AsyncSession, org_id: int):
    ...
    for release_data in releases:
        release = VarietyRelease(organization_id=org_id, **release_data)
        db.add(release)
...
async def create_policy_adoptions(db: AsyncSession, org_id: int):
    ...
    for adoption_data in adoptions:
        adoption = PolicyAdoption(organization_id=org_id, **adoption_data)
        db.add(adoption)
