...
async def login(
    ...
    is_demo = user.email == settings.DEMO_USER_EMAIL or (
        organization and organization.name == settings.DEMO_ORG_NAME
    )
...
async def register(user_in: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    if not is_strong_password(user_in.password):
        raise HTTPException(
            ...
            detail="Password policy violation: must be at least 8 characters and contain an uppercase letter, a lowercase letter, a digit, and a special character.",
        )
...
async def reset_rate_limit(
    ...
    x_ratelimit_reset_token: str | None = Header(None, alias="X-RateLimit-Reset-Token"),
):
    """
    Reset rate limit for the requesting IP or a specific IP.

    SECURITY: Only available in development/test mode (DEBUG=True) AND requires a valid token.
    ...
    # Verify reset token
    if (
        not settings.RATE_LIMIT_RESET_TOKEN
        or x_ratelimit_reset_token != settings.RATE_LIMIT_RESET_TOKEN
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Invalid or missing rate limit reset token",
        )
