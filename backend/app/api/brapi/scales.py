...
def brapi_response(result, page: int = 0, page_size: int = 1000):
    ...
    Args:
        ...
        page (int): The current page number for paginated results.
        page_size (int): The number of items per page for paginated results.
...
def scale_to_brapi(scale: Scale) -> dict:
    """Converts a Scale model object to its BrAPI representation.

    Args:
        scale (Scale): The SQLAlchemy Scale model object.

    Returns:
        dict: A dictionary representing the scale in BrAPI format.
    """
    valid_values = {}
...
async def get_scales(
    ...
    Args:
        ...
        scaleDbId (Optional[str]): The unique identifier for a scale.
        scaleName (Optional[str]): The name of the scale.
        dataType (Optional[str]): The data type of the scale values.
        ontologyDbId (Optional[str]): The unique identifier for an ontology.
        externalReferenceId (Optional[str]): An external reference ID.
        externalReferenceSource (Optional[str]): The source of the external reference.
        page (int): The page number for pagination.
        pageSize (int): The number of items per page.
        db (AsyncSession): The database session dependency.

    Returns:
        dict: A BrAPI-formatted response containing a list of scales.
    """
...
async def get_scale(
    ...
    Args:
        scaleDbId (str): The unique identifier for the scale.
        request (Request): The incoming FastAPI request.
        db (AsyncSession): The database session dependency.

    Returns:
        dict: A BrAPI-formatted response containing the scale, or a 'not found' message.
    """
    org_id = getattr(request.state, "organization_id", None)
    is_superuser = getattr(request.state, "is_superuser", False)
    await set_tenant_context(db, org_id, is_superuser)
...
async def create_scales(
    ...
    Args:
        scales (List[dict]): A list of dictionaries, each representing a scale to be created.
        request (Request): The incoming FastAPI request.
        db (AsyncSession): The database session dependency.
    ...
    Returns:
        dict: A BrAPI-formatted response containing the newly created scales.
    """
    org_id = getattr(request.state, "organization_id", None)
    is_superuser = getattr(request.state, "is_superuser", False)
    await set_tenant_context(db, org_id, is_superuser)
...
async def update_scale(
    ...
    Args:
        scaleDbId (str): The unique identifier for the scale to be updated.
        scale_data (dict): A dictionary containing the updated scale data.
        request (Request): The incoming FastAPI request.
        db (AsyncSession): The database session dependency.

    Returns:
        dict: A BrAPI-formatted response containing the updated scale, or a 'not found' message.
    """
    org_id = getattr(request.state, "organization_id", None)
    is_superuser = getattr(request.state, "is_superuser", False)
    await set_tenant_context(db, org_id, is_superuser)
