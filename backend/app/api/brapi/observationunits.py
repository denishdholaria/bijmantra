...
async def list_observation_units(
    ...
    query = db.query(ObservationUnit)
    
    if studyDbId:
        query = query.filter(ObservationUnit.study_id == int(studyDbId))
    if germplasmDbId:
        query = query.filter(ObservationUnit.germplasm_id == int(germplasmDbId))
    if observationLevel:
        query = query.filter(ObservationUnit.observation_level == observationLevel)
    if observationUnitDbId:
        query = query.filter(ObservationUnit.observation_unit_db_id == observationUnitDbId)
...
async def update_observation_units_bulk(
    ...
    for unit_data in units:
        ...
        unit = db.query(ObservationUnit).filter(
            ObservationUnit.observation_unit_db_id == unit_data.observationUnitDbId
        ).first()
