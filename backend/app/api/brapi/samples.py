...
async def list_samples(
    ...
    """Get list of samples from database"""
    query = db.query(Sample)
    
    if sampleDbId:
        query = query.filter(Sample.sample_db_id == sampleDbId)
    if sampleName:
        query = query.filter(Sample.sample_name.ilike(f"%{sampleName}%"))
    if sampleType:
        query = query.filter(Sample.sample_type == sampleType)
    if plateDbId:
        query = query.filter(Sample.plate_db_id == plateDbId)
    if germplasmDbId:
        query = query.filter(Sample.germplasm_id == int(germplasmDbId))
    if studyDbId:
        query = query.filter(Sample.study_id == int(studyDbId))
    ...
    data = [_model_to_brapi(s) for s in results]
...
async def get_sample(
    ...
    """Get sample by ID from database"""
    sample = db.query(Sample).filter(Sample.sample_db_id == sampleDbId).first()
...
async def update_samples_bulk(
    ...
    for sample_data in samples:
        ...
        sample = db.query(Sample).filter(
            Sample.sample_db_id == sample_data.sampleDbId
        ).first()
