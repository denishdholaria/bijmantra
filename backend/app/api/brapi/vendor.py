...
from app.api.deps import get_optional_user, get_organization_id
from app.core.database import get_db
from app.models.genotyping import Plate, VendorOrder
...
class VendorPlateSubmission(BaseModel):
    ...
    numberOfSamples: int | None = None
    plates: list[dict] | None = None
    sampleType: str | None = None
    serviceIds: list[str] | None = None
...
class VendorOrderRequest(BaseModel):
    ...
    numberOfSamples: int | None = None
    plates: list[dict] | None = None
    requiredServiceInfo: dict | None = None
    sampleType: str | None = None
    serviceIds: list[str] | None = None
...
async def create_vendor_order(
    ...
    new_order = VendorOrder(
        ...
        submitted_date=datetime.now(UTC).isoformat() + "Z",
        plates=order.plates or [],
    ...
    results = [
        {
            ...
            "resultTimestamp": order.completed_date or datetime.now(UTC).isoformat() + "Z"
        },
        {
...
async def submit_vendor_plates(
    ...
    new_submission = {
        ...
        "submittedDate": datetime.now(UTC).isoformat() + "Z",
        "status": "submitted"
    }
