...
async def create_event(
    event: EventCreate, db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)
):
    """Create a new event.
...
async def get_event(
    eventDbId: str, db: AsyncSession = Depends(get_db), current_user=Depends(get_optional_user)
):
