...
async def list_variables(
    ...
    query = db.query(ObservationVariable)
    
    if observationVariableDbId:
        query = query.filter(ObservationVariable.observation_variable_db_id == observationVariableDbId)
    if observationVariableName:
        query = query.filter(ObservationVariable.observation_variable_name.ilike(f"%{observationVariableName}%"))
    if traitClass:
        query = query.filter(ObservationVariable.trait_class.ilike(f"%{traitClass}%"))
    if commonCropName:
        query = query.filter(ObservationVariable.common_crop_name.ilike(f"%{commonCropName}%"))
    if methodDbId:
        query = query.filter(ObservationVariable.method_db_id == methodDbId)
    if scaleDbId:
        query = query.filter(ObservationVariable.scale_db_id == scaleDbId)
    if ontologyDbId:
        query = query.filter(ObservationVariable.ontology_db_id == ontologyDbId)
    
    total = query.count()
    results = query.offset(page * pageSize).limit(pageSize).all()
    data = [_model_to_brapi(v) for v in results]
