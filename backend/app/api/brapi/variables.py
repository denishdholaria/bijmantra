...
def _model_to_brapi(var: ObservationVariable) -> dict:
    ...
    Args:
        var: The ObservationVariable SQLAlchemy model instance.

    Returns:
        A dictionary formatted for BrAPI v2.1 response.
    """
    return {
...
async def list_variables(
    ...
    """Get a list of observation variables.

    Args:
        page: The page number for pagination.
        pageSize: The number of items per page.
        observationVariableDbId: Filter by observation variable DB ID.
        observationVariableName: Filter by observation variable name.
        traitClass: Filter by trait class.
        commonCropName: Filter by common crop name.
        methodDbId: Filter by method DB ID.
        scaleDbId: Filter by scale DB ID.
        ontologyDbId: Filter by ontology DB ID.
        db: The database session.
        current_user: The current authenticated user.

    Returns:
        A dictionary containing the list of observation variables and metadata.
    """
    ...
    if observationVariableName:
        stmt = stmt.where(
            ObservationVariable.observation_variable_name.ilike(f"%{observationVariableName}%")
        )
    if traitClass:
...
async def create_variable(
    ...
    Args:
        variable: The observation variable data to create.
        db: The database session.
        current_user: The current authenticated user.

    Returns:
        A dictionary containing the newly created observation variable and metadata.
    """
...
async def get_variable(
    ...
    """Get an observation variable by its ID.

    Args:
        observationVariableDbId: The ID of the observation variable.
        db: The database session.
        current_user: The current authenticated user.

    Returns:
        A dictionary containing the observation variable data and metadata.

    Raises:
        HTTPException: If the observation variable is not found.
    """
...
async def update_variable(
    ...
    Args:
        observationVariableDbId: The ID of the observation variable to update.
        variable: The observation variable data to update.
        db: The database session.
        current_user: The current authenticated user.

    Returns:
        A dictionary containing the updated observation variable and metadata.
...
async def delete_variable(
    ...
    """Delete an observation variable.

    Args:
        observationVariableDbId: The ID of the observation variable to delete.
        db: The database session.
        current_user: The current authenticated user.
