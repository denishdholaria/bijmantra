...
class CrossingProjectUpdate(BaseModel):
    ...
    programDbId: str | None = None
    programName: str | None = None
    potentialParentDbIds: list[str] | None = None
...
async def get_crossing_projects(
    ...
    if crossingProjectName:
        query = query.where(
            CrossingProjectModel.crossing_project_name.ilike(f"%{crossingProjectName}%")
        )
    if commonCropName:
        ...
        if proj_in.programDbId:
            prog_result = await db.execute(
                select(Program.id).where(
                    Program.program_db_id == proj_in.programDbId, Program.organization_id == org_id
                )
            )
