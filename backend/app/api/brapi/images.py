...
def _model_to_brapi(image: Image) -> dict:
    """Converts an Image SQLAlchemy model to a BrAPI-compliant dictionary.

    Args:
        image (Image): The Image SQLAlchemy model instance.

    Returns:
        A dictionary representation of the image conforming to BrAPI v2.1 specification.
    """
    return {
...
async def list_images(
    ...
    Args:
        ...
        imageDbId (Optional[str]): Filter by image database ID.
        imageName (Optional[str]): Filter by image name (case-insensitive).
        observationUnitDbId (Optional[str]): Filter by observation unit ID.
        observationDbId (Optional[str]): Filter by observation ID.
        db (AsyncSession): The database session.
        current_user (Optional[User]): The currently authenticated user.

    Returns:
        A dictionary containing a paginated list of images and metadata.
    """
    ...
    if current_user and current_user.organization_id:
        stmt = stmt.where(Image.organization_id == current_user.organization_id)
...
async def create_image(
    ...
    Args:
        image (ImageCreate): The image data to create.
        db (AsyncSession): The database session.
        current_user (User): The currently authenticated user.

    Raises:
        HTTPException: If an image with the same name already exists.

    Returns:
        A dictionary containing the newly created image and metadata.
    """
...
async def get_image(
    ...
    """Retrieves a specific image by its database ID.

    Args:
        imageDbId (str): The database ID of the image to retrieve.
        db (AsyncSession): The database session.
        current_user (Optional[User]): The currently authenticated user.
...
async def update_image(
    ...
    Args:
        imageDbId (str): The database ID of the image to update.
        image_data (ImageUpdate): The new data for the image.
        db (AsyncSession): The database session.
        current_user (User): The currently authenticated user.

    Raises:
        HTTPException: If the image with the given ID is not found,
                       or if an image with the same name already exists.

    Returns:
        A dictionary containing the updated image and metadata.
    """
    stmt = select(Image).options(selectinload(Image.observation_unit)).where(
        Image.image_db_id == imageDbId
    )
...
async def update_image_content(
    ...
    """Updates the content of an image (placeholder).

    Note:
        This endpoint is a placeholder and does not currently handle
        actual binary file uploads. It validates the image's existence
    ...
    Returns:
        A dictionary containing the image metadata and a success status.
    """
    stmt = select(Image).options(selectinload(Image.observation_unit)).where(
        Image.image_db_id == imageDbId
    )
...
async def delete_images_bulk(
    ...
    """Deletes multiple images in a single bulk operation.

    Args:
        request (DeleteImagesRequest): A request object containing a list of
                                       image database IDs to delete.
        db (AsyncSession): The database session.
        current_user (User): The currently authenticated user.

    Raises:
        HTTPException: If an image to be deleted is still referenced by other entities.

    Returns:
        A dictionary containing a list of the deleted image IDs and metadata.
    """
    deleted_ids = []
