...
from app.models.germplasm import Germplasm, Seedlot, SeedlotTransaction
...
async def get_seedlot(
    seedLotDbId: str, db: AsyncSession = Depends(get_tenant_db), current_user=Depends(get_optional_user)
):
...
class TransactionBase(BaseModel):
    ...
    transactionTimestamp: str | None = None
    amount: float | None = None
    units: str | None = None
    fromSeedLotDbId: str | None = None
    toSeedLotDbId: str | None = None
    additionalInfo: dict | None = None
    externalReferences: dict | None = None
...
async def list_seedlot_transactions(
    ...
    # Get transactions
    stmt = (
        select(SeedlotTransaction)
        .options(selectinload(SeedlotTransaction.seedlot))
