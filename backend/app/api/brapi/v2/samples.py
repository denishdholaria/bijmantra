...
from app.models.phenotyping import ObservationUnit, Sample
...
class SampleBase(BaseModel):
    ...
    sampleBarcode: str | None = None
    samplePUI: str | None = None
    sampleGroupDbId: str | None = None
    sampleTimestamp: str | None = None
    takenBy: str | None = None
    observationUnitDbId: str | None = None
    germplasmDbId: str | None = None
    studyDbId: str | None = None
    plateDbId: str | None = None
...
def _model_to_brapi(sample: Sample) -> dict:
    ...
    return {
        ...
        "observationUnitDbId": sample.observation_unit.observation_unit_db_id
...
async def create_sample(
    sample: SampleCreate, db: AsyncSession = Depends(get_tenant_db), current_user=Depends(get_current_user)
):
...
async def get_sample(
    sampleDbId: str, db: AsyncSession = Depends(get_tenant_db), current_user=Depends(get_optional_user)
):
    ...
    stmt = (
        select(Sample)
        .options(selectinload(Sample.observation_unit))
        .where(Sample.sample_db_id == sampleDbId)
    )
    result = await db.execute(stmt)
