...
# ===========================================
# BrAPI Response Models (imported from canonical location)
# ===========================================
...
async def get_iot_devices(
    ...
    deviceType: str | None = Query(None, description="Filter by device type"),
    status: str | None = Query(None, description="Filter by status"),
    environmentDbId: str | None = Query(None, description="Filter by environment"),
...
async def get_iot_sensors(
    page: int = Query(0, ge=0, description="Page number"),
    pageSize: int = Query(100, ge=1, le=1000, description="Page size"),
    sensorType: str | None = Query(None, description="Filter by sensor type"),
    deviceDbId: str | None = Query(None, description="Filter by device"),
    db: AsyncSession = Depends(get_tenant_db)
):
    """
...
async def get_iot_telemetry(
    ...
    sensorDbId: str | None = Query(None, description="Sensor ID (optional)"),
    startTime: str = Query(..., description="ISO 8601 start time (required)"),
    endTime: str = Query(..., description="ISO 8601 end time (required)"),
    downsample: str | None = Query(None, description="Downsample interval"),
    page: int = Query(0, ge=0, description="Page number"),
...
async def get_iot_aggregates(
    ...
    startDate: str | None = Query(None, description="Filter by start date"),
    endDate: str | None = Query(None, description="Filter by end date"),
    page: int = Query(0, ge=0, description="Page number"),
    pageSize: int = Query(100, ge=1, le=1000, description="Page size"),
    db: AsyncSession = Depends(get_tenant_db)
):
...
async def get_iot_alerts(
    ...
    deviceDbId: str | None = Query(None, description="Filter by device"),
    startTime: str | None = Query(None, description="Filter alerts after this time"),
    endTime: str | None = Query(None, description="Filter alerts before this time"),
    page: int = Query(0, ge=0, description="Page number"),
    pageSize: int = Query(100, ge=1, le=1000, description="Page size"),
    db: AsyncSession = Depends(get_tenant_db)
):
...
async def get_iot_environment_links(
    ...
    return BrAPIResponse(
        ...
        result={"data": [link_to_response(link) for link in links]}
    )
