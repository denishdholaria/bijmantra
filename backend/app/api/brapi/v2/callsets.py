...
from app.models.genotyping import Call, CallSet, VariantSet
...
def brapi_response(result, page: int = 0, page_size: int = 1000, total: int = None):
    """Wraps the result in a BrAPI standard response object.

    Args:
        result: The result data to be returned.
        ...
        total (int, optional): The total number of items. Defaults to None.

    Returns:
        dict: A BrAPI standard response object.
    """
...
def callset_to_brapi(callset: CallSet) -> dict:
    """Converts a CallSet SQLAlchemy model to a BrAPI-compliant dictionary.

    Args:
        callset (CallSet): The CallSet SQLAlchemy model.

    Returns:
        dict: A BrAPI-compliant dictionary representing the call set.
    """
    return {
...
async def get_callsets(
    ...
    """Retrieves a list of call sets from the database.

    Args:
        callSetDbId (Optional[str], optional): The ID of the call set to retrieve. Defaults to None.
        callSetName (Optional[str], optional): The name of the call set to retrieve. Defaults to None.
        sampleDbId (Optional[str], optional): The ID of the sample to retrieve. Defaults to None.
        variantSetDbId (Optional[str], optional): The ID of the variant set to retrieve. Defaults to None.
        germplasmDbId (Optional[str], optional): The ID of the germplasm to retrieve. Defaults to None.
        page (int, optional): The page number to retrieve. Defaults to Query(0, ge=0).
        pageSize (int, optional): The number of items per page. Defaults to Query(1000, ge=1, le=10000).
        db (AsyncSession, optional): The database session. Defaults to Depends(get_tenant_db).

    Returns:
        dict: A BrAPI-compliant response containing a list of call sets.
    """
    # Build query
...
async def get_callset(
    ...
    """Retrieves a single call set by its ID.

    Args:
        callSetDbId (str): The ID of the call set to retrieve.
        db (AsyncSession, optional): The database session. Defaults to Depends(get_tenant_db).

    Returns:
        dict: A BrAPI-compliant response containing the call set.

    Raises:
        HTTPException: An exception with a 404 status code if the call set
            is not found.
    """
...
async def get_callset_calls(
    ...
    """Retrieves all calls for a specific call set.

    Args:
        callSetDbId (str): The ID of the call set.
    ...
    Returns:
        dict: A BrAPI-compliant response containing a list of calls.
