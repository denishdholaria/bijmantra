...
from app.modules.core.services.job_service import job_service
...
async def search_attributevalues(
    ...
    query = select(GermplasmAttribute).options(
        selectinload(GermplasmAttribute.germplasm)
    ).join(Germplasm)
...
async def search_markerpositions(
    ...
    # Build query from database
    query = select(MarkerPosition).options(
        selectinload(MarkerPosition.genome_map)
    )
...
async def search_allelematrix(
    ...
    # Get variants
    variant_query = select(Variant).options(
    ...
    if call_sets and variants:
        calls_query = select(Call).options(
            selectinload(Call.variant),
            selectinload(Call.call_set),
        ).join(CallSet).join(Variant).where(
