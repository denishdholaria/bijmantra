...
from app.modules.phenotyping.services.image_service import image_service


router = APIRouter()


def create_brapi_response(
    data: any, page: int = 0, page_size: int = 0, total_count: int = 0
) -> BrAPIResponse:
    total_pages = (total_count + page_size - 1) // page_size if page_size > 0 else 0
    return BrAPIResponse(
        metadata=Metadata(
            pagination=Pagination(
                currentPage=page, pageSize=page_size, totalCount=total_count, totalPages=total_pages
            ),
...
def _model_to_dict(image) -> dict:
    ...
    return {
        ...
        "observationUnitDbId": image.observation_unit.observation_unit_db_id
        if image.observation_unit
        else None,
...
async def upload_image_file(file: UploadFile = File(...), current_user=Depends(get_current_user)):
    """
...
async def get_image(
    imageDbId: str, db: AsyncSession = Depends(get_tenant_db), current_user=Depends(get_optional_user)
):
