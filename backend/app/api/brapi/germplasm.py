...
from app.services.germplasm_service import GermplasmService
...
async def create_germplasm(
    ...
    await GermplasmService.log_mutation(db, org_id, getattr(current_user, "id", None), "POST", new_germplasm.germplasm_db_id, {"germplasmName": germplasm.germplasmName})
    await db.commit()
...
async def update_germplasm(
    ...
    await GermplasmService.log_mutation(db, getattr(current_user, "organization_id", None), getattr(current_user, "id", None), "PUT", existing.germplasm_db_id, {"germplasmName": germplasm.germplasmName})
    await db.commit()
...
async def delete_germplasm(
    ...
    if not existing:
        raise HTTPException(status_code=404, detail="Germplasm not found")
    target_id = existing.germplasm_db_id
    await db.delete(existing)
    await db.commit()
    await GermplasmService.log_mutation(db, getattr(current_user, "organization_id", None), getattr(current_user, "id", None), "DELETE", target_id, None)
    await db.commit()
...
async def get_germplasm_pedigree(
    ...
    Raises:
        HTTPException: If the germplasm is not found.
    """
    query = select(GermplasmModel).options(
        selectinload(GermplasmModel.cross).selectinload(Cross.parent1),
        selectinload(GermplasmModel.cross).selectinload(Cross.parent2),
        selectinload(GermplasmModel.cross).selectinload(Cross.crossing_project)
    ).where(GermplasmModel.germplasm_db_id == germplasmDbId)
    ...
    if germ.cross:
        c = germ.cross
        crossingYear = c.crossing_year
        if c.crossing_project:
            crossingProjectDbId = c.crossing_project.crossing_project_db_id

        if c.parent1:
            parents.append({
                "germplasmDbId": c.parent1.germplasm_db_id,
                "germplasmName": c.parent1.germplasm_name,
                "parentType": c.parent1_type or "FEMALE"
            })
        if c.parent2:
            parents.append({
                "germplasmDbId": c.parent2.germplasm_db_id,
                "germplasmName": c.parent2.germplasm_name,
                "parentType": c.parent2_type or "MALE"
            })

    siblings = []
    if includeSiblings and germ.cross_id:
        sib_query = select(GermplasmModel).where(
            GermplasmModel.cross_id == germ.cross_id,
            GermplasmModel.id != germ.id
        )
        sib_result = await db.execute(sib_query)
        sibs = sib_result.scalars().all()
        for s in sibs:
            siblings.append({
                "germplasmDbId": s.germplasm_db_id,
                "germplasmName": s.germplasm_name
            })

    pedigree_str = germ.pedigree
    if not pedigree_str and parents:
        p1_name = parents[0]["germplasmName"] if len(parents) > 0 else "Unknown"
        p2_name = parents[1]["germplasmName"] if len(parents) > 1 else "Unknown"
        pedigree_str = f"{p1_name}/{p2_name}"

    if not pedigree_str:
        pedigree_str = f"{germ.germplasm_name}/Unknown"
