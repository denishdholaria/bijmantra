...
def model_to_dict(ontology: OntologyModel) -> dict[str, Any]:
    """Convert an OntologyModel object to a BrAPI-compliant dictionary.

    Args:
        ontology: The OntologyModel object to convert.

    Returns:
        A dictionary representing the ontology in BrAPI format.
    """
    return {
...
def brapi_response(result, page: int = 0, page_size: int = 1000, total: int = None):
    """Wraps results in a standard BrAPI response format.

    Args:
        result: The data to be included in the response. Can be a list or a single item.
    ...
    Returns:
        A dictionary representing the standard BrAPI response structure.
    """
...
async def get_ontologies(
    ...
    """Retrieves a list of ontologies, optionally filtered by parameters.

    Args:
        ontologyDbId: The unique identifier for an ontology.
        ontologyName: The human-readable name of an ontology.
        page: The page number for pagination.
        pageSize: The number of items per page.
        db: The database session dependency.
        org_id: The organization ID dependency.

    Returns:
        A BrAPI-compliant response containing a list of ontologies.
    """
...
async def get_ontology(
    ...
    """Retrieves a single ontology by its unique identifier.
    ...
    Returns:
        A BrAPI-compliant response containing the requested ontology.
        If the ontology is not found, the response metadata will contain
        an error status message.
    """

    query = select(OntologyModel).where(
        OntologyModel.ontology_db_id == ontologyDbId, OntologyModel.organization_id == org_id
    )
    result = await db.execute(query)
    ...
    if not ontology:
        return {
            "metadata": {
                "status": [
                    {"message": f"Ontology {ontologyDbId} not found", "messageType": "ERROR"}
                ]
            },
            "result": None,
        }
...
async def create_ontologies(
    ...
    """Creates one or more new ontologies.

    Args:
        ontologies: A list of dictionaries, each representing a new ontology to be created.
        db: The database session dependency.
        org_id: The organization ID dependency.

    Returns:
        A BrAPI-compliant response containing the newly created ontologies.
    """
    created = []
...
async def update_ontology(
    ...
    Args:
        ontologyDbId: The unique identifier for the ontology to be updated.
        ontology_data: A dictionary containing the fields to be updated.
        db: The database session dependency.
        org_id: The organization ID dependency.

    Returns:
        A BrAPI-compliant response containing the updated ontology.
        If the ontology is not found, the response metadata will contain
        an error status message.
    """
...
async def delete_ontology(
    ...
    """Deletes an existing ontology.

    Args:
        ontologyDbId: The unique identifier for the ontology to be deleted.
        db: The database session dependency.
        org_id: The organization ID dependency.

    Raises:
        HTTPException: If the ontology with the given ID is not found.

    Returns:
        A dictionary with a message confirming successful deletion.
    """
