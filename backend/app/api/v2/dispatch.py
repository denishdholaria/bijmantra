...
from app.modules.core.services.dispatch_management_service import get_dispatch_service
...
class DispatchItemCreate(BaseModel):
    ...
    unit_price: float | None = Field(None, description="Price per kg")
...
class DispatchCreate(BaseModel):
    ...
    recipient_id: str | None = Field(None, description="Recipient firm ID")
...
class FirmCreate(BaseModel):
    ...
    gst_number: str | None = Field(None, description="GST/Tax number")
...
async def create_dispatch(
    ...
    try:
        ...
        recipient_id = int(request.recipient_id) if request.recipient_id else None
        dispatch = await service.create_dispatch(db, current_user.organization_id, recipient_id=recipient_id,
...
async def list_dispatches(
    ...
    recipient_id: str | None = Query(None, description="Filter by recipient"),
    transfer_type: str | None = Query(None, description="Filter by transfer type"),
...
async def approve_dispatch(
    dispatch_id: int, approved_by: str = Query("Manager"),
    db: AsyncSession = Depends(get_db),
    current_user=Depends(get_current_user),
):
...
async def ship_dispatch(
    dispatch_id: int, request: ShipDispatch,
    db: AsyncSession = Depends(get_db),
    current_user=Depends(get_current_user),
):
...
async def mark_delivered(
    dispatch_id: int, notes: str = Query(""),
    db: AsyncSession = Depends(get_db),
    current_user=Depends(get_current_user),
):
...
async def cancel_dispatch(
    dispatch_id: int, reason: str = Query(...),
    db: AsyncSession = Depends(get_db),
    current_user=Depends(get_current_user),
):
...
async def list_firms(
    firm_type: str | None = Query(None, description="Filter by type"),
    status: str | None = Query(None, description="Filter by status"),
