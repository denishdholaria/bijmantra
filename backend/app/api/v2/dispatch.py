...
@router.post("/orders")
async def create_dispatch(request: DispatchCreate):
    """
...
async def list_dispatches(
    ...
    dispatches = service.list_dispatches(
        status=status,
...
async def submit_dispatch(dispatch_id: str):
    """Submit dispatch for approval"""
    service = get_dispatch_service()
    
    try:
        dispatch = service.submit_for_approval(dispatch_id)
        return {
            "success": True,
...
async def approve_dispatch(dispatch_id: str, approved_by: str = Query("Manager")):
    """Approve a dispatch order"""
    service = get_dispatch_service()
    
    try:
        dispatch = service.approve_dispatch(dispatch_id, approved_by)
        return {
            "success": True,
...
async def start_picking(dispatch_id: str):
    """Start picking process for dispatch"""
    service = get_dispatch_service()
    
    try:
        dispatch = service.start_picking(dispatch_id)
        return {
            "success": True,
...
async def mark_item_picked(dispatch_id: str, item_id: str):
    """Mark a dispatch item as picked"""
    service = get_dispatch_service()
    
    try:
        dispatch = service.mark_item_picked(dispatch_id, item_id)
        return {
            "success": True,
...
async def ship_dispatch(dispatch_id: str, request: ShipDispatch):
    """Mark dispatch as shipped"""
    service = get_dispatch_service()
    
    try:
        dispatch = service.ship_dispatch(
            dispatch_id,
...
async def mark_delivered(dispatch_id: str, notes: str = Query("")):
    """Mark dispatch as delivered"""
    service = get_dispatch_service()
    
    try:
        dispatch = service.mark_delivered(dispatch_id, notes)
        return {
            "success": True,
...
async def cancel_dispatch(dispatch_id: str, reason: str = Query(...)):
    """Cancel a dispatch order"""
    service = get_dispatch_service()
    
    try:
        dispatch = service.cancel_dispatch(dispatch_id, reason)
        return {
            "success": True,
...
async def create_firm(request: FirmCreate):
    ...
    try:
        firm = service.create_firm(**request.model_dump())
        return {
            "success": True,
...
async def update_firm(firm_id: str, request: FirmUpdate):
    ...
    try:
        ...
        firm = service.update_firm(firm_id, **updates)
        return {
            "success": True,
...
async def deactivate_firm(firm_id: str):
    """Deactivate a firm"""
    service = get_dispatch_service()
    
    try:
        firm = service.deactivate_firm(firm_id)
        return {
            "success": True,
...
async def get_statistics():
    ...
    service = get_dispatch_service()
    stats = service.get_dispatch_statistics()
