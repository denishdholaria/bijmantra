...
from app.modules.ai.services.engine import LLMProvider, get_llm_service
    ...
    for service_id, config in SERVICE_CONFIG.items():
        ...
        if service_id == "ollama" and configured:
            try:
                ...
                ollama_state = status.get("providers", {}).get(LLMProvider.OLLAMA.value, {})
                available = bool(ollama_state.get("available"))
                if not available:
