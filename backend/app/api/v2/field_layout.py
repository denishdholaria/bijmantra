...
from app.modules.spatial.services.field_layout_service import FieldLayoutService


router = APIRouter(
    prefix="/field-layout", tags=["Field Layout"], dependencies=[Depends(get_current_user)]
)
...
async def get_plots(
    study_id: str,
    block: int | None = Query(None, description="Filter by block"),
    replicate: int | None = Query(None, description="Filter by replicate"),
