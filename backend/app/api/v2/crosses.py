...
from app.models.genotyping import Call, CallSet
...
async def predict_by_id(
    request: PredictByIdRequest,
    db: AsyncSession = Depends(get_db)
):
    """Predict cross performance using parent IDs (fetches data from DB)"""
    try:
        # 1. Fetch Germplasm
        p1 = await db.scalar(select(Germplasm).where(Germplasm.germplasm_db_id == request.parent1_id))
        p2 = await db.scalar(select(Germplasm).where(Germplasm.germplasm_db_id == request.parent2_id))
