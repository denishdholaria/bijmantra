...
async def predict_by_id(request: PredictByIdRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        # 3. Fetch Genotypes
        # Find CallSets for parents
        stmt_cs = select(CallSet).join(Sample).where(Sample.germplasm_id.in_([p1.id, p2.id]))
