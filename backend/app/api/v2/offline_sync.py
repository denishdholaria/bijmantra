...
from app.models.core import Study, Trial, User
...
async def get_pending_changes(
    status: str | None = Query(None, description="Filter by status: pending, error"),
...
async def queue_change(
    ...
    if existing_item:
        ...
        existing_item.last_modified = datetime.now(UTC)
...
async def clear_cache(
    category: str | None = Query(None, description="Clear specific category, or all if not specified"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Clear cached data"""
