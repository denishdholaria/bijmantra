...
from app.models.climate import CarbonMeasurement, CarbonMeasurementType, CarbonStock
from app.models.core import User
from app.modules.environment.services.carbon_monitoring_service import get_carbon_monitoring_service
...
class CarbonStockCreate(BaseModel):
    ...
    soil_carbon_stock: float | None = Field(None, description="Soil carbon (t/ha)")
    vegetation_carbon_stock: float | None = Field(None, description="Vegetation carbon (t/ha)")
    ...
    gee_image_id: str | None = Field(None, description="Google Earth Engine image ID")
    additional_data: dict | None = Field(None, description="Additional metadata")


class CarbonStockUpdate(BaseModel):
    """Schema for updating carbon stock"""
    soil_carbon_stock: float | None = None
    vegetation_carbon_stock: float | None = None
...
class CarbonMeasurementCreate(BaseModel):
    ...
    location_id: int = Field(..., description="Location/field ID")
    carbon_stock_id: int | None = Field(None, description="Associated carbon stock ID")
    measurement_date: date = Field(..., description="Date of measurement")
    measurement_type: CarbonMeasurementType = Field(..., description="Type of measurement")
    carbon_value: float = Field(..., description="Carbon value")
    unit: str = Field(..., description="Unit of measurement")
    depth_from_cm: int | None = Field(None, description="Depth from (cm)")
    depth_to_cm: int | None = Field(None, description="Depth to (cm)")
    bulk_density: float | None = Field(None, description="Bulk density (g/cm³)")
    sample_id: str | None = Field(None, description="Sample ID")
    method: str | None = Field(None, description="Measurement method")
...
class CarbonMeasurementResponse(BaseModel):
    ...
    organization_id: int
    location_id: int
    carbon_stock_id: int | None
    measurement_date: date
    measurement_type: CarbonMeasurementType
    carbon_value: float
    unit: str
    depth_from_cm: int | None
    depth_to_cm: int | None
...
class CarbonDashboardResponse(BaseModel):
    ...
    sequestration_rate: float | None


class CarbonTimeSeriesResponse(BaseModel):
    """Schema for carbon time series response"""
    date: str
    soil_carbon: float | None
    vegetation_carbon: float | None
    total_carbon: float
...
async def list_carbon_stocks(
    ...
    measurement_type: CarbonMeasurementType | None = Query(None, description="Filter by measurement type"),
...
async def list_carbon_measurements(
    location_id: int | None = Query(None, description="Filter by location"),
    carbon_stock_id: int | None = Query(None, description="Filter by carbon stock"),
    start_date: date | None = Query(None, description="Filter by start date"),
    end_date: date | None = Query(None, description="Filter by end date"),
    limit: int = Query(100, ge=1, le=1000, description="Maximum results"),
    offset: int = Query(0, ge=0, description="Results offset"),
...
async def get_carbon_dashboard(
    ...
    try:
        ...
        # Calculate metrics
        unique_locations = len({s.location_id for s in stocks})
