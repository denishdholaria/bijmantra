...
from app.modules.environment.services.sustainability_metrics_service import get_sustainability_metrics_service
...
class ImpactMetricCreate(BaseModel):
    ...
    program_id: int | None = Field(None, description="Program ID (optional)")
    metric_type: MetricType = Field(..., description="Type of metric")
    metric_name: str = Field(..., description="Metric name")
    metric_value: float = Field(..., description="Metric value")
    unit: str = Field(..., description="Unit of measurement")
    baseline_value: float | None = Field(None, description="Baseline value")
    measurement_date: date = Field(..., description="Date of measurement")
    geographic_scope: str | None = Field(None, description="Geographic scope")
    beneficiaries: int | None = Field(None, description="Number of beneficiaries")
...
class VarietyReleaseCreate(BaseModel):
    ...
    registration_number: str | None = Field(None, description="Official registration number")
    target_environment: str | None = Field(None, description="Target production environment")
    expected_adoption_ha: float | None = Field(None, description="Expected adoption (hectares)")
    actual_adoption_ha: float | None = Field(None, description="Actual adoption (hectares)")
...
class VarietyReleaseResponse(BaseModel):
    ...
    expected_adoption_ha: float | None
    actual_adoption_ha: float | None
...
async def list_impact_metrics(
    program_id: int | None = Query(None, description="Filter by program"),
...
async def list_variety_releases(
    program_id: int | None = Query(None, description="Filter by program"),
    germplasm_id: int | None = Query(None, description="Filter by germplasm"),
    country: str | None = Query(None, description="Filter by country"),
    release_status: ReleaseStatus | None = Query(None, description="Filter by status"),
    limit: int = Query(100, ge=1, le=1000, description="Maximum results"),
    offset: int = Query(0, ge=0, description="Results offset"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
