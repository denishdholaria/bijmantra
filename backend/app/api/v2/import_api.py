...
from app.modules.core.services.import_engine import (
    CSVImporter,
    ExcelImporter,
    GermplasmImporter,
    JSONImporter,
...
def _pick_domain_importer(import_type: str, db: AsyncSession, org_id: int, user_id: int):
    ...
    if normalized in {"qtl", "qtls"}:
        return QTLImporter(db, org_id, user_id)
...
async def _run_job(
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            report = await domain_importer.import_data(
                rows_iter, mapping=mapping, formulas=formulas, dry_run=dry_run
            )
...
async def upload_import(
    ...
    domain_importer = _pick_domain_importer(
        import_type, db, current_user.organization_id, current_user.id
    )
