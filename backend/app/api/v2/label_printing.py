...
from app.modules.core.services.label_printing_service import label_printing_service
...
async def get_template(
    ...
    template = await label_printing_service.get_template(
...
async def create_template(
    ...
    template = await label_printing_service.create_template(
...
async def get_label_data(
    ...
    data = await label_printing_service.get_label_data(
        db,
...
async def get_print_jobs(
    ...
    jobs = await label_printing_service.get_print_jobs(
        db, current_user.organization_id, status=status, limit=limit
    )
...
async def create_print_job(
    ...
    template = await label_printing_service.get_template(
        db, current_user.organization_id, request.template_id
    )
    if not template:
        raise HTTPException(status_code=400, detail="Invalid template ID")

    job = await label_printing_service.create_print_job(
        db,
        current_user.organization_id,
        {
...
async def update_job_status(
    ...
    job = await label_printing_service.update_print_job_status(
        db, current_user.organization_id, job_id, request.status
    )
    if not job:
        raise HTTPException(status_code=404, detail="Print job not found")
    return job
