...
from sqlalchemy import select, func, and_, Float, cast
...
async def get_top_performers(
    ...
    trait_name = trait or "Yield"

    # Get ObservationVariable ID for the trait
    stmt = select(ObservationVariable.id).where(
        ObservationVariable.observation_variable_name == trait_name,
        ObservationVariable.organization_id == organization_id
    )
    var_result = await db.execute(stmt)
    var_id = var_result.scalar_one_or_none()

    check_mean = 0.0

    if var_id:
        # Calculate Check Mean
        # Join Observation -> ObservationUnit -> Study
        # Filter by trial (using trial.id from previous query) and entry_type="CHECK"
        stmt = (
            select(func.avg(cast(Observation.value, Float)))
            ...
            .where(
                and_(
                    Study.trial_id == trial.id,
                    Study.organization_id == organization_id,
                    ObservationUnit.entry_type.ilike("%check%"),
                    Observation.observation_variable_id == var_id
                )
            )
        )
        check_mean_result = await db.execute(stmt)
        check_mean = check_mean_result.scalar() or 0.0

    # Map to response format
    result = []
    for p in performers:
        yield_val = p.get("yield") or 0.0
        change_pct_str = "0%"

        if check_mean > 0:
            change = ((yield_val - check_mean) / check_mean) * 100
            sign = "+" if change > 0 else ""
            change_pct_str = f"{sign}{change:.1f}%"
    ...
    return {"data": result, "trait": trait_name, "message": "Top performers rank based on performance score"}
