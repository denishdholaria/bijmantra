...
- GET /api/v2/quality/certificates - List issued certificates
...
from app.modules.core.services.quality_control_service import get_qc_service


router = APIRouter(prefix="/quality", tags=["Quality Control"], dependencies=[Depends(get_current_user)])
...
async def list_samples(
    status: str | None = Query(None, description="Filter by status: pending, passed, failed"),
...
async def list_certificates():
    """List issued quality certificates"""
    service = get_qc_service()
