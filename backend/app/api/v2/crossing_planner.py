...
router = APIRouter(prefix="/crossing-planner", tags=["Crossing Planner"])
...
async def list_crosses(
    ...
    result = crossing_planner_service.list_crosses(
        status=status,
        priority=priority,
        season=season,
        breeder=breeder,
...
async def get_statistics():
    ...
    stats = crossing_planner_service.get_statistics()
    return {"metadata": {}, "result": stats}


@router.get("/germplasm")
async def list_germplasm(search: Optional[str] = None):
    """List available germplasm for crossing"""
    germplasm = crossing_planner_service.list_germplasm(search=search)
    return {"metadata": {}, "result": {"data": germplasm}}


@router.get("/{crossId}")
async def get_cross(crossId: str):
    """Get a single cross by ID"""
    cross = crossing_planner_service.get_cross(crossId)
    if not cross:
...
@router.post("")
async def create_cross(data: CrossCreate):
    """Create a new planned cross"""
    cross = crossing_planner_service.create_cross(data.model_dump())
    return {"metadata": {}, "result": cross}


@router.put("/{crossId}")
async def update_cross(crossId: str, data: CrossUpdate):
    """Update a cross"""
    cross = crossing_planner_service.update_cross(crossId, data.model_dump(exclude_none=True))
    if not cross:
        raise HTTPException(status_code=404, detail="Cross not found")
    return {"metadata": {}, "result": cross}
...
async def update_status(crossId: str, data: StatusUpdate):
    """Update cross status"""
    cross = crossing_planner_service.update_status(crossId, data.status, data.actualProgeny)
    if not cross:
        raise HTTPException(status_code=404, detail="Cross not found")
    return {"metadata": {}, "result": cross}
...
async def delete_cross(crossId: str):
    """Delete a cross"""
    success = crossing_planner_service.delete_cross(crossId)
    if not success:
        raise HTTPException(status_code=404, detail="Cross not found")
