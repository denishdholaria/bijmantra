...
router = APIRouter(prefix="/seed-inventory", tags=["Seed Inventory"], dependencies=[Depends(get_current_user)])
...
class SeedLotRequest(BaseModel):
    ...
    # Extended fields
    seed_class: str | None = Field(None, description="Seed class (foundation, certified, etc)")
    purity_percent: float | None = Field(None, ge=0, le=100, description="Purity percentage")
    moisture_percent: float | None = Field(None, ge=0, le=100, description="Moisture percentage")
    production_year: int | None = Field(None, description="Year of production")
    producer_name: str | None = Field(None, description="Name of producer")
    production_location: str | None = Field(None, description="Location of production")
...
async def list_seed_lots(
    ...
    storage_type: str | None = Query(None, description="Filter by storage type"),
    db: AsyncSession = Depends(get_db)
):
