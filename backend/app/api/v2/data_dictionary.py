...
router = APIRouter(prefix="/data-dictionary", tags=["Data Dictionary"], dependencies=[Depends(get_current_user)])
...
@router.get("/fields")
async def search_fields(search: str | None = Query(None), entity: str | None = Query(None)):
...
async def get_stats():
    ...
    return {
        ...
        "data": {
            ...
            "brapi_version": "2.1",
            "modules": list({e["brapi_module"] for e in ENTITIES.values()})
        }
    }
