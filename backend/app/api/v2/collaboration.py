...
from app.modules.core.services import collaboration_service
...
async def get_team_members(
    ...
    team_members = await collaboration_service.list_team_members(db, current_user.organization_id)
    return [TeamMemberResponse(**member) for member in team_members]
...
async def get_shared_items(
    ...
    """
    response_items = await collaboration_service.list_shared_items(db, current_user, item_type)
    return [SharedItemResponse(**item) for item in response_items]
...
async def get_activity_feed(
    ...
    """
    response = await collaboration_service.list_activity(db, current_user.organization_id, limit, offset)
    return [ActivityResponse(**activity) for activity in response]
