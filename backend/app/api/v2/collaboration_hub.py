...
router = APIRouter(prefix="/collaboration-hub", tags=["Collaboration Hub"])
...
async def get_collaboration_stats(db: AsyncSession = Depends(get_db)):
    ...
    # Count total members with presence
    members_result = await db.execute(select(func.count(UserPresence.id)))
    ...
    online_members = online_result.scalar() or 0
    ...
    active_workspaces = workspaces_result.scalar() or 0
    ...
    # Count today's activities and comments
...
async def list_workspaces(
    ...
    if member_id:
        ...
        member_ws_ids = {r for r in member_result.scalars().all()}
        workspaces = [w for w in workspaces if w.id in member_ws_ids]
