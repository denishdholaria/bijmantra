...
async def list_qtls(
    ...
    population: str | None = Query(None, description="Filter by mapping population"),
...
async def get_gwas_results(
    ...
    max_p_value: float | None = Query(None, description="Maximum p-value"),
        db: AsyncSession = Depends(get_db),
        current_user=Depends(get_current_user),
):
...
async def get_go_enrichment(
    qtl_ids: str | None = Query(None, description="Comma-separated QTL IDs"),
        db: AsyncSession = Depends(get_db),
        current_user=Depends(get_current_user),
):
