...
from app.modules.breeding.services.yield_gap_service import yield_gap_service
...
async def get_location_yield_gap_summary(
    ...
    try:
        ...
        sensitivity = yield_gap_service.sensitivity_analysis(gap["gap_percent"], environment, soil)
        projected = yield_gap_service.project_climate_scenario(
            potential_yield=potential, delta_temp_c=2.0
        )
        economic = yield_gap_service.economic_impact(gap["gap_absolute"], market_price_per_ton)
        ...
        return YieldGapResponse(
            ...
            limiting_factors=[LimitingFactorSchema(**node.__dict__) for node in limiting_factors],
