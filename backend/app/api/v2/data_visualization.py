...
router = APIRouter(
    prefix="/visualizations", tags=["Data Visualization"], dependencies=[Depends(get_current_user)]
)
...
async def get_charts(
    type: str | None = Query(None, description="Filter by chart type"),
...
async def get_data_sources(
    ...
    trial_count = await db.execute(
        select(func.count()).select_from(Trial).where(Trial.organization_id == organization_id)
    )
    ...
    study_count = await db.execute(
        select(func.count()).select_from(Study).where(Study.organization_id == organization_id)
    )
