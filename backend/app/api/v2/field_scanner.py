...
async def get_scans(
    ...
    plot_id: str | None = Query(None, description="Filter by plot ID"),
    crop: str | None = Query(None, description="Filter by crop type"),
    has_issues: bool | None = Query(None, description="Filter by issue presence"),
...
async def get_scan_stats(
    ...
    plots_scanned = len({s.plot_id for s in scans if s.plot_id})
    ...
    last_scan = None
    if scans:
        latest = max(scans, key=lambda s: s.created_at or datetime.min.replace(tzinfo=UTC))
