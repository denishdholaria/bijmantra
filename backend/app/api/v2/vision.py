...
from app.modules.phenotyping.services.vision.annotation_service import (
    AnnotationStatus,
    AnnotationType,
...
router = APIRouter(
    prefix="/vision",
    tags=["Vision Training Ground"],
    dependencies=[Depends(get_current_user), require_permission("read:plant_sciences")],
)


def _raise_vision_service_error(detail: str) -> None:
    if detail == "Model not found":
        raise HTTPException(404, detail)
    if detail == "Invalid model ID":
...
class ImageUpload(BaseModel):
    ...
    metadata: dict | None = Field(None, description="Additional metadata (GPS, date, etc.)")
    annotation: dict | None = Field(None, description="Annotation data")
...
async def get_model(
    ...
    model = await vision_model_service.get_model(db, current_user.organization_id, model_id)
    if not model:
...
async def submit_annotation_for_review(
    annotation_id: str,
    db: AsyncSession = Depends(get_db),
    current_user=Depends(get_current_user),
):
...
async def get_annotation_task(
    ...
    task = await annotation_task_service.get_task(db, current_user.organization_id, task_id)
    if not task:
...
async def list_annotators(
    ...
    """List all annotators"""
    annotators = await quality_control_service.list_annotators(db, current_user.organization_id)
...
async def get_training_job(
    ...
    job = await vision_training_service.get_job(db, current_user.organization_id, job_id)
    if not job:
...
async def cancel_training_job(
    ...
    result = await vision_training_service.cancel_job(db, current_user.organization_id, job_id)
    if not result:
        raise HTTPException(404, "Job not found")
    if "error" in result:
        raise HTTPException(400, result["error"])
...
async def get_training_logs(
    ...
    logs = await vision_training_service.get_logs(db, current_user.organization_id, job_id, last_n)
...
async def undeploy_model(
    ...
    """Remove a deployment"""
    success = await vision_deployment_service.undeploy(db, current_user.organization_id, deploy_id)
    if not success:
        raise HTTPException(404, "Deployment not found")
...
async def download_registry_model(
    registry_id: str,
    db: AsyncSession = Depends(get_db),
    current_user=Depends(get_current_user),
):
...
async def like_registry_model(
    registry_id: str,
    db: AsyncSession = Depends(get_db),
    current_user=Depends(get_current_user),
):
    """Like a model in the registry"""
    result = await model_registry_service.like_model(db, current_user.organization_id, registry_id)
    if not result:
        raise HTTPException(404, "Model not found in registry")
    return {"success": True, **result}
...
async def list_model_versions(
    ...
    versions = await model_version_service.list_versions(db, current_user.organization_id, model_id)
