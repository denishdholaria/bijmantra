...
def create_brapi_response(data: any, page: int, page_size: int, total_count: int):
    ...
    metadata = Metadata(
        pagination=Pagination(
            current_page=page, page_size=page_size, total_count=total_count, total_pages=total_pages
        ),
...
async def list_trials(
    ...
    trial_list = [Trial.model_validate(t).model_dump(by_alias=True) for t in trials]
...
async def get_trial(
    trialDbId: str, db: AsyncSession = Depends(get_db), org_id: int = Depends(get_organization_id)
):
    ...
    trial_data = Trial.model_validate(trial).model_dump(by_alias=True)
    metadata = Metadata(
...
async def create_trial(
    ...
    await db.commit()
    await TrialService.log_mutation(
        db,
        current_user.organization_id,
        current_user.id,
        "POST",
        trial.trial_db_id or str(trial.id),
        trial_in.model_dump(exclude_none=True),
    )
    await db.commit()
    trial_data = Trial.model_validate(trial).model_dump(by_alias=True)
    metadata = Metadata(
...
async def update_trial(
    ...
    await db.commit()
    payload = trial_in.model_dump(exclude_none=True) if hasattr(trial_in, "model_dump") else {}
    await TrialService.log_mutation(db, org_id, None, "PUT", trialDbId, payload)
    await db.commit()
    trial_data = Trial.model_validate(trial).model_dump(by_alias=True)
    metadata = Metadata(
        pagination=Pagination(current_page=0, page_size=1, total_count=1, total_pages=1),
...
async def delete_trial(
    ...
    await db.commit()
    await TrialService.log_mutation(db, org_id, None, "DELETE", trialDbId, None)
    await db.commit()
    return None
