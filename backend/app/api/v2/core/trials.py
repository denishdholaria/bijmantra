...
async def create_trial(
    ...
    await db.commit()
    await TrialService.log_mutation(db, current_user.organization_id, current_user.id, "POST", trial.trial_db_id or str(trial.id), trial_in.model_dump(exclude_none=True))
    await db.commit()
    trial_data = Trial.model_validate(trial).model_dump(by_alias=True)
    metadata = Metadata(
        pagination=Pagination(current_page=0, page_size=1, total_count=1, total_pages=1),
