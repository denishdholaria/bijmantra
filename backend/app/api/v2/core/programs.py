...
async def create_program(
    ...
    """
    try:
        print(f"[DEBUG] create_program called for user {current_user.email} org_id={current_user.organization_id}")
        await set_tenant_context(db, current_user.organization_id, current_user.is_superuser)
        print(f"[DEBUG] set_tenant_context completed")
        
        program = await program_crud.create(db, obj_in=program_in, org_id=current_user.organization_id)
        print(f"[DEBUG] program_crud.create completed: {program.program_db_id}")
        await db.commit()
        print(f"[DEBUG] db.commit completed")
        ...
        import traceback
        print(f"[ERROR] create_program FAILED: {type(e).__name__}: {e}")
