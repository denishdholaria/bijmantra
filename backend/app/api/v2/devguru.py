...
async def devguru_chat(request: ChatRequest, db: AsyncSession = Depends(get_db)):
    """Chat with DevGuru - PhD mentoring mode (Savant RAG enabled)."""
    devguru_service = get_devguru_service(db)
    
    # We need user_id and org_id - for now assume user/org 1 or get from context if available
    # Ideally should use Depends(get_current_user)
    # Using hardcoded user for now as migration to auth dependency is separate
    # But wait, Veena uses auth. We should support it if possible.
    # For now, pass defaults to avoid breaking if auth not passed
    user_id = 1
    org_id = 1
