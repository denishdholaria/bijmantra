...
@router.put("/{id}", response_model=PestObservation)
async def update_pest_observation(
    id: int,
    record_in: PestObservationUpdate,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_active_user)
):
    """Update an existing pest observation."""
    record = await pest_crud.pest_observation.get(db, id=id)
    if not record or record.organization_id != current_user.organization_id:
        raise HTTPException(status_code=404, detail="Pest observation not found")

    record = await pest_crud.pest_observation.update(
