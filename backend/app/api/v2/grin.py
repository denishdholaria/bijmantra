...
router = APIRouter(prefix="/grin", tags=["GRIN-Global Integration"], dependencies=[Depends(get_current_user)])
...
async def search_grin_global(
    genus: str | None = Query(None, description="Genus name"),
    species: str | None = Query(None, description="Species name"),
    country: str | None = Query(None, description="Country code (ISO 3166-1 alpha-3)"),
...
async def validate_taxonomy(
    ...
    subspecies: str | None = Query(None, description="Subspecies name"),
    db: AsyncSession = Depends(get_db),
    organization_id: int = Depends(get_organization_id),
):
    """
