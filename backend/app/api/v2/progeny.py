...
async def list_parents(
    parent_type: str | None = Query(
        None, description="Filter by parent type: FEMALE, MALE, SELF, POPULATION"
    ),
    species: str | None = Query(None, description="Filter by species"),
    search: str | None = Query(None, description="Search by parent or progeny name"),
    ...
    cross_q = (
        ...
        .options(selectinload(Cross.progeny))
    )
    ...
    g = (
        await db.execute(
            select(Germplasm).where(
                Germplasm.id == parent_id, Germplasm.organization_id == organization_id
            )
        )
    ).scalar_one_or_none()
    if not g:
...
async def get_progeny_by_germplasm(
        ...
        await db.execute(
            select(Germplasm).where(
                ...
                or_(
                    ...
                    Germplasm.id == int(germplasm_id) if germplasm_id.isdigit() else False,
                ),
            )
        )
    ).scalar_one_or_none()
    ...
    ptype = "FEMALE"
    for c in crosses:
        ptype = c.parent1_type or "FEMALE" if c.parent1_db_id == g.id else c.parent2_type or "MALE"
        for prog in c.progeny or []:
            progeny_list.append(
                {
                    "germplasm_id": prog.germplasm_db_id or str(prog.id),
...
async def get_statistics(
    ...
    type_q = await db.execute(
        select(Cross.parent1_type, func.count(func.distinct(Cross.parent1_db_id)))
        .where(Cross.organization_id == organization_id, Cross.parent1_type.isnot(None))
    ...
    sp_q = await db.execute(
        select(Germplasm.common_crop_name, func.count(Germplasm.id))
        .where(Germplasm.organization_id == organization_id, Germplasm.cross_id.isnot(None))
...
async def get_lineage_tree(
    ...
    g = (
        await db.execute(
            select(Germplasm).where(
                Germplasm.id == germplasm_id, Germplasm.organization_id == organization_id
            )
        )
    ).scalar_one_or_none()
    if not g:
