...
router = APIRouter(
    prefix="/warehouse", tags=["Warehouse Management"], dependencies=[Depends(get_current_user)]
)
...
class StorageLocationCreate(BaseModel):
    ...
    storage_type: str = Field(
        "ambient", description="Storage type: cold, ambient, controlled, cryo"
    )
    capacity_kg: float = Field(..., gt=0, description="Total capacity in kg")
    target_temperature: float | None = Field(None, description="Target temperature °C")
...
async def delete_location(
    ...
    if loc.lot_count > 0:
        raise HTTPException(
            status_code=400, detail="Cannot delete location with stored lots. Move lots first."
        )
...
async def get_summary(
    ...
    total_capacity = sum(loc.capacity_kg for loc in locations)
    total_used = sum(loc.used_kg for loc in locations)
    total_lots = sum(loc.lot_count for loc in locations)
    ...
    alerts_count = sum(1 for loc in locations if loc.status in ("warning", "critical"))
