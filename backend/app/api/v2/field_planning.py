...
from app.models.core import Season, Trial
...
async def get_field_plans(
    ...
    query = select(Trial).options(selectinload(Trial.studies)).where(Trial.organization_id == organization_id)
    if status:
        query = query.where(Trial.active == (status == "active"))

    # Optional: Filter by location/field_id if provided
    if field_id:
        # Assuming field_id is Location ID
        try:
            f_id = int(field_id)
            query = query.where(Trial.location_id == f_id)
        except ValueError:
    ...
    for t in trials:
        ...
        plan = {
            ...
            "field_name": None, # Could fetch Location name if joined
            "season": None, # Could fetch Season name if joined
...
async def get_field_plan(
    ...
    t = (await db.execute(
        select(Trial).options(selectinload(Trial.studies)).where(Trial.id == plan_id, Trial.organization_id == organization_id)
    )).scalar_one_or_none()
    if not t:
...
async def get_season_plans(
    ...
    query = select(Season).options(selectinload(Season.trials)).where(Season.organization_id == organization_id)
    if year:
    ...
    plans = []
    for s in seasons:
        # Count trials in this season using loaded relationship
...
async def get_season_plan(
    ...
    s = (await db.execute(
        select(Season).options(selectinload(Season.trials)).where(Season.id == plan_id, Season.organization_id == organization_id)
    )).scalar_one_or_none()
    if not s:
...
async def get_calendar(
    ...
    # Filter in DB for efficiency
    query = select(Trial).where(Trial.organization_id == organization_id)

    # This assumes start_date and end_date are date/datetime columns
    # Extract year from date in SQL is DB-specific (extract('year', ...))
    # For now, let's keep Python filtering but maybe select only relevant columns if optimization needed
    # Or implement DB-side filtering if performance is critical with huge dataset.
    # Given typical trial counts, loading them might be acceptable for now,
    # but let's stick to simple query for "audit fix" scope unless we want to rewrite SQL.
...
async def get_statistics(
    ...
    active_count = (await db.execute(
        select(func.count(Trial.id)).where(Trial.organization_id == organization_id, Trial.active)
    )).scalar() or 0
