...
def get_variant_set(variantSetDbId: str):
...
def create_variant_set(data: VariantSetCreate):
...
def get_call_set(callSetDbId: str):
...
def get_calls_statistics(variantSetDbId: Optional[str] = None):
...
def get_reference(referenceDbId: str):
...
def get_vendor_order(vendorOrderDbId: str):
...
def create_vendor_order(data: VendorOrderCreate):
...
def update_vendor_order_status(vendorOrderDbId: str, data: VendorOrderStatusUpdate):
