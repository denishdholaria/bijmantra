...
async def create_vendor_order(data: VendorOrderCreate, db: AsyncSession = Depends(get_db)):
    """Create a new vendor order"""
    result = await genotyping_service.create_vendor_order(db, data.model_dump())
    return {"metadata": {}, "result": result}
