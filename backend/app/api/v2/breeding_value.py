...
async def estimate_gblup(data: GBLUPRequest):
    ...
    """
    result = await run_in_threadpool(
        breeding_value_service.estimate_gblup,
        phenotypes=data.phenotypes,
...
async def predict_cross(data: CrossPredictionRequest):
    ...
    result = await run_in_threadpool(
        breeding_value_service.predict_cross,
...
async def calculate_accuracy(data: AccuracyRequest):
    ...
    result = await run_in_threadpool(
        breeding_value_service.calculate_accuracy,
...
async def rank_candidates(data: RankCandidatesRequest):
    ...
    result = await run_in_threadpool(
        breeding_value_service.rank_candidates,
...
async def calculate_kinship(data: KinshipRequest):
    ...
    result = await run_in_threadpool(
        breeding_value_service._calculate_g_matrix,
        data.markers
    )
...
async def list_analyses():
    ...
    analyses = await run_in_threadpool(breeding_value_service.list_analyses)
...
async def get_analysis(analysis_id: str):
    ...
    analysis = await run_in_threadpool(breeding_value_service.get_analysis, analysis_id)
    if not analysis:
