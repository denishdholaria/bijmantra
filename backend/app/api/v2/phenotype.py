...
router = APIRouter(prefix="/phenotype", tags=["Phenotype Analysis"])
...
async def selection_index(request: SelectionIndexRequest):
    ...
    Combines multiple traits into a single index value for ranking genotypes.
...
async def anova_rcbd(request: AnovaRequest):
    ...
    Returns F-values for testing significance of genotype differences.
    """
    service = get_phenotype_service()
