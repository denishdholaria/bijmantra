...
router = APIRouter(
    prefix="/resources", tags=["Resource Management"], dependencies=[Depends(get_current_user)]
)
...
async def get_resource_overview(
    ...
    budget_total = (
        await db.execute(
            select(func.sum(BudgetCategory.allocated)).where(
...
async def get_budget_summary(
    ...
    return {
        ...
        "utilization_percent": round(float(total_spent) / float(total_allocated) * 100, 1)
    ...
    cat = (
        await db.execute(
            select(BudgetCategory).where(
                BudgetCategory.id == category_id, BudgetCategory.organization_id == organization_id
            )
        )
    ).scalar_one_or_none()
    if not cat:
...
async def get_field_allocations(
    ...
    for loc in locations:
        trial_count = (
            await db.execute(select(func.count(Trial.id)).where(Trial.location_id == loc.id))
        ).scalar() or 0
        fields.append(
            {
                "id": str(loc.id),
...
async def create_calendar_event(event: CalendarEventCreate):
    ...
    raise HTTPException(
        status_code=501, detail="Calendar events table not yet created. Migration pending."
    )
...
async def create_harvest_record(record: HarvestRecordCreate):
    ...
    raise HTTPException(
        status_code=501, detail="Harvest records table not yet created. Migration pending."
    )
