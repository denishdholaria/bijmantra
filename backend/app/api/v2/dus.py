...
from app.modules.interop.services.dus_testing_service import (
    TrialStatus,
    dus_service,
)


router = APIRouter(prefix="/dus", tags=["DUS Testing"], dependencies=[Depends(get_current_user)])
...
class UniformityRequest(BaseModel):
    ...
    sample_size: int | None = Field(None, description="Sample size (uses trial default if not provided)")
...
async def list_trials(
    crop_code: str | None = Query(None, description="Filter by crop"),
