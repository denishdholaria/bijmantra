...
async def generate_prediction(
    ...
    attribution = await attributor.analyze(
        db, result, request.field_id, request.crop_name, request.features or {}
    )
...
async def get_yield_prediction(
    id: int, db: AsyncSession = Depends(get_tenant_db), org_id: int = Depends(get_organization_id)
):
...
async def get_predictions_by_field(
    field_id: int, db: AsyncSession = Depends(get_tenant_db), org_id: int = Depends(get_organization_id)
):
...
async def get_predictions_by_trial(
    trial_id: int, db: AsyncSession = Depends(get_tenant_db), org_id: int = Depends(get_organization_id)
):
