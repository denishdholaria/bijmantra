...
from app.modules.ai.services.reevu_cognitive_service import (
    ReevuCognitiveService,
    ReevuReasoningEngine,
)


router = APIRouter(prefix="/veena", tags=["REEVU Cognitive"], dependencies=[Depends(get_current_user)])
...
def get_service(db: AsyncSession = Depends(get_tenant_db)) -> ReevuCognitiveService:
    return ReevuCognitiveService(db)
...
async def veena_chat(
    ...
    service: ReevuCognitiveService = Depends(get_service),
):
    context = await service.retrieve_relevant_context(current_user.id, request.query)
    user_context = await service.get_or_create_user_context(
        current_user.id,
    ...
    engine = ReevuReasoningEngine(service)
    engine_result = await engine.run(
        request.session_id,
        current_user.id,
        current_user.organization_id,
        request.query,
        [{"id": m.id} for m in context],
    )
    ...
    audit.generated_draft = content
    audit.flagged_sensitive = service.guardrail_flag_sensitive_draft(content)
