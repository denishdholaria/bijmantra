...
async def get_collaboration_stats(db: AsyncSession = Depends(get_tenant_db)):
    ...
    query = select(
        ...
        select(func.count(CollaborationWorkspace.id)).where(
            CollaborationWorkspace.is_active
        ).scalar_subquery(),
...
async def get_online_members(db: AsyncSession = Depends(get_tenant_db)):
...
async def get_member(member_id: int, db: AsyncSession = Depends(get_tenant_db)):
...
async def get_workspace(workspace_id: int, db: AsyncSession = Depends(get_tenant_db)):
...
async def delete_task(task_id: int, db: AsyncSession = Depends(get_tenant_db)):
    """Delete a task."""
    result = await db.execute(
        select(CollaborationTask).where(CollaborationTask.id == task_id)
    )
...
async def delete_comment(comment_id: int, db: AsyncSession = Depends(get_tenant_db)):
    """Delete a comment."""
    result = await db.execute(
...
async def join_workspace(workspace_id: int, db: AsyncSession = Depends(get_tenant_db)):
...
async def leave_workspace(db: AsyncSession = Depends(get_tenant_db)):
...
async def heartbeat(db: AsyncSession = Depends(get_tenant_db)):
