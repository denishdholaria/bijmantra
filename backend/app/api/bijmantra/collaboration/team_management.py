...
async def get_members(
    ...
    # Single query: load users + their TeamMember rows in one round-trip
    # using a LEFT OUTER JOIN so users with no TeamMember record are included.
    ...
    rows = (await db.execute(user_query)).all()

    # Batch-load all team memberships for the org in one query
    user_ids = [row[0].id for row in rows]
    if user_ids:
        team_rows = (
            await db.execute(
                select(TeamMember.user_id, TeamMember.team_id).where(
                    sa_and(
    ...
    # Build user_id → [team_id] map
    user_team_map: dict[int, list[int]] = {}
    for uid, tid in team_rows:
        user_team_map.setdefault(uid, []).append(tid)
    ...
    for user, member in rows:
        ...
        team_ids = user_team_map.get(user.id, [])
...
async def get_teams(
    ...
    # Batch-load member counts for all teams in one query
    ...
    member_count_map = {row[0]: row[1] for row in count_rows}
    ...
    teams_with_stats = [
        TeamResponse(
            ...
            member_count=member_count_map.get(team.id, 0),
            project_count=0,
        )
        for team in teams
    ]
