...
from app.modules.phenotyping.services import spectral_indices


router = APIRouter(
    prefix="/image-analysis", tags=["Image Analysis"], dependencies=[Depends(get_current_user)]
)
...
async def analyze_spectral_indices(
    ...
    try:
        ...
        if await loop.run_in_executor(None, os.path.exists, tmp_out_path):
            ...
            return {"success": True, "index": idx, "image_data": f"data:image/png;base64,{b64_str}"}
        else:
