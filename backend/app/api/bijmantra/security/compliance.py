...
from app.modules.core.services.compliance.audit import create_audit_record, verify_certificate
from app.modules.core.services.compliance.batch import submit_batch_job
...
async def download_certificate(cert_hash: str, db: AsyncSession = Depends(get_tenant_db)):
...
async def batch_generate(
    request: BatchGenerationRequest, current_user: User = Depends(get_current_user)
):
    # Submit job
...
async def verify(cert_hash: str, db: AsyncSession = Depends(get_tenant_db)):
    record = await verify_certificate(db, cert_hash)
    if not record:
