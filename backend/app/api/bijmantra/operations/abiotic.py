...
from app.models.stress_resistance import AbioticStress, ToleranceGene


router = APIRouter(prefix="/abiotic", tags=["Abiotic Stress"], dependencies=[Depends(get_current_user)])
...
async def get_stress_types(
    ...
    query = select(AbioticStress).where(AbioticStress.is_active)
...
@router.get("/genes")
async def get_genes(
    stress_id: str | None = Query(None, description="Filter by stress type"),
...
async def get_categories(db: AsyncSession = Depends(get_tenant_db)):
    """Get list of stress categories"""
    query = select(AbioticStress.category).where(
        AbioticStress.is_active
    ).distinct()
...
async def get_statistics(db: AsyncSession = Depends(get_tenant_db)):
    ...
    stress_count = await db.execute(
        select(func.count(AbioticStress.id)).where(AbioticStress.is_active)
    )
    total_stresses = stress_count.scalar() or 0

    # Count genes
    gene_count = await db.execute(
        select(func.count(ToleranceGene.id)).where(ToleranceGene.is_active)
    )
    total_genes = gene_count.scalar() or 0

    # Count categories
    cat_result = await db.execute(
        select(AbioticStress.category).where(AbioticStress.is_active).distinct()
    )
    ...
    mas_count = await db.execute(
        select(func.count(ToleranceGene.id)).where(
            (ToleranceGene.is_active) &
            (ToleranceGene.markers is not None) &
    ...
    stress_result = await db.execute(
        select(AbioticStress).where(AbioticStress.is_active)
    )
