...
from app.models.stress_resistance import Disease, PyramidingStrategy, ResistanceGene


router = APIRouter(prefix="/disease", tags=["Disease Resistance"], dependencies=[Depends(get_current_user)])
...
class GeneCreate(BaseModel):
    ...
    resistance_type: str
    source_germplasm: str | None = None
    markers: list[str] = []
...
async def get_diseases(
    ...
    pathogen_type: str | None = Query(None, description="Filter by pathogen type"),
    search: str | None = Query(None, description="Search term"),
    db: AsyncSession = Depends(get_tenant_db),
):
    """Get all diseases with optional filters"""
    query = select(Disease).where(Disease.is_active)
...
@router.get("/genes")
async def get_genes(
    disease_id: str | None = Query(None, description="Filter by disease"),
    resistance_type: str | None = Query(None, description="Filter by resistance type"),
    crop: str | None = Query(None, description="Filter by crop"),
    search: str | None = Query(None, description="Search term"),
    db: AsyncSession = Depends(get_tenant_db),
):
    ...
    ).where(ResistanceGene.is_active)
...
@router.get("/crops")
async def get_crops(db: AsyncSession = Depends(get_tenant_db)):
    """Get list of unique crops"""
    query = select(Disease.crop).where(Disease.is_active).distinct()
...
async def get_pathogen_types(db: AsyncSession = Depends(get_tenant_db)):
    """Get list of pathogen types"""
    query = select(Disease.pathogen_type).where(Disease.is_active).distinct()
...
async def get_resistance_types(db: AsyncSession = Depends(get_tenant_db)):
    """Get list of resistance types"""
    query = select(ResistanceGene.resistance_type).where(
        ResistanceGene.is_active
    ).distinct()
...
async def get_statistics(db: AsyncSession = Depends(get_tenant_db)):
    """Get disease resistance statistics"""
    # Count diseases
    disease_count = await db.execute(
        select(func.count(Disease.id)).where(Disease.is_active)
    )
    total_diseases = disease_count.scalar() or 0

    # Count genes
    gene_count = await db.execute(
        select(func.count(ResistanceGene.id)).where(ResistanceGene.is_active)
    )
    total_genes = gene_count.scalar() or 0

    # Count crops
    crop_result = await db.execute(
        select(Disease.crop).where(Disease.is_active).distinct()
    )
    ...
    mas_count = await db.execute(
        select(func.count(ResistanceGene.id)).where(
            (ResistanceGene.is_active) &
            (ResistanceGene.markers is not None) &
...
async def get_pyramiding_strategies(db: AsyncSession = Depends(get_tenant_db)):
    """Get gene pyramiding strategies"""
    query = select(PyramidingStrategy).where(PyramidingStrategy.is_active)
