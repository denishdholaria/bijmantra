...
from app.modules.seed_bank.mta import BenefitSharingType, MTAStatus, MTAType, mta_service


router = APIRouter(prefix="/mta", tags=["Material Transfer Agreements"], dependencies=[Depends(get_current_user)])
...
class CreateMTARequest(BaseModel):
    ...
    crops: list[str]
    purpose: str
    benefit_sharing: BenefitSharingInfo | None = None
...
@router.get("")
async def list_mtas(
    mta_type: MTAType | None = Query(None, description="Filter by MTA type"),
    status: MTAStatus | None = Query(None, description="Filter by status"),
