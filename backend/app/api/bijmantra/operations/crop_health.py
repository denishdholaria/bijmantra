...
router = APIRouter(prefix="/crop-health", tags=["Crop Health"], dependencies=[Depends(get_current_user)])
...
async def list_trial_health(
    ...
    risk_level: str | None = Query(None, description="Filter by disease risk level"),
...
async def get_trial_health(trial_id: str, db: AsyncSession = Depends(get_tenant_db)):
...
async def record_health_scan(trial_id: str, data: HealthScan, db: AsyncSession = Depends(get_tenant_db)):
    ...
    trial.last_scan_at = datetime.now(UTC)
...
async def get_alert(alert_id: str, db: AsyncSession = Depends(get_tenant_db)):
...
async def acknowledge_alert(alert_id: str, data: AlertAcknowledge, db: AsyncSession = Depends(get_tenant_db)):
...
async def list_locations(db: AsyncSession = Depends(get_tenant_db)):
...
async def list_crops(db: AsyncSession = Depends(get_tenant_db)):
