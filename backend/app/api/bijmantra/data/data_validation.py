...
async def get_validation_issues(
    type: str | None = Query(None, description="Filter by type: error, warning, info"),
...
@router.get("/stats")
async def get_validation_stats(db: AsyncSession = Depends(get_tenant_db)):
...
async def update_validation_rule(rule_id: str, data: dict, db: AsyncSession = Depends(get_tenant_db)):
...
async def update_issue_status(issue_id: str, data: dict, db: AsyncSession = Depends(get_tenant_db)):
...
async def delete_issue(issue_id: str, db: AsyncSession = Depends(get_tenant_db)):
...
@router.post("/run")
async def run_validation(data: dict | None = None, db: AsyncSession = Depends(get_tenant_db)):
    """Trigger a new validation run"""
    entity_types = data.get("entity_types", ["all"]) if data else ["all"]

    # Get enabled rules count
    rules_result = await db.execute(
        select(func.count()).select_from(ValidationRule).where(ValidationRule.enabled)
    )
