...
from app.models.collaboration import SyncSettings as SyncSettingsModel
...
async def get_sync_stats(db: AsyncSession = Depends(get_tenant_db)):
...
async def delete_pending_item(item_id: int, db: AsyncSession = Depends(get_tenant_db)):
...
async def get_sync_status(sync_id: int, db: AsyncSession = Depends(get_tenant_db)):
...
async def get_offline_data(db: AsyncSession = Depends(get_tenant_db)):
...
async def clear_offline_data(category: str, db: AsyncSession = Depends(get_tenant_db)):
...
async def refresh_offline_data(category: str, db: AsyncSession = Depends(get_tenant_db)):
...
async def get_conflicts(db: AsyncSession = Depends(get_tenant_db)):
...
async def get_conflict_details(item_id: int, db: AsyncSession = Depends(get_tenant_db)):
...
async def get_sync_settings(db: AsyncSession = Depends(get_tenant_db)):
...
async def download_updates(
    entity_types: str | None = Query(None, description="Comma-separated entity types"),
    since: datetime | None = Query(None, description="Download changes since this time"),
    db: AsyncSession = Depends(get_tenant_db)
):
