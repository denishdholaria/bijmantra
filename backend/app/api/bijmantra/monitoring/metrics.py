...
METRICS_PATHS = [
    Path(__file__).parent.parent.parent.parent.parent / "metrics.json",
]
...
FALLBACK_METRICS = {
    ...
    "build": {"status": "passing", "pwaEntries": 116, "sizeKB": 7960, "sizeMB": "7.9MB"},
    "milestones": {
...
class ApiMetrics(BaseModel):
    totalEndpoints: int
    brapiEndpoints: int
    brapiPublishedEndpoints: int | None = None
    brapiExposedEndpoints: int | None = None
    brapiCoverage: int
    customEndpoints: int


def _normalize_api_metrics(api_data: dict) -> dict:
    normalized = dict(api_data)
    published = normalized.get("brapiPublishedEndpoints")
    if not isinstance(published, int):
        published = 201
    exposed = normalized.get("brapiExposedEndpoints")
    if not isinstance(exposed, int):
        exposed = normalized.get("brapiEndpoints", published)
    total_endpoints = normalized.get("totalEndpoints", 0)
    custom_endpoints = normalized.get("customEndpoints")
    if not isinstance(custom_endpoints, int):
        custom_endpoints = max(total_endpoints - exposed, 0)
    brapi_coverage = normalized.get("brapiCoverage")
    if not isinstance(brapi_coverage, int):
        brapi_coverage = min(100, int((min(exposed, published) / published) * 100)) if published else 0

    normalized["brapiPublishedEndpoints"] = published
    normalized["brapiExposedEndpoints"] = exposed
    normalized["brapiEndpoints"] = exposed
    normalized["customEndpoints"] = custom_endpoints
    normalized["brapiCoverage"] = brapi_coverage
    return normalized


def _normalize_metrics_payload(data: dict) -> dict:
    normalized = dict(data)
    normalized["api"] = _normalize_api_metrics(data.get("api", {}))
    return normalized
...
async def load_metrics() -> dict:
    ...
    for metrics_path in METRICS_PATHS:
        try:
            await aiofiles.os.stat(metrics_path)
            async with aiofiles.open(metrics_path) as f:
                content = await f.read()
                loaded = json.loads(content)
                if isinstance(loaded, dict):
                    return _normalize_metrics_payload(loaded)
        except (OSError, FileNotFoundError, json.JSONDecodeError):
            continue

    # Return fallback if no file found
    return _normalize_metrics_payload(FALLBACK_METRICS)
...
async def get_version():
    ...
    data = await load_metrics()
    version_data = data.get("version", {"app": "0.2.0", "brapi": "2.1", "schema": "1.0.0"})
    return {
...
async def badge_version():
    ...
    return {"schemaVersion": 1, "label": "version", "message": f"v{version}", "color": "blue"}
