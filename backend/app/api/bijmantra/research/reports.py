...
async def get_report_stats(db: AsyncSession = Depends(get_tenant_db)):
    """Get report system statistics."""
    # Count templates
    templates_result = await db.execute(
        select(func.count(ReportTemplate.id)).where(ReportTemplate.is_active)
    )
...
async def get_template(template_id: int, db: AsyncSession = Depends(get_tenant_db)):
...
async def generate_report(
    ...
    report = GeneratedReport(
        ...
        name=f"{template.name} - {datetime.now(UTC).strftime('%Y-%m-%d')}",
    ...
    template.last_generated = datetime.now(UTC)
...
async def download_report(report_id: str, db: AsyncSession = Depends(get_tenant_db)):
...
async def delete_generated_report(report_id: int, db: AsyncSession = Depends(get_tenant_db)):
...
async def list_schedules(
    status: str | None = Query(None, description="Filter by status: active, paused"),
    db: AsyncSession = Depends(get_tenant_db)
):
...
async def create_schedule(
    ...
    if frequency == ScheduleFrequency.WEEKLY:
        ...
        next_run = datetime.now(UTC) + timedelta(days=30)
    elif frequency == ScheduleFrequency.QUARTERLY:
        next_run = datetime.now(UTC) + timedelta(days=90)
...
async def get_schedule(schedule_id: int, db: AsyncSession = Depends(get_tenant_db)):
...
async def delete_schedule(schedule_id: int, db: AsyncSession = Depends(get_tenant_db)):
...
async def run_schedule_now(schedule_id: int, db: AsyncSession = Depends(get_tenant_db)):
    ...
    report = GeneratedReport(
        ...
        name=f"{schedule.name} - {datetime.now(UTC).strftime('%Y-%m-%d')}",
