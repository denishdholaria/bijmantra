...
class ObservationRequest(BaseModel):
    ...
    timestamp: str | None = Field(None, description="Observation timestamp")
...
async def get_study_by_code(db: AsyncSession, study_id: str) -> FieldBookStudy | None:
...
@router.get("/studies")
async def list_studies(db: AsyncSession = Depends(get_tenant_db)):
    """List studies with field books"""
    query = select(FieldBookStudy).where(FieldBookStudy.is_active)
...
async def record_observation(
    ...
    timestamp = datetime.fromisoformat(request.timestamp) if request.timestamp else datetime.now(UTC)
    ...
    for obs_data in request.observations:
        ...
        if existing:
            if isinstance(value, (int, float)):
                ...
                existing.value_numeric = None
            existing.observation_timestamp = datetime.now(UTC)
        else:
            obs = FieldBookObservation(
                id=uuid_module.uuid4(),
                organization_id=study.organization_id,
...
async def get_collection_summary(
    ...
    for trait in traits:
        ...
        obs_query = select(FieldBookObservation.value_numeric).where(
            (FieldBookObservation.study_id == study.id) &
            (FieldBookObservation.trait_id == trait.id) &
            (FieldBookObservation.value_numeric is not None)
        )
