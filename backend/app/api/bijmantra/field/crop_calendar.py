...
from app.modules.environment.services.crop_calendar_service import get_crop_calendar_service


router = APIRouter(prefix="/crop-calendar", tags=["Crop Calendar"], dependencies=[Depends(get_current_user)])
...
async def list_events(
    crop_id: str | None = Query(None, description="Filter by crop"),
...
async def get_growth_stage(
    event_id: str,
    check_date: str | None = Query(None, description="Date to check (default: today)")
):
    """
...
async def get_upcoming_activities(
    ...
    event_id: str | None = Query(None, description="Filter by planting event")
):
    """
