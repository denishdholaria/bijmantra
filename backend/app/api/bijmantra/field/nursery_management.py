...
from app.models.field_operations import BatchStatus, NurseryLocation, SeedlingBatch
...
class CountsUpdateRequest(BaseModel):
    ...
    germinated: int | None = Field(None, ge=0, description="Number germinated")
    healthy: int | None = Field(None, ge=0, description="Number healthy")
...
@router.get("/stats")
async def get_stats(db: AsyncSession = Depends(get_tenant_db)):
...
async def get_locations(db: AsyncSession = Depends(get_tenant_db)):
    """Get available locations"""
    query = select(NurseryLocation).where(NurseryLocation.is_active)
...
async def get_germplasm_list(db: AsyncSession = Depends(get_tenant_db)):
