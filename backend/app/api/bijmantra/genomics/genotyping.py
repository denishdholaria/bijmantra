...
def _is_missing_variantset_table(error: ProgrammingError) -> bool:
    message = str(error).lower()
    return "variant_set" in message and "does not exist" in message
...
async def list_variant_sets(
    ...
    try:
        ...
        if not _is_missing_variantset_table(error):
            raise
        await db.rollback()
        items, total = [], 0
...
async def get_variant_set(variantSetDbId: str, db: AsyncSession = Depends(get_tenant_db)):
...
async def get_call_set(callSetDbId: str, db: AsyncSession = Depends(get_tenant_db)):
...
async def get_calls_statistics(variantSetDbId: str | None = None, db: AsyncSession = Depends(get_tenant_db)):
...
async def get_reference(referenceDbId: str, db: AsyncSession = Depends(get_tenant_db)):
...
async def get_vendor_order(vendorOrderDbId: str, db: AsyncSession = Depends(get_tenant_db)):
...
async def update_vendor_order_status(vendorOrderDbId: str, data: VendorOrderStatusUpdate, db: AsyncSession = Depends(get_tenant_db)):
...
# Summary endpoint
@router.get("/summary")
async def get_genotyping_summary(db: AsyncSession = Depends(get_tenant_db)):
