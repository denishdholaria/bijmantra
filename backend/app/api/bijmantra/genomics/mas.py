...
from app.modules.genomics.services.marker_assisted_service import get_mas_service


router = APIRouter(prefix="/mas", tags=["Marker-Assisted Selection"], dependencies=[Depends(get_current_user)])
...
async def register_marker(request: MarkerRegisterRequest):
    ...
    try:
        ...
        # Avoid exposing internal error details in response
        raise HTTPException(500, "Failed to register marker")
...
async def list_markers(
    ...
    trait: str | None = Query(None, description="Filter by linked trait")
):
...
async def foreground_selection(request: ForegroundRequest):
    ...
    try:
        ...
        # Avoid exposing internal error details in response
        raise HTTPException(500, "Foreground selection failed")
...
async def background_selection(request: BackgroundRequest):
    ...
    try:
        ...
        # Avoid exposing internal error details in response
        raise HTTPException(500, "Background selection failed")
...
async def mabc_selection(request: MABCRequest):
    ...
    try:
        ...
        # Avoid exposing internal error details in response
        raise HTTPException(500, "MABC selection failed")
