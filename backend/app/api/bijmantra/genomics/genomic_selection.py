...
from app.schemas.genomic_selection import (
    GBLUPRequest,
    GBLUPResponse,
    GMatrixRequest,
    GMatrixResponse,
...
async def run_gblup(request: GBLUPRequest):
    ...
    """
    result = compute_engine.compute_gblup_from_grm(
        phenotypes=np.array(request.phenotypes),
        grm=np.array(request.g_matrix),
        heritability=request.heritability,
    )

    analysis_id = str(uuid.uuid4())
    response = GBLUPResponse(
        gebv=result.breeding_values.tolist(),
        reliability=result.reliability.tolist() if result.reliability is not None else [0.0] * len(request.phenotypes),
        ...
        mean=float(result.fixed_effects[0]) if len(result.fixed_effects) > 0 else float(np.mean(request.phenotypes)),
    )
