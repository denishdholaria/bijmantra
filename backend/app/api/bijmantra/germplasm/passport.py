...
from app.modules.germplasm.services.passport_service import get_passport_service


router = APIRouter(prefix="/passport", tags=["Germplasm Passport"])
...
class AccessionRegisterRequest(BaseModel):
    ...
    acquisition_date: str | None = Field(None, description="Acquisition date (YYYY-MM-DD)")
...
class CollectionSiteRequest(BaseModel):
    ...
    latitude: float | None = Field(None, ge=-90, le=90, description="Latitude (decimal degrees)")
    longitude: float | None = Field(None, ge=-180, le=180, description="Longitude (decimal degrees)")
    elevation: float | None = Field(None, description="Elevation (meters)")
    collection_date: str | None = Field(None, description="Collection date (YYYY-MM-DD)")
...
async def register_accession(
    request: AccessionRegisterRequest,
    ...
    """
    await _require_passport_manage_access(current_user, db)
    organization_id = _current_organization_id(current_user)
...
async def list_accessions(
    ...
    biological_status: str | None = Query(None, description="Filter by biological status code"),
    country: str | None = Query(None, description="Filter by collection country"),
    db: AsyncSession = Depends(get_tenant_db),
    current_user=Depends(get_current_user),
):
    """List germplasm accessions with optional filters"""
    await _require_passport_read_access(current_user, db)
    organization_id = _current_organization_id(current_user)
    service = get_passport_service()
...
async def get_passport(
    ...
    passport = service.get_passport(accession_id, organization_id=organization_id)
    if passport is None:
...
async def add_collection_site(
    accession_id: str,
    request: CollectionSiteRequest,
    db: AsyncSession = Depends(get_tenant_db),
    current_user=Depends(get_current_user),
):
    """
...
async def search_accessions(
    ...
    results = service.search(q, limit, organization_id=organization_id)
...
async def export_mcpd(
    accession_ids: str | None = Query(None, description="Comma-separated accession IDs (empty = all)"),
    db: AsyncSession = Depends(get_tenant_db),
    current_user=Depends(get_current_user),
):
    """
    ...
    mcpd_data = service.export_mcpd(ids, organization_id=organization_id)
...
async def get_statistics(
    ...
    stats = service.get_statistics(organization_id=organization_id)
...
async def list_biological_status_codes(
    ...
    await _require_passport_read_access(current_user, db)
    return legacy_bijmantra_passport_biological_status_codes()
...
async def list_acquisition_source_codes(
    ...
    await _require_passport_read_access(current_user, db)
    return legacy_bijmantra_passport_acquisition_source_codes()


async def _require_passport_read_access(current_user, db: AsyncSession) -> None:
...
async def _require_passport_manage_access(current_user, db: AsyncSession) -> None:
    await require_accession_passport_api_access(
        current_user,
        db=db,
...
def _current_organization_id(current_user: Any) -> int:
    if isinstance(current_user, Mapping):
        return int(current_user["organization_id"])
    return int(getattr(current_user, "organization_id"))
