...
async def get_collections(
    type: str | None = Query(None, description="Filter by type: core, working, active, base, breeding"),
    species: str | None = Query(None, description="Filter by species"),
    curator: str | None = Query(None, description="Filter by curator"),
...
@router.get("/stats")
async def get_collection_stats(db: AsyncSession = Depends(get_tenant_db)):
...
async def get_collection(collection_id: str, db: AsyncSession = Depends(get_tenant_db)):
...
@router.post("")
async def create_collection(data: dict, db: AsyncSession = Depends(get_tenant_db)):
...
async def update_collection(collection_id: str, data: dict, db: AsyncSession = Depends(get_tenant_db)):
    """Update a collection"""
    try:
        coll_uuid = uuid.UUID(collection_id)
    except ValueError:
...
async def delete_collection(collection_id: str, db: AsyncSession = Depends(get_tenant_db)):
    """Delete a collection"""
    try:
        coll_uuid = uuid.UUID(collection_id)
    except ValueError:
...
async def add_accessions(collection_id: str, data: dict, db: AsyncSession = Depends(get_tenant_db)):
