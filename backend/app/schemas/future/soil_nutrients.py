...
class SoilTestBase(BaseModel):
    ...
    ca_ppm: float | None = Field(None, ge=0)
    mg_ppm: float | None = Field(None, ge=0)
    s_ppm: float | None = Field(None, ge=0)
    zn_ppm: float | None = Field(None, ge=0)
    fe_ppm: float | None = Field(None, ge=0)
    mn_ppm: float | None = Field(None, ge=0)
    cu_ppm: float | None = Field(None, ge=0)
    b_ppm: float | None = Field(None, ge=0)
    cec: float | None = Field(None, ge=0, description="Cation Exchange Capacity (meq/100g)")
    # Physics
    texture_class: str | None = Field(None, max_length=100)
    sand_percent: float | None = Field(None, ge=0, le=100)
    silt_percent: float | None = Field(None, ge=0, le=100)
    clay_percent: float | None = Field(None, ge=0, le=100)
    notes: str | None = None
...
class SoilHealthScoreBase(BaseModel):
    ...
    physical_score: float | None = Field(None, ge=0, le=100)
    chemical_score: float | None = Field(None, ge=0, le=100)
    biological_score: float | None = Field(None, ge=0, le=100)
    organic_carbon_percent: float | None = Field(None, ge=0)
    aggregate_stability: float | None = Field(None, ge=0, le=100, description="Water-stable aggregates %")
    water_infiltration_rate: float | None = Field(None, ge=0, description="mm/hour")
    earthworm_count: int | None = Field(None, ge=0, description="Per m²")
    microbial_biomass: float | None = Field(None, ge=0, description="mg/kg")
    respiration_rate: float | None = Field(None, ge=0, description="mg CO2/kg/day")
...
class FertilizerRecommendationBase(BaseModel):
    ...
    s_kg_ha: float | None = Field(default=0.0, ge=0, description="Sulfur kg/ha")
    zn_kg_ha: float | None = Field(default=0.0, ge=0, description="Zinc kg/ha")
    application_timing: dict[str, Any] | None = Field(None, description="Split application schedule")
...
class CarbonSequestrationBase(BaseModel):
    ...
    sequestration_rate: float | None = Field(None, description="tonnes C/ha/year")
    measurement_depth_cm: int = Field(..., gt=0)
    methodology: str | None = Field(None, max_length=255)
    verification_status: str | None = Field(default="Pending", max_length=50)
    carbon_credits_potential: float | None = Field(None, ge=0)
    practice_changes: dict[str, Any] | None = None
