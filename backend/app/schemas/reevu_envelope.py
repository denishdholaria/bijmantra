...
class ClaimTrace(BaseModel):
    """A display-oriented claim segment with explicit support classification."""

    statement: str = Field(..., description="Human-readable claim or synthesis sentence")
    support_type: Literal["retrieval_backed", "calculation_backed", "model_synthesis"] = Field(
        ...,
        description=(
            "Whether the sentence is backed by retrieval evidence, backed by deterministic "
            "calculation provenance, or remains model synthesis text."
        ),
    )
    evidence_refs: list[str] = Field(
        default_factory=list,
        description="Evidence refs explicitly attached to this claim segment",
    )
    calculation_ids: list[str] = Field(
        default_factory=list,
        description="Calculation ids explicitly attached to this claim segment",
    )
...
class ReevuEnvelope(BaseModel):
    ...
    claim_traces: list[ClaimTrace] = Field(
        default_factory=list,
        description="Claim segments classified by whether they are retrieval-backed or model synthesis",
    )
    ...
    missing_evidence_signals: list[str] = Field(
        default_factory=list,
        description="Explicit trust signals when evidence or calculation provenance is missing",
    )
