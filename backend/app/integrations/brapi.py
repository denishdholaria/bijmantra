...
class BrAPIAdapter(IntegrationAdapter):
    ...
    def capability_tags(self) -> list[str]:
        return ["germplasm", "trials", "observations", "seedlots", "programs"]

    @property
    def function_mappings(self) -> dict[str, str]:
        return {
            "search_germplasm": "GET /brapi/v2/germplasm",
            "get_germplasm_details": "GET /brapi/v2/germplasm/{germplasmDbId}",
            "list_programs": "GET /brapi/v2/programs",
            "search_trials": "GET /brapi/v2/trials",
            "search_observations": "GET /brapi/v2/observations",
            "get_seedlots": "GET /brapi/v2/seedlots",
        }
    ...
    def __init__(self, config: IntegrationConfig):
        ...
        self._client = create_traced_async_client(timeout=config.timeout_seconds)
    ...
    async def push_data(self, data_type: str, data: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        endpoint_map = {"germplasm": "germplasm", "programs": "programs", "trials": "trials"}

        if data_type not in endpoint_map:
            raise ValueError(
                f"Unsupported data type: {data_type}. Supported: {list(endpoint_map.keys())}"
            )
