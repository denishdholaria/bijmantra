...
def get_current_user_id_sql() -> str:
    """Return SQL expression for the current authenticated user id."""
...
def generate_user_owned_rls_policy_sql(table_name: str) -> str:
    """Generate SQL for tenant-scoped tables that are also user-owned."""
...
-- Drop existing policy if exists
DROP POLICY IF EXISTS {table_name}_tenant_isolation ON {table_name};
{drop_legacy_policy_sql}-- Create tenant + user isolation policy
CREATE POLICY {table_name}_tenant_isolation ON {table_name}
    FOR ALL
    USING (
        COALESCE(current_setting('app.current_organization_id', true), '-1')::integer = 0
        OR (
            organization_id = COALESCE(current_setting('app.current_organization_id', true), '-1')::integer
...
def generate_reasoning_trace_rls_policy_sql(table_name: str) -> str:
    """Generate SQL for Veena reasoning traces using session ownership evidence."""
    current_user_sql = get_current_user_id_sql()
    return f"""
-- Enable RLS on {table_name}
ALTER TABLE {table_name} ENABLE ROW LEVEL SECURITY;
...
-- Read access is scoped through the current user's audit-log-backed session history.
CREATE POLICY {table_name}_tenant_isolation ON {table_name}
    FOR ALL
    USING (
        COALESCE(current_setting('app.current_organization_id', true), '-1')::integer = 0
        OR (
            ...
            AND EXISTS (
                ...
                WHERE audit.session_id = {table_name}.session_id
                  AND audit.organization_id = COALESCE(current_setting('app.current_organization_id', true), '-1')::integer
...
def generate_table_rls_policy_sql(table_name: str) -> str:
    """Dispatch RLS generation for generic and special-case tenant tables."""
    if table_name in {"veena_memories_v2", "veena_user_contexts_v2", "veena_audit_logs"}:
        return generate_user_owned_rls_policy_sql(table_name)
    if table_name == "veena_reasoning_traces_v2":
        return generate_reasoning_trace_rls_policy_sql(table_name)
    return generate_rls_policy_sql(table_name)
...
""",
    ...
    for table in RLS_ENABLED_TABLES:
        ...
        sql_parts.append(generate_table_rls_policy_sql(table))
        sql_parts.append("")
