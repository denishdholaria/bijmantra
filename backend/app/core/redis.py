...
class RedisClient:
    ...
    async def keys(self, pattern: str) -> list[str]:
        """Get keys matching pattern using non-blocking SCAN instead of KEYS.

        KEYS is O(N) and blocks the Redis server for the full scan duration.
        SCAN iterates in small batches and is safe under production keyspace sizes.
        """
        if not self._available:
            return []
    ...
    async def delete_pattern(self, pattern: str) -> int:
        ...
        Uses SCAN + pipeline DELETE in batches of 100 to avoid blocking Redis
        and to keep individual pipeline payloads small.
        """
        if not self._available:
            return 0
