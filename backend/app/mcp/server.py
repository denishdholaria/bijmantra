...
try:
    ...
    MCP_AVAILABLE = False
    logger.warning("[MCP] mcp package not installed. Install with: pip install mcp")
...
class BijmantraClient:
    ...
    async def chat_with_reevu(self, message: str) -> dict:
        """Chat with REEVU, the Bijmantra agricultural intelligence assistant."""
        return await self._request(
            "POST",
            "/api/v2/chat",
    ...
    async def chat_with_veena(self, message: str) -> dict:
        """Legacy alias for chat_with_reevu."""
        return await self.chat_with_reevu(message)
...
if MCP_AVAILABLE:
    ...
    async def list_tools() -> list[Tool]:
        ...
        return [
            ...
            Tool(
                name="ask_reevu",
                description="Ask REEVU, the AI breeding assistant, a question about plant breeding",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "question": {"type": "string", "description": "Your question for REEVU"}
                    },
                    "required": ["question"]
                }
            ),
            Tool(
                name="ask_veena",
                description="Legacy alias for asking REEVU a plant breeding question",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "question": {"type": "string", "description": "Your question for REEVU"}
    ...
    async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
        ...
        try:
            if name == "list_programs":
                ...
                result = await client.chat_with_reevu(arguments["question"])
            elif name == "ask_veena":
                result = await client.chat_with_reevu(arguments["question"])
...
def run_server():
    ...
    logger.info(f"[Bijmantra MCP] Starting server v{MCP_SERVER_VERSION}")
    logger.info(f"[Bijmantra MCP] Connecting to API at {BIJMANTRA_API_URL}")
    asyncio.run(main())
