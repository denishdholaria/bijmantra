...
from app.models.social import Report, ReportStatus, Reputation
...
class ModerationService:
    ...
    async def get_reports(self, status: ReportStatus | None = None, skip: int = 0, limit: int = 20) -> list[Report]:
        stmt = select(Report)
        if status:
    ...
    async def resolve_report(self, report_id: int, resolved_by_id: int, resolution_note: str, status: ReportStatus = ReportStatus.RESOLVED) -> Report | None:
