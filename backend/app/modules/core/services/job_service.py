...
from app.core.redis import _fallback, redis_client
...
class JobService:
    ...
    async def list_jobs(self, job_type: str | None = None) -> list[dict[str, Any]]:
        """
        ...
        if redis_client.is_available:
            ...
            for key in keys:
                ...
                if job and (job_type is None or job.get("job_type") == job_type):
                    jobs.append(job)
        else:
    ...
    async def get_search_result(self, search_id: str) -> dict[str, Any] | None:
        """
