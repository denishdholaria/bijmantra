    ...
    for user, profile in result.all():
        ...
        if last_active:
            ...
            if time_diff < timedelta(minutes=5):
                ...
                last_active_str = (
                    f"{hours} hours ago" if hours < 24 else f"{int(hours / 24)} days ago"
                )
...
async def list_shared_items(
    db: AsyncSession, current_user: User, item_type: str | None = None
) -> list[dict]:
...
async def list_activity(
    db: AsyncSession, organization_id: int, limit: int, offset: int
) -> list[dict]:
    stmt = (
    ...
    for activity in result.scalars().all():
        ...
        if activity.user:
            user_name = (
                ...
                or getattr(activity.user, "username", None)
        ...
        out.append(
            {
                ...
                or f"{activity.activity_type} {activity.entity_type}",
