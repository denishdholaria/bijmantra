...
from .observer import EventSeverity, ObservationLayer, SecurityEvent
...
class ThreatAnalyzer:
    ...
    def _generate_assessment_id(self) -> str:
        ...
        return f"THR-{datetime.now(UTC).strftime('%Y%m%d')}-{self._assessment_counter:06d}"
    ...
    def get_recent_assessments(self, limit: int = 100,
                               min_confidence: ThreatConfidence | None = None) -> list[dict]:
