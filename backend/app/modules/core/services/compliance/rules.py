...
def check_ista_rules(batch: SeedBatchInfo) -> list[str]:
    ...
    if batch.crop in CROP_STANDARDS:
        ...
        if batch.purity_percentage < std["purity"]:
            errors.append(
                f"Purity {batch.purity_percentage}% is below ISTA standard {std['purity']}%."
            )
...
def validate_germination(batch: SeedBatchInfo) -> list[str]:
    ...
    if batch.crop in CROP_STANDARDS:
        ...
        if batch.germination_percentage < std["germination"]:
            errors.append(
                f"Germination {batch.germination_percentage}% is below standard {std['germination']}%."
            )
    else:
        # Default fallback
        if batch.germination_percentage < 75.0:
            errors.append(
                f"Germination {batch.germination_percentage}% is below general standard 75.0%."
            )
