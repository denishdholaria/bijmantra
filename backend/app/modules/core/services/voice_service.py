...
from app.core.http_tracing import merge_trace_headers

logger = logging.getLogger(__name__)


# UP042 Fix: Use StrEnum
...
class VibeVoiceService:
    ...
    async def check_health(self) -> VoiceServiceStatus:
        ...
        try:
            import aiohttp
            # SIM117 Fix: Flatten nested with (although this already looks flat, ensuring formatting)
...
class UnifiedVoiceService:
    ...
    async def synthesize(
        ...
        """
        # Try VibeVoice first
        # SIM102 Fix: Nested if merged into one
        if backend in (None, "vibevoice") and (await self.vibevoice.check_health()).available:
            try:
        ...
        # Try Edge TTS
        # SIM102 Fix: Nested if merged
        if backend in (None, "edge_tts") and await self.edge_tts.check_available():
            try:
...
# Singleton instances
_voice_service: VibeVoiceService | None = None
_unified_service: UnifiedVoiceService | None = None
