...
from app.models.iot import IoTAlertEvent, IoTDevice, IoTSensor, IoTTelemetry
...
def _is_missing_iot_schema_error(error: ProgrammingError) -> bool:
    message = str(getattr(error, 'orig', error))
    return (
        'UndefinedTableError' in message
        or 'UndefinedColumnError' in message
    ) and any(name in message for name in ('iot_devices', 'iot_sensors', 'iot_telemetry', 'iot_alert_events', 'sensor_type_id'))
...
class VaultSensorService:
    ...
    async def get_sensor_readings(
        ...
        try:
            ...
            if not _is_missing_iot_schema_error(error):
                raise
            await db.rollback()
            return []
    ...
    async def get_vault_conditions(
        ...
        alert_stmt = select(func.count(IoTAlertEvent.id)).where(
            and_(
                ...
                IoTAlertEvent.acknowledged.is_(False)
            )
        )
        try:
