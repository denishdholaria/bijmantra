...
RATE_LIMITS: dict[RateLimitType, RateLimitConfig] = {
    RateLimitType.LOGIN: RateLimitConfig(
        max_requests=1000,  # Relaxed for development
        window_seconds=900,  # 15 minutes
        block_duration_seconds=1,  # Minimal block
    ),
...
class RateLimiter:
    ...
    async def check(
        ...
        if await self._is_blocked(limit_type, identifier):
            ...
            return RateLimitResult(
                allowed=False,
                remaining=0,
                reset_at=datetime.fromtimestamp(now + max(ttl, 0), tz=UTC),
    ...
    async def _check_redis(
        ...
        try:
            ...
            reset_at = datetime.fromtimestamp(now + config.window_seconds, tz=UTC)
