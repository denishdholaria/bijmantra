...
class AuditEntry:
    ...
    action: str
    actor: str | None  # User ID or system component
    target: str | None  # Resource being accessed/modified
    source_ip: str | None
...
class SecurityAuditLog:
    ...
    def get_stats(self, hours: int = 24) -> dict[str, Any]:
        ...
        return {
            ...
            "unique_actors": len({e.actor for e in recent if e.actor}),
            "unique_ips": len({e.source_ip for e in recent if e.source_ip}),
        }
    ...
    def search(
        ...
        for entry in reversed(self._entries):
            if query_lower in entry.action.lower() or query_lower in str(entry.details).lower() or entry.actor and query_lower in entry.actor.lower() or entry.target and query_lower in entry.target.lower():
