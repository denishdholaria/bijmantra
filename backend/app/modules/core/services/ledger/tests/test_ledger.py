...
from backend.app.services.ledger.models import Transaction, TransactionType
from backend.app.services.ledger.service import LedgerService
...
def test_transaction_hashing():
    tx = Transaction(id="1", type=TransactionType.SOWING, data={"test": "data"}, sender="sender")
    h = tx.calculate_hash()
    assert h

    # Verify hash changes with content
    tx2 = Transaction(id="1", type=TransactionType.SOWING, data={"test": "data2"}, sender="sender")
...
def test_blockchain_mining_and_verification():
    ...
    # Add transaction
    tx = Transaction(id="1", type=TransactionType.SOWING, data={"lot_id": "L1"}, sender="farm")
...
def test_ledger_service_instance():
    ...
    # Record transaction
    tx = service.record_transaction(TransactionType.HARVEST, {"quantity": 100}, "farmer_john")
