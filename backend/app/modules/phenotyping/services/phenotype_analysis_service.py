...
class HeritabilityResult:
    ...
    h2_narrow: float | None  # Narrow-sense (if available)
...
class PhenotypeAnalysisService:
    ...
    def anova_rcbd(
        ...
        """
        # Organize data
        genotypes = list({d["genotype"] for d in data})
        blocks = list({d["block"] for d in data})
    ...
    def estimate_heritability_multi_env(
        self,
        data: list[dict[str, Any]],
        trait: str,
        ...
        """
        Estimate heritability from multi-environment data (ANOVA method).

        Model: y = mean + G + E + GxE + Rep(E) + error

        Variance Estimation (Random Model):
        - Vg = (MS_g - MS_ge) / (r * e)
        - Vge = (MS_ge - MS_error) / r
        - Ve = MS_error

        Heritability (Entry-mean basis):
        H² = Vg / (Vg + Vge/e + Ve/(re))

        Args:
            data: List of dicts with genotype, environment, rep, trait_value
            trait: Trait name
            genotype_col: Column name for genotype
            env_col: Column name for environment
            rep_col: Column name for replicate (optional, assumed balanced if inferred)

        Returns:
            HeritabilityResult with Vg, Vge, Ve components
        """
        # 1. Extract Factors
        genotypes = list({d[genotype_col] for d in data})
        envs = list({d[env_col] for d in data})

        g = len(genotypes)
        e = len(envs)
        n_total = len(data)

        # Infer reps (assuming balanced for ANOVA approximation)
        r = n_total / (g * e)
        # If not integer, might be unbalanced. For now, use average r

        # 2. Calculate Means
        grand_mean = sum(d["value"] for d in data) / n_total

        # Marginal means
        geno_means = dict.fromkeys(genotypes, 0.0)
        geno_counts = dict.fromkeys(genotypes, 0)

        env_means = dict.fromkeys(envs, 0.0)
        env_counts = dict.fromkeys(envs, 0)

        ge_means = {} # Key: (geno, env)
        ge_counts = {}
        ...
        for d in data:
            ...
            key = (geno, env)
            ge_means[key] = ge_means.get(key, 0.0) + val
            ge_counts[key] = ge_counts.get(key, 0) + 1

        # Normalize means
        for k in geno_means:
            geno_means[k] /= geno_counts[k]
        for k in env_means:
            env_means[k] /= env_counts[k]
        for k in ge_means:
            ge_means[k] /= ge_counts[k]

        # 3. Sum of Squares (Type III / Balanced ANOVA Logic)

        # Correction Factor
        (grand_mean * n_total) ** 2 / n_total # or just (sum Y)^2 / N
        # Actually standard definition: SS_Total = sum(Y^2) - CF
        sum_sq_total = sum(d["value"]**2 for d in data) - (sum(d["value"] for d in data)**2 / n_total)

        # SS Genotype = r*e * sum((mean_g - grand)^2)
        ss_g = sum(r * e * (geno_means[geno] - grand_mean)**2 for geno in genotypes)

        # SS Environment = r*g * sum((mean_e - grand)^2)
        ss_e = sum(r * g * (env_means[env] - grand_mean)**2 for env in envs)

        # SS GxE = r * sum((mean_ge - mean_g - mean_e + grand)^2)
        ss_ge = sum(
            r * (ge_means.get((geno, env), grand_mean) - geno_means[geno] - env_means[env] + grand_mean)**2
            for geno in genotypes for env in envs
        )

        # Note: Rep(Env) is usually nested.
        # For simplicity in this service which handles mean data or raw data,
        # let's compute Error as residual.
        # SS_Total = SS_G + SS_E + SS_GE + SS_Rep(E) + SS_Error
        # If we ignore Rep(E) (treat as Error), then Residual includes Rep variance.
        # Let's approximate Residual = Total - G - E - GE
        ss_residual = sum_sq_total - ss_g - ss_e - ss_ge

        # 4. Degrees of Freedom
        df_g = g - 1
        df_e = e - 1
        df_ge = (g - 1) * (e - 1)
        # df_residual = df_total - df_g - df_e - df_ge
        # In full model with reps: df_error = e * g * (r - 1)
        df_residual = n_total - 1 - df_g - df_e - df_ge

        # 5. Mean Squares
        ms_g = ss_g / df_g if df_g > 0 else 0
        ms_ge = ss_ge / df_ge if df_ge > 0 else 0
        ms_residual = ss_residual / df_residual if df_residual > 0 else 0

        # 6. Variance Components (Random Model)
        # Ve = MS_residual
        ve = ms_residual

        # Vge = (MS_ge - MS_residual) / r
        vge = (ms_ge - ms_residual) / r
        vge = max(0, vge)
        ...
        # 7. Heritability (Entry Mean Basis)
        # Vp_mean = Vg + Vge/e + Ve/(re)
        vp_mean = vg + (vge / e) + (ve / (r * e))
        h2 = vg / vp_mean if vp_mean > 0 else 0

        # Approximate SE (Knapp 1985)
        # SE = SE(H2) ... complicated for multi-env. Return 0 for now or simple approx.
        se = 0.0
        ...
        return HeritabilityResult(
            ...
            h2_narrow=None,
            vg=vg,
            ve=ve, # This is residual plot error
            vp=vp_mean, # Phenotypic variance of the mean
            se=se
        )


# Singleton
_phenotype_service: PhenotypeAnalysisService | None = None
