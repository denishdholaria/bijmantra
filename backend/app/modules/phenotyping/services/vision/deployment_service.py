...
from app.models.vision import VisionDeployment, VisionModel
...
class VisionDeploymentService:
        ...
        if cmd:
            try:
                ...
                if process.returncode != 0:
                    error_output = stderr.decode().strip() or "Unknown conversion failure"
                    logger.error(f"Conversion failed: {error_output}")
                    raise RuntimeError(error_output)

            except FileNotFoundError:
                logger.warning(f"Conversion tool not found: {cmd[0]}")
                raise RuntimeError(f"Required conversion tool is not installed: {cmd[0]}")
        else:
            # No conversion mapping found or implemented
            logger.warning(f"No conversion mapping for {source_format} -> {target_format}")
            raise RuntimeError(f"No conversion mapping is implemented for {source_format} -> {target_format.value}")
    ...
    async def export_model(
        ...
        stmt = select(VisionModel).where(
            VisionModel.id == m_id, VisionModel.organization_id == organization_id
        )
        ...
        if format == ExportFormat.TFJS:
            ...
            ext = format.value
            if format == ExportFormat.TFLITE:
                ext = "tflite"
            elif format == ExportFormat.ONNX:
                ext = "onnx"
            elif format == ExportFormat.TORCHSCRIPT:
                ext = "pt"
            elif format == ExportFormat.SAVEDMODEL:
                ext = "pb"  # simplified
        ...
        if not os.path.exists(model.file_path):
            return {"error": f"Model artifact is missing at {model.file_path}"}
        ...
        if not os.path.exists(export_path):
            return {"error": "Model export did not produce an artifact"}
    ...
    async def get_deployment(
        ...
        stmt = select(VisionDeployment).where(
            VisionDeployment.id == d_id, VisionDeployment.organization_id == organization_id
        )
        result = await db.execute(stmt)
...
class ModelRegistryService:
    ...
    async def publish_model(
        ...
        Args:
            ...
            _organization_id: Organization ID for multi-tenant isolation
    ...
    async def search_registry(
        ...
        _crop: str | None = None,
        _task: str | None = None,
        _min_accuracy: float | None = None,
        _sort_by: str = "downloads",
    ) -> list[dict]:
