...
from app.schemas.images import ImageNewRequest, ImageSearchRequest, ImageUpdateRequest
from app.modules.genomics.compute.analytics.storage import storage_service
...
class ImageService:
    ...
    async def get_image(self, db: AsyncSession, image_db_id: str, organization_id: int) -> Image | None:
        """
    ...
    async def update_image(
        ...
        if image_data.imageName is not None:
            image.image_name = image_data.imageName
        if image_data.imageFileName is not None:
            image.image_file_name = image_data.imageFileName
        if image_data.imageFileSize is not None:
            image.image_file_size = image_data.imageFileSize
        if image_data.imageHeight is not None:
            image.image_height = image_data.imageHeight
        if image_data.imageWidth is not None:
            image.image_width = image_data.imageWidth
        if image_data.mimeType is not None:
            image.mime_type = image_data.mimeType
        if image_data.imageURL is not None:
            image.image_url = image_data.imageURL
        if image_data.imageTimeStamp is not None:
            image.image_time_stamp = image_data.imageTimeStamp
        if image_data.copyright is not None:
            image.copyright = image_data.copyright
        if image_data.description is not None:
            image.description = image_data.description
        if image_data.descriptiveOntologyTerms is not None:
            image.descriptive_ontology_terms = image_data.descriptiveOntologyTerms
        if image_data.observationDbId is not None:
            image.observation_db_id = image_data.observationDbId
        if image_data.imageLocation is not None:
            image.image_location = image_data.imageLocation
        if image_data.additionalInfo is not None:
            image.additional_info = image_data.additionalInfo
        if image_data.externalReferences is not None:
            image.external_references = image_data.externalReferences
