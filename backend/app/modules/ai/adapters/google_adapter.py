...
class GoogleAdapter(IProviderAdapter):
    ...
    async def call(
        ...
        try:
            ...
            logger.error("[Veena] Google AI error: %s", e)
        return None
        ...
        try:
            ...
            async with client.stream(
                ...
                if response.status_code != 200:
                    ...
                    logger.error("[Veena] Google API error %s: %s", response.status_code, error_text)
                    ...
                    if line:
                        try:
                            ...
                            if "error" in data:
                                ...
                                logger.error("[Veena] Google API returned error: %s", error_msg)
                                yield f"[Error: {error_msg}]"
                                return
                        except json.JSONDecodeError:
                            logger.debug("[Veena] Skipping non-JSON line: %s", line[:100])
                            continue
        except Exception as e:
            logger.error("[Veena] Google streaming error: %s", e)
            yield f"[Error: {str(e)}]"