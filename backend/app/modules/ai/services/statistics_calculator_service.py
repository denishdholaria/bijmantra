...
class TrialStatistics:
    def __init__(self, data: list[ObservationData]):
        ...
        self.germplasms = {d.germplasm_id for d in self.data}
    ...
    def calculate_heritability(self, ms_genotype: float, ms_error: float) -> float | None:
        """
    ...
    def calculate_lsd(self, ms_error: float, alpha: float = 0.05) -> float | None:
        """
    ...
    def calculate_genetic_gain(self, h2: float, phenotypic_std_dev: float, selection_intensity: float = 1.755) -> float | None:
        """
