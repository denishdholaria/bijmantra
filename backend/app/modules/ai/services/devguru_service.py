...
from app.models.core import Program, Study, Trial
from app.models.devguru import MilestoneStatus, ResearchMilestone, ResearchPhase, ResearchProject
from app.models.veena_core import ReasoningStage
from app.modules.ai.services.engine import get_llm_service
from app.modules.breeding.services.knowledge_service import BreedingKnowledgeBase


logger = logging.getLogger(__name__)

# We need to import services inside methods or use a different pattern to avoid circular imports if any
# but commonly we can import them here if they are in different modules.
...
Always consider the practical constraints of PhD research (time, resources, scope)."""
    ...
    async def generate_rag_response(
        ...
        """
        Generate a response using RAG (Savant Strategy).
        1. Context Retrieval (Vector Store)
        2. Cognitive Processing (LLM)
        3. Reasoning Trace Logging (Veena)
        """
        from app.models.core import User
        from app.modules.ai.services.memory import BreedingVectorService, VectorStoreService
        from app.modules.ai.services.veena_service import VeenaService

        # Initialize services
        vector_store = VectorStoreService(self.db)
        breeding_vector_service = BreedingVectorService(vector_store)
        veena_service = VeenaService(self.db)
        llm_service = get_llm_service()

        # 1. Retrieve Context (Savant: Observation)
        # Search for breeding knowledge relevant to the query
        rag_results = await breeding_vector_service.search_breeding_knowledge(query, limit=3)

        # Format context
        context_str = "\\n".join([
            f"- [{r.doc_type.upper()}] {r.title}: {r.content[:200]}..."
            for r in rag_results
        ])

        # Get Project Context
        project = await self.get_project(project_id)
        system_prompt = self.get_system_prompt(project)

        if context_str:
            system_prompt += f"\\n\\nRELEVANT KNOWLEDGE BASE:\\n{context_str}"

        # 2. Generate Response (Savant: Hypothesis/Deduction)
        ...
        # 3. Log Reasoning Trace (Savant: Reflection)
        # We assume the user exists for logging
        try:
            # Need a User object for logging, fetch it
            user_result = await self.db.execute(select(User).where(User.id == user_id))
            user = user_result.scalar_one_or_none()

            if user:
                # Log the "Thought" process (Concept)
                concept_trace = await veena_service.log_reasoning_trace(
                    user=user,
                    session_id=__import__('uuid').uuid4(), # New session or pass one
                    stage=ReasoningStage.CONCEPT,
                    content=f"Query: {query}\\nContext Found: {len(rag_results)} items",
                    confidence=0.9
                )

                # Log the "Conclusion"
                await veena_service.log_reasoning_trace(
                    user=user,
                    session_id=__import__('uuid').uuid4(), # Should match above ideally
            ...
            logger.error(f"Failed to log reasoning trace: {e}")
            concept_trace = None

        return {
            "message": response.content,
            "provider": response.provider.value,
            "sources": [r.model_dump() for r in rag_results],
            "reasoning": concept_trace.content if concept_trace else None
        }

    def get_system_prompt(self, project: ResearchProject | None = None) -> str:
...
"""
    ...
    async def list_projects(self, user_id: int | None = None) -> list[ResearchProject]:
    ...
    async def link_program(self, project_id: str, program_id: int) -> ResearchProject | None:
    ...
    async def get_research_programs(self) -> list[Program]:
        ...
        result = await self.db.execute(
            select(Program)
            .where(Program.is_research_project)
...
Keep the response concise and actionable."""
    ...
    async def add_paper(
        ...
        from app.models.devguru import ReadStatus, ResearchPaper
    ...
    async def update_paper(self, paper_id: str, **kwargs) -> dict[str, Any] | None:
        """Update a paper"""
        from app.models.devguru import ResearchPaper
        result = await self.db.execute(
    ...
    async def create_chapter(
        ...
        from app.models.devguru import ChapterStatus, ThesisChapter
    ...
    async def update_chapter(self, chapter_id: str, **kwargs) -> dict[str, Any] | None:
        """Update a chapter"""
        from app.models.devguru import ThesisChapter
        result = await self.db.execute(
    ...
    async def update_committee_member(self, member_id: str, **kwargs) -> dict[str, Any] | None:
    ...
    async def update_meeting(self, meeting_id: str, **kwargs) -> dict[str, Any] | None:
        """Update a meeting"""
        from app.models.devguru import CommitteeMeeting
        result = await self.db.execute(
    ...
    async def update_feedback(self, feedback_id: str, **kwargs) -> dict[str, Any] | None:
