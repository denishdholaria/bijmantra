...
async def _build_gblup_compute_response(
    shared: CalculateHandlerSharedContext,
    *,
    organization_id: int,
    trait: str,
    ...
    genomic_prediction = await GenomicPredictionService(
        compute_engine=shared.compute_engine,
    ).compute_gebv(
        db=None,
        organization_id=organization_id,
        trait_name=trait,
    ...
    if genomic_prediction.insufficient_data:
        raise ValueError(genomic_prediction.reason or "GBLUP compute inputs were insufficient")
    ...
    output = GBLUPOutput(
        breeding_values=[entry.gebv for entry in genomic_prediction.gebvs],
        reliability=[entry.reliability or 0.0 for entry in genomic_prediction.gebvs],
        accuracy=(
            [genomic_prediction.accuracy] * len(genomic_prediction.gebvs)
            if genomic_prediction.accuracy is not None
            else None
        ),
        genetic_variance=genomic_prediction.genetic_variance,
        error_variance=genomic_prediction.error_variance,
        mean=genomic_prediction.mean or 0.0,
        converged=bool(genomic_prediction.converged),
    )
    ...
    breeding_values = [entry.gebv for entry in genomic_prediction.gebvs]
    reliabilities = [entry.reliability or 0.0 for entry in genomic_prediction.gebvs]
    labels = [entry.germplasm_id for entry in genomic_prediction.gebvs]
    ranked_items = sorted(
        [
            {
                "candidate": label,
    ...
    return shared.with_plan_execution_summary(
        {
            ...
            "gebv_result": _serialize_gebv_result(genomic_prediction),
...
def _serialize_gebv_result(result: GEBVResult) -> dict[str, Any]:
    return {
        ...
        "gebvs": [
            {
                ...
                "germplasm_name": entry.germplasm_name,
                "gebv": entry.gebv,
        ...
        "genetic_variance": result.genetic_variance,
        "error_variance": result.error_variance,
        "mean": result.mean,
        "converged": result.converged,
        "evidence_refs": [ref.model_dump() for ref in result.evidence_refs],
        "calculation_steps": [step.model_dump() for step in result.calculation_steps],
    }
    ...
    if function_name == "calculate_breeding_value":
        ...
        try:
            if method == "GBLUP":
                ...
                return await _build_gblup_compute_response(
                    shared,
                    organization_id=org_id,
                    trait=trait,
                    heritability=heritability,
