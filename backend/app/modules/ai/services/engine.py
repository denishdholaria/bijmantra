...
- VEENA_LLM_PROVIDER: Legacy compatibility provider override
"""
...
class MultiTierLLMService:
    ...
    SYSTEM_PROMPT_TEMPLATE = """You are REEVU, the Bijmantra scientific intelligence system for plant breeding and agricultural research.

REEVU is the canonical in-app AI identity presented to users across the Bijmantra experience.

═══════════════════════════════════════════════════════════════
YOUR IDENTITY (CRITICAL - ALWAYS ACCURATE)
═══════════════════════════════════════════════════════════════

You are REEVU, a plant breeding scientific reasoning system.
Your responses are powered by: {provider_name}
{model_info}

CRITICAL IDENTITY RULES:
1. You are REEVU, NOT the underlying model directly
2. When asked "what AI are you?" or "what model are you?":
    - Say "I am REEVU, powered by {provider_name}"
...
Remember: A breeder making decisions based on your advice could affect crop yields for thousands of farmers. Truth matters more than comfort."""
    ...
    def _initialize_adapters(self) -> None:
        ...
        self._adapters = {
            ...
            LLMProvider.NVIDIA_NIM: NvidiaNimAdapter(self._get_client),
        }

    def _get_ollama_tags_url(self, base_url: str | None) -> str | None:
        if not base_url:
            return None
    ...
    async def check_ollama_available(self, config: LLMConfig | None = None) -> bool:
        ...
        status_url = self._get_ollama_tags_url(ollama_config.base_url)
        if not status_url:
            return False
        ...
        try:
            ...
            logger.debug("[REEVU] Ollama availability check failed: %s", exc)
            return False

    async def list_ollama_models(self, config: LLMConfig | None = None) -> list[str]:
        """List available models from the configured Ollama host."""
        ollama_config = config or self.providers.get(LLMProvider.OLLAMA)
        if ollama_config is None:
            return []

        models_url = self._get_ollama_tags_url(ollama_config.base_url)
        if not models_url:
            return []
            ...
            for model in models:
                if isinstance(model, dict):
                    ...
                    if isinstance(model_name, str) and model_name.strip():
                        available_model_names.append(model_name.strip())
            return available_model_names
        except Exception as exc:
            logger.debug("[REEVU] Could not list Ollama models: %s", exc)
            return []
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        ...
        if self._http_client is None or self._http_client.is_closed:
            self._http_client = create_traced_async_client(timeout=60.0)
    ...
    def _generate_template_response(
        ...
        if any(word in user_message for word in ["hello", "hi", "namaste", "hey"]):
            return (
                "Hello! 🙏 I'm REEVU, your plant breeding analysis system. "
                "I'm currently running in offline mode with limited capabilities. "
                "For full AI-powered responses, please configure a managed REEVU provider "
                "in AI Settings. Ollama remains available as an advanced local option. "
        ...
        if any(word in user_message for word in ["help", "what can you do", "capabilities"]):
            return (
                ...
                "template mode (no managed provider configured). For intelligent responses:\n\n"
                "**Recommended path:**\n"
                "• Configure a managed REEVU provider in AI Settings\n"
                "• Or enable Ollama locally for advanced on-device routing\n\n"
        ...
        if "cross" in user_message or "breeding" in user_message:
            return (
                ...
                "Please configure a managed REEVU provider in AI Settings, "
                "or enable Ollama for advanced local routing. "
        ...
        return (
            ...
            "For intelligent responses, configure REEVU in AI Settings with a managed provider.\n"
            "If you need advanced local routing, you can also enable Ollama on-device.\n\n"
            "See /ai-settings for configuration help."
        )
    ...
    async def generate(
        ...
        for config in available:
            ...
            logger.info(f"[REEVU] Trying provider: {config.provider.value} ({config.model})")
            ...
            try:
                ...
                if result and result.content:
                    ...
                    logger.info(f"[REEVU] Response from {config.provider.value} ({actual_model}, confirmed={model_confirmed}) in {latency:.0f}ms")
                    return response
        ...
        logger.info("[REEVU] Using template fallback")
    ...
    async def chat(
        ...
        """
        High-level chat interface for REEVU.
    ...
    async def stream_chat(
        ...
        try:
            ...
            logger.info(f"[REEVU] Streaming from {active_config.provider.value} ({active_config.model})")
...
async def veena_chat(
    ...
    """Legacy compatibility alias for REEVU chat."""
