...
def _build_cross_domain_retrieval_tables(results: dict[str, Any]) -> list[str]:
    ...
    return _dedupe_string_values(
        [
            ...
            "CropCalendar" if (results.get("field") or {}).get("crop_calendar_events") else None,
...
def _build_cross_domain_retrieval_entities(
    *,
    ...
    add_entity(
        entities,
        "field_calendar_event_count",
        len((results.get("field") or {}).get("crop_calendar_events") or []),
    )
    add_entity(
        entities,
        "field_layout_count",
        len((results.get("field") or {}).get("field_layouts") or []),
    )
