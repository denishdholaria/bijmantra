...
class FunctionExecutor:
    ...
    def _build_interpretation_records_from_observations(
        ...
        for observation in observations:
            ...
            entity_id = str(germplasm.get("accession") or germplasm.get("id") or "").strip()
        ...
        if function_name == "search_germplasm":
            ...
            try:
                if not self.germplasm_search_service:
                    raise FunctionExecutionError("Germplasm search service not available")
            ...
            try:
                results = await self.trial_search_service.search(
                    db=self.db,
                    organization_id=org_id,
                    query=query,
                    crop=crop,
            ...
            try:
                results = await self.cross_search_service.search(
                    db=self.db,
                    organization_id=org_id,
                    query=query,
                    parent_id=parent_id,
            ...
            try:
                ...
                if country:
                    locations = await self.location_search_service.search(
                        db=self.db,
                        organization_id=org_id,
                        country=country,
                        limit=100
            ...
            try:
                results = await self.location_search_service.search(
                    db=self.db,
                    organization_id=org_id,
                    query=query,
                    country=country,
    ...
    async def _handle_statistics(self, params: dict) -> dict:
        ...
        try:
            ...
            germplasm_stats = await self.germplasm_search_service.get_statistics(
                db=self.db,
                organization_id=org_id
            )

            trial_stats = await self.trial_search_service.get_statistics(
                db=self.db,
                organization_id=org_id
            )
            ...
            cross_stats = await self.cross_search_service.get_statistics(
                db=self.db,
                organization_id=org_id
            )

            location_stats = await self.location_search_service.get_statistics(
                db=self.db,
                organization_id=org_id
            )
    ...
    async def _handle_cross_domain_query(self, params: dict) -> dict:
        ...
        try:
            ...
            if germplasm_query or trait_query or crop_query:
                germplasm_results = await self.germplasm_search_service.search(
                    db=self.db,
                    organization_id=org_id,
            ...
            if location_query or program_query or crop_query:
                trial_results = await self.trial_search_service.search(
                    db=self.db,
                    organization_id=org_id,
                    query=None,
                    crop=crop_query,
            ...
            if location_query:
                location_results = await self.location_search_service.search(
                    db=self.db,
                    organization_id=org_id,
    ...
    async def _handle_get(self, function_name: str, params: dict) -> dict:
        ...
        if function_name == "get_germplasm_details":
            ...
            try:
                result = await self.germplasm_search_service.get_by_id(
                    db=self.db,
                    organization_id=org_id,
            ...
            try:
                # Get trial details
                trial = await self.trial_search_service.get_by_id(
                    db=self.db,
                    organization_id=org_id,
                ...
                records = self._build_interpretation_records_from_observations(observations)
                ...
                interpretation = phenotype_interpretation_service.build_interpretation(
                    ...
                    evidence_refs=[f"db:trial:{trial.get('trial_db_id') or trial_id}"],
                )
                ...
                return {
                    ...
                    "data": {
                        ...
                        "top_performers": self._build_ranked_candidates(interpretation)[:5],
            ...
            try:
                result = await self.location_search_service.get_by_id(
                    db=self.db,
                    organization_id=org_id,
            ...
            try:
                result = await self.cross_search_service.get_by_id(
                    db=self.db,
                    organization_id=org_id,
            ...
            try:
                ...
                summary = self.weather_service.get_veena_summary(forecast)
    ...
    async def _handle_compare(self, function_name: str, params: dict) -> dict:
        ...
        if function_name == "compare_germplasm":
            ...
            try:
                germplasm_data = await self._resolve_germplasm_requests(
                    organization_id=org_id,
                ...
                interpretation = phenotype_interpretation_service.build_interpretation(
                    ...
                    entity_order=[
                        str(germplasm.get("accession") or germplasm.get("id"))
                        for germplasm in germplasm_data
                    ],
                    baseline_entity_id=baseline_record.entity_db_id if baseline_record else None,
                    baseline_entity_name=baseline_record.entity_name if baseline_record else None,
                    baseline_selection="inferred_check_entry" if baseline_record else None,
                    evidence_refs=[
                        f"db:germplasm:{germplasm.get('accession') or germplasm.get('id')}"
                        for germplasm in germplasm_data
                    ],
                )
                candidates = self._build_ranked_candidates(interpretation)
        ...
        if function_name == "calculate_breeding_value":
            ...
            try:
                ...
                if method == "GBLUP":
                    ...
                    result = self.breeding_value_service.estimate_blup(
    ...
    async def _handle_predict(self, function_name: str, params: dict) -> dict:
        ...
        if function_name == "predict_cross":
            ...
            try:
                ...
                parent1 = await self.germplasm_search_service.get_by_id(
                    db=self.db,
                    organization_id=org_id,
                    germplasm_id=str(parent1_id)
                )
                parent2 = await self.germplasm_search_service.get_by_id(
                    db=self.db,
                    organization_id=org_id,
                ...
                prediction = self.breeding_value_service.predict_cross(
