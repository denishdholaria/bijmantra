...
async def get_trial_summary(*args: Any, **kwargs: Any) -> Any:
    """Compatibility wrapper for extracted get-handler injection."""
    from app.api.bijmantra.trials.trial_summary import get_trial_summary as _get_trial_summary

    return await _get_trial_summary(*args, **kwargs)


async def get_comparison_statistics(*args: Any, **kwargs: Any) -> Any:
    """Compatibility wrapper for extracted get-handler injection."""
    from app.api.bijmantra.phenotyping.phenotype_comparison import (
        get_comparison_statistics as _get_comparison_statistics,
    )

    return await _get_comparison_statistics(*args, **kwargs)
...
class FunctionExecutor:
    ...
    def __init__(
        ...
        crop_calendar_service: Optional[Any] = None,
    ):
        self.db = db
        ...
        self.crop_calendar_service = crop_calendar_service or CropCalendarService(db)
        # Keep these repo-owned service singletons on the executor so the
        # StepExecutor uses the same injected/test-patchable surfaces as the
        # rest of the REEVU handler family.
        self.observation_search_service = observation_search_service
        self.seedlot_search_service = seedlot_search_service
        self.trait_search_service = trait_search_service
        self.get_qtl_mapping_service = get_qtl_mapping_service
    ...
    async def _handle_get(self, function_name: str, params: dict) -> dict:
        ...
        if function_name == "get_trait_distribution":
            query = str(
                params.get("query")
                or params.get("q")
                or f"distribution of {params.get('trait') or 'phenotypic observations'}"
            ).strip()
            return await self._handle_cross_domain_query({**params, "query": query})

        if function_name == "get_seed_inventory":
            query = str(
                params.get("query")
                or params.get("q")
                or f"seed inventory for {params.get('germplasm') or params.get('germplasm_id') or 'requested scope'}"
            ).strip()
            return await self._handle_cross_domain_query({**params, "query": query})

        if function_name == "get_field_info":
            query = str(
                params.get("query")
                or params.get("q")
                or f"field information for {params.get('location') or 'requested scope'}"
            ).strip()
            return await self._handle_cross_domain_query({**params, "query": query})

        if function_name == "get_crop_calendar":
            query = str(
                params.get("query")
                or params.get("q")
                or f"crop calendar for {params.get('crop') or params.get('location') or 'requested scope'}"
            ).strip()
            return await self._handle_cross_domain_query({**params, "query": query})

        if function_name == "get_observations":
            query = str(params.get("query") or params.get("q") or "").strip()
            if query and "phenotyping" in _detect_domains(query):
                return await self._handle_cross_domain_query({**params, "query": query})
