        ...
        thoughts = [
            {
                ...
                "tool_used": "intent-parser",
                "outcome": "intent-captured",
            },
            {
                "step_id": "retrieve",
                "thought": "Fetch relevant memory",
                "tool_used": "pgvector",
                "outcome": f"context_hits={len(context)}",
            },
        ]
        for step in thoughts:
            await self.service.log_reasoning_step(
                session_id,
                step,
...
class VeenaCognitiveService:
    ...
    async def _resolve_organization_id(self, user_id: int, organization_id: int | None = None) -> int:
        if organization_id is not None:
            return organization_id

        result = await self.db.execute(select(User.organization_id).where(User.id == user_id))
        resolved_organization_id = result.scalar_one_or_none()
        if resolved_organization_id is None:
            raise ValueError(f"Unable to resolve organization for user {user_id}")
    ...
    async def store_memory(
        ...
        resolved_organization_id = await self._resolve_organization_id(user_id, organization_id)
        memory = VeenaMemory(
            ...
            embedding=interaction_data.get("embedding")
            or await self._embed_text(f"{query} {response}"),
    ...
    async def retrieve_relevant_context(
        self, user_id: int, current_query: str, limit: int = 5
    ) -> list[VeenaMemory]:
    ...
    def apply_expertise_filter(
        self, content: str, user_id: int | None = None, expertise_level: str | None = None
    ) -> str:
    ...
    async def feedback_loop(
        self, trace_id: int, approved: bool, correction: str | None = None
    ) -> None:
