...
class PostgresMissionStateRepository(MissionStateRepository):
    ...
    async def save_mission(self, mission: MissionRecord) -> MissionRecord:
        ...
        if instance is None:
            instance = OrchestratorMission(
                ...
                source_board_concurrency_token=mission.source_board_concurrency_token,
            ...
            instance.queue_job_id = mission.queue_job_id
            instance.source_lane_id = mission.source_lane_id
            instance.source_board_concurrency_token = mission.source_board_concurrency_token
    ...
    async def list_missions(
        ...
        *,
        ...
        if owner is not None:
            stmt = stmt.where(OrchestratorMission.owner == owner)
        if queue_job_id is not None:
            stmt = stmt.where(OrchestratorMission.queue_job_id == queue_job_id)
        if source_lane_id is not None:
            stmt = stmt.where(OrchestratorMission.source_lane_id == source_lane_id)
        stmt = stmt.order_by(OrchestratorMission.created_at.asc(), OrchestratorMission.id.asc())
    ...
    def _mission_record(self, instance: OrchestratorMission) -> MissionRecord:
        return MissionRecord(
            ...
            queue_job_id=instance.queue_job_id,
            source_lane_id=instance.source_lane_id,
            source_board_concurrency_token=instance.source_board_concurrency_token,
