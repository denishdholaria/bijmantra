...
def _build_cross_domain_plan_execution_summary(
    *,
    ...
    for step in plan.steps:
        ...
        if step.domain == "breeding":
        ...
        elif step.domain == "field":
            services = _dedupe_string_values(
                [
                    "location_search_service.search" if "field" in requested_domains else None,
            ...
            add_output(
                ...
                identifiers=_sample_record_identifiers(
                    results.get("locations"),
                    keys=("location_db_id", "locationDbId", "id"),
                ),
            )
            field_results = results.get("field") or {}
            field_layouts = field_results.get("field_layouts") or []
            crop_calendar_events = field_results.get("crop_calendar_events") or []
            if field_layouts:
                actual_outputs.append("field_layouts")
                output_counts["field_layouts"] = len(field_layouts)
            if crop_calendar_events:
                actual_outputs.append("crop_calendar_events")
                output_counts["crop_calendar_events"] = len(crop_calendar_events)
            if field_results.get("season_info"):
                actual_outputs.append("season_data")
                output_counts["season_data"] = 1
            ...
            phenotyping = results.get("phenotyping") or {}
            if phenotyping.get("distribution"):
                actual_outputs.append("trait_distribution")
                output_counts["trait_distribution"] = 1
            if phenotyping.get("summary_stats"):
                actual_outputs.append("observation_statistics")
                output_counts["observation_statistics"] = 1
        elif step.domain == "seed_ops":
            services = _dedupe_string_values(
            ...
            seed_ops = results.get("seed_ops") or {}
            inventory_summary = seed_ops.get("inventory_summary") or {}
            if inventory_summary:
                actual_outputs.append("inventory_levels")
                output_counts["inventory_levels"] = int(inventory_summary.get("total_lots") or 0)
                output_metadata["low_stock_entries"] = inventory_summary.get("low_stock_entries", [])
        ...
        if not completed:
            missing_reason = {
                ...
                "field": "no field location or crop-calendar evidence matched the query scope",
                "genomics": "no trait-linked genomics evidence matched the query",
                "phenotyping": "no phenotypic observations matched the query scope",
                "seed_ops": "no seed inventory evidence matched the query scope",
