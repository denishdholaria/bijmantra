...
class BiologicalStatus(StrEnum):
    WILD = "100"
    WEEDY = "200"
    LANDRACE = "300"
    BREEDING_LINE = "400"
...
class GermplasmPassport:
    ...
    donor_institute: str = ""
    donor_accession: str = ""
    collection_site: CollectionSite | None = None
    pedigree: str = ""
    remarks: str = ""
    ...
    def to_mcpd(self) -> dict[str, Any]:
        ...
        return legacy_bijmantra_passport_to_mcpd(self)
...
class GermplasmPassportService:
    ...
    def __init__(self):
        self.passports_by_organization: dict[int, dict[str, GermplasmPassport]] = {}
    ...
    def register_accession(
        ...
        self._passports_for_organization(organization_id)[accession_id] = passport
        return passport
    ...
    def add_collection_site(
        ...
        passports = self._passports_for_organization(organization_id)
        if accession_id not in passports:
        ...
        passports[accession_id].collection_site = site
        return passports[accession_id]

    def get_passport(self, accession_id: str, organization_id: int = 0) -> dict[str, Any] | None:
        """Get passport data for an accession"""
        passports = self._passports_for_organization(organization_id)
        if accession_id not in passports:
            return None
        return passports[accession_id].to_dict()
    ...
    def list_passports(
        ...
        result = []
        for passport in self._passports_for_organization(organization_id).values():
    ...
    def export_mcpd(
        ...
        passports_by_id = self._passports_for_organization(organization_id)
        if accession_ids:
            passports = [passports_by_id[aid] for aid in accession_ids if aid in passports_by_id]
        else:
            passports = list(passports_by_id.values())

        return export_legacy_bijmantra_passports_to_mcpd(passports)
    ...
    def get_statistics(self, organization_id: int = 0) -> dict[str, Any]:
        ...
        return {
            ...
            "with_coordinates": sum(1 for p in passports.values()
                                   if p.collection_site and p.collection_site.latitude),
        }

    def _passports_for_organization(self, organization_id: int) -> dict[str, GermplasmPassport]:
        return self.passports_by_organization.setdefault(int(organization_id), {})


# Singleton
_passport_service: GermplasmPassportService | None = None
