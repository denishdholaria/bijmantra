...
from app.modules.bio_analytics.models import GEBVPrediction, GSModel, MarkerEffect
...
class GenomicPredictionService:
    ...
    async def train_rrblup_model(
        ...
        # LHS = Z'Z + I*lambda
        ZtZ = np.dot(Z.T, Z)
        identity_matrix = np.eye(n_markers)
        LHS = ZtZ + (identity_matrix * lambda_val)
    ...
    async def train_from_variant_set(
        ...
        def load_and_align():
            ...
            if 'variants/ID' in callset:
                ...
                marker_names = [f"{c}_{p}" for c, p in zip(chroms, positions, strict=False)]
    ...
    async def predict_new(
        ...
        return {
            "predictions": [
                ...
                for gid, val in zip(germplasm_ids, gebvs, strict=False)
            ]
        }
