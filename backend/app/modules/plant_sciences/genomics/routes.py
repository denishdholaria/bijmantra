...
from app.modules.genomics.services.genomics_data_service import genomics_data_service
...
async def calculate_grm(request: GRMRequest, background_tasks: BackgroundTasks):
    ...
    if request.method not in valid_methods:
        ...
        if request.method == "vanraden":
            request.method = "vanraden1"
        elif request.method not in valid_methods:
            raise HTTPException(
                status_code=400, detail=f"Invalid method. Choose from: {', '.join(valid_methods)}"
            )

    # Create a job to track status
    job_id = await job_service.create_job(
        job_type="grm",
        metadata={"marker_data_id": request.marker_data_id, "method": request.method},
    )
...
async def predict_gebv(request: GEBVRequest, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(
        _run_gebv_job, job_id, request.training_data_id, request.target_trait, request.heritability
    )
...
async def _run_grm_job(job_id: str, marker_data_id: str, method: str):
    ...
    try:
        ...
        await job_service.update_job(job_id, status=JobStatus.FAILED, error=str(e))
...
async def _run_gebv_job(job_id: str, training_data_id: str, trait: str, heritability: float):
    ...
    try:
        ...
        gebv_results = [
            ...
            for sid, bv in zip(sample_ids, breeding_values, strict=False)
        ]
