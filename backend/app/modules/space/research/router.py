...
from app.modules.environment.services.space_research_service import get_space_research_service
...
async def get_space_crops(
    ...
    crops = await service.get_space_crops(db, current_user.organization_id, status)
    return {
        "success": True,
        "data": crops,
        "count": len(crops),
...
async def get_space_crop(
    ...
    service = get_space_research_service()
    crop = await service.get_space_crop(db, current_user.organization_id, crop_id)
    if not crop:
...
async def get_crop_environment(
    crop_id: str,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    service = get_space_research_service()
    params = await service.get_controlled_environment_params(db, current_user.organization_id, crop_id)
    if "error" in params:
...
async def get_experiments(
    ...
    service = get_space_research_service()
    experiments = await service.get_experiments(db, current_user.organization_id, status)
    return {
        "success": True,
        "data": experiments,
...
async def get_experiment(
    ...
    service = get_space_research_service()
    exp = await service.get_experiment(db, current_user.organization_id, exp_id)
    if not exp:
