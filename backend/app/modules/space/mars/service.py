...
class MarsService:
    ...
    async def create_environment(
        db: AsyncSession, profile: MarsEnvironmentProfileCreate, organization_id: int
    ) -> MarsEnvironmentProfile:
        db_profile = MarsEnvironmentProfile(organization_id=organization_id, **profile.model_dump())
        db.add(db_profile)
        await db.commit()
    ...
    async def get_environment(
        db: AsyncSession, environment_id: UUID, organization_id: int
    ) -> MarsEnvironmentProfile:
        result = await db.execute(
            select(MarsEnvironmentProfile).filter(
    ...
    async def simulate_trial(
        db: AsyncSession, request: SimulationRequest, organization_id: int
    ) -> MarsTrial:
        # 1. Fetch Environment
        profile = await MarsService.get_environment(
            db, request.environment_profile_id, organization_id
        )
    ...
    async def get_trial(db: AsyncSession, trial_id: UUID, organization_id: int) -> MarsTrial:
        stmt = (
            select(MarsTrial)
            .options(selectinload(MarsTrial.closed_loop_metrics))
            .filter(MarsTrial.id == trial_id, MarsTrial.organization_id == organization_id)
        )
