...
class LunarService:
    ...
    async def create_environment_profile(
        self, profile: LunarEnvironmentProfileCreate, organization_id: int
    ) -> LunarEnvironmentProfile:
        db_profile = LunarEnvironmentProfile(
            **profile.model_dump(), organization_id=organization_id
        )
    ...
    async def get_environment_profile(
        self, profile_id: UUID, organization_id: int
    ) -> LunarEnvironmentProfile | None:
    ...
    async def create_trial(self, trial: LunarTrialCreate, organization_id: int) -> LunarTrial:
        ...
        db_trial = LunarTrial(**trial.model_dump(), organization_id=organization_id)
    ...
    async def get_trial(self, trial_id: UUID, organization_id: int) -> LunarTrial | None:
        query = select(LunarTrial).where(
            LunarTrial.id == trial_id, LunarTrial.organization_id == organization_id
        )
        result = await self.db.execute(query)
        return result.scalar_one_or_none()

    async def simulate(
        self, request: LunarSimulationRequest, organization_id: int
    ) -> LunarSimulationResponse:
        # Fetch environment
