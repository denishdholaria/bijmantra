...
class QTLMappingService:
    ...
    @staticmethod
    def _is_missing_table_error(error: ProgrammingError, table_names: tuple[str, ...]) -> bool:
        message = str(getattr(error, 'orig', error))
        return 'UndefinedTableError' in message and any(table_name in message for table_name in table_names)
    ...
    async def list_qtls(
        ...
        try:
            ...
            if not self._is_missing_table_error(error, ('bio_qtls', 'bio_candidate_genes')):
                raise
            await db.rollback()
            return []
    ...
    async def get_gwas_results(
        ...
        try:
            ...
            if not self._is_missing_table_error(error, ('bio_gwas_results', 'bio_gwas_runs')):
                raise
            await db.rollback()
            return []
    ...
    async def get_candidate_genes(
        ...
        try:
            ...
            if not self._is_missing_table_error(error, ('bio_candidate_genes', 'bio_qtls')):
                raise
            await db.rollback()
            return []
    ...
    async def get_go_enrichment(
        ...
        try:
            ...
            if not self._is_missing_table_error(error, ('genes', 'go_terms', 'gene_go_terms', 'bio_candidate_genes', 'bio_qtls')):
                raise
            await db.rollback()
            return []
        ...
        try:
            ...
            if not self._is_missing_table_error(error, ('genes',)):
                raise
            await db.rollback()
            return []
        ...
        for go_id, k in candidate_go_counts.items():
            ...
            try:
                ...
                if not self._is_missing_table_error(error, ('genes', 'go_terms', 'gene_go_terms')):
                    raise
                await db.rollback()
                return []
    ...
    async def get_qtl_summary(
        ...
        traits = list({q.get("trait") for q in qtls if q.get("trait")})
        chromosomes = list({q.get("chromosome") for q in qtls if q.get("chromosome")})
        lod_values = [q.get("lod") for q in qtls if q.get("lod") is not None]
        total_pve = sum(q.get("pve") for q in qtls if q.get("pve") is not None)
        avg_lod = sum(lod_values) / len(lod_values) if lod_values else 0
        ...
        return {
            ...
            "major_qtls": len([q for q in qtls if q.get("pve") is not None and q["pve"] >= 15]),
            "minor_qtls": len([q for q in qtls if q.get("pve") is not None and q["pve"] < 15]),
        }
        ...
        candidate_genes = [
            {
                ...
                "go_terms": gene.go_terms or [],
            }
            for gene in (qtl.candidate_genes or [])
        ] or (qtl.candidate_genes_json or [])
