...
class DoubledHaploidService:
    ...
    async def _generate_protocol_code(self, db: AsyncSession, organization_id: int) -> str:
    ...
    async def _generate_batch_code(self, db: AsyncSession, organization_id: int) -> str:
        """Generate unique batch code."""
        stmt = select(func.count(DHBatch.id)).where(DHBatch.organization_id == organization_id)
        result = await db.execute(stmt)
        count = result.scalar() or 0
    ...
    async def get_protocols(
        ...
        stmt = select(DHProtocol).where(DHProtocol.organization_id == organization_id)
    ...
    async def get_protocol(
        ...
        stmt = select(DHProtocol).where(
            and_(DHProtocol.organization_id == organization_id, DHProtocol.id == protocol_id)
        )
        result = await db.execute(stmt)
    ...
    async def get_batches(
        ...
        stmt = (
            select(DHBatch)
            .options(selectinload(DHBatch.protocol))
            .where(DHBatch.organization_id == organization_id)
        )
    ...
    async def get_batch(
        ...
        stmt = (
            select(DHBatch)
            .options(selectinload(DHBatch.protocol))
            .where(and_(DHBatch.organization_id == organization_id, DHBatch.id == batch_id))
        )
        result = await db.execute(stmt)
    ...
    async def update_batch_progress(
        ...
        **updates,
        ...
        stmt = select(DHBatch).where(
            and_(DHBatch.organization_id == organization_id, DHBatch.id == batch_id)
        )
        result = await db.execute(stmt)
        batch = result.scalar_one_or_none()
    ...
    async def get_statistics(
        ...
        active_stmt = select(func.count(DHBatch.id)).where(
            and_(DHBatch.organization_id == organization_id, DHBatch.status == "active")
        )
        active_result = await db.execute(active_stmt)
        active_batches = active_result.scalar() or 0

        completed_stmt = select(func.count(DHBatch.id)).where(
            and_(DHBatch.organization_id == organization_id, DHBatch.status == "completed")
        )
        ...
        crops_stmt = (
            select(DHProtocol.crop).where(DHProtocol.organization_id == organization_id).distinct()
        )
        crops_result = await db.execute(crops_stmt)
        crops = [row[0] for row in crops_result.all()]

        methods_stmt = (
            select(DHProtocol.method)
            .where(DHProtocol.organization_id == organization_id)
            .distinct()
        )
