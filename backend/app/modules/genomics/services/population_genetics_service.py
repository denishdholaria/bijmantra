...
class PopulationGeneticsService:
    ...
    async def list_populations(
        ...
        stmt = (
            ...
            .group_by(Germplasm.common_crop_name, Germplasm.country_of_origin_code)
        )
    ...
    async def get_structure_analysis(
        ...
        stmt = select(func.count(CallSet.id)).where(CallSet.organization_id == organization_id)
        result = await db.execute(stmt)
                ...
                "fst_ranges": [
                    {
                        "range": "0-0.05",
                        "level": "Little",
                        "description": "Little genetic differentiation",
                    },
                    {
                        "range": "0.05-0.15",
                        "level": "Moderate",
                        "description": "Moderate differentiation",
                    },
                    {
                        "range": "0.15-0.25",
                        "level": "Great",
                        "description": "Great differentiation",
                    },
                    {
                        "range": ">0.25",
                        "level": "Very great",
                        "description": "Very great differentiation",
                    },
                ],
            },
