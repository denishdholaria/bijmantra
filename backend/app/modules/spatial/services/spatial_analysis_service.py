...
class SpatialAnalysisService:
    ...
    def get_field(self, field_id: str) -> dict | None:
        """Get field details"""
    ...
    def moving_average_adjustment(
        ...
        # Create grid
        max(v[row_key] for v in values)
        max(v[col_key] for v in values)
    ...
    def nearest_neighbor_analysis(
        ...
        for i, p1 in enumerate(points):
            ...
            for j, p2 in enumerate(points):
                if i != j:
                    dist = math.sqrt((p1[x_key] - p2[x_key]) ** 2 + (p1[y_key] - p2[y_key]) ** 2)
    ...
    def row_column_trend(
        ...
        return {
            ...
            "row_trend_direction": "increasing"
            if row_trend > 0
            else "decreasing"
            if row_trend < 0
            else "flat",
            "column_trend_direction": "increasing"
            if col_trend > 0
            else "decreasing"
            if col_trend < 0
            else "flat",
