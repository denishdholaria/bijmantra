...
from app.modules.environment.services.environmental_physics_service import environmental_service
from app.modules.core.services.math_utils_service import clamp, safe_divide
...
# B019 Fix: Move lru_cache outside class method
@lru_cache(maxsize=512)
def _calculate_potential_yield(
    crop: str,
    # ARG002 Fix: Removed unused location_id
    gdd: float,
    ptu: float,
    water: float,
    nitrogen: float,
    ...
    # Resolve model directly here as we moved out of class
    model = POTENTIAL_YIELD_MODELS.get(crop.lower())
    if not model:
        # Fallback or raise? Original raised.
        raise ValueError(
...
class YieldGapService:
    ...
    async def calculate_potential_yield(
        ...
        # Call standalone cached function
        return _calculate_potential_yield(
            crop=crop,
    ...
    async def fetch_actual_yields(
        ...
        try:
            async with BrAPIAdapter(
                IntegrationConfig(base_url="http://localhost:8000")
            ) as adapter:
                trials = await adapter.get_trials(page=0, page_size=50)
                _ = [
                    t
                    for t in trials
                    if str(t.get("locationDbId", "")) == str(location_id)
                ]
        except Exception:
            logger.debug(
