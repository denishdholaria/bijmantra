...
class PerformanceRankingService:
    ...
    async def get_rankings(
        ...
        for unit in units:
            ...
            if search:
                ...
                if not (
                    search_lower in name.lower() or any(search_lower in t.lower() for t in traits)
                ):
                    continue
            ...
            entry = {
                ...
                "program_id": str(unit.study.program_id)
                if unit.study and unit.study.program_id
                else None,
                "program_name": None,  # Would need join to Program
                "trial_id": str(unit.study.trial_id)
                if unit.study and unit.study.trial_id
                else None,
                "trial_name": None,  # Would need join to Trial
                "generation": unit.germplasm.additional_info.get("generation")
                if unit.germplasm.additional_info
                else None,
    ...
    async def get_programs(
        ...
        stmt = select(Program).where(Program.organization_id == organization_id)

        result = await db.execute(stmt)
        programs = result.scalars().all()

        return [{"id": str(p.id), "name": p.program_name or str(p.id)} for p in programs]
    ...
    async def get_trials(
        ...
        stmt = select(Trial).where(Trial.organization_id == organization_id)
