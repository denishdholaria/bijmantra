...
class QuickEntryService:
    ...
    async def get_entries(
        ...
        """
        async def fetch_germplasm():
            async with AsyncSessionLocal() as session:
                # Set RLS context for the new session
                try:
                    await session.execute(text(f"SET LOCAL app.current_organization_id = '{organization_id}'"))
                except Exception:
                    # Ignore if not supported (e.g. SQLite)
                    pass
        ...
        async def fetch_observations():
            async with AsyncSessionLocal() as session:
                try:
                    await session.execute(text(f"SET LOCAL app.current_organization_id = '{int(organization_id)}'"))
                except Exception as _rls_exc:
                    # RLS context must be set; if it fails the query must not run
                    # without tenant isolation. Log and re-raise.
                    logger.error(
                        "[QuickEntry] Failed to set RLS context for observations org=%s: %s",
                        organization_id, _rls_exc,
                    )
                    raise
        ...
        async def fetch_crosses():
            async with AsyncSessionLocal() as session:
                try:
                    await session.execute(text(f"SET LOCAL app.current_organization_id = '{int(organization_id)}'"))
                except Exception as _rls_exc:
                    logger.error(
                        "[QuickEntry] Failed to set RLS context for crosses org=%s: %s",
                        organization_id, _rls_exc,
                    )
                    raise
        ...
        if entry_type is None:
            ...
            if entry_type == "germplasm":
                entries.extend(await fetch_germplasm())
            elif entry_type == "observation":
                entries.extend(await fetch_observations())
            elif entry_type == "cross":
                entries.extend(await fetch_crosses())
