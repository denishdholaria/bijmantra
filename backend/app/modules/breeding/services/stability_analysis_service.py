...
from app.models.biometrics import StabilityResult
...
class StabilityAnalysisService:
    ...
    async def get_varieties(
        ...
        # Check for existing stability results
        stmt_results = select(StabilityResult).where(
            StabilityResult.organization_id == organization_id,
            StabilityResult.germplasm_id.in_([g.id for g in germplasm_list]),
        )
        results = await db.execute(stmt_results)
        stability_map = {r.germplasm_id: r for r in results.scalars().all()}

        output = []
        for g in germplasm_list:
            res = stability_map.get(g.id)
            output.append(
                {
                    "id": str(g.id),
                    "name": g.germplasm_name,
                    "mean_yield": res.mean_yield if res else None,
                    "slope": res.slope if res else None,
                    "deviation": res.deviation if res else None,
                    "ranking": res.ranking if res else None,
                    "classification": res.classification if res else None,
                    "years_tested": res.years_tested if res else 0,
                }
            )
    ...
    async def analyze(
        ...
        # 1. Try to fetch existing results first
        # ... (In a real scenario, we might re-run analysis if requested,
        # but for now we fallback to the empty calculator if no results exist)
    ...
    async def get_recommendations(
        ...
        return {
            "wide_adaptation": {
                ...
                "criteria": "bi ≈ 1, low S²di, low σ²i",
                "varieties": [
                    {"id": v["id"], "name": v["name"], "mean_yield": v.get("mean_yield")}
                    for v in wide
                ],
            },
            "favorable_environments": {
    ...
    async def get_statistics(
        ...
        return {
            ...
            "favorable_adaptation": len(
                [v for v in varieties if v.get("recommendation") == "favorable"]
            ),
            "unfavorable_adaptation": len(
                [v for v in varieties if v.get("recommendation") == "unfavorable"]
            ),
            ...
            "environments_tested": max(
                (v.get("environments_tested", 0) for v in varieties), default=0
            ),
