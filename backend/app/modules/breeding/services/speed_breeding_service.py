...
class SpeedBreedingService:
    ...
    # Verified implementation: Queries database for protocols
    async def get_protocols(
        ...
        if crop:
            query = query.where(func.lower(SpeedBreedingProtocol.crop) == crop.lower())
        if status:
            query = query.where(func.lower(SpeedBreedingProtocol.status) == status.lower())

        result = await db.execute(query)
        return result.scalars().all()

    async def create_protocol(
        self, db: AsyncSession, organization_id: int, protocol: SpeedBreedingProtocolCreate
    ) -> SpeedBreedingProtocol:
        """Create a new speed breeding protocol."""
        db_protocol = SpeedBreedingProtocol(
            **protocol.model_dump(), organization_id=organization_id
        )
        db.add(db_protocol)
        await db.commit()
    ...
    async def get_protocol(
        self, db: AsyncSession, organization_id: int, protocol_id: str
    ) -> SpeedBreedingProtocol | None:
    ...
    async def get_batches(
        ...
        query = (
            select(SpeedBreedingBatch)
            .options(
                selectinload(SpeedBreedingBatch.protocol), selectinload(SpeedBreedingBatch.chamber)
            )
            .where(SpeedBreedingBatch.organization_id == organization_id)
        )
        ...
        if chamber:
            batches = [
                b
                for b in batches
                if b.chamber and (b.chamber.name == chamber or str(b.chamber.id) == chamber)
            ]

        return batches

    async def get_batch(
        self, db: AsyncSession, organization_id: int, batch_id: str
    ) -> SpeedBreedingBatch | None:
        ...
        query = (
            ...
            .where(
                SpeedBreedingBatch.id == b_id, SpeedBreedingBatch.organization_id == organization_id
            )
        )
        result = await db.execute(query)
        return result.scalars().first()
    ...
    async def create_batch(
        self, db: AsyncSession, organization_id: int, batch: SpeedBreedingBatchCreate
    ) -> SpeedBreedingBatch:
        """Create a new batch."""
        db_batch = SpeedBreedingBatch(**batch.model_dump(), organization_id=organization_id)
        db.add(db_batch)
        await db.commit()
    ...
    async def create_chamber(
        self, db: AsyncSession, organization_id: int, chamber: SpeedBreedingChamberCreate
    ) -> SpeedBreedingChamber:
        db_chamber = SpeedBreedingChamber(**chamber.model_dump(), organization_id=organization_id)
        db.add(db_chamber)
        await db.commit()
        await db.refresh(db_chamber)
        return db_chamber

    async def get_statistics(self, db: AsyncSession, organization_id: int) -> dict[str, Any]:
        """Get speed breeding statistics from database."""

        # Total protocols
        count_query = (
            select(func.count())
            .select_from(SpeedBreedingProtocol)
            .where(SpeedBreedingProtocol.organization_id == organization_id)
        )
        ...
        # Chambers in use
        chamber_ids = {b.chamber_id for b in active_batches if b.chamber_id}
        chambers_in_use = len(chamber_ids)

        # Crops
        crops_query = (
            select(SpeedBreedingProtocol.crop)
            .where(SpeedBreedingProtocol.organization_id == organization_id)
            .distinct()
        )
    ...
    async def get_chamber_statistics(
        ...
        # Query chambers
        chambers_query = select(SpeedBreedingChamber).where(
            SpeedBreedingChamber.organization_id == organization_id
        )
        chambers_res = await db.execute(chambers_query)
        chambers = chambers_res.scalars().all()

        total_chambers = len(chambers)
        active_chambers = len([c for c in chambers if c.status == "active"])
        maintenance_chambers = len([c for c in chambers if c.status == "maintenance"])
        offline_chambers = len([c for c in chambers if c.status == "offline"])
        total_capacity = sum(c.capacity for c in chambers if c.capacity)

        # Query active batches for used capacity
        batches_query = select(SpeedBreedingBatch.entries).where(
            SpeedBreedingBatch.organization_id == organization_id,
            SpeedBreedingBatch.chamber_id.is_not(None),
            SpeedBreedingBatch.status.in_(["growing", "flowering", "harvesting"]),
        )
        batches_res = await db.execute(batches_query)
        entries = batches_res.scalars().all()
        used_capacity = sum(e for e in entries if e)
        ...
        return {
            ...
            "maintenance_chambers": maintenance_chambers,
            "offline_chambers": offline_chambers,
