...
class BreedingPipelineService:
    ...
    async def list_entries(
        ...
        query = select(Germplasm).where(
            ...
            func.json_extract(Germplasm.additional_info, '$.breeding_pipeline_stage') is not None
        )
    ...
    async def get_stage_summary(self) -> list[dict[str, Any]]:
        ...
        # 1. Get counts for all stages
        count_stmt = select(
            func.json_extract(Germplasm.additional_info, '$.breeding_pipeline_stage').label('stage'),
            func.count(Germplasm.id).label('count')
        ).where(
            Germplasm.organization_id == self.organization_id,
            func.json_extract(Germplasm.additional_info, '$.breeding_pipeline_stage').in_([s["id"] for s in PIPELINE_STAGES]),
            func.json_extract(Germplasm.additional_info, '$.breeding_pipeline_status') == "active"
        ).group_by(
            func.json_extract(Germplasm.additional_info, '$.breeding_pipeline_stage')
        )

        # 2. Get top 5 entries for each stage using window function
        stage_col = func.json_extract(Germplasm.additional_info, '$.breeding_pipeline_stage').label('stage')
        ...
        subquery = select(
            ...
            func.row_number().over(
                partition_by=func.json_extract(Germplasm.additional_info, '$.breeding_pipeline_stage'),
                order_by=desc(Germplasm.id)
            ).label('rn')
        ).where(
            Germplasm.organization_id == self.organization_id,
            func.json_extract(Germplasm.additional_info, '$.breeding_pipeline_stage').in_([s["id"] for s in PIPELINE_STAGES]),
        ...
        entries_stmt = select(
            subquery.c.germplasm_db_id,
            subquery.c.germplasm_name,
            subquery.c.common_crop_name,
            subquery.c.stage
        ).where(subquery.c.rn <= 5)

        # Execute queries
        count_result = await self.db.execute(count_stmt)
        entries_result = await self.db.execute(entries_stmt)

        counts = {row.stage: row.count for row in count_result.all()}

        entries_by_stage = {}
        for row in entries_result.all():
            if row.stage not in entries_by_stage:
                entries_by_stage[row.stage] = []
            entries_by_stage[row.stage].append({
                "id": row.germplasm_db_id,
                "name": row.germplasm_name,
        ...
        for stage in PIPELINE_STAGES:
            ...
            summary.append({
                ...
                "entries": entries_by_stage.get(stage_id, []),
            })
    ...
    async def advance_stage(self, entry_id: str, decision: str, notes: str = "") -> dict[str, Any] | None:
