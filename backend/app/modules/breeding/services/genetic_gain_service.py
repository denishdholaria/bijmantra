...
class GeneticGainService:
    ...
    async def get_program_summary(
        ...
        stmt = select(Program).where(
            Program.organization_id == organization_id, Program.program_db_id == program_id
        )
        result = await db.execute(stmt)
        ...
        stmt_trials = select(func.count(Trial.id)).where(
            Trial.organization_id == organization_id, Trial.program_id == program.id
        )
    ...
    async def get_statistics(
        ...
        stmt_trials = select(func.count(Trial.id)).where(Trial.organization_id == organization_id)
        result_trials = await db.execute(stmt_trials)
        trial_count = result_trials.scalar() or 0
