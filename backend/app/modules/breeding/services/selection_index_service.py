...
class SelectionIndexService:
    ...
    def tandem_selection(
        ...
        for i, trait in enumerate(trait_sequence):
            ...
            # Sort by trait value
            sorted_pop = sorted(
                current_population, key=lambda x: x.get(trait, float("-inf")), reverse=True
            )
    ...
    def predict_response(
        ...
        Phenotypic: R = i * h² * σp
        Genomic: R = i * r * σa, where σa = h * σp -> R = i * r * h * σp
        """
        if accuracy is not None:
            # Genomic Selection
            h = math.sqrt(heritability)
            response_per_cycle = selection_intensity * accuracy * h * phenotypic_std
            formula = "R = i × r × h × σp"
        else:
            # Phenotypic Selection
            response_per_cycle = selection_intensity * heritability * phenotypic_std
            formula = "R = i × h² × σp"

        annual_response = response_per_cycle / generation_interval if generation_interval > 0 else 0
        ...
        return {
            ...
            "accuracy": accuracy,
            "response_per_cycle": round(response_per_cycle, 4),
            "annual_response": round(annual_response, 4),
            "predicted_response": round(response_per_cycle, 4),  # Backward compatibility
            "formula": formula,
        }
