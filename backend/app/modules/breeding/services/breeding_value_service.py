...
class BreedingValueService:
    ...
    def estimate_blup(
        ...
        pedigree: list[dict[str, str]] | None = None,
        ...
        breeding_values = []
        for i, (ind_id, value) in enumerate(zip(ids, values, strict=False)):
        ...
        for i in range(n):
            for j in range(i, n):
                ...
                if geno_i and geno_j and len(geno_i) == len(geno_j):
                    matches = sum(1 for a, b in zip(geno_i, geno_j, strict=False) if a == b)
    ...
    def calculate_accuracy(
        ...
        numerator = sum((p - mean_pred) * (o - mean_obs) for p, o in zip(predicted, observed, strict=False))
        ...
        # Mean squared error
        mse = sum((p - o) ** 2 for p, o in zip(predicted, observed, strict=False)) / n
        rmse = math.sqrt(mse)

        # Bias
        bias = sum(p - o for p, o in zip(predicted, observed, strict=False)) / n
    ...
    async def get_breeding_values_from_db(
        ...
        # Factors
        genotypes = sorted({r.germplasm_name for r in rows})
        environments = sorted({r.study_name for r in rows})
        ...
        # 7. Format Results
        results = []
        np.mean(fixed) # Rough approximation of intercept
