...
class TrialNetworkService:
    ...
    def _extract_coordinates(self, location) -> dict[str, float] | None:
    ...
    def _map_site_to_dict(self, loc: Any) -> dict[str, Any]:
        """Maps a Location model to a dictionary."""
        return {
            "id": str(loc.id),
            "name": loc.location_name,
            "location": loc.institute_address or loc.location_name,
    ...
    async def get_site(
        ...
        return self._map_site_to_dict(loc)
    ...
    async def get_site_comparison(
        ...
        site_db_ids = []
        valid_site_ids_order = []
        for sid in site_ids:
            try:
                site_db_ids.append(int(sid))
                valid_site_ids_order.append(str(sid))
        ...
        stmt = (
            select(Location)
            .where(Location.organization_id == organization_id)
            .where(Location.id.in_(site_db_ids))
            .options(selectinload(Location.trials))
        )
        ...
        loc_map = {str(loc.id): loc for loc in locations}

        sites = []
        for sid in valid_site_ids_order:
            if sid in loc_map:
                sites.append(self._map_site_to_dict(loc_map[sid]))
