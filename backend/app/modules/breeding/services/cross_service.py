...
from app.schemas.germplasm import Cross as CrossSchema
from app.schemas.germplasm import CrossCreate, CrossStats, CrossUpdate
    ...
    async def create_cross(
        ...
        if cross_data.crossingProjectDbId:
            stmt = select(CrossingProject).where(
                ...
                CrossingProject.organization_id == organization_id,
            )
            result = await db.execute(stmt)
            project = result.scalar_one_or_none()
            if not project:
                raise ValueError("Crossing project not found")
        ...
        if cross_data.parent1DbId:
            stmt = select(Germplasm).where(
                Germplasm.germplasm_db_id == cross_data.parent1DbId,
                Germplasm.organization_id == organization_id,
            )
            result = await db.execute(stmt)
            p1 = result.scalar_one_or_none()
            if not p1:
                raise ValueError("Parent 1 germplasm not found")
            parent1_id = p1.id
        if cross_data.parent2DbId:
            stmt = select(Germplasm).where(
                Germplasm.germplasm_db_id == cross_data.parent2DbId,
                Germplasm.organization_id == organization_id,
            )
            result = await db.execute(stmt)
            p2 = result.scalar_one_or_none()
            if not p2:
                raise ValueError("Parent 2 germplasm not found")
            parent2_id = p2.id
