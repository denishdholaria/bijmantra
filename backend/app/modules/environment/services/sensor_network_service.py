...
from app.models.iot import IoTAlertEvent, IoTAlertRule, IoTDevice, IoTSensor, IoTTelemetry
...
class SensorNetworkService:
    ...
    async def get_device(self, db: AsyncSession, device_id: str) -> dict | None:
        """Get device by ID."""
    ...
    async def get_latest_readings(self, db: AsyncSession, device_id: str) -> dict[str, dict]:
        ...
        # Optimized: Fetch latest telemetry for all sensors in a single query
        # using DISTINCT ON (sensor_id) which is efficient in Postgres
        t_query = select(IoTTelemetry).where(
            IoTTelemetry.device_id == device.id
        ).distinct(IoTTelemetry.sensor_id).order_by(IoTTelemetry.sensor_id, desc(IoTTelemetry.timestamp))

        t_result = await db.execute(t_query)
        telemetry_records = t_result.scalars().all()

        # Map telemetry by sensor_id for quick lookup
        telemetry_map = {t.sensor_id: t for t in telemetry_records}

        latest = {}
        for s in sensors:
            t = telemetry_map.get(s.id)
            if t:
    ...
    async def list_alert_rules(self, db: AsyncSession, organization_id: int | None = None, enabled_only: bool = False) -> list[dict]:
        ...
        if enabled_only:
            conditions.append(IoTAlertRule.enabled)
    ...
    async def update_alert_rule(self, db: AsyncSession, rule_id: str, updates: dict) -> dict | None:
        ...
        if "notify_sms" in updates:
            rule.notify_sms = updates["notify_sms"]
    ...
    async def _check_alerts(self, db: AsyncSession, device: IoTDevice, sensor: IoTSensor, value: float):
        ...
        for rule in rules:
            ...
            triggered = False
            if rule.condition == "above" and value > rule.threshold or rule.condition == "below" and value < rule.threshold or rule.condition == "equals" and abs(value - rule.threshold) < 0.01:
                triggered = True
    ...
    async def acknowledge_alert(self, db: AsyncSession, event_id: str, user: str) -> dict | None:
    ...
    # Statistics
    async def get_network_stats(self, db: AsyncSession, organization_id: int | None = None) -> dict:
        ...
        alert_query = select(func.count(IoTAlertEvent.id)).join(IoTAlertRule).where(not IoTAlertEvent.acknowledged)
        if organization_id:
