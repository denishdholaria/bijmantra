...
class WeatherForecastUnavailableError(RuntimeError):
    """Raised when grounded weather evidence cannot be retrieved."""
...
class WeatherService:
    ...
    async def get_forecast(
        ...
        if lat is not None and lon is not None:
            try:
                ...
                logger.warning(
                    "Weather API failed for %s (%s); falling back to generated forecast.",
                    location_name,
                    location_id,
                    exc_info=exc,
                )
            ...
            if not allow_generated_fallback:
                raise WeatherForecastUnavailableError(
                    "location coordinates are required for live weather forecast"
                )
            daily_forecast = self._generate_mock_forecast(location_id, location_name, days)
    ...
    def _map_api_to_weather_data(self, api_data: dict[str, Any], location_id: str, location_name: str) -> list[WeatherData]:
        ...
        for i, time_str in enumerate(times):
            ...
            # E701 Fix: Split into multiple lines
            if precip > 20:
