...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, enabled: bool = True):
        ...
        self._prahari_logged = False
        self._rakshaka_logged = False
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            from app.modules.core.services.prahari import threat_responder
            ...
            if not self._prahari_logged:
                logger.warning("PRAHARI security module not available. Security checks skipped.")
                self._prahari_logged = True
        ...
        try:
            from app.modules.core.services.rakshaka import health_monitor
            ...
            if not self._rakshaka_logged:
                logger.warning("RAKSHAKA health module not available. Health recording skipped.")
                self._rakshaka_logged = True
        except Exception as e:
    ...
    async def _observe_request(
        ...
        try:
            from app.modules.core.services.prahari import security_observer
            ...
            if event and event.severity.value in ['medium', 'high', 'critical']:
                from app.modules.core.services.prahari import threat_analyzer, threat_responder
            ...
            if not self._prahari_logged:
                logger.warning("PRAHARI security module not available. Security observation skipped.")
                self._prahari_logged = True
        except Exception as e:
            logger.error(f"Security observation error: {e}")

    def _get_user_id(self, request: Request) -> str | None:
        """Extract user ID from request state (set by TenantContextMiddleware).

        TenantContextMiddleware runs before SecurityMiddleware and fully verifies
        the JWT signature before writing to request.state. Reading from state here
        avoids re-decoding the token and eliminates the previous pattern of decoding
