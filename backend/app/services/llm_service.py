...
Remember: A breeder making decisions based on your advice could affect crop yields for thousands of farmers. Truth matters more than comfort."""
    ...
    PROVIDER_DISPLAY_NAMES = {
        LLMProvider.OLLAMA: "Ollama (Local LLM)",
    ...
    def _initialize_providers(self):
        ...
        self.providers[LLMProvider.GOOGLE] = LLMConfig(
            ...
            model=os.getenv("GOOGLE_MODEL", "gemini-2.0-flash"),  # Updated Dec 2025
            api_key=google_key,
    ...
    async def check_ollama_available(self) -> bool:
        """Check if Ollama is running and has models - updates config with ACTUAL available model"""
        try:
            ...
            if response.status_code == 200:
                ...
                if models:
                    ...
                    # IMPORTANT: Use the ACTUAL first available model from Ollama
                    # This ensures we report truthfully what model is available
                    preferred_model = os.getenv("OLLAMA_MODEL", "llama3.2:3b")
                    if preferred_model in models:
                        config.model = preferred_model
                    elif any(preferred_model in m for m in models):
                        # Partial match (e.g., "llama3.2" matches "llama3.2:3b")
                        config.model = next(m for m in models if preferred_model in m)
                    else:
                        # Use first available model - this is the ACTUAL model
                        config.model = models[0]
                    logger.info(f"[Veena] Ollama available. Using model: {config.model}. All models: {models}")
                    return True
    ...
    async def _call_ollama(
        ...
        """Call Ollama API - returns actual model from response"""
        try:
            client = await self._get_client()
            ...
            if response.status_code == 200:
                data = response.json()
                # Ollama returns the actual model used in response
        ...
        for config in available:
            ...
            try:
                if config.provider == LLMProvider.OLLAMA:
                    result = await self._call_ollama(messages, config)
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "model_source": "detected" if active_config and active_config.provider == LLMProvider.OLLAMA else "configured",
