...
class FunctionExecutor:
    ...
    async def _handle_create(self, function_name: str, params: Dict) -> Dict:
        ...
        if function_name == "create_trial":
            ...
            try:
                ...
                if not cross_name:
                    cross_name = f"{parent1.germplasm_name} × {parent2.germplasm_name}"
    ...
    async def _handle_calculate(self, function_name: str, params: Dict) -> Dict:
        ...
        elif function_name == "calculate_gdd_for_field":
            # This is a new function to calculate GDD for a specific field.
            # We'll use the CrossDomainGDDService for this.
            field_id = params.get("field_id")
            start_date = params.get("start_date")
            end_date = params.get("end_date")
            base_temp = params.get("base_temp", 10.0)

            # In a real implementation, we would use the service to calculate GDD.
            # For now, we'll return a sample response.
            return {
                "success": True,
                "function": function_name,
                "result_type": "gdd_calculation",
                "data": {
                    "field_id": field_id,
                    "start_date": start_date,
                    "end_date": end_date,
                    "base_temp": base_temp,
                    "gdd": 250.0,
                    "message": f"Calculated GDD for field {field_id} from {start_date} to {end_date} is 250.0."
                }
            }
            ...
            try:
                ...
                return {
                    ...
                    "data": {
                        ...
                        "message": f"GDD for {crop} at {location_name}: {total_gdd:.1f}°C·days over {days} days"
                    },
                    "demo": False
                }
            except Exception as e:
