...
class ContextService:
    ...
    async def get_context_documents(
        ...
        docs = await breeding_service.search_breeding_knowledge(
            query=query,
            include_germplasm="germplasm" in (doc_types or []) or doc_types is None,
            include_trials="trial" in (doc_types or []) or doc_types is None,
            include_protocols="protocol" in (doc_types or []) or doc_types is None,
            limit=limit,
        )
        return docs
