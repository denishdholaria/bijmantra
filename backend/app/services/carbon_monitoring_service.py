...
from app.models.climate import CarbonMeasurementType, CarbonStock
...
class CarbonMonitoringService:
    ...
    async def get_carbon_stocks(
        ...
        measurement_type: CarbonMeasurementType | None = None,
        limit: int = 100,
        offset: int = 0
    ) -> list[CarbonStock]:
        """
    ...
    async def calculate_carbon_sequestration_rate(
        ...
        if len(stocks) < 2:
            logger.warning("Insufficient data for sequestration rate calculation")
            return None
