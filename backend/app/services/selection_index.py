...
class SelectionIndexService:
    ...
    def smith_hazel_index(
        ...
        index_coefficients = [
            h2 * w for h2, w in zip(heritabilities, economic_weights)
        ]
        ...
        return {
            ...
            "index_coefficients": dict(zip(trait_names, index_coefficients)),
            "results": results,
    ...
    def desired_gains_index(
        ...
        weights = []
        for i, (gain, h2) in enumerate(zip(desired_gains, heritabilities)):
            if h2 > 0:
        ...
        return {
            ...
            "n_individuals": n_individuals,
            "trait_names": trait_names,
            "desired_gains": dict(zip(trait_names, desired_gains)),
            "current_means": dict(zip(trait_names, current_means)),
            "calculated_weights": dict(zip(trait_names, normalized_weights)),
            "results": results,
            "top_10_percent": results[:max(1, n_individuals // 10)],
        }
    ...
    def base_index(
        ...
        return {
            ...
            "n_individuals": n_individuals,
            "trait_names": trait_names,
            "weights": dict(zip(trait_names, weights)),
            "results": results,
        }
    ...
    def independent_culling(
        ...
        return {
            ...
            "n_traits": len(trait_names),
            "n_individuals": n_individuals,
            "thresholds": dict(zip(trait_names, thresholds)),
            "threshold_types": dict(zip(trait_names, threshold_types)),
