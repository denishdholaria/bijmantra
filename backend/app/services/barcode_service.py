...
class BarcodeService:
    ...
    async def get_scan_history(
        ...
        Args:
            ...
            barcode_value: Optional filter by barcode value
        ...
        stmt = (
            select(BarcodeScan)
            .where(BarcodeScan.organization_id == organization_id)
            .order_by(BarcodeScan.scanned_at.desc())
            .limit(limit)
        )
        if barcode_value:
            stmt = stmt.where(BarcodeScan.barcode_value == barcode_value)
    ...
    async def get_statistics(
        ...
        total_scans = await db.scalar(
            select(func.count(BarcodeScan.id))
            .where(BarcodeScan.organization_id == organization_id)
        ) or 0

        successful_scans = await db.scalar(
            select(func.count(BarcodeScan.id))
            .where(BarcodeScan.organization_id == organization_id)
            .where(BarcodeScan.found == True)
        ) or 0
