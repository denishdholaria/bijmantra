...
class PhenotypeAnalysisService:
    ...
    def estimate_heritability_multi_env(
        ...
        """
        # 1. Extract Factors
        genotypes = list(set(d[genotype_col] for d in data))
        envs = list(set(d[env_col] for d in data))
        ...
        # Marginal means
        geno_means = {geno: 0.0 for geno in genotypes}
        geno_counts = {geno: 0 for geno in genotypes}

        env_means = {env: 0.0 for env in envs}
        env_counts = {env: 0 for env in envs}
        ...
        # Normalize means
        for k in geno_means: geno_means[k] /= geno_counts[k]
        for k in env_means: env_means[k] /= env_counts[k]
        for k in ge_means: ge_means[k] /= ge_counts[k]

        # 3. Sum of Squares (Type III / Balanced ANOVA Logic)

        # Correction Factor
        cf = (grand_mean * n_total) ** 2 / n_total # or just (sum Y)^2 / N
