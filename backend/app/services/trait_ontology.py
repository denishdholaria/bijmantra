...
class Scale:
    ...
    categories: list[dict[str, str]] | None = None  # For nominal/ordinal
...
class TraitOntologyService:
    ...
    def search_traits(self, query: str) -> list[dict[str, Any]]:
        ...
        for trait in self.traits.values():
            if query_lower in trait.name.lower() or trait.synonyms and any(query_lower in s.lower() for s in trait.synonyms):
...
# Singleton
_ontology_service: TraitOntologyService | None = None
