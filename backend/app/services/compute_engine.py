...
class ComputeEngine:
    ...
    def _check_fortran(self) -> bool:
        ...
        try:
            ...
            # from bijmantra_compute import fortran_ffi
            return False  # Disabled until library is built
        except ImportError:
            return False
    ...
    def _blup_fortran(self, *args) -> BLUPResult:
        """Fortran implementation via Rust FFI"""
        # TODO: Implement when Fortran library is built
        logger.warning("Fortran backend not available, falling back to NumPy")
        return self._blup_numpy(*args)
    
    def _gblup_fortran(self, *args) -> BLUPResult:
        """Fortran implementation via Rust FFI"""
        logger.warning("Fortran backend not available, falling back to NumPy")
        return self._gblup_numpy(*args)
    
    def _grm_fortran(self, genotypes: np.ndarray, method: str) -> np.ndarray:
        """Fortran implementation via Rust FFI"""
        logger.warning("Fortran backend not available, falling back to NumPy")
        return self._grm_numpy(genotypes, method)
    
    def _reml_fortran(self, *args) -> REMLResult:
        """Fortran implementation via Rust FFI"""
        logger.warning("Fortran backend not available, falling back to NumPy")
        return self._reml_numpy(*args)
