...
class SoilProfile:
    ...
    ec: float | None = None  # Electrical Conductivity
...
class FieldEnvironmentService:
    ...
    def get_soil_profiles(self, field_id: str | None = None) -> list[SoilProfile]:
    ...
    def get_soil_profile(self, profile_id: str) -> SoilProfile | None:
    ...
    # Input Log Methods
    def get_input_logs(self, field_id: str | None = None, input_type: InputType | None = None) -> list[InputLog]:
        """Get input logs with optional filters"""
        logs = list(self._input_logs.values())
        if field_id:
            logs = [log for log in logs if log.field_id == field_id]
        if input_type:
            logs = [log for log in logs if log.input_type == input_type]
        return sorted(logs, key=lambda log: log.application_date, reverse=True)
    ...
    # Irrigation Methods
    def get_irrigation_events(self, field_id: str | None = None) -> list[IrrigationEvent]:
