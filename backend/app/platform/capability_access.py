...
FastAPI route guards should use ``app.platform.capability_guards`` so HTTP
error shaping and persisted-install fallback stay centralized.
"""
...
def evaluate_capability_manifest_access(
    manifest: CapabilityManifest,
    context: CapabilityAccessContext,
) -> CapabilityAccessDecision:
    """Return whether a context satisfies a capability manifest's default grants."""
    ...
    missing_data_scopes = tuple(
        data_scope for data_scope in manifest.data_scopes if data_scope not in context.data_scopes
    )
    if missing_data_scopes:
        return CapabilityAccessDecision(
            allowed=False,
