...
async def list_capability_installations(
    db: AsyncSession,
    *,
    organization_id: int,
) -> list[OrganizationCapabilityInstallation]:
    """Return all capability installation rows for one organization."""

    result = await db.execute(
        select(OrganizationCapabilityInstallation)
        .where(OrganizationCapabilityInstallation.organization_id == organization_id)
        .order_by(OrganizationCapabilityInstallation.capability_id)
    )
    return list(result.scalars().all())
...
async def build_persisted_capability_access_context_if_present(
    db: AsyncSession,
    actor: Any,
    *,
    capability_id: str,
) -> CapabilityAccessContext | None:
    """Return persisted access context only when a capability row exists.

    This keeps existing organizations legacy-compatible until default
    capability bootstrap is in place, while making explicit persisted
    install/disable state authoritative as soon as a row exists.
    """
    ...
    if installation is None:
        return None
    return await build_persisted_capability_access_context(db, actor)
