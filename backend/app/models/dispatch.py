...
class Firm(BaseModel):
    ...
    )  # dealer, distributor, retailer, farmer, institution, government
    ...
    __table_args__ = (Index("ix_firms_city_state", "city", "state"),)
...
class Dispatch(BaseModel):
    ...
    transfer_type = Column(
        String(50), nullable=False, index=True
    )  # sale, internal, donation, sample, return
    ...
    __table_args__ = (Index("ix_dispatches_created_at", "created_at"),)
...
class DispatchItem(BaseModel):
    ...
    # Parent dispatch
    dispatch_id = Column(
        Integer, ForeignKey("dispatches.id", ondelete="CASCADE"), nullable=False, index=True
    )
