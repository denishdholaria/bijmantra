...
def naive_utc_now() -> datetime:
    """Return a naive UTC datetime for timestamp columns stored without timezone."""
...
class ValidationIssueStatus(enum.StrEnum):
...
class Backup(Base):
    ...
    # Timing
    started_at = Column(DateTime, default=naive_utc_now)
    completed_at = Column(DateTime)
    ...
    updated_at = Column(DateTime, default=naive_utc_now, onupdate=naive_utc_now)
...
class DiseaseRiskLevel(enum.StrEnum):
    """Disease risk levels"""
    LOW = "low"
    MEDIUM = "medium"
    HIGH = "high"
