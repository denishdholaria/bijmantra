...
class ActivityLog(BaseModel):
    ...
    # Relationships
    organization = relationship("Organization")
    # lazy="raise" enforces that callers must use selectinload(ActivityLog.user)
    # on any query that accesses this relationship. Prevents silent N+1 queries
    # on the activity feed endpoints.
    user = relationship("User", lazy="raise")


class UserDockPreference(BaseModel):
    """User dock preferences for Mahasarthi navigation system

    Stores pinned pages, recent pages, and dock display preferences.
    Enables cross-device sync and role-based defaults.

    """

    __tablename__ = "user_dock_preferences"

    organization_id = Column(Integer, ForeignKey("organizations.id"), nullable=False, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, unique=True, index=True)

    # Dock items stored as JSON arrays
    # Each item: {id, path, label, icon, isPinned, lastVisited?, visitCount?}
    pinned_items = Column(JSON, nullable=False, default=list)
    recent_items = Column(JSON, nullable=False, default=list)

    # Dock preferences
    max_pinned = Column(Integer, nullable=False, default=8)
    max_recent = Column(Integer, nullable=False, default=4)
