...
Implements: docs/gupt/SensorIOT/sensor-iot.md specification
"""
...
class IoTSensorType(BaseModel):
    """
    IoT Sensor Type - Catalog of available sensor types

    Defines metadata for sensor types like 'Temperature', 'Humidity', etc.
    """

    __tablename__ = "iot_sensor_types"
    ...
    # Metadata
    properties = Column(JSON)  # Additional properties like accuracy, precision, etc.

    # Relationships
    sensors = relationship("IoTSensor", back_populates="sensor_type_rel")

    def __repr__(self):
        return f"<IoTSensorType {self.code}: {self.name}>"
...
class IoTDevice(BaseModel):
    ...
    commands = relationship("IoTDeviceCommand", back_populates="device", cascade="all, delete-orphan")
    connectivity_logs = relationship("IoTConnectivityLog", back_populates="device", cascade="all, delete-orphan")
...
class IoTSensor(BaseModel):
    ...
    sensor_type_id = Column(Integer, ForeignKey("iot_sensor_types.id"), nullable=True, index=True) # Optional link to types
    ...
    sensor_type_rel = relationship("IoTSensorType", back_populates="sensors")
...
class IoTDeviceCommand(BaseModel):
    """
    IoT Device Command - Commands sent to devices
    """
    ...
    def __repr__(self):
        return f"<IoTDeviceCommand {self.command} -> {self.device_id}>"


class IoTConnectivityLog(BaseModel):
    """
    IoT Connectivity Log - History of device connectivity status
    """
    __tablename__ = "iot_connectivity_logs"
    ...
    # Relationships
    device = relationship("IoTDevice", back_populates="connectivity_logs")

    def __repr__(self):
        return f"<IoTConnectivityLog {self.device_id}: {self.status} at {self.timestamp}>"
