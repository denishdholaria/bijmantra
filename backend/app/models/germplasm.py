...
from sqlalchemy import JSON, BigInteger, Column, Date, Float, ForeignKey, Integer, String, Text
...
class Germplasm(BaseModel):
    ...
    # Lineage
    cross_id = Column(BigInteger, ForeignKey("crosses.id"), index=True)

    # Relationships
    organization = relationship("Organization")
    attributes = relationship(
        "GermplasmAttribute", back_populates="germplasm", cascade="all, delete-orphan"
    )
...
class GermplasmAttribute(BaseModel):
    ...
    organization_id = Column(BigInteger, ForeignKey("organizations.id"), nullable=False, index=True)
    germplasm_id = Column(BigInteger, ForeignKey("germplasm.id"), nullable=False, index=True)
...
class Cross(BaseModel):
    ...
    organization_id = Column(BigInteger, ForeignKey("organizations.id"), nullable=False, index=True)
    crossing_project_id = Column(BigInteger, ForeignKey("crossing_projects.id"), index=True)
    ...
    # Parents
    parent1_db_id = Column(BigInteger, ForeignKey("germplasm.id"))
    parent1_type = Column(String(20))  # FEMALE, MALE, SELF, POPULATION
    parent2_db_id = Column(BigInteger, ForeignKey("germplasm.id"))
...
class CrossingProject(BaseModel):
    ...
    organization_id = Column(BigInteger, ForeignKey("organizations.id"), nullable=False, index=True)
    program_id = Column(BigInteger, ForeignKey("programs.id"), index=True)
...
class Seedlot(BaseModel):
    ...
    organization_id = Column(BigInteger, ForeignKey("organizations.id"), nullable=False, index=True)
    germplasm_id = Column(BigInteger, ForeignKey("germplasm.id"), index=True)
    location_id = Column(BigInteger, ForeignKey("locations.id"), index=True)
    program_id = Column(BigInteger, ForeignKey("programs.id"), index=True)
    ...
    # Conservation Link
    accession_uuid = Column(
        UUID(as_uuid=True), ForeignKey("seed_bank_accessions.id"), nullable=True, index=True
    )
    ...
    transactions = relationship(
        "SeedlotTransaction", back_populates="seedlot", cascade="all, delete-orphan"
    )
...
class SeedlotTransaction(BaseModel):
    ...
    organization_id = Column(BigInteger, ForeignKey("organizations.id"), nullable=False, index=True)
    seedlot_id = Column(BigInteger, ForeignKey("seedlots.id"), nullable=False, index=True)
