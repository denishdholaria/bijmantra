...
class Organization(BaseModel):
    ...
    ai_daily_limit = Column(Integer, default=50, nullable=False)
...
class Program(BaseModel):
    ...
    lead_person_db_id = Column(BigInteger, ForeignKey("people.id"))

    # Research project flag for DevGuru integration
    is_research_project = Column(Boolean, default=False, nullable=False, index=True)
    research_context = Column(
        JSON
    )  # PhD metadata: student_name, supervisor, expected_end_date, etc.
...
class Location(BaseModel):
    ...
    # Environmental
    environmental_units = relationship("EnvironmentalUnit", back_populates="location")
    soil_profile = relationship("SoilProfile", uselist=False, back_populates="location")
...
class Trial(BaseModel):
    ...
    organization_id = Column(BigInteger, ForeignKey("organizations.id"), nullable=False, index=True)
    program_id = Column(BigInteger, ForeignKey("programs.id"), nullable=False, index=True)
    location_id = Column(BigInteger, ForeignKey("locations.id"), index=True)
    season_id = Column(BigInteger, ForeignKey("seasons.id"), index=True)
    ...
    season = relationship("Season", back_populates="trials")
...
class Study(BaseModel):
    ...
    organization_id = Column(BigInteger, ForeignKey("organizations.id"), nullable=False, index=True)
    trial_id = Column(BigInteger, ForeignKey("trials.id"), nullable=False, index=True)
    location_id = Column(BigInteger, ForeignKey("locations.id"), index=True)
...
class Season(BaseModel):
    ...
    # Relationships
    organization = relationship("Organization")
    trials = relationship("Trial", back_populates="season")
