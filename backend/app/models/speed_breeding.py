...
class SpeedBreedingProtocol(BaseModel):
    ...
    batches: Mapped[list["SpeedBreedingBatch"]] = relationship(
        "SpeedBreedingBatch", back_populates="protocol"
    )
...
class SpeedBreedingChamber(BaseModel):
    ...
    organization: Mapped["Organization"] = relationship("Organization")
    batches: Mapped[list["SpeedBreedingBatch"]] = relationship(
        "SpeedBreedingBatch", back_populates="chamber"
    )
...
class SpeedBreedingBatch(BaseModel):
    ...
    protocol_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("speed_breeding_protocols.id"), nullable=False
    )
    chamber_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("speed_breeding_chambers.id")
    )
    ...
    status: Mapped[str | None] = mapped_column(
        String(50), default="growing"
    )  # growing, flowering, harvesting, completed
    progress: Mapped[int | None] = mapped_column(Integer, default=0)  # 0-100

    # Relationships
    organization: Mapped["Organization"] = relationship("Organization")
    protocol: Mapped["SpeedBreedingProtocol"] = relationship(
        "SpeedBreedingProtocol", back_populates="batches"
    )
    chamber: Mapped[Optional["SpeedBreedingChamber"]] = relationship(
        "SpeedBreedingChamber", back_populates="batches"
    )
