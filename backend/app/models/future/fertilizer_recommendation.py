...
class FertilizerRecommendation(BaseModel):
    ...
    target_yield = Column(Float, nullable=False, comment="Target yield value")
    yield_unit = Column(
        String(20), nullable=False, default="t/ha", comment="Unit for yield (t/ha, kg/ha, bu/ac)"
    )

    # Macronutrient recommendations (kg/ha)
    n_kg_ha = Column(Float, nullable=False, default=0.0, comment="Nitrogen recommendation in kg/ha")
    p_kg_ha = Column(
        Float, nullable=False, default=0.0, comment="Phosphorus (P2O5) recommendation in kg/ha"
    )
    k_kg_ha = Column(
        Float, nullable=False, default=0.0, comment="Potassium (K2O) recommendation in kg/ha"
    )

    # Secondary and micronutrient recommendations (kg/ha)
    s_kg_ha = Column(Float, nullable=True, default=0.0, comment="Sulfur recommendation in kg/ha")
    zn_kg_ha = Column(Float, nullable=True, default=0.0, comment="Zinc recommendation in kg/ha")

    # Application timing (JSON for flexibility)
    application_timing = Column(
        JSONB, nullable=True, comment="Split application schedule with timing and amounts"
    )
    ...
    # Cost estimation
    estimated_cost = Column(Float, nullable=True, comment="Total estimated fertilizer cost")
    currency = Column(String(3), nullable=True, default="USD", comment="ISO 4217 currency code")

    # Validity period
    recommendation_date = Column(
        Date, nullable=False, default=date.today, comment="Date recommendation was generated"
    )
    valid_until = Column(Date, nullable=True, comment="Recommendation expiration date")

    # Additional notes
    notes = Column(Text, nullable=True, comment="Additional recommendations or caveats")
