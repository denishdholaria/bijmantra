...
class GrowingDegreeDayLog(BaseModel):
    ...
    # GDD values
    daily_gdd = Column(Float, nullable=False, comment="GDD accumulated on this day (always >= 0)")
    cumulative_gdd = Column(Float, nullable=False, comment="Total GDD since planting date")

    # Temperature data
    base_temperature = Column(
        Float, nullable=False, default=10.0, comment="Base temperature in Celsius (crop-specific)"
    )
    max_temperature = Column(Float, nullable=True, comment="Daily maximum temperature in Celsius")
    min_temperature = Column(Float, nullable=True, comment="Daily minimum temperature in Celsius")
