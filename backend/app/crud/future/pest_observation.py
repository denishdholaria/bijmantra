...
class CRUDPestObservation(CRUDBase[PestObservation, PestObservationCreate, PestObservationUpdate]):
    ...
    async def get_by_severity(
        ...
        *,
        ...
        query = select(self.model).where(
            self.model.organization_id == org_id,
            self.model.severity_score >= min_severity
        ).order_by(self.model.severity_score.desc())
