    ...
    try:
        async with async_session_maker() as db:
            ...
            stmt_reset = (
                update(AIUsageDaily)
                .where(
                    AIUsageDaily.organization_id == org.id, AIUsageDaily.usage_date == date.today()
                )
            ...
            for i in range(3):
                allowed = await AIQuotaService.check_and_increment_usage(db, org.id, increment=True)
            ...
            try:
                await AIQuotaService.check_and_increment_usage(db, org.id, increment=True)
