...
async def my_db_session():
    ...
    tables = [Base.metadata.tables["vision_models"], Base.metadata.tables["vision_deployments"]]
...
async def test_deploy_model(my_db_session):
    ...
    deployment = await vision_deployment_service.get_deployment(
        my_db_session, organization_id=1, deploy_id=str(deploy_id)
    )
    ...
    deployments = await vision_deployment_service.list_deployments(
        my_db_session, organization_id=1, model_id=str(model.id)
    )
