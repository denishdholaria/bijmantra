...
@pytest.mark.asyncio
async def test_get_protocols_filters_crop_and_status_case_insensitively(async_db_session):
    org = Organization(name="Case Org", id=2)
    async_db_session.add(org)
    async_db_session.add(
        SpeedBreedingProtocol(
            organization_id=2,
            name="Rice Rapid Advance",
            crop="Rice",
            status="Active",
        )
    )
    ...
    protocols = await speed_breeding_service.get_protocols(
        async_db_session,
        2,
        crop="rice",
        status="active",
    ...
    assert len(protocols) == 1
    assert protocols[0].name == "Rice Rapid Advance"
