...
async def async_db_engine():
    ...
    async with engine.begin() as conn:
        ...
        await conn.run_sync(
            Base.metadata.create_all, tables=[Organization.__table__, PrintJob.__table__]
        )
    ...
    async with engine.begin() as conn:
        await conn.run_sync(
            Base.metadata.drop_all, tables=[Organization.__table__, PrintJob.__table__]
        )
...
async def test_get_print_jobs(db, test_organization):
    ...
    assert (
        len(retrieved_job["items"]) == 0
    )  # Default empty list in create_print_job unless provided
    ...
    completed_jobs = await label_printing_service.get_print_jobs(
        db, test_organization.id, status=PrintJobStatus.COMPLETED
    )
    assert len(completed_jobs) >= 1
    assert job1["id"] in [j["id"] for j in completed_jobs]

    pending_jobs = await label_printing_service.get_print_jobs(
        db, test_organization.id, status=PrintJobStatus.PENDING
    )
