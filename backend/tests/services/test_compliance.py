...
from app.modules.core.services.compliance.rules import validate_batch
...
def test_rules_engine_germination_fallback(sample_batch):
    # Use a crop that is in ISTA_CROPS (so no "not covered" error)
    # but NOT in CROP_STANDARDS (so it uses the fallback logic)
    sample_batch.crop = "Barley"

    # Case 1: Germination below default standard (75.0%)
    sample_batch.germination_percentage = 70.0
    result = validate_batch(sample_batch, ComplianceType.ISTA)
    assert result["valid"] is False
    assert any("Germination 70.0% is below general standard 75.0%." in err for err in result["errors"])

    # Case 2: Germination above default standard
    sample_batch.germination_percentage = 76.0
    result = validate_batch(sample_batch, ComplianceType.ISTA)
    assert result["valid"] is True
