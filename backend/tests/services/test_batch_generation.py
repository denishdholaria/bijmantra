...
from app.services.compliance import batch
...
async def test_process_batch_generation_success(mock_output_dir, sample_batches):
    # Mock dependencies
    with (
        patch("app.services.compliance.batch.generate_certificate_pdf") as mock_gen_pdf,
        patch("app.services.compliance.batch.send_email_with_attachment") as mock_email,
    ):
