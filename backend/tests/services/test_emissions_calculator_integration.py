...
from app.modules.environment.services.emissions_calculator_service import EmissionsCalculatorService
...
async def test_calculate_variety_footprint_with_yield():
    ...
    emission_sources = [
        EmissionSource(
            id=1, organization_id=1, co2e_emissions=100.0, category=EmissionCategory.FERTILIZER
        ),
        EmissionSource(
            id=2, organization_id=1, co2e_emissions=50.0, category=EmissionCategory.FUEL
        ),
    ]
    ...
    footprint = await service.calculate_variety_footprint(
        db=db, organization_id=1, germplasm_id=101, trial_id=201, study_id=301, location_id=401
    )
    ...
    service.get_emission_sources.assert_called_once_with(db, 1, trial_id=201, study_id=301)
...
async def test_calculate_variety_footprint_no_yield():
    ...
    # Mock emissions
    service.get_emission_sources = AsyncMock(
        return_value=[EmissionSource(co2e_emissions=100.0, category=EmissionCategory.FERTILIZER)]
    )
