...
def test_domain_registry_is_open_not_a_fixed_taxonomy() -> None:
    ...
    assert "germplasm.accession_passport" in DOMAIN_CAPABILITY_CODE_ROOTS
    assert "field_operations.trial_analysis" in DOMAIN_CAPABILITY_CODE_ROOTS
...
def test_active_capability_manifests_have_canonical_backend_packs() -> None:
    for manifest in CAPABILITY_MANIFESTS.values():
        if manifest.lifecycle != "active":
            continue

        assert manifest.backend_code_root is not None
        capability_root = REPO_ROOT / manifest.backend_code_root
        capability_key = f"{manifest.owner_domain}.{capability_root.name}"
        capability_pack = yaml.safe_load((capability_root / "capability.yaml").read_text())

        assert capability_root.is_dir()
        assert capability_key in DOMAIN_CAPABILITY_CODE_ROOTS
        assert DOMAIN_CAPABILITY_CODE_ROOTS[capability_key].backend_root == manifest.backend_code_root
        assert capability_pack["id"] == manifest.id
...
def test_research_asset_core_is_the_fdca_reconciliation_capability() -> None:
    ...
    assert research_assets.lifecycle == "active"
    ...
    assert research_assets.audit_events == FEDERATED_AUDIT_ACTIONS
...
def test_seed_lot_traceability_manifest_owns_adjustment_write_candidate() -> None:
    manifest = resolve_capability_manifest("seedops_commercialization.seed_lot_traceability")
    assert manifest is not None

    assert manifest.dominion == "seedops_commercialization"
    assert manifest.owner_domain == "germplasm"
    assert manifest.lifecycle == "planned"
    assert "seedops.seed_lots.adjust" in manifest.required_permissions
    assert "organization" in manifest.data_scopes
    assert "lot" in manifest.data_scopes
    assert "seed_lot.adjusted" in manifest.audit_events
    assert "/api/v2/seed-inventory/*" in manifest.backend_routes
    assert "contracts/fastapi-parity/seedlot-inventory-adjustment-write-contract.json" in (
        manifest.source_context
    )
    assert "docs/rust-id-strategy.md" in manifest.source_context
    assert "crates/bijmantra-server/tests/rust_330_gates.rs" in manifest.tests

    candidate_capabilities = capabilities_for_backend_route("/api/v2/seed-inventory/adjustments")
    assert "seedops_commercialization.seed_lot_traceability" in {
        capability.id for capability in candidate_capabilities
    }
...
def test_research_asset_manifest_points_to_canonical_capability_pack() -> None:
    manifest = resolve_capability_manifest(
        "scientific_publishing_fair_exchange.research_asset_core"
    )

    assert manifest is not None
    assert manifest.backend_code_root == "backend/app/domains/knowledge/capabilities/research_asset_core"
    assert (REPO_ROOT / manifest.backend_code_root).is_dir()
    assert "tests/domains/knowledge/capabilities/research_asset_core" in manifest.tests


def test_germplasm_accession_passport_manifest_points_to_canonical_capability_pack() -> None:
    manifest = resolve_capability_manifest("germplasm_global_seed_registry.accession_passport")

    assert manifest is not None
    assert manifest.backend_code_root == "backend/app/domains/germplasm/capabilities/accession_passport"
    assert (REPO_ROOT / manifest.backend_code_root).is_dir()
    assert "BrAPI v2.1" in manifest.standards
    assert "MCPD" in manifest.standards


def test_trial_analysis_manifest_points_to_canonical_capability_pack() -> None:
    manifest = resolve_capability_manifest("plant_science_rnd.trial_analysis")

    assert manifest is not None
    assert manifest.backend_code_root == "backend/app/domains/field_operations/capabilities/trial_analysis"
    assert (REPO_ROOT / manifest.backend_code_root).is_dir()
    assert "BrAPI v2.1" in manifest.standards
    assert "MIAPPE" in manifest.standards
...
def test_backend_capability_pack_manifests_match_registered_capabilities() -> None:
    ...
    for manifest_path in manifest_paths:
        ...
        assert capability_manifest["lifecycle"] == registered.lifecycle
...
def test_research_asset_capability_manifest_matches_domain_audit_contract() -> None:
    manifest = resolve_capability_manifest(
        "scientific_publishing_fair_exchange.research_asset_core"
    )
    assert manifest is not None
    ...
    assert manifest.audit_events == FEDERATED_AUDIT_ACTIONS
    assert tuple(capability_manifest["audit_events"]) == FEDERATED_AUDIT_ACTIONS
...
def test_frontend_capability_seed_matches_knowledge_graph_manifest() -> None:
    ...
    for audit_event in manifest.audit_events:
        assert f"'{audit_event}'" in frontend_source


def test_frontend_capability_seed_matches_research_asset_manifest() -> None:
    manifest = resolve_capability_manifest(
        "scientific_publishing_fair_exchange.research_asset_core"
    )
    assert manifest is not None
    ...
    assert f"id: '{manifest.id}'" in frontend_source
    assert f"ownerDomain: '{manifest.owner_domain}'" in frontend_source
    assert f"lifecycle: '{manifest.lifecycle}'" in frontend_source
    for route in manifest.frontend_routes:
        assert f"'{route}'" in frontend_source
    for route in manifest.backend_routes:
        assert f"'{route}'" in frontend_source
...
def test_frontend_capability_seed_matches_accession_passport_manifest() -> None:
    manifest = resolve_capability_manifest("germplasm_global_seed_registry.accession_passport")
    assert manifest is not None
    frontend_source = (
        REPO_ROOT / "frontend/src/framework/registry/capabilities.ts"
    ...
    for permission in manifest.required_permissions:
        assert f"'{permission}'" in frontend_source
    for data_scope in manifest.data_scopes:
        assert f"'{data_scope}'" in frontend_source
    for audit_event in manifest.audit_events:
        assert f"'{audit_event}'" in frontend_source
