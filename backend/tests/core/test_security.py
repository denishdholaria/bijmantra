...
# This allows the unit tests to run even when full dependencies (like bcrypt or jwt)
...
def _setup_mocks():
    ...
    deps_to_mock = ["bcrypt", "jwt", "fastapi"]
    for dep in deps_to_mock:
        try:
            ...
            if dep not in sys.modules:
                ...
                if dep == "jwt":
                    # Ensure InvalidTokenError exists for exception handling
                    mock_mod.InvalidTokenError = type('InvalidTokenError', (Exception,), {})
...
class TestSecurity(unittest.TestCase):
    ...
    def test_decode_access_token_failure(self):
        ...
        with patch("app.core.security.jwt") as mocked_jwt:
            mocked_jwt.InvalidTokenError = jwt.InvalidTokenError
            mocked_jwt.decode.side_effect = jwt.InvalidTokenError("Invalid token")
            token = "invalid_token"
