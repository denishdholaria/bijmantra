...
def test_intelligence_knowledge_graph_vocabulary_normalization_is_domain_policy() -> None:
    assert require_supported_relationship_type("Has Trait") == "has_trait"
    assert require_supported_relationship_type("resistant-to") == "resistant_to"
    assert require_supported_direction("Incoming") == "incoming"
    assert require_supported_result_side("target") == "target"
