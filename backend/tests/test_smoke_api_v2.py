...
API_V2_ROUTES = [
    ...
    # ── Biosimulation ─────────────────────────────────────────────────────
    ("/api/v2/biosimulation/models", "biosimulation"),
    # ── Breeding Pipeline ─────────────────────────────────────────────────
...
async def _smoke_clients():
    ...
    with SyncSession() as sess:
        ...
        sess.commit()
        sess.refresh(org)
        user = User(email="smoke@test.com", hashed_password="x", organization_id=org.id, is_superuser=True)
        sess.add(user)
        sess.commit()
        sess.refresh(user)
        user_id = user.id
...
_PROTECTED_ROUTES = [(path, mod) for path, mod in ALL_ROUTES if path not in PUBLIC_ENDPOINTS]

_PUBLIC_ROUTE_LIST = [(path, mod) for path, mod in ALL_ROUTES if path in PUBLIC_ENDPOINTS]

_JSON_ROUTES = [(path, mod) for path, mod in ALL_ROUTES if path not in STREAMING_ENDPOINTS]
...
class TestAuthenticatedSmoke:
    ...
    async def test_get_authenticated_no_500(self, auth_client: AsyncClient, path: str, module: str):
        if path in SQLITE_INCOMPATIBLE:
            pytest.xfail(f"SQLite-incompatible: {path}")
        response = await auth_client.get(path)
        assert response.status_code != 500, (
            f"500 Internal Server Error on {path} ({module}): {response.text[:300]}"
        )
...
class TestUnauthenticatedGuard:
    ...
    async def test_get_unauth_rejected(self, anon_client: AsyncClient, path: str, module: str):
        if path in SQLITE_INCOMPATIBLE:
            pytest.xfail(f"SQLite-incompatible: {path}")
...
class TestPublicEndpoints:
    ...
    async def test_public_returns_200(self, anon_client: AsyncClient, path: str, module: str):
        if path in SQLITE_INCOMPATIBLE:
            pytest.xfail(f"SQLite-incompatible: {path}")
        response = await anon_client.get(path)
        assert response.status_code == 200, (
            f"Public endpoint {path} ({module}) returned {response.status_code} — expected 200"
        )
...
class TestResponseFormat:
    ...
    async def test_returns_json(self, auth_client: AsyncClient, path: str, module: str):
        if path in SQLITE_INCOMPATIBLE:
            pytest.xfail(f"SQLite-incompatible: {path}")
        response = await auth_client.get(path)
