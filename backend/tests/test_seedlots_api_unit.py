...
from app.api.brapi.v2.seedlots import create_transaction, TransactionCreate
...
async def test_create_transaction_adds_user_info():
    ...
    transaction_request = TransactionCreate(
        seedLotDbId="seedlot1", amount=10.0, transactionDescription="Test transaction"
    )

    # Mock the SeedlotTransaction model
    with patch("app.api.brapi.v2.seedlots.SeedlotTransaction") as MockTransaction:
        ...
        result = await create_transaction(
            transaction=transaction_request, db=mock_db, current_user=mock_user
        )
