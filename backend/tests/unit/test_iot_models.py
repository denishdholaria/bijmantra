...
async def test_create_iot_telemetry(async_db_session):
    ...
    org = Organization(name="IoT Test Org", description="Test Organization for IoT")
    ...
    telemetry = IoTTelemetry(
        timestamp=timestamp, device_id=device.id, sensor_id=sensor.id, value=25.5, quality="good"
    )
    ...
    stmt = select(IoTTelemetry).where(
        IoTTelemetry.device_id == device.id, IoTTelemetry.sensor_id == sensor.id
    )
