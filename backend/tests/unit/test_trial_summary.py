...
from app.api.bijmantra.trials.trial_summary import get_top_performers
...
async def test_get_top_performers_change_percent_calculated():
    ...
    mock_db.execute.side_effect = [mock_trial_result, mock_var_result, mock_check_mean_result]
    ...
    with patch("app.api.bijmantra.trials.trial_summary.performance_ranking_service") as mock_service:
        mock_service.get_top_performers = AsyncMock(return_value=mock_performers)

        response = await get_top_performers(
            trial_id=trial_id, limit=10, trait="Yield", db=mock_db, organization_id=mock_org_id
        )
