...
def setup_db():
    ...
    tables = [
        ...
        "perf_query_cache",
        "perf_frontend_bundles",
        "perf_asset_optimizations",
        "perf_server_responses",
        # QTL & GO
        "genes",
        "go_terms",
        "gene_go_terms",
        # Bio-analytics QTL & GWAS
        "bio_candidate_genes",
    ]
...
def test_superuser(db_session: Session) -> User:
    """
    Creates a test superuser and organization using sync session.
    """
    org = db_session.query(Organization).filter_by(name="Test Admin Org").first()
    if not org:
        org = Organization(name="Test Admin Org")
        db_session.add(org)
        db_session.commit()
        db_session.refresh(org)
...
@pytest.fixture
async def superuser_client(test_superuser: User) -> AsyncClient:
    """
    Creates an authenticated superuser client.
    """
    access_token_expires = timedelta(minutes=30)
    access_token = create_access_token(
        data={"sub": str(test_superuser.id)}, expires_delta=access_token_expires
    )
