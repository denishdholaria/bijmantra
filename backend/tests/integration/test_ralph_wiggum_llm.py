...
from app.modules.ai.services.engine import MultiTierLLMService, LLMProvider, LLMConfig
...
async def test_ralph_wiggum_fail_softly():
    ...
    with patch.dict("os.environ", {}, clear=True):
        ...
        assert (
            "template mode" in response.content.lower()
            or "offline mode" in response.content.lower()
        )
...
async def test_ralph_wiggum_self_awareness():
    ...
    prompt_unconfirmed = service.get_system_prompt(
        LLMProvider.OLLAMA, "llama3", model_confirmed=False
    )
    ...
    prompt_unknown = service.get_system_prompt(
        LLMProvider.OLLAMA, model=None, model_confirmed=False
    )
...
async def test_ralph_wiggum_naive_input_handling():
    ...
    response = await service.chat(nonsense_input, preferred_provider=LLMProvider.TEMPLATE)
...
async def test_ralph_wiggum_guardrails_identity():
    ...
    assert "You are REEVU, NOT the underlying model directly" in prompt
