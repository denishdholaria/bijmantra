...
def test_apex_module_routes_use_canonical_prefixes():
    ...
    assert "/api/v2/seed-bank/mcpd/template" in route_paths
...
def test_seed_bank_mcpd_routes_are_mounted_once_at_public_paths():
    route_paths = [
        getattr(route, "path", "")
        for route in app.routes
        if "/api/v2/seed-bank/mcpd" in getattr(route, "path", "")
    ]

    expected = [
        "/api/v2/seed-bank/mcpd/export/csv",
        "/api/v2/seed-bank/mcpd/export/json",
        "/api/v2/seed-bank/mcpd/import",
        "/api/v2/seed-bank/mcpd/template",
        "/api/v2/seed-bank/mcpd/codes/biological-status",
        "/api/v2/seed-bank/mcpd/codes/acquisition-source",
        "/api/v2/seed-bank/mcpd/codes/storage-type",
        "/api/v2/seed-bank/mcpd/codes/countries",
    ]

    assert sorted(route_paths) == sorted(expected)
