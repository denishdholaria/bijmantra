...
async def test_get_dock_state_role_based(
    ...
    for r_data in roles_data:
        role = Role(organization_id=org_id, role_id=r_data["role_id"], name=r_data["name"])
    ...
    user_role = UserRole(user_id=user.id, role_id=roles["breeder"].id, granted_at=datetime.utcnow())
    ...
    user_role_tech = UserRole(
        user_id=user.id, role_id=roles["technician"].id, granted_at=datetime.utcnow()
    )
