...
from app.api.bijmantra.phenotyping.image_analysis import router
...
def override_get_current_user():
    ...
    return User(id=1, email="test@example.com", organization_id=1, hashed_password="fake")
...
@pytest.fixture
def mock_tempfile():
    with patch("app.api.bijmantra.phenotyping.image_analysis.tempfile") as mock:
        yield mock


@pytest.fixture
def mock_shutil():
    with patch("app.api.bijmantra.phenotyping.image_analysis.shutil") as mock:
        yield mock


@pytest.fixture
def mock_os():
    with patch("app.api.bijmantra.phenotyping.image_analysis.os") as mock:
...
@pytest.fixture
def mock_spectral():
    with patch("app.api.bijmantra.phenotyping.image_analysis.spectral_indices") as mock:
        yield mock
