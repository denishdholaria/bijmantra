...
async def test_list_workspaces_performance(
    authenticated_client: AsyncClient, test_user: User, async_db_session: AsyncSession
):
    # 1. Setup Data
    NUM_WORKSPACES = 2000
    ...
    for i in range(NUM_MEMBER_WORKSPACES):
        member = WorkspaceMember(
            workspace_id=workspaces[i].id, user_id=test_user.id, role=MemberRole.EDITOR
        )
    ...
    response = await authenticated_client.get(
        f"/api/v2/collaboration-hub/workspaces?member_id={test_user.id}"
    )
    end_time = time.time()
