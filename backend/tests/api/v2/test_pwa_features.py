...
from app.api.bijmantra.system import pwa_notifications
...
async def test_pwa_subscribe_unauthorized(client: AsyncClient):
    ...
    response = await client.post(
        "/api/v2/pwa/notifications/subscribe",
        json={"endpoint": "https://example.com/push", "keys": {"p256dh": "key", "auth": "secret"}},
    )
...
async def test_pwa_list_subscriptions(authenticated_client: AsyncClient):
    ...
    endpoint = "https://example.com/push/list-test"
    ...
    assert endpoint not in data["subscriptions"]


@pytest.mark.asyncio
async def test_pwa_dispatch_notification(superuser_client: AsyncClient):
    ...
    payload = {"title": "Test Notification", "body": "This is a test.", "data": {"url": "/home"}}
    response = await superuser_client.post("/api/v2/pwa/notifications/dispatch", json=payload)
    assert response.status_code == 200
    data = response.json()
...
@pytest.mark.asyncio
async def test_pwa_dispatch_requires_superuser(authenticated_client: AsyncClient):
    response = await authenticated_client.post(
        "/api/v2/pwa/notifications/dispatch",
        json={
            "title": "Nope",
...
@pytest.mark.asyncio
async def test_pwa_subscribe_returns_503_when_redis_is_required(
    authenticated_client: AsyncClient, monkeypatch: pytest.MonkeyPatch
):
    async def unavailable_redis():
        return None, False

    monkeypatch.setattr(pwa_notifications, "_redis_available", unavailable_redis)
    monkeypatch.setattr(pwa_notifications.settings, "ENVIRONMENT", "production")

    response = await authenticated_client.post(
        "/api/v2/pwa/notifications/subscribe",
        json={
            "endpoint": "https://example.com/push/prod-gate",
            "keys": {"p256dh": "key", "auth": "secret"},
        },
    )

    assert response.status_code == 503
    assert response.json()["detail"] == "Redis-backed PWA notification storage is unavailable"
...
async def test_pwa_sync_unauthorized(client: AsyncClient):
    ...
    response = await client.post(
        "/api/v2/pwa/drafts/sync", json={"drafts": [{"type": "test", "data": {}}]}
    )
    assert response.status_code == 401
...
@pytest.mark.asyncio
async def test_pwa_sync_rejects_cross_tenant_draft(authenticated_client: AsyncClient):
    response = await authenticated_client.post(
        "/api/v2/pwa/drafts/sync",
        json={
            "drafts": [{"type": "observation", "organization_id": 999999, "data": {"value": 12}}]
        },
    )
