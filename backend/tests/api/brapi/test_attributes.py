...
# Patch set_tenant_context in attributes module to avoid Postgres-specific SQL in SQLite
# We need to do this after imports because app.main imports the router which imports attributes
from app.api.brapi.v2 import attributes
attributes.set_tenant_context = AsyncMock()
...
# Mock SpatiaLite functions for SQLite (Copied from conftest.py)
...
async def token():
    ...
    async with TestingSessionLocal() as db:
        ...
        access_token = create_access_token(
            data={"sub": str(user.id), "organization_id": org.id}, expires_delta=access_token_expires
        )
        return access_token
...
async def test_germplasm_attributes_crud(client, token):
    ...
    # 4. Update
    update_data = {"attributeName": "Updated Name", "attributeCategory": "Agronomic"}
    response = await client.put(
        f"/brapi/v2/attributes/{attr_id}", json=update_data, headers=headers
    )
