...
async def conversation(
    async_db_session: AsyncSession, test_user: User, other_user: User
) -> Conversation:
    """Creates a conversation between test_user and other_user."""
    conv = Conversation(
        organization_id=test_user.organization_id, type=ConversationType.DIRECT, name="Test Chat"
    )
...
async def test_send_message_broadcasts_websocket(
    authenticated_client: AsyncClient, conversation: Conversation, test_user: User
):
    # Mock socketio
    with patch("app.api.bijmantra.collaboration.sio", new_callable=AsyncMock) as mock_sio:
        payload = {"conversation_id": str(conversation.id), "content": "Hello World"}
