...
# Compatibility aliases for extracted functions
_build_reevu_envelope = MessageService.build_reevu_envelope
_validate_response_content = MessageService.validate_response_content
from app.api.bijmantra.trials.trial_summary import _build_trial_evidence_envelope
...
class TestCrossContractEnvelopeIntegration:
    ...
    def test_chat_envelope_accepts_germplasm_lookup_artifacts(self):
        validation, evidence_pack = _validate_response_content(
            content="Germplasm IR64 is grounded in database evidence [[ref:db:germplasm:IR64]].",
            context_docs=None,
            function_call_name="get_germplasm_details",
            function_result={
                "evidence_refs": ["db:germplasm:IR64", "db:observation:OBS-1"],
            },
        )
        ...
        assert any(ref["entity_id"] == "db:germplasm:IR64" for ref in envelope["evidence_refs"])
        assert any(ref["entity_id"] == "db:observation:OBS-1" for ref in envelope["evidence_refs"])
        assert "no_evidence_for_claims" not in envelope["policy_flags"]

    def test_chat_envelope_accepts_trait_summary_artifacts(self):
        validation, evidence_pack = _validate_response_content(
            content="Trait summary for IR64 and Swarna is grounded in database evidence [[ref:db:germplasm:IR64]].",
            context_docs=None,
            function_call_name="get_trait_summary",
            function_result={
        ...
        assert any(ref["entity_id"] == "db:germplasm:IR64" for ref in envelope["evidence_refs"])
        assert any(step["step_id"] == "calc:yield:mean" for step in envelope["calculation_steps"])
        assert "no_evidence_for_claims" not in envelope["policy_flags"]

    def test_chat_envelope_accepts_marker_association_artifacts(self):
        validation, evidence_pack = _validate_response_content(
            content="Blast resistance markers are grounded in genomics evidence [[ref:db:qtl:qtl_blast_1]].",
            context_docs=None,
            function_call_name="get_marker_associations",
            function_result={
                "evidence_refs": ["db:qtl:qtl_blast_1", "db:marker:M123"],
                "calculation_ids": ["calc:genomics:marker_lookup"],
            },
        )
        ...
        assert any(ref["entity_id"] == "db:qtl:qtl_blast_1" for ref in envelope["evidence_refs"])
        assert any(ref["entity_id"] == "db:marker:M123" for ref in envelope["evidence_refs"])
        assert any(step["step_id"] == "calc:genomics:marker_lookup" for step in envelope["calculation_steps"])
        assert "no_evidence_for_claims" not in envelope["policy_flags"]

    def test_chat_envelope_accepts_cross_domain_query_artifacts(self):
        validation, evidence_pack = _validate_response_content(
            content="Joined breeding, trial, and weather evidence supports this recommendation [[ref:db:trial:TRIAL-1]].",
            context_docs=None,
            function_call_name="cross_domain_query",
            function_result={
                "evidence_refs": ["db:germplasm:IR64", "db:trial:TRIAL-1", "fn:weather.forecast"],
        ...
        assert any(ref["entity_id"] == "db:trial:TRIAL-1" for ref in envelope["evidence_refs"])
        assert any(ref["entity_id"] == "fn:weather.forecast" for ref in envelope["evidence_refs"])
        assert any(step["step_id"] == "calc:cross_domain:join" for step in envelope["calculation_steps"])
        assert "no_evidence_for_claims" not in envelope["policy_flags"]

    def test_chat_envelope_emits_explicit_missing_evidence_signals(self):
        validation, evidence_pack = _validate_response_content(
            content="Yield improved by 42% under this recommendation.",
            context_docs=None,
    ...
    def test_chat_envelope_classifies_retrieval_and_synthesis_claim_traces(self):
        content = (
            ...
            "I infer that additional monitoring may still be needed."
        )
        validation, evidence_pack = _validate_response_content(
            content=content,
            context_docs=None,
            ...
            function_result={
                "evidence_refs": ["db:trial:TRIAL-22"],
            },
        )
        ...
        assert any(
            trace["support_type"] == "retrieval_backed"
            and trace["statement"] == "Database evidence supports this recommendation."
            and trace["evidence_refs"] == ["db:trial:TRIAL-22"]
            for trace in envelope["claim_traces"]
        )
        assert any(
            trace["support_type"] == "model_synthesis"
            and trace["statement"] == "I infer that additional monitoring may still be needed."
            for trace in envelope["claim_traces"]
        )
