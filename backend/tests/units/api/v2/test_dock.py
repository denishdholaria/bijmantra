...
from app.api.bijmantra.operations.dock import (
    get_dock_state,
    update_dock_state,
    pin_item,
    unpin_item,
...
class TestDockApi:
    ...
    def setup_mocks(self):
        ...
        # SQLAlchemy Session.add() is synchronous, so keep this a sync mock.
    ...
    async def test_get_dock_state_returns_defaults_for_new_user(self):
        ...
        # Act
        response = await get_dock_state(db=self.mock_db, current_user=self.mock_user)
    ...
    async def test_get_dock_state_user_not_found(self):
        ...
        with pytest.raises(HTTPException) as exc:
            await get_dock_state(db=self.mock_db, current_user=self.mock_user)
    ...
    async def test_update_dock_state_creates_new_preferences(self):
        ...
        update_data = UpdateDockRequest(
            ...
            preferences=DockPreferences(
                maxPinned=8, maxRecent=4, showLabels=False, compactMode=False
            ),
        )
        ...
        response = await update_dock_state(
            data=update_data, db=self.mock_db, current_user=self.mock_user
        )
    ...
    async def test_update_dock_state_updates_existing_preferences(self):
        ...
        update_data = UpdateDockRequest(
            pinnedItems=[
                DockItem(
                    id="trials", path="/trials", label="Trials", icon="FlaskConical", isPinned=True
                )
            ],
    ...
    async def test_pin_item_success(self):
        ...
        pin_data = PinItemRequest(id="programs", path="/programs", label="Programs", icon="Wheat")

        # Act
        response = await pin_item(data=pin_data, db=self.mock_db, current_user=self.mock_user)
    ...
    async def test_pin_item_max_limit_reached(self):
        ...
        pin_data = PinItemRequest(id="new-item", path="/new-item", label="New Item", icon="Plus")

        # Act & Assert
        with pytest.raises(HTTPException) as exc:
            await pin_item(data=pin_data, db=self.mock_db, current_user=self.mock_user)
    ...
    async def test_unpin_item_success(self):
        ...
        # Act
        response = await unpin_item(path="/dashboard", db=self.mock_db, current_user=self.mock_user)
    ...
    async def test_unpin_item_not_in_pinned(self):
        ...
        # Act
        response = await unpin_item(path="/nonexistent", db=self.mock_db, current_user=self.mock_user)
    ...
    async def test_reorder_pinned_success(self):
        ...
        response = await reorder_pinned(from_index=0, to_index=2, db=self.mock_db, current_user=self.mock_user)
    ...
    async def test_reorder_pinned_negative_index(self):
        ...
        # Act & Assert
        with pytest.raises(HTTPException) as exc:
            await reorder_pinned(from_index=-1, to_index=0, db=self.mock_db, current_user=self.mock_user)
        assert exc.value.status_code == 400
        assert "Invalid from_index" in exc.value.detail
    ...
    async def test_reorder_pinned_no_preferences(self):
        ...
        # Act & Assert
        with pytest.raises(HTTPException) as exc:
            await reorder_pinned(from_index=0, to_index=1, db=self.mock_db, current_user=self.mock_user)
        assert exc.value.status_code == 404
    ...
    async def test_record_visit_adds_to_recent(self):
        ...
        response = await record_visit(
            data=visit_data, db=self.mock_db, current_user=self.mock_user
        )
    ...
    async def test_record_visit_trims_to_max_recent(self):
        ...
        visit_data = RecordVisitRequest(
            id="new-item", path="/new-item", label="New Item", icon="Plus"
        )
    ...
    async def test_clear_recent_success(self):
        ...
        # Act
        response = await clear_recent(db=self.mock_db, current_user=self.mock_user)
    ...
    async def test_update_preferences_success(self):
        ...
        new_prefs = DockPreferences(maxPinned=12, maxRecent=6, showLabels=True, compactMode=True)

        # Act
        response = await update_preferences(
            data=new_prefs, db=self.mock_db, current_user=self.mock_user
        )
    ...
    async def test_update_preferences_creates_if_not_exist(self):
        ...
        new_prefs = DockPreferences(maxPinned=10, maxRecent=5, showLabels=True, compactMode=False)
    ...
    async def test_get_default_dock_for_seed_company(self):
        ...
        # Seed company should have Seed Commerce pages
    ...
    async def test_reset_dock_to_defaults_success(self):
        ...
        response = await reset_dock_to_defaults(
            role="breeder", db=self.mock_db, current_user=self.mock_user
        )
    ...
    async def test_reset_dock_creates_preferences_if_not_exist(self):
        ...
        # Act
        response = await reset_dock_to_defaults(
            role="researcher", db=self.mock_db, current_user=self.mock_user
        )
    ...
    async def test_reset_dock_with_invalid_role_uses_default(self):
        ...
        # Act
        response = await reset_dock_to_defaults(
            role="invalid_role", db=self.mock_db, current_user=self.mock_user
        )
    ...
    async def test_reset_dock_returns_pinned_count(self):
        ...
        # Act
        response = await reset_dock_to_defaults(
            role="seed_company", db=self.mock_db, current_user=self.mock_user
        )
