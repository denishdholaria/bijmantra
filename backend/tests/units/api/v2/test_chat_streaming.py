...
class TestChatStreamingEndpoint:
    ...
    def _disable_quota_limits(self):
        ...
        with patch("app.api.bijmantra.chat.AIQuotaService.check_and_increment_usage", return_value=None):
            yield
    ...
    def client(self, mock_breeding_service):
        ...
        from app.api.bijmantra.ai.chat import get_breeding_service, get_reevu_service, router
    ...
    def mock_llm_service(self):
        ...
        with patch("app.services.chat.session_service.get_llm_service") as mock:
            service = MagicMock()
    ...
    def mock_breeding_service(self):
        ...
        with patch("app.api.bijmantra.chat.get_breeding_service") as mock:
            service = MagicMock()
    ...
    def test_streaming_policy_validation_emits_safe_failure_on_invalid_response(self, client):
        ...
        with patch(
            ...
            "app.api.bijmantra.chat._validate_response_content",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                ["app.api.bijmantra.trial_summary.get_trial_summary"],
    ...
    def test_streaming_route_uses_request_scoped_registry_metadata(self, client):
        ...
        with patch("app.services.chat.session_service.MultiTierLLMService", FakeMultiTierLLMService), patch(
            "app.services.chat.session_service.get_llm_service", return_value=FakeMultiTierLLMService()
        ), patch("app.modules.ai.service.get_ai_provider_service", return_value=provider_service), patch(
            "app.services.chat.orchestration_service.FunctionCallingService.detect_function_call",
    ...
    def test_streaming_blocked_internal_functiongemma_output_falls_back_without_execution(
        ...
        with patch.dict(
            ...
            "app.services.chat.orchestration_service.FunctionCallingService._get_client",
            new=mock_get_client,
        ), patch(
            "app.api.bijmantra.chat.FunctionExecutor.execute",
    ...
    def test_streaming_route_passes_prompt_modes_from_agent_setting(self, client):
        ...
        with patch("app.services.chat.session_service.MultiTierLLMService", FakeMultiTierLLMService), patch(
            "app.services.chat.session_service.get_llm_service", return_value=base_service
        ), patch("app.modules.ai.service.get_ai_provider_service", return_value=provider_service), patch(
            "app.services.chat.orchestration_service.FunctionCallingService.detect_function_call",
            side_effect=no_function_call,
        ):
...
class TestRequestScopedLLMService:
    def test_get_request_llm_service_returns_non_multitier_base_service(self):
        ...
        async def run_test():
            with patch("app.services.chat.session_service.get_llm_service", return_value=base_service), patch(
                "app.modules.ai.service.get_ai_provider_service"
            ) as mock_provider_service:
                resolved = await SessionService.get_request_llm_service(
                    db=SimpleNamespace(),
    ...
    def test_get_request_llm_service_loads_org_registry_into_new_multitier_service(self):
        ...
        async def run_test():
            with patch("app.services.chat.session_service.MultiTierLLMService", FakeMultiTierLLMService), patch(
                "app.services.chat.session_service.get_llm_service", return_value=base_service
            ), patch("app.modules.ai.service.get_ai_provider_service", return_value=provider_service):
                resolved = await SessionService.get_request_llm_service(db=db, current_user=user)
