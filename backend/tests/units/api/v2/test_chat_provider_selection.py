...
_derive_routing_decisions = SessionService.derive_routing_decisions
...
def mock_chat_infra():
    ...
    with patch("app.api.bijmantra.chat.AIQuotaService.check_and_increment_usage", new=AsyncMock(return_value=None)) as mock_quota, patch(
        ...
        "app.services.chat.message_service.MessageService.validate_response_content",
        return_value=(
...
def test_chat_route_honors_preferred_provider_and_byok(client, mock_chat_infra):
    ...
    with patch("app.api.bijmantra.chat.MultiTierLLMService", FakeMultiTierLLMService), patch(
        "app.api.bijmantra.chat.get_llm_service", return_value=base_service
    ), patch("app.api.bijmantra.chat.get_ai_provider_service", return_value=provider_service):
        response = test_client.post(
            "/api/v2/chat",
            json={
...
def test_chat_route_records_token_telemetry_for_managed_requests(client, mock_chat_infra):
    ...
    with patch("app.api.bijmantra.chat.MultiTierLLMService", FakeMultiTierLLMService), patch(
        "app.api.bijmantra.chat.get_llm_service", return_value=FakeMultiTierLLMService()
    ), patch("app.api.bijmantra.chat.get_ai_provider_service", return_value=provider_service), patch.object(
...
def test_usage_route_returns_richer_telemetry_payload(client):
    ...
    with patch("app.api.bijmantra.chat.MultiTierLLMService", FakeMultiTierLLMService), patch(
        ...
        "app.api.bijmantra.chat.AIQuotaService.get_usage_stats",
...
def test_chat_route_filters_function_manifest_using_agent_capabilities(client):
    ...
    with patch("app.api.bijmantra.chat.MultiTierLLMService", FakeMultiTierLLMService), patch(
        ...
        "app.services.chat.orchestration_service.FunctionCallingService", RecordingFunctionCallingService
    ):
        response = test_client.post(
            "/api/v2/chat",
            json={"message": "Hello", "include_context": False},
...
def test_function_chat_route_records_retrieval_execution_in_metrics(client):
    ...
    with patch("app.api.bijmantra.chat.MultiTierLLMService", FakeMultiTierLLMService), patch(
        ...
        "app.api.bijmantra.chat.FunctionExecutor.execute",
...
def test_diagnostics_route_returns_provider_latencies_and_safe_failures():
    ...
    metrics.record_request(
            ...
            "step_execution_trace": [
                {
                    ...
                    "duration_ms": 120.0,
                    "error_category": None,
                    "metadata": {"narrowing_applied": True},
                },
                {
        ...
        plan_execution_summary={
            "domains_involved": ["trials", "analytics"],
            "is_compound": True,
            "steps": [
                {
                    "step_id": "step-trials",
                    "domain": "trials",
                    "status": "completed",
                    "actual_outputs": ["trial-1"],
                },
                {
                    "step_id": "step-analytics",
                    "domain": "analytics",
                    "status": "completed",
                    "actual_outputs": ["insight-1"],
                },
            ],
        },
    )
            ...
            "step_execution_trace": [
                {
                    ...
                    "status": "failed",
                    "duration_ms": 55.0,
                    "error_category": "weather_service_unavailable",
                    "metadata": {"narrowing_skipped": True},
                },
                {
                    "step_id": "step-breeding",
                    "domain": "breeding",
                    "status": "skipped",
                    "duration_ms": 0.0,
                    "error_category": None,
                    "metadata": {"skipped_prerequisite": "step-weather"},
                },
            ],
        },
        plan_execution_summary={
            ...
            "steps": [
                ...
                },
                {
                    ...
                    "status": "skipped",
                    "actual_outputs": [],
                    "missing_reason": "blocked_on_weather",
                },
            ],
        },
    )
    ...
    with TestClient(app) as test_client, patch(
        ...
        "app.api.bijmantra.chat._load_reevu_benchmark_status",
        return_value={
            "available": True,
    ...
    assert payload["retrieval_execution"] == {
        ...
        "functions": [],
        "domains": [
            {"domain": "analytics", "count": 1},
            {"domain": "breeding", "count": 1},
            {"domain": "trials", "count": 1},
            {"domain": "weather", "count": 1},
        ],
        "services": [
            {"service": "trial_search_service.search", "count": 1},
            {"service": "weather_service.get_forecast", "count": 1},
        ],
    }
    assert payload["step_execution_traces"] == {
        "total_steps_observed": 4,
        "status_distribution": [
    ...
    assert payload["narrowing_effectiveness"] == {
        "applied_non_empty": 1,
        "applied_empty": 0,
        "fell_back_to_unnarrowed": 1,
        "domain_breakdown": [
            {
                "domain": "trials",
                "applied_non_empty": 1,
    ...
    assert payload["retrieval_outcomes"] == [
        {"domain": "analytics", "success": 1, "partial": 0, "failure": 0},
        {"domain": "trials", "success": 1, "partial": 0, "failure": 0},
        {"domain": "weather", "success": 0, "partial": 0, "failure": 1},
    ]
    assert payload["safe_failure_distribution"] == [
        {
            "domain": "weather",
            "count": 1,
            "categories": [{"error_category": "weather_service_unavailable", "count": 1}],
        }
    ]
    assert payload["benchmark_status"] == {
        "available": True,
        "runtime_status": "blocked",
        "generated_at": "2026-04-05T10:38:21.986969+00:00",
        "runtime_target": "in_process_app",
...
def test_diagnostics_route_requires_superuser(client):
    test_client, _ = client
