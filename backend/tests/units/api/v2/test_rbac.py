...
from app.api.bijmantra.security.rbac import (
    list_roles,
    create_role,
    get_user_roles,
    assign_role_to_user,
...
class TestRbacApi:
    ...
    async def test_create_role_success_by_superuser(self):
        ...
        response = await create_role(
            role_data=role_data, db=self.mock_db, current_user=self.mock_superuser
        )
    ...
    async def test_create_role_fails_on_duplicate_id(self):
        ...
        self.mock_scalar_one_or_none.return_value = Role(
            id=1, role_id="existing_role", name="Old Name"
        )
    ...
    async def test_get_user_roles_success(self):
        ...
        # Act
        response = await get_user_roles(user_id=3, db=self.mock_db, current_user=self.mock_user)
    ...
    async def test_get_user_roles_not_found(self):
        ...
        # Act & Assert
        with pytest.raises(HTTPException) as exc:
            await get_user_roles(user_id=999, db=self.mock_db, current_user=self.mock_user)
    ...
    async def test_assign_role_success_by_superuser(self):
        ...
        response = await assign_role_to_user(
            user_id=3, request=request, db=self.mock_db, current_user=self.mock_superuser
        )
    ...
    async def test_assign_role_fails_if_user_not_found(self):
        ...
        with pytest.raises(HTTPException) as exc:
            await assign_role_to_user(
                user_id=999, request=request, db=self.mock_db, current_user=self.mock_superuser
            )
        assert exc.value.status_code == 404
        assert "User not found" in exc.value.detail
    ...
    async def test_remove_role_success_by_superuser(self):
        ...
        response = await remove_role_from_user(
            user_id=3, role_id="role_to_remove", db=self.mock_db, current_user=self.mock_superuser
        )
