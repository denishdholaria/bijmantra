...
from app.api.bijmantra.future.soil_moisture import (
...
class TestSoilMoistureApi:
    ...
    async def test_list_readings(self):
        ...
        try:
            # Act
            result = await list_soil_moisture_readings(skip=0, limit=100, db=self.mock_db, org_id=1)
    ...
    async def test_get_device_readings(self):
        ...
        try:
            # Act
            result = await get_device_readings(device_id=123, db=self.mock_db, org_id=1)
