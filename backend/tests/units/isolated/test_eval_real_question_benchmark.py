...
DEFAULT_LOCAL_REPORT_PATH = BACKEND_ROOT / "test_reports" / "reevu_real_question_local.json"
DEMO_LOCAL_REPORT_PATH = BACKEND_ROOT / "test_reports" / "reevu_real_question_local_org2.json"
DEFAULT_LOCAL_PASSED_CASES_FLOOR = 3
DEFAULT_LOCAL_TOTAL_CASES = 14
DEFAULT_LOCAL_PASS_RATE_FLOOR = round(DEFAULT_LOCAL_PASSED_CASES_FLOOR / DEFAULT_LOCAL_TOTAL_CASES, 4)
...
class TestEvalRealQuestionBenchmark(unittest.TestCase):
    ...
    def test_local_benchmark_artifacts_meet_documented_retrieval_quality_floors(self) -> None:
        default_payload = json.loads(DEFAULT_LOCAL_REPORT_PATH.read_text(encoding="utf-8"))
        demo_payload = json.loads(DEMO_LOCAL_REPORT_PATH.read_text(encoding="utf-8"))

        self.assertEqual(default_payload["runtime_path"], "local")
        self.assertEqual(default_payload["runtime_target"], "in_process_app")
        self.assertEqual(default_payload["local_organization_id"], 1)
        self.assertEqual(default_payload["total_cases"], DEFAULT_LOCAL_TOTAL_CASES)
        self.assertGreaterEqual(default_payload["passed_cases"], DEFAULT_LOCAL_PASSED_CASES_FLOOR)
        self.assertGreaterEqual(default_payload["pass_rate"], DEFAULT_LOCAL_PASS_RATE_FLOOR)

        default_attribution = default_payload.get("failure_attribution_summary", {})
        self.assertEqual(default_attribution.get("executor_regression_or_contract_gap", 0), 0)

        self.assertEqual(demo_payload["runtime_path"], "local")
        self.assertEqual(demo_payload["runtime_target"], "in_process_app")
        self.assertEqual(demo_payload["local_organization_id"], 2)
        self.assertEqual(demo_payload["total_cases"], DEFAULT_LOCAL_TOTAL_CASES)
        self.assertEqual(demo_payload["passed_cases"], DEFAULT_LOCAL_TOTAL_CASES)
        self.assertEqual(demo_payload["failed_cases"], 0)
        self.assertEqual(demo_payload["pass_rate"], 1.0)

    def test_main_writes_local_benchmark_summary_at_documented_floor(self) -> None:
        results = [
            {
                "benchmark_id": f"rq-pass-{index}",
                "passed": True,
                "failed_checks": [],
                "domain_group": "breeding",
                "cross_domain_class": "single_domain",
                ...
                "safe_failure_observed": False,
                "safe_failure_source": None,
                "safe_failure_payload": None,
            }
            for index in range(DEFAULT_LOCAL_PASSED_CASES_FLOOR)
        ]
        results.extend(
            {
                "benchmark_id": f"rq-blocked-{index}",
                "passed": False,
                "failed_checks": ["safe_failure.unexpected_safe_failure"],
                "domain_group": "breeding",
                "cross_domain_class": "single_domain",
                "question_family": "breeding::single_domain",
                "expected_function": "get_trait_summary",
                "expected_domains": ["breeding"],
                "safe_failure_expected": False,
                "safe_failure_observed": True,
                "safe_failure_source": "function_result",
                "safe_failure_payload": {
                    ...
                    "searched": ["phenotype_comparison.statistics"],
                },
            }
            for index in range(DEFAULT_LOCAL_TOTAL_CASES - DEFAULT_LOCAL_PASSED_CASES_FLOOR)
        )
        readiness = {
            "organization_id": 1,
            "surface_counts": {"germplasm": 2, "trials": 2, "observations": 0, "bio_gwas_runs": 0, "bio_qtls": 0},
            "surface_examples": {"germplasm": ["IR64"], "trials": ["Trial A"]},
            "surface_errors": {},
            "readiness_flags": ["observations.empty", "bio_gwas_runs.empty", "bio_qtls.empty"],
        }
        ...
        with patch.object(benchmark_eval, "_load_fixture", return_value=[]), patch.object(
            ...
            benchmark_eval,
            "_inspect_local_runtime_readiness",
            return_value=readiness,
        ), patch.object(benchmark_eval, "_print_summary", return_value=None):
            with self.subTest("local benchmark summary artifact"):
                output_path = BACKEND_ROOT / "test_reports" / "_tmp_reevu_regression_floor.json"
                try:
                    exit_code = benchmark_eval.main(
                        [
                            "--runtime-path",
        ...
        self.assertEqual(payload["runtime_status"], "blocked")
        self.assertEqual(payload["passed_cases"], DEFAULT_LOCAL_PASSED_CASES_FLOOR)
        self.assertEqual(payload["total_cases"], DEFAULT_LOCAL_TOTAL_CASES)
        self.assertEqual(payload["pass_rate"], DEFAULT_LOCAL_PASS_RATE_FLOOR)
        self.assertGreaterEqual(payload["pass_rate"], DEFAULT_LOCAL_PASS_RATE_FLOOR)
