...
def test_user_create_valid_password():
    ...
    user_data = {"email": "test@example.com", "password": "ValidPassword1!", "organization_id": 1}
...
def test_user_create_invalid_password(password, error_message):
    ...
    with pytest.raises(ValidationError) as excinfo:
        UserCreate(email="test@example.com", password=password, organization_id=1)
...
def test_user_model_config_from_attributes():
    ...
    class MockUserORM:
        def __init__(
            self, id, email, organization_id, is_active, is_superuser, created_at, full_name=None
        ):
            self.id = id
            self.email = email
