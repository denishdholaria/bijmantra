...
class TestReevuPlanner:
    ...
    def test_trials_phenotype_environment_compound_plan(self):
        ...
        assert plan.is_compound is True
        assert "trials" in plan.domains_involved
        assert "phenotyping" in plan.domains_involved
        assert "weather" in plan.domains_involved

    def test_phenotyping_keywords(self):
        for query in (
            "show observation records",
            "find phenotype data",
            "list trait data",
            "get plant height measurements",
            "show grain yield observations",
        ):
            assert "phenotyping" in _detect_domains(query)

    def test_phenotyping_domain_order(self):
    ...
    def test_seed_ops_keywords(self):
        for query in (
            "show seed lot inventory",
            "how much seed stock is available",
            "check seed quantity in storage",
            "list accession inventory",
            "find seed bank viability",
        ):
            assert "seed_ops" in _detect_domains(query)

    def test_seed_ops_domain_order(self):
        from app.modules.ai.services.reevu.planner import DOMAIN_ORDER

        assert DOMAIN_ORDER["breeding"] < DOMAIN_ORDER["seed_ops"] < DOMAIN_ORDER["protocols"]
    ...
    def test_prerequisites_chain(self):
        ...
        if plan.total_steps >= 2:
            # Dependency-aware DAG plans should attach prerequisites where the
            # query implies chained execution, without forcing a purely linear
            # chain across otherwise independent domains.
...
# Compatibility alias for extracted function
_maybe_format_comparison = MessageService.maybe_format_comparison
