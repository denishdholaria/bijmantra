from app.modules.ai.services.reevu import ClaimItem, EvidencePack, PolicyGuard, ResponseValidator
...
def test_policy_guard_allowlist_matches_current_trust_domains():
    guard = PolicyGuard()

    assert guard.ALLOWED_DOMAIN_SCOPES == {
        "analytics",
        "breeding",
        "genomics",
        "operations",
...
def test_policy_guard_blocks_non_allowlisted_scope():
    guard = PolicyGuard()

    decision = guard.evaluate_access(domain_scope="future", entity="gdd", operation="read")

    assert decision.allowed is False
    assert decision.tier == "D"
    assert "not in REEVU allowlist" in decision.reason


def test_policy_guard_requires_confirmation_for_non_read_operations():
    guard = PolicyGuard()

    decision = guard.evaluate_access(domain_scope="operations", entity="crossing_planner", operation="write")
