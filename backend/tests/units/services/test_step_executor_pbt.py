...
class TestProperty3QueryNarrowingExtraction:
    ...
    def test_narrow_from_phenotyping_extracts_trait_ids(self, entity_ids: list[str]):
        """For any successful phenotyping StepResult, entity_ids pass through as trait_ids."""
        from unittest.mock import MagicMock
        ...
        narrowing = se._narrow_from_phenotyping(ctx, "prereq-phenotyping")

        assert narrowing["trait_ids"] == entity_ids
...
# ---------------------------------------------------------------------------
# Phenotyping Property: Distribution Bin Coverage
# ---------------------------------------------------------------------------


class TestPhenotypingDistributionProperties:
    @given(
        values=st.lists(
            st.floats(
    ...
    @settings(max_examples=50)
    def test_distribution_bin_coverage(self, values: list[float], n_bins: int):
        distribution = StepExecutor._compute_trait_distribution(values, n_bins=n_bins)

        assert distribution is not None
        assert sum(distribution["bin_counts"]) == len(values)
        assert len(distribution["bin_edges"]) == n_bins + 1
...
REQUIRED_KEYS = {
    ...
    "locations", "weather", "field", "genomics", "phenotyping", "traits", "seedlots", "seed_ops",
}
LIST_KEYS = {
    "germplasm", "trials", "observations", "protocols",
    "locations", "traits", "seedlots",
}
...
_DOMAINS_WITH_RECORDS = {
    ...
    "field": {
        "locations": [{"id": 11, "name": "Ludhiana", "latitude": 30.9, "longitude": 75.8}],
        "field_layouts": [{"location_id": "11", "plots": 4}],
        "crop_calendar_events": [{"event_type": "planting", "planned_date": "2026-11-15"}],
        "season_info": {"season": "rabi"},
    },
    "genomics": {"genomics": {"markers": ["SNP1", "SNP2"]}},
    "phenotyping": {
        "observations": [{"id": 8}],
        "traits": [{"id": 9}],
        "summary_stats": {"overall": {"n": 1, "mean": 1.0, "min": 1.0, "max": 1.0}},
        "observation_count": 1,
    },
    "seed_ops": {
        "seedlots": [{"id": 10, "seedlot_db_id": "SL-10"}],
        "inventory_summary": {"total_lots": 1, "total_quantity_grams": 120},
        "availability": [{"germplasm_id": "G1", "available": True}],
    },
...
class TestProperty9ResponseShapePreservation:
    ...
    def test_required_keys_present(self, outcome: ExecutionOutcome):
        """
        For any ExecutionOutcome, the assembled results dict contains the flat
        contract keys consumed by the handler before it adds derived insights.
