...
@pytest.mark.asyncio
async def test_get_trait_distribution_routes_through_cross_domain_query(monkeypatch):
    captured = {}

    async def fake_cross_domain(self, params):
        captured["params"] = params
        return {"success": True, "function": "cross_domain_query"}

    monkeypatch.setattr(FunctionExecutor, "_handle_cross_domain_query", fake_cross_domain)

    executor = FunctionExecutor(db=SimpleNamespace())

    result = await executor._handle_get(
        "get_trait_distribution",
        {"organization_id": 1, "trait": "plant height"},
    )

    assert result == {"success": True, "function": "cross_domain_query"}
    assert captured["params"]["query"] == "distribution of plant height"
    assert captured["params"]["trait"] == "plant height"


@pytest.mark.asyncio
async def test_get_observations_routes_phenotyping_query_through_cross_domain_query(monkeypatch):
    captured = {}
    ...
    result = await executor._handle_get(
        "get_observations",
        {"organization_id": 1, "query": "show plant height observations"},
    )

    assert result == {"success": True, "function": "cross_domain_query"}
    assert captured["params"]["query"] == "show plant height observations"


@pytest.mark.asyncio
async def test_get_seed_inventory_routes_through_cross_domain_query(monkeypatch):
    captured = {}
    ...
    result = await executor._handle_get(
        "get_seed_inventory",
        {"organization_id": 1, "germplasm": "IR64"},
    )

    assert result == {"success": True, "function": "cross_domain_query"}
    assert captured["params"]["query"] == "seed inventory for IR64"


@pytest.mark.asyncio
async def test_get_field_info_routes_through_cross_domain_query(monkeypatch):
    captured = {}
    ...
    result = await executor._handle_get(
        "get_field_info",
        {"organization_id": 1, "location": "Ludhiana"},
    )

    assert result == {"success": True, "function": "cross_domain_query"}
    assert captured["params"]["query"] == "field information for Ludhiana"


@pytest.mark.asyncio
async def test_get_crop_calendar_routes_through_cross_domain_query(monkeypatch):
    captured = {}
    ...
    result = await executor._handle_get(
        "get_crop_calendar",
        {"organization_id": 1, "crop": "wheat"},
    )

    assert result == {"success": True, "function": "cross_domain_query"}
    assert captured["params"]["query"] == "crop calendar for wheat"
...
async def test_compare_germplasm_returns_shared_interpretation(monkeypatch):
    async def fake_get_by_germplasm(db, organization_id, germplasm_id, limit=100):
        if germplasm_id == 101:
            return [
                {
                    "id": f"obs-ir64-{index}",
                    "observation_db_id": f"OBS-IR64-{index}",
                    "value": str(value),
                    "trait": {"name": "Yield", "trait_name": "Yield", "scale": "t/ha", "data_type": "numeric"},
                    "germplasm": {"id": "101", "name": "IR64", "accession": "IR64"},
                }
                for index, value in enumerate([4.0, 4.1, 4.2], start=1)
            ]
        return [
            {
                "id": f"obs-swarna-{index}",
                "observation_db_id": f"OBS-SWARNA-{index}",
                "value": str(value),
                "trait": {"name": "Yield", "trait_name": "Yield", "scale": "t/ha", "data_type": "numeric"},
                "germplasm": {"id": "102", "name": "Swarna", "accession": "Swarna"},
            }
            for index, value in enumerate([5.3, 5.5, 5.7], start=1)
        ]
    ...
    assert result["statistical_comparison"]["test_type"] == "t_test"
    assert isinstance(result["statistical_comparison"]["p_value"], float)
    assert result["statistical_comparison"]["significant"] is (
        result["statistical_comparison"]["p_value"] < 0.05
    )
    assert result["trait_profile"]["traits"] == ["Yield"]
    normalized_values = [
        profile["normalized_values"]["Yield"]
        for profile in result["trait_profile"]["profiles"]
    ]
...
async def test_get_trait_summary_returns_contract_metadata(monkeypatch):
    ...
    assert result["retrieval_audit"]["services"] == [
        "app.api.bijmantra.phenotype_comparison.get_comparison_statistics"
    ]
    assert result["retrieval_audit"]["tables"] == [
        "Germplasm",
        "Observation",
...
async def test_cross_domain_query_returns_safe_failure_when_trial_phenotype_missing(monkeypatch):
    ...
    assert result["success"] is False
    assert result["error"] == "Cross-domain query could not retrieve requested domains: phenotyping"
    assert result["safe_failure"]["error_category"] == "insufficient_retrieval_scope"
    assert "phenotyping" in result["plan_execution_summary"]["missing_domains"]
...
async def test_calculate_breeding_value_gblup_uses_matrix_backed_compute(monkeypatch):
    ...
    assert result["gebv_result"]["method"] == "GBLUP"
    assert result["gebv_result"]["trait_name"] == "yield"
    assert result["gebv_result"]["gebvs"][0] == {
        "germplasm_id": "IR64",
        "germplasm_name": "IR64",
        "gebv": 0.45,
        "reliability": 0.91,
    ...
    assert result["gebv_result"]["n_individuals"] == 2
    assert result["gebv_result"]["n_markers"] == 3
    assert result["gebv_result"]["insufficient_data"] is False
...
async def test_calculate_breeding_value_gblup_resolves_database_training_population(monkeypatch):
    ...
    assert [entry["germplasm_id"] for entry in result["gebv_result"]["gebvs"]] == [
        "HD2967",
        "Kalyan Sona",
        "PBW343",
    ]
    assert all(
        0.0 <= entry["reliability"] <= 1.0
        for entry in result["gebv_result"]["gebvs"]
    )
    assert result["evidence_refs"] == ["fn:compute.gblup"]
    assert result["calculation_ids"] == ["fn:compute.gblup"]
    assert result["calculation_method_refs"] == ["fn:compute.gblup"]
...
async def test_get_trial_results_includes_interpretation(monkeypatch):
    ...
    assert result["retrieval_audit"]["services"] == ["app.api.bijmantra.trial_summary.get_trial_summary"]
    assert result["retrieval_audit"]["tables"] == [
        "Trial",
        "Study",
        "Observation",
