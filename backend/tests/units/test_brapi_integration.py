...
def test_brapi_adapter_exposes_capability_metadata(adapter):
    assert adapter.capability_tags == ["germplasm", "trials", "observations", "seedlots", "programs"]
    assert adapter.function_mappings["search_germplasm"] == "GET /brapi/v2/germplasm"
    assert adapter.function_mappings["search_trials"] == "GET /brapi/v2/trials"
    assert adapter.adapter_type == "api"
    assert adapter.performance_profile == {
        "latency_ms": 200,
