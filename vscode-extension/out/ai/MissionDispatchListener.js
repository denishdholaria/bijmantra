...
exports.MissionDispatchListener = void 0;
 ...
 */
class MissionDispatchListener {
    static isExecuting = false;
    static currentJobId = null;
    static iterationCount = 0;
    static maxIterations = 50; // Safety cap per mission
    static activate(context) {
        ...
        const handleDispatch = async (uri) => {
    ...
    static async executeMission(dispatch) {
        ...
        try {
            ...
            const availability = await CopilotAdapter_1.CopilotAdapter.checkAvailability();
    ...
    static async runMissionLoop(dispatch) {
        ...
        while (MissionDispatchListener.iterationCount < MissionDispatchListener.maxIterations) {
            ...
            try {
                responseText = await vscode.window.withProgress({
                    ...
                    return await CopilotAdapter_1.CopilotAdapter.runDryRunPlan(prompt);
                });
            }
            catch (e) {
            ...
            switch (action.action) {
                case 'edit':
                    ...
                    await WorkspaceTools_1.WorkspaceTools.applyWorkspaceEditDryRun(action.path, action.content);
                    // Brief pause before next iteration
                    await MissionDispatchListener.sleep(2000);
                    break;
                case 'verify':
                    vscode.window.showInformationMessage(`[Mission] Verifying: ${action.command}`);
                    const result = await WorkspaceTools_1.WorkspaceTools.runVerification(action.command);
                    ...
                    if (vscode.workspace.workspaceFolders) {
                        const logUri = vscode.Uri.joinPath(vscode.workspace.workspaceFolders[0].uri, '.agent/state/latest_verification.log');
                        await vscode.workspace.fs.writeFile(logUri, new TextEncoder().encode(`COMMAND: ${action.command}\n\nRESULT:\n${result}`));
                    }
                    await MissionDispatchListener.sleep(2000);
                    break;
                case 'stop':
        ...
        // Hit iteration cap
        await MissionDispatchListener.writeResult(jobId, 'iteration-limit', `Reached ${MissionDispatchListener.maxIterations} iterations without completion`, verificationResults);
    }
    static buildPrompt(dispatch, previousVerification) {
        const inputFiles = (dispatch.input_files || []).map((f) => `- \`${f}\``).join('\n');
        const criteria = (dispatch.success_criteria || []).map((c) => `- ${c}`).join('\n');
        const verifyCommands = (dispatch.verification_commands || []).map((c) => `\`${c}\``).join(', ');
        const blockedPaths = (dispatch.blocked_paths || []).map((p) => `- ${p}`).join('\n');
...
`;
    ...
    static async runJobVerification(dispatch, results) {
        const commands = dispatch.verification_commands || [];
        ...
        for (const cmd of commands) {
            ...
            const result = await WorkspaceTools_1.WorkspaceTools.runVerification(cmd);
     ...
     */
    static async writeResult(jobId, status, summary, verificationResults) {
        if (!vscode.workspace.workspaceFolders || vscode.workspace.workspaceFolders.length === 0) {
            return;
        }
        const rootUri = vscode.workspace.workspaceFolders[0].uri;
        ...
        await vscode.workspace.fs.writeFile(resultUri, new TextEncoder().encode(JSON.stringify(payload, null, 2)));
        ...
        try {
            await vscode.workspace.fs.writeFile(receiptUri, new TextEncoder().encode(JSON.stringify(receipt, null, 2)));
        }
        catch (e) {
            console.error('[MissionBridge] Failed to write closeout receipt:', e);
        }
    }
    static deleteDispatchFile() {
        if (!vscode.workspace.workspaceFolders) {
            return;
        }
        const rootUri = vscode.workspace.workspaceFolders[0].uri;
...
exports.MissionDispatchListener = MissionDispatchListener;
//# sourceMappingURL=MissionDispatchListener.js.map