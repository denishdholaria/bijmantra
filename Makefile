.PHONY: help dev start stop restart logs clean build test lint format install dx-check reevu-gate overnight-plan update-state
...
dev-backend: ## Start backend development server
	cd backend && . venv/bin/activate && uvicorn app.main:app --reload --reload-include "*.yaml" --reload-include "*.yml" --reload-include "*.json" --host 0.0.0.0 --port 8000
