...
extern "C" {
    ...
    fn compute_epistatic_matrix(g: *const c_double, e: *mut c_double, n: c_int) -> c_int;

    /// Estimate Variance Components using REML
    #[link_name = "reml_estimate"]
    fn c_reml_estimate(
        y: *const c_double,
        x: *const c_double,
        z: *const c_double,
        ...
        max_iter: c_int,
        tol: c_double,
        var_a_out: *mut c_double,
        var_e_out: *mut c_double,
        iterations_out: *mut c_int,
        converged_out: *mut c_int,
        log_lik_out: *mut c_double,
        n: c_int,
        p: c_int,
        q: c_int,
    ) -> c_int;
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ComputeError::ConvergenceFailure { iterations } => {
                write!(
                    f,
                    "Convergence not achieved after {} iterations",
                    iterations
                )
            }
...
pub fn compute_epistatic(grm: &[f64], n: usize) -> ComputeResult<Vec<f64>> {
    ...
    let status = unsafe { compute_epistatic_matrix(grm.as_ptr(), e.as_mut_ptr(), n as c_int) };
...
/// Safe wrapper for REML variance component estimation
///
/// # Arguments
...
/// * `var_a_init` - Initial additive variance
/// * `var_e_init` - Initial residual variance
/// * `method` - REML method
