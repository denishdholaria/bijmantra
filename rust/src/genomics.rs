...
pub fn calculate_allele_frequencies(
    ...
    let mut sums = vec![0.0; n_markers];
    let mut het_counts = vec![0usize; n_markers];
    let mut valid_counts = vec![0usize; n_markers];
    let mut missing_counts = vec![0usize; n_markers];
    ...
    for j in 0..n_markers {
        let valid_count = valid_counts[j];
        if valid_count > 0 {
            let freq = sums[j] / (2.0 * valid_count as f64);
            maf.push(freq.min(1.0 - freq));
            het.push(het_counts[j] as f64 / valid_count as f64);
        } else {
            maf.push(0.0);
            het.push(0.0);
        }
        missing.push(missing_counts[j] as f64 / n_samples as f64);
    }
...
pub fn calculate_ld_matrix(genotypes: &[i32], n_samples: usize, n_markers: usize) -> Vec<f64> {
    ...
    for j in 0..n_markers {
        let count = counts[j];
        if count > 0 {
            let mean = means[j] / count as f64;
            vars[j] = vars[j] / count as f64 - mean * mean;
            means[j] = mean;
        }
    }
...
pub fn impute_missing_mean(genotypes: &[i32], n_samples: usize, n_markers: usize) -> Vec<f64> {
    ...
    for i in 0..n_samples {
        ...
        for j in 0..n_markers {
            ...
            imputed[idx] = if geno >= 0 { geno as f64 } else { means[j] };
        }
    }
