...
pub fn calculate_diversity(genotypes: &[i32], n_samples: usize, n_markers: usize) -> JsValue {
    ...
    let result = DiversityMetrics {
        ...
        inbreeding_coefficient: f.clamp(-1.0, 1.0),
...
/// Calculate Fst between populations (Implementation)
pub fn calculate_fst_impl(
    genotypes: &[i32],
    population_ids: &[i32],
    n_samples: usize,
    n_markers: usize,
    ...
    // Use HashSet for O(N) deduplication
    let unique_pops: HashSet<i32> = population_ids.iter().cloned().collect();
    let mut pops: Vec<i32> = unique_pops.into_iter().collect();
    // Sort to ensure deterministic output order and consistent indexing
    pops.sort_unstable();
    let n_pops = pops.len();
    ...
    // Precompute population indices for O(1) lookup
    let pop_indices: HashMap<i32, usize> =
        pops.iter().enumerate().map(|(i, &id)| (id, i)).collect();
    ...
    for j in 0..n_markers {
        ...
        for i in 0..n_samples {
            ...
            // Use O(1) lookup instead of O(P) linear scan
            let pop_idx = *pop_indices.get(&population_ids[i]).unwrap();
        ...
        per_marker_fst.push(fst_marker.clamp(0.0, 1.0));
    ...
    let fst = if total_ht > 0.0 {
        ((total_ht - total_hs) / total_ht).clamp(0.0, 1.0)
    } else {
        0.0
    };
...
pub fn calculate_fst(
    ...
    let result = calculate_fst_impl(genotypes, population_ids, n_samples, n_markers);
    serde_wasm_bindgen::to_value(&result).unwrap()
}
...
/// Perform PCA on genotype matrix (Implementation)
pub fn calculate_pca_impl(genotypes: &[i32], n_samples: usize, n_markers: usize) -> PCAResult {
    ...
    for pc in 0..3 {
        ...
        let mut v: Vec<f64> = (0..n_samples).map(|_| rng.random::<f64>() - 0.5).collect();
...
pub fn calculate_ammi(
    ...
    for g in 0..n_genotypes {
        for e in 0..n_environments {
            let val = phenotypes[g * n_environments + e];
            if !val.is_nan() {
                interaction[g * n_environments + e] =
                    val - geno_means[g] - env_means[e] + grand_mean;
            }
        }
    }
